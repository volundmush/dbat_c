...
extern bool Obj_to_store(struct obj_data *obj, FILE *fl, int location);
...
extern bool Crash_delete_file(char *name);

extern bool Crash_delete_crashfile(struct char_data *ch);

extern bool Crash_clean_file(char *name);

extern void Crash_listrent(struct char_data *ch, char *name);

extern bool Crash_load(struct char_data *ch);
