...
// functions
extern bool shop_producing(struct obj_data *item, int shop_nr);

extern bool ok_damage_shopkeeper(struct char_data *ch, struct char_data *victim);

extern void boot_the_shops(FILE *shop_f, char *filename, int rec_count);

extern void assign_the_shopkeepers(void);

extern bool ok_shop_room(int shop_nr, room_vnum room);
