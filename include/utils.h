...
void assign_affect(struct char_data *ch, int aff_flag, int skill, int dur, int str, int con, int intel, int agl,
                   int wis, int spd);
...
#define CHECK_PLAYER_SPECIAL(ch, var)                                                                                  \
    (*(((ch)->player_specials == &dummy_mob)                                                                           \
               ? (log("OHNO: Mob using '" #var "' at %s:%d.", __FILE__, __LINE__), &(var))                             \
               : &(var)))
#else
...
#define PLANET_ZENITH(room)                                                                                            \
    ((GET_ROOM_VNUM(room) >= 3400 && GET_ROOM_VNUM(room) <= 3599)                                                      \
     || (GET_ROOM_VNUM(room) >= 62900 && GET_ROOM_VNUM(room) <= 62999) || (GET_ROOM_VNUM(room) == 19600))
...
#define GET_CLASS_RANKS(ch, whichclass) (GET_CLASS_NONEPIC(ch, whichclass) + GET_CLASS_EPIC(ch, whichclass))
...
#define GET_PC_HEIGHT(ch)                                                                                              \
    (!IS_NPC(ch) ? age(ch)->year <= 10   ? (int) ((ch)->height * 0.68)                                                 \
                   : age(ch)->year <= 12 ? (int) ((ch)->height * 0.72)                                                 \
                   : age(ch)->year <= 14 ? (int) ((ch)->height * 0.85)                                                 \
                   : age(ch)->year <= 16 ? (int) ((ch)->height * 0.92)                                                 \
                                         : (ch)->height                                                                \
                 : (ch)->height)
#define GET_PC_WEIGHT(ch)                                                                                              \
    (!IS_NPC(ch) ? age(ch)->year <= 10   ? (int) ((ch)->weight * 0.48)                                                 \
                   : age(ch)->year <= 12 ? (int) ((ch)->weight * 0.55)                                                 \
                   : age(ch)->year <= 14 ? (int) ((ch)->weight * 0.7)                                                  \
                   : age(ch)->year <= 16 ? (int) ((ch)->weight * 0.85)                                                 \
                                         : (ch)->weight                                                                \
                 : (ch)->weight)
...
#define GET_MUTBOOST(ch)                                                                                               \
    (IS_MUTANT(ch) ? ((GET_GENOME(ch, 0) == 1 || GET_GENOME(ch, 1) == 1)                                               \
                              ? (GET_SPEEDCALC(ch) + GET_SPEEDBONUS(ch) + GET_SPEEDBOOST(ch)) * 0.3                    \
                              : 0)                                                                                     \
                   : 0)
...
#define GET_SPEEDCALC(ch)                                                                                              \
    (IS_GRAP(ch) ? GET_CHA(ch)                                                                                         \
                 : (IS_INFERIOR(ch) ? (AFF_FLAGGED(ch, AFF_FLYING) ? (GET_SPEEDVAR(ch) * 1.25) : GET_SPEEDVAR(ch))     \
                                    : GET_SPEEDVAR(ch)))
#define GET_SPEEDBONUS(ch)                                                                                             \
    (IS_ARLIAN(ch) ? AFF_FLAGGED(ch, AFF_SHELL)                                                                        \
                             ? GET_SPEEDVAR(ch) * -0.5                                                                 \
                             : (IS_MALE(ch) ? (AFF_FLAGGED(ch, AFF_FLYING) ? (GET_SPEEDVAR(ch) * 0.5) : 0) : 0)        \
                   : 0)
...
#define GET_SPEEDINT(ch)                                                                                               \
    (IS_BIO(ch) ? ((GET_CHA(ch) * GET_DEX(ch)) * (GET_MAX_HIT(ch) / 1200) / 1200)                                      \
...
#define GET_KI(ch) ((ch)->mana)
#define GET_MAX_KI(ch) ((ch)->max_mana)
...
#define GET_BLESSBONUS(ch)                                                                                             \
    (AFF_FLAGGED(ch, AFF_BLESS)                                                                                        \
             ? (GET_BLESSLVL(ch) >= 100  ? ((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * 0.1                 \
                : GET_BLESSLVL(ch) >= 60 ? ((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * 0.05                \
                : GET_BLESSLVL(ch) >= 40 ? ((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * 0.02                \
                                         : 0)                                                                          \
             : 0)
#define GET_POSELF(ch)                                                                                                 \
    (!IS_NPC(ch) ? PLR_FLAGGED(ch, PLR_POSE) ? GET_SKILL(ch, SKILL_POSE) >= 100  ? 0.15                                \
                                               : GET_SKILL(ch, SKILL_POSE) >= 60 ? 0.1                                 \
                                               : GET_SKILL(ch, SKILL_POSE) >= 40 ? 0.05                                \
                                                                                 : 0                                   \
                                             : 0                                                                       \
                 : 0)
#define GET_POSEBONUS(ch) (((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * GET_POSELF(ch))
#define GET_LIFEBONUS(ch)                                                                                              \
    (IS_ARLIAN(ch) ? ((GET_MAX_MANA(ch) * 0.01) * (GET_MOLT_LEVEL(ch) / 100))                                          \
                             + ((GET_MAX_MOVE(ch) * 0.01) * (GET_MOLT_LEVEL(ch) / 100))                                \
                   : 0)
#define GET_LIFEBONUSES(ch)                                                                                            \
    ((ch)->lifebonus > 0                                                                                               \
             ? (GET_LIFEBONUS(ch) + GET_BLESSBONUS(ch) + GET_POSEBONUS(ch)) * (((ch)->lifebonus + 100) * 0.01)         \
             : (GET_LIFEBONUS(ch) + GET_BLESSBONUS(ch) + GET_POSEBONUS(ch)))
#define GET_LIFEMAX(ch)                                                                                                \
    (IS_DEMON(ch)                                                                                                      \
             ? (((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * 0.75) + GET_LIFEBONUSES(ch)                    \
             : (IS_KONATSU(ch) ? (((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * 0.85) + GET_LIFEBONUSES(ch)  \
                               : (GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5) + GET_LIFEBONUSES(ch)))
...
#define SET_SKILL_BONUS(ch, i, value)                                                                                  \
    do {                                                                                                               \
        (ch)->skillmods[i] = value;                                                                                    \
    } while(0)
#define SET_SKILL_PERF(ch, i, value)                                                                                   \
    do {                                                                                                               \
        (ch)->skillperfs[i] = value;                                                                                   \
    } while(0)
...
#define ALIGN_TYPE(ch) ((IS_GOOD(ch) ? 0 : (IS_EVIL(ch) ? 6 : 3)) + (IS_LAWFUL(ch) ? 0 : (IS_CHAOTIC(ch) ? 2 : 1)))
...
#define IS_PLAYING(d)                                                                                                  \
    (STATE(d) == CON_TEDIT || STATE(d) == CON_REDIT || STATE(d) == CON_MEDIT || STATE(d) == CON_OEDIT                  \
     || STATE(d) == CON_ZEDIT || STATE(d) == CON_SEDIT || STATE(d) == CON_CEDIT || STATE(d) == CON_PLAYING             \
     || STATE(d) == CON_TRIGEDIT || STATE(d) == CON_AEDIT || STATE(d) == CON_GEDIT || STATE(d) == CON_IEDIT            \
     || STATE(d) == CON_HEDIT || STATE(d) == CON_NEWSEDIT || STATE(d) == CON_POBJ)
#define IS_INMENU(d)                                                                                                   \
    (STATE(d) == CON_MENU || STATE(d) == CON_EXDESC || STATE(d) == CON_UMENU || STATE(d) == CON_GET_USER               \
     || STATE(d) == CON_GET_EMAIL || STATE(d) == CON_CHPWD_GETOLD || STATE(d) == CON_CHPWD_GETNEW                      \
     || STATE(d) == CON_CHPWD_VRFY || STATE(d) == CON_DELCNF1 || STATE(d) == CON_DELCNF2 || STATE(d) == CON_QRACE      \
     || STATE(d) == CON_QCLASS || STATE(d) == CON_CLASS_HELP || STATE(d) == CON_RACE_HELP || STATE(d) == CON_BONUS     \
...
#define IS_CORPSE(obj) (GET_OBJ_TYPE(obj) == ITEM_CONTAINER && GET_OBJ_VAL((obj), VAL_CONTAINER_CORPSE) == 1)
...
#define LIGHT_OK(sub)                                                                                                  \
    (!AFF_FLAGGED(sub, AFF_BLIND) && !PLR_FLAGGED(sub, PLR_EYEC)                                                       \
     && (IS_LIGHT(IN_ROOM(sub)) || AFF_FLAGGED((sub), AFF_INFRAVISION)                                                 \
         || (IS_MUTANT(sub) && (GET_GENOME(sub, 0) == 4 || GET_GENOME(sub, 1) == 4))                                   \
         || PLR_FLAGGED(sub, PLR_AURALIGHT)))

#define INVIS_OK(sub, obj) (!AFF_FLAGGED((obj), AFF_INVISIBLE) || AFF_FLAGGED(sub, AFF_DETECT_INVIS))
...
#define CAN_SEE(sub, obj)                                                                                              \
    (SELF(sub, obj)                                                                                                    \
     || ((GET_ADMLEVEL(sub) >= (IS_NPC(obj) ? 0 : GET_INVIS_LEV(obj))) && IMM_CAN_SEE(sub, obj)                        \
         && (NOT_HIDDEN(obj) || GET_ADMLEVEL(sub) > 0)))
...
#define MORT_CAN_SEE_OBJ(sub, obj)                                                                                     \
    ((LIGHT_OK(sub) || obj->carried_by == sub || obj->worn_by) && INVIS_OK_OBJ(sub, obj)                               \
...
#define CAN_GET_OBJ(ch, obj)                                                                                           \
    (CAN_WEAR((obj), ITEM_WEAR_TAKE) && !SITTING(obj) && CAN_CARRY_OBJ((ch), (obj)) && CAN_SEE_OBJ((ch), (obj)))

#define DISG(ch, vict)                                                                                                 \
    ((!PLR_FLAGGED(ch, PLR_DISGUISED)) || (PLR_FLAGGED(ch, PLR_DISGUISED) && (GET_ADMLEVEL(vict) > 0 || IS_NPC(vict))))
...
#define PERS(ch, vict)                                                                                                 \
    ((DISG(ch, vict)                                                                                                   \
              ? (CAN_SEE(vict, ch) ? (INTROD(vict, ch) ? (ISWIZ(ch, vict) ? GET_NAME(ch) : get_i_name(vict, ch))       \
                                                       : introd_calc(ch))                                              \
                                   : "Someone")                                                                        \
...
#define JUGGLERACE(ch)                                                                                                 \
    (IS_HOSHIJIN(ch) ? (GET_MIMIC(ch) > 0 ? pc_race_types[GET_MIMIC(ch) - 1] : pc_race_types[GET_RACE(ch)])            \
                     : handle_racial(ch, 1))
#define LRACE(ch) (JUGGLERACELOWER(ch))
#define JUGGLERACELOWER(ch)                                                                                            \
    (IS_HOSHIJIN(ch) ? (GET_MIMIC(ch) > 0 ? race_names[GET_MIMIC(ch) - 1] : race_names[GET_RACE(ch)])                  \
                     : handle_racial(ch, 0))
...
#define SPAR_TRAIN(ch)                                                                                                 \
    (FIGHTING(ch) && !IS_NPC(ch) && PLR_FLAGGED(ch, PLR_SPAR) && !IS_NPC(FIGHTING(ch))                                 \
     && PLR_FLAGGED(FIGHTING(ch), PLR_SPAR))
#define IS_NONPTRANS(ch)                                                                                               \
    (IS_HUMAN(ch)                                                                                                      \
     || ((IS_SAIYAN(ch) || IS_HALFBREED(ch)) && !IS_FULLPSSJ(ch) && !PLR_FLAGGED(ch, PLR_LSSJ)                         \
         && !PLR_FLAGGED(ch, PLR_OOZARU))                                                                              \
     || IS_NAMEK(ch) || IS_MUTANT(ch) || IS_ICER(ch) || IS_KAI(ch) || IS_KONATSU(ch) || IS_DEMON(ch)                   \
     || IS_KANASSAN(ch))
#define IS_FULLPSSJ(ch)                                                                                                \
    ((IS_SAIYAN(ch) && PLR_FLAGGED(ch, PLR_FPSSJ) && PLR_FLAGGED(ch, PLR_TRANS1))                                      \
     || (IS_HALFBREED(ch) && PLR_FLAGGED(ch, PLR_FPSSJ) && PLR_FLAGGED(ch, PLR_TRANS1)))
#define IS_TRANSFORMED(ch)                                                                                             \
    (PLR_FLAGGED(ch, PLR_TRANS1) || PLR_FLAGGED(ch, PLR_TRANS2) || PLR_FLAGGED(ch, PLR_TRANS3)                         \
...
#define ETHER_STREAM(ch)                                                                                               \
    (ROOM_FLAGGED(IN_ROOM(ch), ROOM_EARTH) || ROOM_FLAGGED(IN_ROOM(ch), ROOM_AETHER)                                   \
     || ROOM_FLAGGED(IN_ROOM(ch), ROOM_NAMEK) || PLANET_ZENITH(IN_ROOM(ch)))
#define HAS_MOON(ch)                                                                                                   \
    (ROOM_FLAGGED(IN_ROOM(ch), ROOM_VEGETA) || ROOM_FLAGGED(IN_ROOM(ch), ROOM_EARTH)                                   \
     || ROOM_FLAGGED(IN_ROOM(ch), ROOM_FRIGID) || ROOM_FLAGGED(IN_ROOM(ch), ROOM_AETHER))
#define HAS_ARMS(ch)                                                                                                   \
    (((IS_NPC(ch) && (MOB_FLAGGED(ch, MOB_LARM) || MOB_FLAGGED(ch, MOB_RARM))) || GET_LIMBCOND(ch, 1) > 0              \
      || GET_LIMBCOND(ch, 2) > 0 || PLR_FLAGGED(ch, PLR_CRARM) || PLR_FLAGGED(ch, PLR_CLARM))                          \
     && ((!GRAPPLING(ch) && !GRAPPLED(ch)) || (GRAPPLING(ch) && GRAPTYPE(ch) == 3)                                     \
         || (GRAPPLED(ch) && GRAPTYPE(ch) != 1 && GRAPTYPE(ch) != 4)))
#define HAS_LEGS(ch)                                                                                                   \
    (((IS_NPC(ch) && (MOB_FLAGGED(ch, MOB_LLEG) || MOB_FLAGGED(ch, MOB_RLEG))) || GET_LIMBCOND(ch, 3) > 0              \
      || GET_LIMBCOND(ch, 4) > 0 || PLR_FLAGGED(ch, PLR_CRLEG) || PLR_FLAGGED(ch, PLR_CLLEG))                          \
     && ((!GRAPPLING(ch) && !GRAPPLED(ch)) || (GRAPPLING(ch) && GRAPTYPE(ch) == 3)                                     \
...
#define OUTSIDE_ROOMFLAG(ch)                                                                                           \
    (!ROOM_FLAGGED(IN_ROOM(ch), ROOM_INDOORS) && !ROOM_FLAGGED(IN_ROOM(ch), ROOM_UNDERGROUND)                          \
     && !ROOM_FLAGGED(IN_ROOM(ch), ROOM_SPACE))

#define OUTSIDE_SECTTYPE(ch)                                                                                           \
    ((SECT(IN_ROOM(ch)) != SECT_INSIDE) && (SECT(IN_ROOM(ch)) != SECT_UNDERWATER)                                      \
     && (SECT(IN_ROOM(ch)) != SECT_IMPORTANT) && (SECT(IN_ROOM(ch)) != SECT_SHOP)                                      \
     && (SECT(IN_ROOM(ch)) != SECT_SPACE))

#define DIRT_ROOM(ch)                                                                                                  \
    (OUTSIDE_SECTTYPE(ch) && ((SECT(IN_ROOM(ch)) != SECT_WATER_NOSWIM) && (SECT(IN_ROOM(ch)) != SECT_WATER_SWIM)))
...
#define SET_FEAT(ch, i, value)                                                                                         \
    do {                                                                                                               \
        CHECK_PLAYER_SPECIAL((ch), (ch)->feats[i]) = value;                                                            \
    } while(0)
#define GET_SPELL_MASTERY_POINTS(ch) (ch->player_specials->spell_mastery_points)
#define GET_FEAT_POINTS(ch) (ch->player_specials->feat_points)
#define GET_EPIC_FEAT_POINTS(ch) (ch->player_specials->epic_feat_points)
#define GET_CLASS_FEATS(ch, cl) (ch->player_specials->class_feat_points[cl])
#define GET_EPIC_CLASS_FEATS(ch, cl) (ch->player_specials->epic_class_feat_points[cl])
...
#define IS_COLOR_CHAR(c)                                                                                               \
    (c == 'n' || c == 'b' || c == 'B' || c == 'c' || c == 'C' || c == 'g' || c == 'G' || c == 'm' || c == 'M'          \
     || c == 'r' || c == 'R' || c == 'y' || c == 'Y' || c == 'w' || c == 'W' || c == 'k' || c == 'K' || c == '0'       \
     || c == '2' || c == '3' || c == '4' || c == '5' || c == '6' || c == '7' || c == 'o' || c == 'e' || c == 'u'       \
     || c == 'l')
