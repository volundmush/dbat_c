...
static bool is_tell_ok(struct char_data *ch, struct char_data *vict);
...
ACMD(do_languages) {
    ...
    if (CONFIG_ENABLE_LANGUAGES) {
        ...
        if (!*arg)
            ...
            for (i = MIN_LANGUAGES; i <= MAX_LANGUAGES; i++) {
                if ((search_block(arg, languages, 0) == i - MIN_LANGUAGES) && GET_SKILL(ch, i)) {
                    SPEAKING(ch) = i;
    ...
    *buf2 = '\0';
    ...
    if (!*argument) {
        ...
        if (!sch) {
            ...
            act(buf2, 1, ch, 0, sch, TO_VICT);
            act(buf, 1, ch, 0, sch, TO_NOTVICT);
        }
            ...
            if (SHENRON == 1) {
                if (GET_ROOM_VNUM(IN_ROOM(ch)) == DRAGONR && GET_ROOM_VNUM(IN_ROOM(EDRAGON)) == DRAGONR) {
                    if (strstr(argument, "wish")) {
                        ...
                        if (granted == 0 && strstr(argument, "knowledge")) {
                            if (wch != NULL) {
                        ...
                        if (granted == 0 && strstr(argument, "speed")) {
                            if (wch != NULL) {
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a speed wish on %s.", GET_NAME(ch),
                                       GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end speed wish if */

                        if (granted == 0 && strstr(argument, "tough")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a tough wish on %s.", GET_NAME(ch),
                                       GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end tough wish if */

                        if (granted == 0 && strstr(argument, "strength")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a strength wish on %s.",
                                       GET_NAME(ch), GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end strength wish if */

                        if (granted == 0 && strstr(argument, "intelligence")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a intelligence wish on %s.",
                                       GET_NAME(ch), GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end intelligence wish if */

                        if (granted == 0 && strstr(argument, "wisdom")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a wisdom wish on %s.", GET_NAME(ch),
                                       GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end wisdom wish if */

                        if (granted == 0 && strstr(argument, "agility")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a agility wish on %s.",
                                       GET_NAME(ch), GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end agility wish if */

                        if (granted == 0 && strstr(argument, "constitution")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a constitutionwish on %s.",
                                       GET_NAME(ch), GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end constitution wish if */

                        if (granted == 0 && strstr(argument, "skill")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a skill wish on %s.", GET_NAME(ch),
                                       GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end skill wish if */
/* Rillao: transloc, add new transes here */
                        if (granted == 0 && strstr(argument, "power")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                        ...
                        if (granted == 0 && strstr(argument, "money")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a money wish on %s.", GET_NAME(ch),
                                       GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end money wish if */

                        if (granted == 0 && strstr(argument, "immunity")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a immunity wish on %s.",
                                       GET_NAME(ch), GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end money wish if */

                        if (granted == 0 && strstr(argument, "vitality")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                        ...
                        if (granted == 0 && strstr(argument, "revive")) {
                            int count = 0;
                            if (wch != NULL) {
                                count += 1;
                            }
                            ...
                            if (count == 1) {
                                if (!AFF_FLAGGED(wch, AFF_SPIRIT)) {
                                    ...
                                    granted = 1;
                                    SELFISHMETER -= 2;
                                    mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a revive wish on %s.",
                                           GET_NAME(ch), GET_NAME(wch));
                                }
                            } /* is there a target for the wish? */
                            if (count == 2) {
                            ...
                            if (count == 3) {
                                ...
                                if (!AFF_FLAGGED(wch3, AFF_SPIRIT)) {
                                    ...
                                    granted = 1;
                                    SELFISHMETER -= 3;
                                    mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a revive wish on %s and %s.",
                        ...
                        if (granted == 0 && strstr(argument, "immortal") && WISH[0] == 0) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 4;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a immortal wish on %s.",
                                       GET_NAME(ch), GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end immortal wish if */

                        if (granted == 0 && strstr(argument, "immortal") && WISH[0] == 1) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                        ...
                        if (granted == 0 && strstr(argument, " mortal")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                ...
                                granted = 1;
                                SELFISHMETER += 4;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a mortal wish on %s.", GET_NAME(ch),
                                       GET_NAME(wch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            } /* is there a target for the wish? */
                        } /* end mortal wish if */

                        if (granted == 0 && strstr(argument, "senzu")) {
                            if (wch != NULL) {
                                ...
                                granted = 1;
                                SELFISHMETER += 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a senzu wish.", GET_NAME(ch));
                            } /* is there a target for the wish? */
                        } /* end senzu wish if */

                        if (granted == 0 && strstr(argument, "roleplay")) {
                            if (wch != NULL) {
                                send_to_room(real_room(DRAGONR),
                                             "@wShenron says, '@CYour wish has been granted, %s!%s@w'@n\r\n",
                                             GET_NAME(wch), WISH[0] ? "" : " Now make your second wish.");
                                granted = 1;
                                mudlog(NRM, ADMLVL_GOD, 1, "Shenron: %s has made a roleplay wish.", GET_NAME(ch));
                                WAIT_STATE(ch, PULSE_4SEC);
                            }
                        }
    ...
    if (SHENRON == 0 || (SHENRON == 1 && IN_ROOM(ch) != real_room(DRAGONR))) {
        ...
        for (f = k->followers; f; f = f->next)
            if (AFF_FLAGGED(f->follower, AFF_GROUP) && (f->follower != ch) && AWAKE(f->follower)) {
                ...
                if (CONFIG_ENABLE_LANGUAGES) {
                    ...
                    act(blah, 1, ch, 0, f->follower, TO_VICT | TO_SLEEP);
                }
            }
...
static bool is_tell_ok(struct char_data *ch, struct char_data *vict) {
    ...
    if (ch == vict)
        ...
        act("$E's linkless at the moment.", 0, ch, 0, vict, TO_CHAR | TO_SLEEP);
    else if (PLR_FLAGGED(vict, PLR_WRITING))
        act("$E's writing a message right now; try again later.", 0, ch, 0, vict, TO_CHAR | TO_SLEEP);
    else if ((!IS_NPC(vict) && GET_ADMLEVEL(ch) < 1 && PRF_FLAGGED(vict, PRF_NOTELL)) ||
             ROOM_FLAGGED(IN_ROOM(vict), ROOM_SOUNDPROOF))
        act("$E can't hear you.", 0, ch, 0, vict, TO_CHAR | TO_SLEEP);
    else
        return true;
...
ACMD(do_tell) {
    ...
    if (!IS_NPC(ch)) {
        ...
        for (k = descriptor_list; k; k = k->next) {
            ...
            if (k->user == NULL)
                continue;
            if (found == 0 && !IS_NPC(ch) && (!strcasecmp(k->user, buf) || strstr(k->user, buf))) {
                vict = k->character;
                found = 1;
            } else if (!IS_NPC(ch) && found == 0 &&
...
ACMD(do_spec_comm) {
    ...
    if (!*buf || !*buf2)
        ...
        if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_NOREPEAT)) {
            ...
            act(blum, 1, ch, 0, vict, TO_CHAR);
        }

        if (subcmd == SCMD_WHISPER) {
            act(action_others, 0, ch, 0, vict, TO_NOTVICT);
...
ACMD(do_write) {
    ...
    if (obj) {                /* then there IS a board! */
        ...
        act("$n begins to write a note on $p.", 1, ch, obj, 0, TO_ROOM);
        return;
    }
            ...
            if (ADM_FLAGGED(ch, ADM_TELLALL)) {
                for (d = descriptor_list; d; d = d->next)
                    if (STATE(d) == CON_PLAYING && d->character)
                        act(buf, 0, ch, 0, d->character, TO_VICT);
            } else
    ...
    *buf2 = '\0';
    ...
    for (i = descriptor_list; i; i = i->next) {
        if (STATE(i) == CON_PLAYING && i != ch->desc && i->character &&
            ...
            msg = act(buf1, 0, ch, 0, i->character, TO_VICT | TO_SLEEP);
...
ACMD(do_qcomm) {
    ...
    if (!*argument)
        ...
        if (PRF_FLAGGED(ch, PRF_NOREPEAT))
            ...
            act(buf, 0, ch, 0, argument, TO_CHAR);
        } else
            act(argument, 0, ch, 0, argument, TO_CHAR);
