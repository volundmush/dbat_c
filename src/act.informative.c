...
static void search_room(struct char_data *ch) {
    ...
    reveal_hiding(ch, 0);
    act("@y$n@Y begins searching the room carefully.@n", 1, ch, 0, 0, TO_ROOM);
    ...
    for (vict = world[IN_ROOM(ch)].people; vict; vict = next_v) {
        ...
        if (AFF_FLAGGED(vict, AFF_HIDE) && vict != ch) {
            ...
            if (perc * bonus >= prob * terrain) { /* Found them. */
                act("@YYou find @y$N@Y hiding nearby!@n", 1, ch, 0, vict, TO_CHAR);
                act("@y$n@Y has found your hiding spot!@n", 1, ch, 0, vict, TO_VICT);
                act("@y$n@Y has found @y$N's@Y hiding spot!@n", 1, ch, 0, vict, TO_NOTVICT);
    ...
    for (obj = world[IN_ROOM(ch)].contents; obj; obj = obj->next_content) {
        if (OBJ_FLAGGED(obj, ITEM_BURIED) && perc * bonus > rand_number(50, 200)) {
            act("@YYou uncover @y$p@Y, which had been burried here.@n", 1, ch, obj, 0, TO_CHAR);
            act("@y$n@Y uncovers @y$p@Y, which had burried here.@n", 1, ch, obj, 0, TO_ROOM);
...
ACMD(do_mimic) {
    ...
    if (israce == 1) {
        ...
        act("@mYou concentrate for a moment and release the illusion that was mimicing another race.@n", 1, ch, 0, 0,
            TO_CHAR);
        act("@M$n@m concentrates for a moment and SUDDENLY $s appearance changes some what!@n", 1, ch, 0, 0,
            TO_ROOM);
        GET_MIMIC(ch) = 0;
    } else {
        send_to_char(ch,
...
ACMD(do_kyodaika) {
    ...
    if (GET_GENOME(ch, 0) == 0) {
        act("@GYou growl as your body grows to ten times its normal size!@n", 1, ch, 0, 0, TO_CHAR);
        act("@g$n@G growls as $s body grows to ten times its normal size!@n", 1, ch, 0, 0, TO_ROOM);
        ...
        act("@GYou growl as your body shrinks to its normal size!@n", 1, ch, 0, 0, TO_CHAR);
        act("@g$n@G growls as $s body shrinks to its normal size!@n", 1, ch, 0, 0, TO_ROOM);
        send_to_char(ch, "@cStrength@D: @C-5\r\n@cSpeed@D: @c+2@n\r\n");
        ch->real_abils.str -= 5;
        ch->real_abils.cha += 2;
        GET_GENOME(ch, 0) = 0;
...
ACMD(do_shuffle) {
    ...
    act("$n shuffles their deck.", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_hand) {
    ...
    if (!strcasecmp("look", arg)) {
        ...
        act("$n looks at $s hand.", 1, ch, 0, 0, TO_ROOM);
        if (count == 0) {
            send_to_char(ch, "No cards.");
            act("There were no cards.", 1, ch, 0, 0, TO_ROOM);
        } else if (count > 7) {
            act("You have more than seven cards in your hand.", 1, ch, 0, 0, TO_CHAR);
            act("$n has more than seven cards in $s hand.", 1, ch, 0, 0, TO_ROOM);
        } else {
            char buf[200];
        ...
        for (obj = ch->carrying; obj; obj = obj->next_content) {
            ...
            if (obj) {
                count += 1;
                act("$p", 1, ch, obj, 0, TO_ROOM);
            }
        }
        if (count == 0) {
            act("No cards.", 1, ch, 0, 0, TO_ROOM);
        }
        if (count > 7) {
            act("You have more than seven cards in your hand.", 1, ch, 0, 0, TO_CHAR);
            act("$n has more than seven cards in $s hand.", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_post) {
    ...
    if (!*arg2) {
        ...
        act("@WYou post $p@W on a nearby structure.@n", 1, ch, obj, 0, TO_CHAR);
        act("@C$n@W posts $p@W on a nearby structure.@n", 1, ch, obj, 0, TO_ROOM);
        obj_from_char(obj);
...
ACMD(do_play) {
    ...
    act("You play $p on your table.", 1, ch, obj, 0, TO_CHAR);
    act("$n plays $p on $s table.", 1, ch, obj, 0, TO_ROOM);
    obj_from_char(obj);
    obj_to_obj(obj, obj2);
}
...
ACMD(do_showoff) {
    ...
    *arg2 = '\0';
    ...
    if (!(obj = get_obj_in_list_vis(ch, arg, NULL, ch->carrying))) {
        ...
        act("@WYou hold up $p@W for @C$N@W to see:@n", 1, ch, obj, vict, TO_CHAR);
        act("@C$n@W holds up $p@W for you to see:@n", 1, ch, obj, vict, TO_VICT);
        act("@C$n@W holds up $p@W for @c$N@W to see.@n", 1, ch, obj, vict, TO_NOTVICT);
...
ACMD(do_intro) {
    ...
    if (readIntro(vict, ch) == 2) {
        ...
        act("You decide to call $M, $N.", 1, ch, 0, vict, TO_CHAR);
        act("$n seems to decide something about you.", 1, ch, 0, vict, TO_VICT);
        act("$n seems to decide something about $N.", 1, ch, 0, vict, TO_NOTVICT);
        return;
    }
}
...
static void show_obj_to_char(struct obj_data *obj, struct char_data *ch, int mode) {
     ...
     */
            ...
            if (OBJ_FLAGGED(obj, ITEM_BURIED)) {
                ...
                if (spotted == 1 && SECT(IN_ROOM(obj)) != SECT_DESERT) {
                    send_to_char(ch, "@yA %s soft dirt is here.@n\r\n", bury);
                    return;
                } else if (spotted == 1 && SECT(IN_ROOM(obj)) == SECT_DESERT) {
...
static void look_at_char(struct char_data *i, struct char_data *ch) {
    ...
    send_to_char(ch, "\r\n");
    if (GET_CLAN(i) != NULL && strstr(GET_CLAN(i), "None") == 0) {
    ...
    if ((!PLR_FLAGGED(i, PLR_DISGUISED) && (readIntro(ch, i) == 1 && !IS_NPC(i)))) {
        ...
        if (GET_SEX(i) == SEX_NEUTRAL)
            send_to_char(ch, "%c%s appears to be %s %s, ", toupper(*GET_NAME(i)), GET_NAME(i) + 1, AN(RACE(i)),
                         LRACE(i));
        else
            send_to_char(ch, "%c%s appears to be %s %s %s, ", toupper(*GET_NAME(i)), GET_NAME(i) + 1, AN(MAFE(i)),
                         MAFE(i), LRACE(i));
    } else {
        if (GET_SEX(i) == SEX_NEUTRAL)
    ...
    if (found && (!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_NOEQSEE))) {
        ...
        if (!PLR_FLAGGED(i, PLR_DISGUISED)) {
            act("$n is using:", 0, i, 0, ch, TO_VICT);
        } else {
            act("The disguised person is using:", 0, i, 0, ch, TO_VICT);
        }
    ...
    if (ch != i && ((GET_SKILL(ch, SKILL_KEEN) && AFF_FLAGGED(ch, AFF_SNEAK)) || GET_ADMLEVEL(ch))) {
        found = 0;
        act("\r\nYou attempt to peek at $s inventory:", 0, i, 0, ch, TO_VICT);
        if (CAN_SEE(i, ch))
            act("$n tries to evaluate what you have in your inventory.", 1, ch, 0, i, TO_VICT);
        if (GET_SKILL(ch, SKILL_KEEN) > axion_dice(0) && (!IS_NPC(i) || GET_ADMLEVEL(ch) > 1)) {
            ...
            act("You are unsure about $s inventory.", 0, i, 0, ch, TO_VICT);
            if (CAN_SEE(i, ch))
                act("$n didn't seem to get a good enough look.", 1, ch, 0, i, TO_VICT);
            improve_skill(ch, SKILL_KEEN, 1);
            return;
        }
        if (!found) {
...
static void list_one_char(struct char_data *i, struct char_data *ch) {
    ...
    if (IS_NPC(i) && i->long_descr && GET_POS(i) == GET_DEFAULT_POS(i) && !FIGHTING(i)) {
        ...
        if (IS_NPC(i) && GET_HIT(i) >= gear_pl(i) * .9 && GET_HIT(i) != gear_pl(i))
            act("@R...Some slight wounds on $s body.@w", 1, i, 0, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= gear_pl(i) * .8 && GET_HIT(i) < gear_pl(i) * .9)
            act("@R...A few wounds on $s body.@w", 1, i, 0, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= gear_pl(i) * .7 && GET_HIT(i) < gear_pl(i) * .8)
            act("@R...Many wounds on $s body.@w", 1, i, 0, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= gear_pl(i) * .6 && GET_HIT(i) < gear_pl(i) * .7)
            act("@R...Quite a few wounds on $s body.@w", 1, i, 0, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= gear_pl(i) * .5 && GET_HIT(i) < gear_pl(i) * .6)
            act("@R...Horrible wounds on $s body.@w", 1, i, 0, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= gear_pl(i) * .4 && GET_HIT(i) < gear_pl(i) * .5)
            act("@R...Blood is seeping from the wounds on $s body.@w", 1, i, 0, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= gear_pl(i) * .3 && GET_HIT(i) < gear_pl(i) * .4)
            act("@R...$s body is in terrible shape.@w", 1, i, 0, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= gear_pl(i) * .2 && GET_HIT(i) < gear_pl(i) * .3)
            act("@R...Is absolutely covered in wounds.@w", 1, i, 0, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= gear_pl(i) * .1 && GET_HIT(i) < gear_pl(i) * .2)
            act("@R...Is on $s last leg.@w", 1, i, 0, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) < gear_pl(i) * .1)
            act("@R...Should be DEAD soon.@w", 1, i, 0, ch, TO_VICT);


        if (GET_EAVESDROP(i) > 0) {
            char eaves[300];
            sprintf(eaves, "@w...$e is spying on everything to the @c%s@w.", dirs[GET_EAVESDIR(i)]);
            act(eaves, 1, i, 0, ch, TO_VICT);
        }
        if (AFF_FLAGGED(i, AFF_FLYING) && GET_ALT(i) == 1)
            act("...$e is in the air!", 0, i, 0, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_FLYING) && GET_ALT(i) == 2)
            act("...$e is high in the air!", 0, i, 0, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_SANCTUARY) && !GET_SKILL(i, SKILL_AQUA_BARRIER))
            act("...$e has a barrier around $s body!", 0, i, 0, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_FIRESHIELD))
            act("...$e has @rf@Rl@Ya@rm@Re@Ys@w around $s body!", 0, i, 0, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_SANCTUARY) && GET_SKILL(i, SKILL_AQUA_BARRIER))
            act("...$e has a @Gbarrier@w of @cwater@w and @Cki@w around $s body!", 0, i, 0, ch, TO_VICT);
        if (!IS_NPC(i) && PLR_FLAGGED(i, PLR_SPIRAL))
            act("...$e is spinning in a vortex!", 0, i, 0, ch, TO_VICT);
        if (GET_CHARGE(i))
            act("...$e has a bright %s aura around $s body!", 0, i, 0, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_METAMORPH))
            act("@w...$e has a dark, @rred@w aura and menacing presence.", 0, i, 0, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_HAYASA))
            act("@w...$e has a soft @cblue@w glow around $s body!", 0, i, 0, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_BLIND))
            act("...$e is groping around blindly!", 0, i, 0, ch, TO_VICT);
        if (affected_by_spell(i, SPELL_FAERIE_FIRE))
            act("@m...$e @mis outlined with purple fire!@m", 0, i, 0, ch, TO_VICT);
        if (GET_FEATURE(i)) {
            char woo[MAX_STRING_LENGTH];
            sprintf(woo, "@C%s@n", GET_FEATURE(i));
            act(woo, 0, i, 0, ch, TO_VICT);
        }
    ...
    if (IS_NPC(i) && !FIGHTING(i) && GET_POS(i) != POS_SITTING && GET_POS(i) != POS_SLEEPING)
        ...
        send_to_char(ch, "@w%c%s is being absorbed from by %s!", toupper(*i->short_descr), i->short_descr + 1,
        ...
        send_to_char(ch, "@w%c%s is being absorbed from by %s!", toupper(*i->short_descr), i->short_descr + 1,
             ...
             is_sparring(i))
        send_to_char(ch, "@w%c%s is sparring with %s!", toupper(*i->short_descr), i->short_descr + 1,
        ...
        send_to_char(ch, "@w%c%s is fighting %s!", toupper(*i->short_descr), i->short_descr + 1,
                     GET_ADMLEVEL(ch) ? GET_NAME(FIGHTING(i)) : (readIntro(ch, FIGHTING(i)) == 1 ? get_i_name(ch,
                                                                                                              FIGHTING(
                                                                                                                      i))
                                                                                                 : LRACE(FIGHTING(i))));
    ...
    if (!IS_NPC(i)) {
        if (PLR_FLAGGED(i, PLR_FISHING)) {
            act("@w...$e is @Cfishing@w.@n", 1, i, 0, ch, TO_VICT);
        }
    }
    if (PLR_FLAGGED(i, PLR_AURALIGHT)) {
        ...
        act(bloom, 1, i, 0, ch, TO_VICT);
    }
    if (AFF_FLAGGED(i, AFF_SANCTUARY) && !GET_SKILL(i, SKILL_AQUA_BARRIER))
        act("@w...$e has a @bbarrier@w around $s body!", 1, i, 0, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_FIRESHIELD))
        act("@w...$e has @rf@Rl@Ya@rm@Re@Ys@w around $s body!", 0, i, 0, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_HEALGLOW))
        act("@w...$e has a serene @Cblue@Y glow@w around $s body.", 1, i, 0, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_EARMOR))
        act("@w...$e has ghostly @Ggreen@w ethereal armor around $s body.", 1, i, 0, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_SANCTUARY) && GET_SKILL(i, SKILL_AQUA_BARRIER))
        act("@w...$e has a @bbarrier@w of @cwater@w and @CKi@w around $s body!", 1, i, 0, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_FLYING) && GET_ALT(i) == 1)
        act("@w...$e is in the air!", 1, i, 0, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_FLYING) && GET_ALT(i) == 2)
        act("@w...$e is high in the air!", 1, i, 0, ch, TO_VICT);
    if (GET_KAIOKEN(i) > 0)
        act("@w...@r$e has a red aura around $s body!", 1, i, 0, ch, TO_VICT);
    if (!IS_NPC(i) && PLR_FLAGGED(i, PLR_SPIRAL))
        act("@w...$e is spinning in a vortex!", 0, i, 0, ch, TO_VICT);
    if (IS_TRANSFORMED(i) && !IS_ANDROID(i) && !IS_SAIYAN(i) && !IS_HALFBREED(i))
        act("@w...$e has energy crackling around $s body!", 1, i, 0, ch, TO_VICT);
    if (GET_CHARGE(i) && !IS_SAIYAN(i) && !IS_HALFBREED(i)) {
        ...
        act(aura, 1, i, 0, ch, TO_VICT);
    }
    if (!PLR_FLAGGED(i, PLR_OOZARU) && GET_CHARGE(i) && IS_TRANSFORMED(i) && (IS_SAIYAN(i) || IS_HALFBREED(i)))
        act("@w...$e has a @Ybright @Yg@yo@Yl@yd@Ye@yn@w aura around $s body!", 1, i, 0, ch, TO_VICT);
    if (!PLR_FLAGGED(i, PLR_OOZARU) && GET_CHARGE(i) && !IS_TRANSFORMED(i) && (IS_SAIYAN(i) || IS_HALFBREED(i))) {
        char aura[MAX_INPUT_LENGTH];
        sprintf(aura, "@w...$e has a @Ybright@w %s aura around $s body!", aura_types[GET_AURA(i)]);
        act(aura, 1, i, 0, ch, TO_VICT);
    ...
    if (PLR_FLAGGED(i, PLR_OOZARU) && GET_CHARGE(i) && (IS_SAIYAN(i) || IS_HALFBREED(i)))
        act("@w...$e is in the form of a @rgreat ape@w!", 1, i, 0, ch, TO_VICT);
    if (GET_GENOME(i, 0) == 11)
        act("@w...$e has expanded $s body size@w!", 1, i, 0, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_HAYASA))
        act("@w...$e has a soft @cblue@w glow around $s body!", 0, i, 0, ch, TO_VICT);
    if (PLR_FLAGGED(i, PLR_OOZARU) && !GET_CHARGE(i) && (IS_SAIYAN(i) || IS_HALFBREED(i)))
        act("@w...$e has energy crackling around $s @rgreat ape@w body!", 1, i, 0, ch, TO_VICT);
    if (GET_FEATURE(i)) {
        char woo[MAX_STRING_LENGTH];
        sprintf(woo, "@C%s@n", GET_FEATURE(i));
        act(woo, 0, i, 0, ch, TO_VICT);
...
static void do_auto_exits(room_rnum target_room, struct char_data *ch, int exit_mode) {
    int door, door_found = 0, has_light = 0, i;
    char dlist1[500];
    char dlist2[500];
    char dlist3[500];
    char dlist4[500];
    ...
    *dlist12 = '\0';
    ...
    if (exit_mode == EXIT_NORMAL && space == 0 && IN_ROOM(ch) == target_room) {
    ...
    if (exit_mode == EXIT_NORMAL && space == 1) {
        /* printmap */
    ...
    if (exit_mode == EXIT_COMPLETE || (exit_mode == EXIT_NORMAL && space == 0 && IN_ROOM(ch) != target_room)) {
...
ACMD(do_autoexit) {
    ...
    if (((tp = search_block(arg, exitlevels, 0)) == -1)) {
...
void look_at_room(room_rnum target_room, struct char_data *ch, int ignore_brief) {
    ...
    list_obj_to_char(world[target_room].contents, ch, SHOW_OBJ_LONG, 0);
...
*/
        ...
        if (OBJVAL_FLAGGED(obj, CONT_CLOSED))
            ...
            if (GET_OBJ_TYPE(obj) == ITEM_CONTAINER &&
                ...
                act("$n looks in $p.", 1, ch, obj, 0, TO_ROOM);
            }
            switch (bits) {
                case FIND_OBJ_INV:
            ...
            list_obj_to_char(obj->contains, ch, SHOW_OBJ_SHORT, 1);
        }
...
static void look_at_target(struct char_data *ch, char *arg, int cmread) {
    int bits, found = 0, j, fnum, i = 0, msg = 1;
        ...
        if (found_char != NULL) {
            ...
            if (ch != found_char) {
                if (!AFF_FLAGGED(ch, AFF_HIDE)) {
                    act("$n looks at you.", 1, ch, 0, found_char, TO_VICT);
                    act("$n looks at $N.", 1, ch, 0, found_char, TO_NOTVICT);
                }
            }
            return;
        }
    ...
    if (!viewport) {
        ...
        if (target_room == NOWHERE) {
            ...
            if (viewport->action_description)
                act(viewport->action_description, 1, ch, viewport, 0, TO_CHAR);
            else
                act("$n looks out the window.", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_rptrans) {
    ...
    mudlog(NRM, MAX(ADMLVL_IMPL, GET_INVIS_LEV(ch)), 1, "EXCHANGE: %s gave %d RPP to user %s", GET_NAME(ch), amt,
           arg);
    save_char(ch);
}
...
ACMD(do_rpbank) {
    ...
    mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1, "RPP Bank: %s has put %d RPP into their bank",
           GET_NAME(ch), amt);
...
ACMD(do_rbanktrans) {
    ...
    send_to_char(ch, "@wWELL @xGOLLY @xGEE @xWILLICKERS@x! @wYOU @RZ@YIM @RZ@YAMMED @C%d @wRIPROOZLES TO @C%s!@n\r\n",
                 amt, CAP(arg));
    mudlog(NRM, MAX(ADMLVL_IMPL, GET_INVIS_LEV(ch)), 1, "EXCHANGE: %s gave %d Banked RPP to user %s", GET_NAME(ch),
           amt, arg);
    save_char(ch);
}
        ...
        if (!*arg) {
            if (subcmd == SCMD_SEARCH) {
                ...
                if (GET_ADMLEVEL(ch) < 1 && !AFF_FLAGGED(ch, AFF_HIDE)) {
                    act("@w$n@w looks around the room.@n", 1, ch, 0, 0, TO_ROOM);
                }
            }
            ...
            look_out_window(ch, arg2);
        } else if ((look_type = search_block(arg, dirs, 0)) >= 0 ||
                   (look_type = search_block(arg, abbr_dirs, 0)) >= 0) {
            if (subcmd == SCMD_SEARCH)
                search_in_direction(ch, look_type);
            else
                look_in_direction(ch, look_type);
        } else if ((is_abbrev(arg, "towards")) &&
                   ((look_type = search_block(arg2, dirs, 0)) >= 0 ||
                    (look_type = search_block(arg2, abbr_dirs, 0)) >= 0)) {
            if (subcmd == SCMD_SEARCH)
                search_in_direction(ch, look_type);
            else
                look_in_direction(ch, look_type);
...
ACMD(do_score) {
    ...
    if (view == full || view == personal) {
        ...
        send_to_char(ch, "  @D|  @CRace@D: @W%10s@D,  @CSensei@D: @W%15s@D,     @CArt@D: @W%-17s@D|@n\n",
                     pc_race_types[(int) GET_RACE(ch)], pc_class_types[(int) GET_CLASS(ch)],
    ...
    if (view == full || view == health) {
        ...
        if (!IS_ANDROID(ch) && GET_LIFEFORCE(ch) > 0) {
            ...
            send_to_char(ch, "    @wLife Force@D-[@C%16s@D%s@c%16s@D]- @wLife Percent@D-[@Y%3d%s@D]@n\n", add_commas(0),
                         "/", add_commas(GET_LIFEMAX(ch)), GET_LIFEPERC(ch), "%");
        }
    }
    if (view == full || view == stats) {
        ...
        send_to_char(ch,
                     "        @D[    @RStrength@D|@G%3d@D] [     @YAgility@D|@G%3d@D] [       @BSpeed@D|@G%3d@D]@n\n",
                     GET_STR(ch), GET_DEX(ch), GET_CHA(ch));
        send_to_char(ch,
                     "        @D[@gConstitution@D|@G%3d@D] [@CIntelligence@D|@G%3d@D] [      @MWisdom@D|@G%3d@D]@n\n",
...
ACMD(do_inventory) {
    ...
    list_obj_to_char(ch->carrying, ch, SHOW_OBJ_SHORT, 1);
    send_to_char(ch, "\n");
}
...
ACMD(do_help) {
    ...
    if ((mid = search_help(argument, GET_ADMLEVEL(ch))) == NOWHERE) {
        ...
        if (GET_ADMLEVEL(ch) < 3) {
            mudlog(NRM, MAX(ADMLVL_IMPL, GET_INVIS_LEV(ch)), 1, "%s tried to get help on %s", GET_NAME(ch),
                   argument);
        }
            ...
            if (short_list) {
                ...
                send_to_char(ch, "%s               @D<@C%-12s@D> %s@w%s", line_color,
                             GET_ADMLEVEL(ch) > 0 ? GET_NAME(tch) : (GET_ADMLEVEL(tch) > 0 ? GET_NAME(tch) : (GET_USER(
                                     tch) ? GET_USER(tch) : "NULL")), GET_ADMLEVEL(ch) > 0 ? usr : "", line_color);
...
static void perform_immort_where(struct char_data *ch, char *arg) {
    ...
    if (!*arg) {
        ...
        mudlog(NRM, MAX(ADMLVL_GRGOD, GET_INVIS_LEV(ch)), 1, "GODCMD: %s has checked where for the location of %s",
               GET_NAME(ch), arg);
        for (i = character_list; i; i = i->next) {
        ...
        for (k = object_list; k; k = k->next)
            if (CAN_SEE_OBJ(ch, k) && isname(arg, k->name)) {
                found = 1;
                print_object_location(++num, k, ch, 1);
            }
        if (!found) {
...
ACMD(do_color) {
    ...
    if (((tp = search_block(arg, ctypes, 0)) == -1)) {
        send_to_char(ch, "Usage: color [ off | on ]\r\n");
        return;
    }
    switch (tp) {
...
ACMD(do_history) {
    ...
    type = search_block(arg, history_types, 0);
    if (!*arg || type < 0) {
        int i;
    ...
    for (i = 0; i < 10; i++) {
        if (EXIT(ch, i)) {
            ...
            if (CAN_GO(ch, i)) {
                ...
                list_obj_to_char(world[newroom].contents, ch, SHOW_OBJ_LONG, 0);
    ...
    if (!*argument) {
        ...
        if (load_char(argument, victim) >= 0) {
            ...
            if (GET_ADMLEVEL(victim) >= ADMLVL_IMMORT) {
                ...
                send_to_char(ch,
                             "@cName  @D: @w%s\r\n@cSensei@D: @w%s\r\n@cRace  @D: @w%s\r\n@cTitle @D: @w%s@n\r\n@cClan  @D: @w%s@n\r\n",
                             GET_NAME(victim), pc_class_types[(int) GET_CLASS(victim)],
                             pc_race_types[(int) GET_RACE(victim)], GET_TITLE(victim), clan ? buf : "None.");
                if (clan == 1 && !strstr(GET_CLAN(victim), "Applying")) {
                    if (checkCLAN(victim) == 1) {
...
static void search_in_direction(struct char_data *ch, int dir) {
    int check = 0, skill_lvl, dchide = 20;

    send_to_char(ch, "You search for secret doors.\r\n");
    act("$n searches the area intently.", 1, ch, 0, 0, TO_ROOM);
