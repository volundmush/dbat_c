...
static bool can_take_obj(struct char_data *ch, struct obj_data *obj);
...
static bool can_harvest(struct obj_data *plant);
...
static void harvest_plant(struct char_data *ch, struct obj_data *plant) {
    int extract = 0, reward = rand_number(5, 15), count = reward;
    ...
    if (*arg) {
        if (!strcasecmp(arg, "collect")) {
            ...
            if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_FERTILE1)) {
                ...
                obj_to_char(soil, ch);
                act("@yYou sink your shovel into the soft ground and manage to dig up a pile of fertile soil!@n", 1,
                    ch, 0, 0, TO_CHAR);
                act("@w$n@y sinks $s shovel into the soft ground and manages to dig up a pile of fertile soil!@n", 1,
                    ch, 0, 0, TO_ROOM);
                ...
                struct obj_data *soil = read_object(255, VIRTUAL);
                obj_to_char(soil, ch);
                act("@yYou sink your shovel into the soft ground and manage to dig up a pile of good soil!@n", 1, ch,
                    0, 0, TO_CHAR);
                act("@w$n@y sinks $s shovel into the soft ground and manages to dig up a pile of good soil!@n", 1,
                    ch, 0, 0, TO_ROOM);
                ...
                struct obj_data *soil = read_object(255, VIRTUAL);
                obj_to_char(soil, ch);
                act("@yYou sink your shovel into the soft ground and manage to dig up a pile of soil!@n", 1, ch, 0,
                    0, TO_CHAR);
                act("@w$n@y sinks $s shovel into the soft ground and manages to dig up a pile of soil!@n", 1, ch, 0,
                    0, TO_ROOM);
    ...
    if (GET_MOVE(ch) < cost) {
        ...
        if (!strcasecmp(arg2, "water")) {
            ...
            if (!water) {
                ...
                act("@GAs you go to water @g$p@G you end up sloppily wasting about half of it on the ground.@n", 1,
                    ch, obj, 0, TO_CHAR);
                act("@g$n@G takes a bottle of grow water and sloshes some of it on @g$p@G.@n", 1, ch, obj, 0,
                    TO_ROOM);
                GET_MOVE(ch) -= cost;
                ...
                act("@GYou calmly and expertly pour the grow water on @g$p@G.@n", 1, ch, obj, 0, TO_CHAR);
                act("@g$n@G calmly and expertly pours some grow water on @g$p@G.@n", 1, ch, obj, 0, TO_ROOM);
                GET_MOVE(ch) -= cost;
            ...
            if (!clippers) {
                ...
                act("@GAs you go to harvest @g$p@G you end up cutting it in half instead!@n", 1, ch, obj, 0,
                    TO_CHAR);
                ...
                act("@GYou calmly and expertly harvest @g$p@G.@n", 1, ch, obj, 0, TO_CHAR);
                act("@g$n@G calmly and expertly harvests @g$p@G.@n", 1, ch, obj, 0, TO_ROOM);
                GET_MOVE(ch) -= cost;
            ...
            if (!shovel) {
                ...
                act("@GYou calmly dig up @g$p@G.@n", 1, ch, obj, 0, TO_CHAR);
                act("@g$n@G calmly digs up @g$p@G.@n", 1, ch, obj, 0, TO_ROOM);
                GET_MOVE(ch) -= cost;
                obj_from_room(obj);
                obj_to_char(obj, ch);
            ...
            if (!soil) {
                ...
                act("@GYou end up digging a hole too shallow to hold @g$p@G. Better try again.@n", 1, ch, obj, 0,
                    TO_CHAR);
                ...
                act("@GYou dig a proper sized hole and plant @g$p@G in it.@n", 1, ch, obj, 0, TO_CHAR);
                act("@g$n@G digs a proper sized hole in a planter and plants @g$p@G in it.@n", 1, ch, obj, 0,
                    TO_ROOM);
                obj_from_char(obj);
            ...
            if (!OBJ_FLAGGED(obj, ITEM_MATURE)) {
                ...
                act("@GYou end up shredding @g$p@G with your clumsy unskilled hands.@n", 1, ch, obj, 0, TO_CHAR);
                act("@g$n@G grabs a hold of @g$p@G and shreds it in an attempt to pick it.@n", 1, ch, obj, 0,
                    TO_ROOM);
                return;
            } else {
                act("@GYou grab a hold of @g$p@G and carefully pick it out of the soil.@n", 1, ch, obj, 0, TO_CHAR);
                act("@g$n@G grabs a hold of @g$p@G and carefully picks it out of the soil.@n", 1, ch, obj, 0,
                    TO_ROOM);
                obj_from_room(obj);
                obj_to_char(obj, ch);
                GET_MOVE(ch) -= cost;
...
ACMD(do_deploy) {
    ...
    if (capsule == 0) {
        ...
        return;
    } else if (GET_RP(ch) < 10 && furniture == 0) {
        send_to_char(ch, "You are required to have (not spend) 10 RPP in order to place a house.\r\n");
        return;
    } else if (furniture == 1 && (!ROOM_FLAGGED(IN_ROOM(ch), ROOM_HOUSE) || ROOM_FLAGGED(IN_ROOM(ch), ROOM_SHIP))) {
        ...
        send_to_char(ch, "You can't deploy house furniture capsules here.\r\n");
        return;
    } else if (furniture == 0 && (SECT(IN_ROOM(ch)) == SECT_INSIDE || SECT(IN_ROOM(ch)) == SECT_WATER_NOSWIM ||
    ...
    int rnum = 18800, giveup = 0, cont = 0, found = 0, type = 0;
...
ACMD(do_twohand) {
    ...
    if (!GET_EQ(ch, WEAR_WIELD1) && !PLR_FLAGGED(ch, PLR_THANDW)) {
        ...
        act("$n stops wielding $s weapon with both hands.", 1, ch, 0, 0, TO_ROOM);
        REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_THANDW);
        return;
    } else {
        send_to_char(ch, "You grab your weapon with both hands.\r\n");
        act("$n starts wielding $s weapon with both hands.", 1, ch, 0, 0, TO_ROOM);
...
void dball_load() {
    ...
    int found4 = 0, found5 = 0, load = 0, num = -1;
    int found6 = 0, found7 = 0, room = 0, loaded = 0;
    int hunter1 = 0, hunter2 = 0;
    struct obj_data *k;
...
ACMD(do_bid) {
    ...
    char arg[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
    int found = 0, list = 0, masterList = 0;
    room_vnum auct_room;
    ...
    if (!strcasecmp(arg, "list")) {
            ...
            if (obj) {
                ...
                if (GET_AUCTIME(obj) + 86400 > time(0) && GET_CURBID(obj) <= -1) {
                    send_to_char(ch,
                                 ...
                                 list, get_name_by_id(GET_AUCTER(obj)) != NULL ? CAP(get_name_by_id(GET_AUCTER(obj)))
                                                                               : "Nobody",
                    ...
                    send_to_char(ch,
                                 ...
                                 list, get_name_by_id(GET_AUCTER(obj)) != NULL ? CAP(get_name_by_id(GET_AUCTER(obj)))
                                                                               : "Nobody",
                                 count_color_chars(obj->short_description) + 30, obj->short_description,
                                 get_name_by_id(GET_CURBID(obj)) != NULL ? CAP(get_name_by_id(GET_CURBID(obj)))
        ...
        if (!obj2) {
            ...
            if (GET_SKILL(ch, SKILL_APPRAISE) < rand_number(1, 101)) {
                ...
                act("@c$n@w looks stumped about something they viewed on their scouter screen.@n", 1, ch, 0, 0,
                    TO_ROOM);
                WAIT_STATE(ch, PULSE_2SEC);
                return;
            } else {
                send_to_char(ch, "You look at images of the object on your scouter.\r\n");
                act("@c$n@w looks at something on their scouter screen.@n", 1, ch, 0, 0, TO_ROOM);
                send_to_char(ch, "@c------------------------------------------------------------------------\n");
                send_to_char(ch, "@GOwner       @W: @w%s@n\n",
                             get_name_by_id(GET_AUCTER(obj2)) != NULL ? CAP(get_name_by_id(GET_AUCTER(obj2)))
                                                                      : "Nobody");
...
static void auc_stat(struct char_data *ch, struct obj_data *obj) {
    ...
    if (aucstat == AUC_NULL_STATE) {
        ...
        act(buf, 1, ch_selling, obj, ch, TO_VICT | TO_SLEEP);
        GET_GOLD(ch) -= 500;
...
static void auc_send_to_all(char *messg, bool buyer) {
    ...
    for (i = descriptor_list; i; i = i->next) {
        ...
        if (ROOM_FLAGGED(IN_ROOM(i->character), ROOM_PAST))
            continue;
        if (buyer)
            act(messg, 1, ch_buying, obj_selling, i->character, TO_VICT | TO_SLEEP);
        else
            act(messg, 1, ch_selling, obj_selling, i->character, TO_VICT | TO_SLEEP);
...
ACMD(do_assemble) {
    ...
    if (*argument == '\0') {
        ...
        send_to_char(ch, "You can't %s %s %s.\r\n", CMD_NAME, AN(argument), argument);
        return;
    } else if (!assemblyCheckComponents(lVnum, ch, 0)) {
    ...
    if (tool) {
        act("@WYou open up your toolkit and take out the necessary tools.@n", 1, ch, 0, 0, TO_CHAR);
        act("@C$n@W opens up $s toolkit and takes out the necessary tools.@n", 1, ch, 0, 0, TO_ROOM);
    } else {
    ...
    if (strcasecmp(argument, "campfire")) {
        ...
        if (GET_SKILL(ch, SKILL_BUILD) <= roll) {
            ...
            if (assemblyCheckComponents(lVnum, ch, 1)) {
        ...
        if (survival <= roll) {
            ...
            if (tool && rand_number(1, 3) == 3 && GET_OBJ_VAL(tool, VAL_ALL_HEALTH) > 0) {
                ...
                act("@RYour toolset is looking a bit more worn.@n", 1, ch, 0, 0, TO_CHAR);
    ...
    if (GET_OBJ_VNUM(pObject) != 1611) {
        ...
        if (IS_TRUFFLE(ch)) {
            int count = 0, plused = 0, hasstat = 0, failsafe = 0;

            while (count < 6) {
                if (pObject->affected[count].location > 0 && rand_number(1, 6) <= 2 && plused == 0) {
...
static void perform_put(struct char_data *ch, struct obj_data *obj,
    ...
    if (OBJ_FLAGGED(cont, ITEM_FORGED)) {
        act("$P is forged and won't hold anything.", 0, ch, 0, cont, TO_CHAR);
        return;
    }
    ...
    if ((GET_OBJ_TYPE(cont) == ITEM_CONTAINER) && (GET_OBJ_VAL(cont, VAL_CONTAINER_CAPACITY) == 0))
        act("$p won't fit in $P.", 0, ch, obj, cont, TO_CHAR);
        ...
        act("You can't get $p out of your hand.", 0, ch, obj, NULL, TO_CHAR);
        ...
        if (!OBJ_FLAGGED(obj, ITEM_ANTI_HIEROPHANT)) {
            act("$n puts $p in $P.", 1, ch, obj, cont, TO_ROOM);
        } else {
            act("$n puts an @DA@wd@cv@Ce@Wnt @DD@wu@ce@Cl @mC@Ma@Wr@wd@n in $P.", 1, ch, obj, cont, TO_ROOM);
        }

        /* Yes, I realize this is strange until we have auto-equip on rent. -gg */
        if (OBJ_FLAGGED(obj, ITEM_NODROP) && !OBJ_FLAGGED(cont, ITEM_NODROP)) {
            SET_BIT_AR(GET_OBJ_EXTRA(cont), ITEM_NODROP);
            act("You get a strange feeling as you put $p in $P.", 0,
                ch, obj, cont, TO_CHAR);
        } else
            act("You put $p in $P.", 0, ch, obj, cont, TO_CHAR);
        ...
        if ((GET_OBJ_TYPE(cont) == ITEM_PORTAL) ||
            ...
            if (GET_OBJ_TYPE(cont) == ITEM_PORTAL) {
                act("What? $U$p disappears from $P in a puff of smoke!",
                    1, ch, obj, cont, TO_ROOM);
                act("What? $U$p disappears from $P in a puff of smoke!",
                    0, ch, obj, cont, TO_CHAR);
            }
        }
    }
}
    ...
    if (!*theobj)
        ...
        if (!cont)
            send_to_char(ch, "You don't see %s %s here.\r\n", AN(thecont), thecont);
                 ...
                 (GET_OBJ_TYPE(cont) != ITEM_VEHICLE))
            act("$p is not a container.", 0, ch, cont, 0, TO_CHAR);
...
static bool can_take_obj(struct char_data *ch, struct obj_data *obj) {
    if (!(CAN_WEAR(obj, ITEM_WEAR_TAKE))) {
        act("$p: you can't take that!", 0, ch, obj, 0, TO_CHAR);
        return false;
    } else if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch)) {
        act("$p: your arms are full!", 0, ch, obj, 0, TO_CHAR);
        return false;
    } else if ((gear_weight(ch) + GET_OBJ_WEIGHT(obj)) > CAN_CARRY_W(ch)) {
        act("$p: you can't carry that much weight.", 0, ch, obj, 0, TO_CHAR);
        return false;
    } else if ((GET_OBJ_WEIGHT(obj) + ROOM_GRAVITY(IN_ROOM(ch))) + gear_weight(ch) > CAN_CARRY_W(ch)) {
        act("$p: you can't carry that much weight because of the gravity.", 0, ch, obj, 0, TO_CHAR);
        return false;
    }
    return true;
}
...
static void get_check_money(struct char_data *ch, struct obj_data *obj) {
    ...
    if (GET_GOLD(ch) + value > GOLD_CARRY(ch)) {
        ...
        act("@w$n @wdrops some onto the ground.@n", 0, ch, 0, 0, TO_ROOM);
        extract_obj(obj);
        int diff = 0;
...
static void perform_get_from_container(struct char_data *ch, struct obj_data *obj,
    ...
    if (mode == FIND_OBJ_INV || mode == FIND_OBJ_EQUIP || can_take_obj(ch, obj)) {
        if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch)) {
            act("$p: you can't hold any more items.", 0, ch, obj, 0, TO_CHAR);
            return;
        }
        if (SITS(ch) && GET_OBJ_VNUM(SITS(ch)) > 603 && GET_OBJ_VNUM(SITS(ch)) < 608 &&
            ...
            obj_to_char(obj, ch);
            if (OBJ_FLAGGED(cont, ITEM_SHEATH)) {
                act("You draw $p from $P.", 0, ch, obj, cont, TO_CHAR);
                act("$n draws $p from $P.", 1, ch, obj, cont, TO_ROOM);
            } else {
                act("You get $p from $P.", 0, ch, obj, cont, TO_CHAR);
                act("$n gets $p from $P.", 1, ch, obj, cont, TO_ROOM);
            }
            if (OBJ_FLAGGED(obj, ITEM_HOT)) {
                if (GET_BONUS(ch, BONUS_FIREPROOF) <= 0 && !IS_DEMON(ch)) {
                    ...
                    act("@RYou are burned by it!@n", 1, ch, 0, 0, TO_CHAR);
                    act("@R$n@R is burned by it!@n", 1, ch, 0, 0, TO_ROOM);
                }
            }
            if (IS_NPC(ch)) {
                item_check(obj, ch);
...
static void get_from_container(struct char_data *ch, struct obj_data *cont,
    ...
    if (OBJVAL_FLAGGED(cont, CONT_CLOSED))
        act("$p is closed.", 0, ch, cont, 0, TO_CHAR);
        ...
        if (!found) {
            if (obj_dotmode == FIND_ALL)
                act("$p seems to be empty.", 0, ch, cont, 0, TO_CHAR);
            else {
                char buf[MAX_STRING_LENGTH];
    ...
    switch (mode) {
        case SCMD_DROP:
            if (!OBJ_FLAGGED(obj, ITEM_UNBREAKABLE) && ROOM_EFFECT(IN_ROOM(ch)) == 6) {
                act("$p melts in the lava!", 0, ch, obj, 0, TO_CHAR);
                act("$p melts in the lava!", 0, ch, obj, 0, TO_ROOM);
                extract_obj(obj);
            } else if (ROOM_EFFECT(IN_ROOM(ch)) == 6) {
                act("$p plops down on some cooled lava!", 0, ch, obj, 0, TO_CHAR);
                act("$p plops down on some cooled lava!", 0, ch, obj, 0, TO_ROOM);
                obj_to_room(obj, IN_ROOM(ch));
                if (GET_ADMLEVEL(ch) > 0) {
                    send_to_imm("IMM DROP: %s dropped %s in room [%d]", GET_NAME(ch), obj->short_description,
                                GET_ROOM_VNUM(IN_ROOM(obj)));
        ...
        case SCMD_DONATE:
            obj_to_room(obj, RDR);
            act("$p suddenly appears in a puff a smoke!", 0, 0, obj, 0, TO_ROOM);
            return false;
        case SCMD_JUNK:
...
static void perform_give(struct char_data *ch, struct char_data *vict,
    ...
    if (OBJ_FLAGGED(obj, ITEM_NODROP)) {
        act("You can't let go of $p!!  Yeech!", 0, ch, obj, 0, TO_CHAR);
        return;
    }
    if (IS_CARRYING_N(vict) >= CAN_CARRY_N(vict)) {
        act("$N seems to have $S hands full.", 0, ch, 0, vict, TO_CHAR);
        if (IS_NPC(ch)) {
            act("$n@n drops $p because you can't carry anymore.", 1, ch, obj, vict, TO_VICT);
            act("$n@n drops $p on the ground since $N's unable to carry it.", 1, ch, obj, vict, TO_NOTVICT);
            obj_from_char(obj);
            obj_to_room(obj, IN_ROOM(ch));
        }
        return;
    ...
    if (IS_NPC(vict) && (OBJ_FLAGGED(obj, ITEM_FORGED) || OBJ_FLAGGED(obj, ITEM_FORGED))) {
        act("$n tries to hand $p to $N.", 1, ch, obj, vict, TO_NOTVICT);
    ...
    if ((GET_OBJ_WEIGHT(obj) + ROOM_GRAVITY(IN_ROOM(vict))) + gear_weight(vict) > CAN_CARRY_W(vict)) {
        act("$E can't carry that much weight because of the gravity.", 0, ch, 0, vict, TO_CHAR);
        if (IS_NPC(ch)) {
            act("$n@n drops $p because you can't carry anymore.", 1, ch, obj, vict, TO_VICT);
            act("$n@n drops $p on the ground since $N's unable to carry it.", 1, ch, obj, vict, TO_NOTVICT);
            obj_from_char(obj);
    ...
    if (!IS_NPC(vict) && !IS_NPC(ch)) {
        if (PRF_FLAGGED(vict, PRF_NOGIVE)) {
            act("$N refuses to accept $p at this time.", 0, ch, obj, vict, TO_CHAR);
            act("$n tries to give you $p, but you are refusing to be handed things.", 0, ch, obj, vict, TO_VICT);
            act("$n tries to give $N, $p, but $E is refusing to be handed things right now.", 0, ch, obj, vict,
                TO_NOTVICT);
            return;
        }
    }
    ...
    if (OBJ_FLAGGED(obj, ITEM_HOT)) {
        if (GET_BONUS(vict, BONUS_FIREPROOF) <= 0 && !IS_DEMON(vict)) {
            ...
            act("@RYou are burned by it!@n", 1, vict, 0, 0, TO_CHAR);
            act("@R$n@R is burned by it!@n", 1, vict, 0, 0, TO_ROOM);
        }
    }
...
ACMD(do_drink) {
    ...
    if (!*arg && !IS_NPC(ch)) {
        ...
        switch (SECT(IN_ROOM(ch))) {
            ...
            case SECT_UNDERWATER:
                ...
                if (GET_SKILL(ch, SKILL_WELLSPRING) && GET_MANA(ch) < GET_MAX_MANA(ch) && wasthirsty <= 30 &&
                    subcmd != SCMD_SIP) {
                    GET_MANA(ch) += ((GET_MAX_MANA(ch) * 0.005) + (GET_WIS(ch) * rand_number(80, 100))) *
    ...
    if ((GET_OBJ_TYPE(temp) != ITEM_DRINKCON) &&
        ...
        if (GET_OBJ_VNUM(temp) == 86 && on_ground != 1) {
            act("@wYou uncork the $p and tip it to your lips. Drinking it down you feel a warmth flow through your body and your ki returns to full strength .@n",
                1, ch, temp, 0, TO_CHAR);
            act("@C$n@w uncorks the $p and tips it to $s lips. Drinking it down and then smiling.@n", 1, ch, temp, 0,
                TO_ROOM);
            obj_from_char(temp);
            extract_obj(temp);
            GET_MANA(ch) = GET_MAX_MANA(ch);
    ...
    if (IS_NPC(ch)) {     /* Cannot use GET_COND() on mobs. */
        act("$n@w drinks from $p.", 1, ch, temp, 0, TO_ROOM);
        obj_from_char(temp);
        extract_obj(temp);
        return;
    }
    if ((GET_COND(ch, DRUNK) > 10) && (GET_COND(ch, THIRST) > 0)) {
        ...
        act("$n tries to drink but misses $s mouth!", 1, ch, 0, 0, TO_ROOM);
        return;
    }
    ...
    if (subcmd == SCMD_DRINK) {
        ...
        if (temp->action_description)
            act(temp->action_description, 1, ch, temp, 0, TO_CHAR);
      ...
      amount = rand_number(3, 10);*/
        ...
        act("$n sips from $p.", 1, ch, temp, 0, TO_ROOM);
...
ACMD(do_eat) {
    ...
    if (subcmd == SCMD_EAT) {
        act("You eat some of $p.", 0, ch, food, 0, TO_CHAR);
        if (food->action_description)
            act(food->action_description, 0, ch, food, 0, TO_CHAR);
        act("$n eats some of $p.", 1, ch, food, 0, TO_ROOM);
    } else {
        act("You nibble a little bit of $p.", 0, ch, food, 0, TO_CHAR);
        act("$n tastes a little bit of $p.", 1, ch, food, 0, TO_ROOM);
...
ACMD(do_pour) {
    ...
    if (subcmd == SCMD_FILL) {
        ...
        if (GET_OBJ_TYPE(to_obj) != ITEM_DRINKCON) {
            act("You can't fill $p!", 0, ch, to_obj, 0, TO_CHAR);
            return;
        }
        if (!*arg2) {        /* no 2nd argument */
            act("What do you want to fill $p from?", 0, ch, to_obj, 0, TO_CHAR);
            return;
        }
        ...
        if (GET_OBJ_TYPE(from_obj) != ITEM_FOUNTAIN && !OBJ_FLAGGED(from_obj, ITEM_BROKEN)) {
            act("You can't fill something from $p.", 0, ch, from_obj, 0, TO_CHAR);
            return;
        } else if (GET_OBJ_TYPE(from_obj) == ITEM_FOUNTAIN && OBJ_FLAGGED(from_obj, ITEM_BROKEN)) {
            act("You can't fill something from a broken fountain.", 0, ch, from_obj, 0, TO_CHAR);
            return;
        }
    }
    ...
    if (subcmd == SCMD_POUR) {    /* pour */
        ...
        if (!strcasecmp(arg2, "out")) {
            if (GET_OBJ_VAL(from_obj, VAL_DRINKCON_CAPACITY) > 0) {
                ...
                act("You empty $p.", 0, ch, from_obj, 0, TO_CHAR);
    ...
    if (subcmd == SCMD_FILL) {
        act("You gently fill $p from $P.", 0, ch, to_obj, from_obj, TO_CHAR);
        act("$n gently fills $p from $P.", 1, ch, to_obj, from_obj, TO_ROOM);
    }
    /* New alias */
    if (GET_OBJ_VAL(to_obj, VAL_DRINKCON_HOWFULL) == 0)
...
static void wear_message(struct char_data *ch, struct obj_data *obj, int where) {
    ...
    act(wear_messages[where][0], 1, ch, obj, 0, TO_ROOM);
    act(wear_messages[where][1], 0, ch, obj, 0, TO_CHAR);
}
...
void perform_wear(struct char_data *ch, struct obj_data *obj, int where) {
   ...
   */
    ...
    if (!CAN_WEAR(obj, wear_bitvectors[where])) {
        act("You can't wear $p there.", 0, ch, obj, 0, TO_CHAR);
        return;
    }
...
int find_eq_pos(struct char_data *ch, struct obj_data *obj, char *arg) {
    ...
    } else if (((where = search_block(arg, keywords, 0)) < 0) || (*arg == '!')) {
        ...
        for (obj = ch->carrying; obj; obj = next_obj) {
            ...
            if (CAN_SEE_OBJ(ch, obj) && (where = find_eq_pos(ch, obj, 0)) >= 0) {
                if (GET_LEVEL(ch) < GET_OBJ_LEVEL(obj)) {
                    ...
                    act("$p: it seems to be broken.", 0, ch, obj, 0, TO_CHAR);
                } else if (OBJ_FLAGGED(obj, ITEM_FORGED)) {
                    act("$p: it seems to be fake...", 0, ch, obj, 0, TO_CHAR);
                } else {
                    items_worn++;
        ...
        if (!(obj = get_obj_in_list_vis(ch, arg1, NULL, ch->carrying)))
            ...
            while (obj) {
                ...
                if ((where = find_eq_pos(ch, obj, 0)) >= 0) {
                    ...
                    act("You can't wear $p.", 0, ch, obj, 0, TO_CHAR);
                obj = next_obj;
            }
    } else {
        if (!(obj = get_obj_in_list_vis(ch, arg1, NULL, ch->carrying)))
    ...
    if (!*arg)
        ...
        if (GET_OBJ_TYPE(obj) == ITEM_LIGHT) {
            ...
            if (GET_OBJ_VAL(obj, VAL_LIGHT_HOURS) > 0 || GET_OBJ_VAL(obj, VAL_LIGHT_HOURS) < 0) {
                act("@wYou light $p@w.", 1, ch, obj, 0, TO_CHAR);
                act("@C$n@w lights $p@w.", 1, ch, obj, 0, TO_ROOM);
            }
            if (GET_OBJ_VAL(obj, VAL_LIGHT_HOURS) == 0) {
                act("@wYou try to light $p@w but it is burnt out.", 1, ch, obj, 0, TO_CHAR);
                act("@C$n@w tries to light $p@w but nothing happens.", 1, ch, obj, 0, TO_ROOM);
            }
        } else {
...
void perform_remove(struct char_data *ch, int pos) {
     ...
     */
        ...
        act("You can't remove $p, it must be CURSED!", 0, ch, obj, 0, TO_CHAR);
    else if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
        act("$p: your arms are full!", 0, ch, obj, 0, TO_CHAR);
    else {
        ...
        act("You stop using $p.", 0, ch, obj, 0, TO_CHAR);
        act("$n stops using $p.", 1, ch, obj, 0, TO_ROOM);
...
ACMD(do_sac) {
    ...
    act("$n sacrifices $p.", 0, ch, j, 0, TO_ROOM);
