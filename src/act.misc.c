...
/* local functions  */
static void
generate_multiform(struct char_data *ch, struct char_data *multi1, struct char_data *multi2, struct char_data *multi3);

static void resolve_song(struct char_data *ch);

static bool campfire_cook(int recipe);

static bool valid_recipe(struct char_data *ch, int recipe, int type);

static bool has_pole(struct char_data *ch);

static void catch_fish(struct char_data *ch, int quality);

static bool valid_silk(struct obj_data *obj);
        ...
        if (AFF_FLAGGED(ch, AFF_SPIRITCONTROL)) {
            ...
            if (GET_MOVE(ch) < cost) {
                ...
                act("@YYou concentrate and quantify every last bit of your spiritual and mental energies. You have full control of them and can bring them forth in an instant.@n",
                    1, ch, 0, 0, TO_CHAR);
                act("@y$n@Y seems to concentrate hard for a moment.@n", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_multiform) {
    ...
    if (num >= 3) {
        if (!strcasecmp(arg, "merge")) {
            ...
            send_to_char(ch,
                         "You have the maximum number of bodies multiform can produce.\nTo merge use: multiform merge.\r\n");
            return;
        }
    } else {
        if (!strcasecmp(arg, "merge")) {
            ...
            if (GET_HIT(ch) < gear_pl(ch) * 0.4) {
                send_to_char(ch,
                             "Your powerlevel is too weakened to split! It can't be below 40%s of your weighted max.\r\n",
                             "%");
                return;
                ...
                act("@YYou focus your ki into your body while concentrating on the image of your body splitting into two. @yYou lose your concentration and fail to split though...@n",
                    1, ch, 0, 0, TO_CHAR);
                act("@y$n@Y seems to concentrate really hard for a moment, before relaxing.@n", 1, ch, 0, 0,
                    TO_ROOM);
                GET_MANA(ch) -= cost;
                GET_MOVE(ch) -= cost;
...
/* Rillao: transloc, add new transes here */
    ...
    } else if (PLR_FLAGGED(ch, PLR_TRANS1) || PLR_FLAGGED(ch, PLR_TRANS2) || PLR_FLAGGED(ch, PLR_TRANS3) ||
               PLR_FLAGGED(ch, PLR_TRANS4) || PLR_FLAGGED(ch, PLR_TRANS5) || PLR_FLAGGED(ch, PLR_TRANS6)) {
...
static void resolve_song(struct char_data *ch) {
    ...
    if (!obj2) {
        ...
        act("@c$n@C stops playing $s song.@n", 1, ch, 0, 0, TO_ROOM);
        GET_SONG(ch) = 0;
        return;
    }
    ...
    if (skill > diceroll) {
        ...
        act("@CYou continue playing your song.@n", 1, ch, 0, 0, TO_CHAR);
        act(buf, 1, ch, 0, 0, TO_ROOM);
    } else {
        act("@CYou mess up a portion of the song, but continue playing.@n", 1, ch, 0, 0, TO_CHAR);
        act("@c$n@C messes up a portion of $s song, but continues to play.@n", 1, ch, 0, 0, TO_ROOM);
        return;
    }
        ...
        switch (GET_SONG(ch)) {
                    ...
                    if ((AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) || vict == ch) {
                        if (ch == vict->master || ch->master == vict || ch->master == vict->master || vict == ch) {
                            if (skill > diceroll) {
                                ...
                                act("@c$n@C continues playing $s ocarina!@n", 1, ch, 0, vict, TO_NOTVICT);
            ...
            case SONG_TELEPORT_EARTH:
                ...
                if (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                    if (ch == vict->master || ch->master == vict || ch->master == vict->master) {
                        if (skill > diceroll) {
                            act("@CYour Teleportation Melody has transported @c$N@C to Earth in a flash!@n", 1, ch,
                                0, vict, TO_CHAR);
                            act("@c$n's@C Teleportation Melody has transported you to Earth in a flash!@n", 1, ch, 0,
                                vict, TO_VICT);
                            act("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n", 1, ch, 0,
                                vict, TO_NOTVICT);
                            char_from_room(vict);
            ...
            case SONG_TELEPORT_VEGETA:
                ...
                if (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                    if (ch == vict->master || ch->master == vict || ch->master == vict->master) {
                        if (skill > diceroll) {
                            act("@CYour Teleportation Melody has transported @c$N@C to Vegeta in a flash!@n", 1, ch,
                                0, vict, TO_CHAR);
                            act("@c$n's@C Teleportation Melody has transported you to Vegeta in a flash!@n", 1, ch,
                                0, vict, TO_VICT);
                            act("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n", 1, ch, 0,
                                vict, TO_NOTVICT);
                            char_from_room(vict);
            ...
            case SONG_TELEPORT_FRIGID:
                ...
                if (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                    if (ch == vict->master || ch->master == vict || ch->master == vict->master) {
                        if (skill > diceroll) {
                            act("@CYour Teleportation Melody has transported @c$N@C to Frigid in a flash!@n", 1, ch,
                                0, vict, TO_CHAR);
                            act("@c$n's@C Teleportation Melody has transported you to Frigid in a flash!@n", 1, ch,
                                0, vict, TO_VICT);
                            act("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n", 1, ch, 0,
                                vict, TO_NOTVICT);
                            char_from_room(vict);
            ...
            case SONG_TELEPORT_KONACK:
                ...
                if (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                    if (ch == vict->master || ch->master == vict || ch->master == vict->master) {
                        if (skill > diceroll) {
                            act("@CYour Teleportation Melody has transported @c$N@C to Konack in a flash!@n", 1, ch,
                                0, vict, TO_CHAR);
                            act("@c$n's@C Teleportation Melody has transported you to Konack in a flash!@n", 1, ch,
                                0, vict, TO_VICT);
                            act("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n", 1, ch, 0,
                                vict, TO_NOTVICT);
                            char_from_room(vict);
            ...
            case SONG_TELEPORT_NAMEK:
                ...
                if (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                    if (ch == vict->master || ch->master == vict || ch->master == vict->master) {
                        if (skill > diceroll) {
                            act("@CYour Teleportation Melody has transported @c$N@C to Namek in a flash!@n", 1, ch,
                                0, vict, TO_CHAR);
                            act("@c$n's@C Teleportation Melody has transported you to Namek in a flash!@n", 1, ch, 0,
                                vict, TO_VICT);
                            act("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n", 1, ch, 0,
                                vict, TO_NOTVICT);
                            char_from_room(vict);
            ...
            case SONG_TELEPORT_ARLIA:
                ...
                if (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                    if (ch == vict->master || ch->master == vict || ch->master == vict->master) {
                        if (skill > diceroll) {
                            act("@CYour Teleportation Melody has transported @c$N@C to Arlia in a flash!@n", 1, ch,
                                0, vict, TO_CHAR);
                            act("@c$n's@C Teleportation Melody has transported you to Arlia in a flash!@n", 1, ch, 0,
                                vict, TO_VICT);
                            act("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n", 1, ch, 0,
                                vict, TO_NOTVICT);
                            char_from_room(vict);
            ...
            case SONG_TELEPORT_AETHER:
                ...
                if (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                    if (ch == vict->master || ch->master == vict || ch->master == vict->master) {
                        if (skill > diceroll) {
                            act("@CYour Teleportation Melody has transported @c$N@C to Aether in a flash!@n", 1, ch,
                                0, vict, TO_CHAR);
                            act("@c$n's@C Teleportation Melody has transported you to Aether in a flash!@n", 1, ch,
                                0, vict, TO_VICT);
                            act("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n", 1, ch, 0,
                                vict, TO_NOTVICT);
                            char_from_room(vict);
            ...
            case SONG_TELEPORT_KANASSA:
                ...
                if (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                    if (ch == vict->master || ch->master == vict || ch->master == vict->master) {
                        if (skill > diceroll) {
                            act("@CYour Teleportation Melody has transported @c$N@C to Kanassa in a flash!@n", 1, ch,
                                0, vict, TO_CHAR);
                            act("@c$n's@C Teleportation Melody has transported you to Kanassa in a flash!@n", 1, ch,
                                0, vict, TO_VICT);
                            act("@c$n's@C Teleportation Melody has transported @c$N@C away in a flash!@n", 1, ch, 0,
                                vict, TO_NOTVICT);
                            char_from_room(vict);
            ...
            case SONG_SHIELDING:
                if (vict == ch || (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP))) {
                    if (ch == vict->master || ch->master == vict || ch->master == vict->master || vict == ch) {
                        if (skill > diceroll) {
                            if (vict != ch) {
                                act("@CYour triumphant and soaring music has powered a barrier around @c$N@C!@n", 1,
                                    ch, 0, vict, TO_CHAR);
                                act("@c$n's@C triumphant and soaring music has powered a barrier around you!@n", 1,
                                    ch, 0, vict, TO_VICT);
                            } else {
                                act("@CYour triumphant and soaring music has powered a barrier around yourself@C!@n",
                                    1, ch, 0, vict, TO_CHAR);
                            }
                            act("@c$n's@C triumphant and soaring music has powered a barrier around @c$N@C!@n", 1,
...
ACMD(do_song) {
    ...
    if (GET_SONG(ch)) {
        act("@cYou stop playing your ocarina.@n", 1, ch, 0, 0, TO_CHAR);
        act("@c$n stops playing their ocarina.@n", 1, ch, 0, 0, TO_ROOM);
        GET_SONG(ch) = 0;
        return;
    } else {
        ...
        if (!*arg) {
            ...
            if (modifier == 0) {
                ...
                act("@CYou begin to play the Song of Safety! Your fingers lightly glide over the ocarina and as you blow into it sweet music similar to a lullaby issues forth from the intrument.@n",
                    1, ch, 0, 0, TO_CHAR);
                act("@c$n@C begins to play a song on $s ocarina. The music seems to be some sort of lullaby.@n", 1,
                    ch, 0, 0, TO_ROOM);
    ...
    for (vict = world[IN_ROOM(ch)].people; vict; vict = next_v) {
        ...
        if (AFF_FLAGGED(vict, AFF_GROUP)) {
            if (ch->master == vict->master || vict->master == ch || ch->master == vict) {
                ...
                act("@CYou breathe in the dust and are healed by it somewhat!@n", 1, vict, 0, 0, TO_CHAR);
                act("@c$n@C breathes in the dust and is healed somewhat!@n", 1, vict, 0, 0, TO_ROOM);
...
ACMD(do_shell) {
    ...
    if (AFF_FLAGGED(ch, AFF_SHELL)) {
        act("@mYou quickly absorb the armor carapace covering your body back inside.@n", 1, ch, 0, 0, TO_CHAR);
        act("@M$n's@m armored carapce retreats back to its original size.@n", 1, ch, 0, 0, TO_ROOM);
    ...
    if (!strcasecmp(arg, "hide")) {
        ...
        if (axion_dice(0) > GET_LEVEL(ch)) {
            act("@MYour body starts to become loose and sag, but you lose focus and it reverts to its original shape!@n",
                1, ch, 0, 0, TO_CHAR);
            act("@m$n@M's body starts to become loose and sag, but $e seems to return normal a moment later.@n", 1,
                ch, 0, 0, TO_ROOM);
...
ACMD(do_defend) {
    ...
    if (!*arg && GET_DEFENDING(ch) == NULL) {
        ...
        act("@YYou stop defending @y$N@Y.@n", 1, ch, 0, GET_DEFENDING(ch), TO_CHAR);
        act("@y$n@Y stops defending you.@n", 1, ch, 0, GET_DEFENDING(ch), TO_VICT);
        act("@y$n@Y stops defending @y$N@Y.@n", 1, ch, 0, GET_DEFENDING(ch), TO_NOTVICT);
    ...
    if (!(vict = get_char_vis(ch, arg, NULL, FIND_CHAR_ROOM))) {
        ...
        act("@YYou start defending @y$N@Y.@n", 1, ch, 0, vict, TO_CHAR);
        act("@y$n@Y starts defending you.@n", 1, ch, 0, vict, TO_VICT);
        act("@y$n@Y starts defending @y$N@Y.@n", 1, ch, 0, vict, TO_NOTVICT);
    ...
    if (!strcasecmp(arg, "cast")) {
        ...
        if (!PLR_FLAGGED(ch, PLR_FISHING)) {
            ...
            act("@CYou pull hard but the fish spits the hook out a second before you pull! You return to waiting for a bite...@n",
                1, ch, 0, 0, TO_CHAR);
            act("@c$n@C pulls hard on $s fishing line, but a moment later $e frowns and returns to fishing.@n", 1,
                ch, 0, 0, TO_ROOM);
            ...
            act("@CYou pull hard on your line and feel that you have managed to hook the fish! Better @Greel@C it in!@n",
                1, ch, 0, 0, TO_CHAR);
            act("@c$n@C pulls hard on $s fishing line and starts to struggle with the fish on the other end!@n", 1,
                ch, 0, 0, TO_ROOM);
        ...
        if (!PLR_FLAGGED(ch, PLR_FISHING)) {
            ...
            reveal_hiding(ch, 0);
            act("@CYou begin reeling the fish in slowly.@n", 1, ch, 0, 0, TO_CHAR);
            act("@c$n@C begins to reel the line on $s pole in.@n", 1, ch, 0, 0, TO_ROOM);
        ...
        if (!GET_EQ(ch, WEAR_WIELD2)) {
            ...
            if (GET_OBJ_TYPE(pole) != ITEM_FISHPOLE) {
                ...
                if (!(bait = get_obj_in_list_vis(ch, arg2, NULL, ch->carrying))) {
                    ...
                    reveal_hiding(ch, 0);
                    act("@CYou carefully apply the $p@C to your hook.@n", 1, ch, bait, 0, TO_CHAR);
                    act("@c$n@C carefully applies $p@C to $s fishing pole's hook.@n", 1, ch, bait, 0, TO_ROOM);
        ...
        if (!PLR_FLAGGED(ch, PLR_FISHING)) {
            ...
            reveal_hiding(ch, 0);
            act("@CYou reel in your line and stop fishing.@n", 1, ch, 0, 0, TO_CHAR);
            act("@c$n@C reels in $s fishing line and stops fishing.@n", 1, ch, 0, 0, TO_ROOM);
...
void fish_update(void) {
    ...
    for (i = character_list; i; i = next_char) {
        ...
        if (ROOM_FLAGGED(IN_ROOM(i), ROOM_FISHING)) {
            if (PLR_FLAGGED(i, PLR_FISHING) && has_pole(i) == 1) {
                ch = i;
                if (GET_FISHD(ch) <= 0 && GET_FISHSTATE(ch) == FISH_REELING) { /* We've caught it */
                    ...
                    if (GET_FISHSTATE(ch) == FISH_REELING && rand_number(1, 100) <= 80) {
                        ...
                        act("@CYou reel the line on your pole some.@n", 1, ch, 0, 0, TO_CHAR);
                        act("@c$n@C reels the line on $s pole slowly.@n", 1, ch, 0, 0, TO_ROOM);
                        send_to_char(ch, "@D[@wDistance@D: @Y%d@D]@n\r\n", GET_FISHD(ch) > 0 ? GET_FISHD(ch) : 0);
                    } else if (GET_FISHSTATE(ch) == FISH_REELING && rand_number(1, 58) <= 55) {
                        act("@CYou struggle as the fish fights against your attempts to reel it in!@n", 1, ch, 0, 0,
                            TO_CHAR);
                        act("@c$n@C struggles with the fish on the end of $s pole!@n", 1, ch, 0, 0, TO_ROOM);
                        ...
                        act("@CYou feel the line go slack and realize you've lost the fish! You reel your line back in...@n",
                            1, ch, 0, 0, TO_CHAR);
                        act("@c$n@C frowns and then begins to reel in $s line.@n", 1, ch, 0, 0, TO_ROOM);
                        GET_FISHD(ch) = 0;
                        GET_FISHSTATE(ch) = FISH_NOFISH;
                        REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_FISHING);
                        if (has_pole(ch) == 1) {
                        ...
                        act("@CYou feel as if the fish has stopped biting...@n", 1, ch, 0, 0, TO_CHAR);
                        GET_FISHSTATE(ch) = FISH_NOFISH;
                        ...
                        act("@CYou feel a fish biting on your line! Better @Ghook@C it!@n", 1, ch, 0, 0, TO_CHAR);
                        GET_FISHSTATE(ch) = FISH_BITE;
                    }
                } /* End reel section */
            } else if (PLR_FLAGGED(i, PLR_FISHING) && has_pole(i) == 0) { /* End of, Is Fishing */
...
static void catch_fish(struct char_data *ch, int quality) {
    ...
    act("@CYou manage to pull a $p@C from the water and onto the ground in front of you!@n", 1, ch, fish, 0,
        TO_CHAR);
    act("@c$n@C manages to pull a $p@C from the water and onto the ground in front of $m!@n", 1, ch, fish, 0,
        TO_ROOM);
...
ACMD(do_scry) {
    ...
    if (GET_PRACTICES(ch, GET_CLASS(ch)) < cost) {
        ...
        act("@GYou see @C$n@G begin to focus, and then without warning, your mind is flooded painfully with images, energy and information. The data streams in a mad torrent through your psyche, and just when you think snapping is possible, the voice of @C$n@G comes to you and eases and guides you. You see images of potential futures, information not yet known, knowledge yet undiscovered. Though you could not fully  grasp what is to come, you feel more prepared at facing the unknown.@n",
            1, ch, 0, vict, TO_VICT);
        act("@C$n@W appears to be performing some sort of ritual or something with @c$N@W.@n", 1, ch, 0, vict,
            TO_NOTVICT);
            ...
            if (GET_OBJ_VNUM(obj) == 1306) {
                if (axion_dice(0) > GET_CON(ch)) {
                    if (!IS_ANDROID(ch) && !IS_DEMON(ch) && !IS_ICER(ch)) {
                        ...
                        act("@RYou choke on the the burning hot @Da@Ws@wh@Dc@Wl@wo@Du@Wd@R!@n", 1, ch, 0, 0,
                            TO_CHAR);
                        act("@r$n@R chokes on the burning hot @Da@Ws@wh@Dc@Wl@wo@Du@Wd@R!@n", 1, ch, 0, 0, TO_ROOM);
                    }
                    if (!IS_ANDROID(ch) && !IS_DEMON(ch) && !IS_NPC(ch)) {
                        if (!PLR_FLAGGED(ch, PLR_EYEC) && !AFF_FLAGGED(ch, AFF_BLIND)) {
                            reveal_hiding(ch, 0);
                            act("@DYour eyes sting from the hot ash! You can't see!@n", 1, ch, 0, 0, TO_CHAR);
                            act("@r$n@D eyes appear to have been hurt by the ash!@n", 1, ch, 0, 0, TO_ROOM);
                            int duration = 1;
            ...
            if (!wearable_obj(obj)) {
                ...
                if (GET_MOVE(ch) < GET_OBJ_WEIGHT(obj) + (GET_MAX_MOVE(ch) / 40)) {
                    ...
                    if (GET_OBJ_SIZE(obj) == SIZE_SMALL) {
                        ...
                        act("@WYou carefully adjust the size of @c$p@W.@n", 1, ch, obj, 0, TO_CHAR);
                        act("@C$n@W carefully adjusts the size of @c$p@W.@n", 1, ch, obj, 0, TO_ROOM);
...
ACMD(do_amnisiac) {
    ...
    if (cost > GET_MANA(ch)) {
        ...
        act("@WYou attempt to grab @C$N@W to kiss $M, but $E evades!@n", 1, ch, 0, vict, TO_CHAR);
        act("@M$n@W attempts to grab you and leans in with puckered lips, but you managed to evade!@n", 1, ch, 0,
            vict, TO_VICT);
        act("@M$n@W attempts to grab @C$N@W and leans in with puckered lips, but $E manages to evade!@n", 1, ch, 0,
            vict, TO_NOTVICT);
        GET_MANA(ch) -= cost;
        return;
    } else {
...
ACMD(do_metamorph) {
    ...
    if (perc < chance) {
        act("@WYou focus your energies and prepare your @RDark Metamorphisis@W but screw up your focus!@n", 1, ch, 0,
            0, TO_CHAR);
        act("@WA dark @Rred@W glow starts to surround @C$n@W, but it fades quickly.@n", 1, ch, 0, 0, TO_ROOM);
        GET_MANA(ch) -= (cost / 2);
        return;
    } else {
...
ACMD(do_hydromancy) {
    ...
    if (!strcasecmp(arg, "spike")) {
        ...
        if (GET_OBJ_WEIGHT(obj) + gear_weight(ch) <= CAN_CARRY_W(ch))
            ...
            act("@C$n@w drops an ice spike.@n", 1, ch, 0, 0, TO_ROOM);
        ...
        attempt = search_block(arg2, dirs, 0);
        ...
        if (CAN_GO(ch, attempt)) {
            ...
            if (skill < chance) {
                act("@BUsing your ki you attempt to create a rush of water! @RYou fail!@n", 1, ch, 0, 0, TO_CHAR);
                act("@b$n@B seems to attempt to create water with $s ki! @RHowever, $e fails!@n", 1, ch, 0, 0,
                    TO_ROOM);
                GET_MANA(ch) -= cost;
                WAIT_STATE(ch, PULSE_2SEC);
            } else {
                ...
                sprintf(bunn, "@B$n@B uses $s ki to create a rush of water flooding away toward the @C%s@B!@n",
                        dirs[attempt]);
                act(bun, 1, ch, 0, 0, TO_CHAR);
                act(bunn, 1, ch, 0, 0, TO_ROOM);
                ...
                for (vict = world[IN_ROOM(ch)].people; vict; vict = next_v) {
                    ...
                    if (!can_kill(ch, vict, NULL, 1)) {
                        act("@CYou are protected from the water!@n", 1, vict, 0, 0, TO_VICT);
                        act("@C$n@C is protected from the water!@n", 1, vict, 0, 0, TO_ROOM);
                    } else if (IS_KANASSAN(vict)) {
                        act("@CYou effortlessly swim against the current.@n", 1, vict, 0, 0, TO_CHAR);
                        act("@C$n@C effortlessly swims against the current.@n", 1, vict, 0, 0, TO_ROOM);
                    } else if (GET_SKILL_BASE(vict, SKILL_BALANCE) >= axion_dice(-10)) {
                        act("@CYou manage to keep your balance and are not swept away!@n", 1, vict, 0, 0, TO_CHAR);
                        act("@C$n@C manages to keep $s balance and is not swept away!@n", 1, vict, 0, 0, TO_ROOM);
                    } else if (AFF_FLAGGED(ch, AFF_FLYING)) {
                        act("@CYou fly above the rushing waters and are untouched.@n", 1, vict, 0, 0, TO_CHAR);
                        act("@C$n@C flies above the rushing waters and is untouched.@n", 1, vict, 0, 0, TO_ROOM);
                    } else {
                        act("@cYou are caught by the rushing waters and sent tumbling away!@n", 1, vict, 0, 0,
                            TO_CHAR);
                        act("@c$n@c is caught by the rushing waters and sent tumbling away!@n", 1, vict, 0, 0,
                            TO_ROOM);
...
ACMD(do_kanso) {
    ...
    if (skill < dice) { /* Failed the technique, user's fault */
        act("You close your eyes and focus, before bounding effortlessly toward $N. Closing the distance, you place your hands on $N's chest but nothing happens!\r\n",
            1, ch, 0, vict, TO_CHAR); /* Message Character $n sees */
        ...
        if (skill > pdice && !AFF_FLAGGED(vict, AFF_PARA)) { /* Paralyze them too */
            act("@R$N@W is paralyzed by the attack!@n", 1, ch, 0, vict, TO_CHAR); /* Message Character $n sees */
            act("@RYou are paralyzed by the attack!@n", 1, ch, 0, vict, TO_VICT); /* Message Vict $N Sees */
            act("@R$N@Wis paralyzed by the attack!@n", 1, ch, 0, vict, TO_NOTVICT); /* Message everyone else sees */
...
ACMD(do_arena) {
    ...
    if (!*arg) {
        ...
        if (GET_ROOM_VNUM(IN_ROOM(ch)) == 17875) {
            ...
            for (d = descriptor_list; d; d = d->next) {
                ...
                if (IN_ARENA(d->character)) {
                    ...
                    act(buf, 1, ch, 0, d->character, TO_CHAR);
                    found = 1;
                }
            }
        ...
        if (num < 0) {
            ...
            if (found == 1) {
                act("@wYou start watching the action surrounding that particular fighter in the arena.@n", 1, ch, 0,
                    0, TO_CHAR);
                act("@C$n@w starts watching the action in the arena.@n", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_silk) {
    ...
    if (!strcasecmp(arg, "weave")) {
        ...
        int found = 0, armor = 500, str = 0, intel = 0, olevel = 0;
        double price = 1;
        ...
        if (!obj) {
            ...
            if (!strcasecmp(arg2, "head")) {
                if (prob <= perc) {
                    ...
                    act("@WYou attempt to weave the bundle and manage to create $p@W!@n", 1, ch, weaved, 0, TO_CHAR);
                    act("@C$n@W attempts to weave a bundle into something and manages to create $p@W!@n", 1, ch,
                        weaved, 0, TO_ROOM);
        ...
        if (GET_MANA(ch) < cost) {
            ...
            WAIT_STATE(ch, PULSE_3SEC);
            int super = 0, superoll = rand_number(1, 100);
            if (IS_KURZAK(ch)) {
        ...
        if (!*arg || !*arg2) {
            ...
            if (!strcasecmp(arg, "pl") || !strcasecmp(arg, "PL")) {
                act("@GYou focus your mind and begin to overwork your powerful adrenal glands and your wounds begin to heal!@n",
                    1, ch, 0, 0, TO_CHAR);
                act("@g$n@G seems to concentrate and $s wounds begin to heal!@n", 1, ch, 0, 0, TO_ROOM);
                ...
                act("@GYou focus your mind and begin to overwork your powerful adrenal glands and you feel your ki replenish!@n",
                    1, ch, 0, 0, TO_CHAR);
                act("@g$n@G seems to concentrate and $e appears energized!@n", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_cook) {
    ...
    if (!*arg) {
        ...
        if (pass == 1) {
            if (skill < prob) {
                act("@wYou screw up the preparation of the recipe and end up wasting the ingredients!@n", 1, ch, 0,
                    0, TO_CHAR);
                act("@C$n@w starts to prepare some food, but ends up ruining the ingredients instead!@n", 1, ch, 0,
                    0, TO_ROOM);
...
ACMD(do_warppool) {
    ...
    int prob = axion_dice(0), cost = GET_MAX_MANA(ch) / 20, pass = 0;
    char arg[MAX_INPUT_LENGTH];
                ...
                if (GET_OBJ_COST(obj) == dir2) {
                    if (skill < prob) {
                        act("@CYou place your hands on the glacial wall and concentrate. You fail to undo the composition of the wall!@n",
                            1, ch, 0, 0, TO_CHAR);
                        act("@c$n@C places $s hands on the glacial wall and concentrates. Nothing happens...@n", 1,
                            ch, 0, 0, TO_ROOM);
                        GET_MANA(ch) -= cost / 2;
                    } else {
...
ACMD(do_feed) {
    ...
    act("@WYou take $p@W and pop it into @C$N@W's mouth!@n", 1, ch, obj, vict, TO_CHAR);
    act("@C$n@W takes $p@W and pops it into YOUR mouth!@n", 1, ch, obj, vict, TO_VICT);
    act("@C$n@W takes $p@W and pops it into @c$N@W's mouth!@n", 1, ch, obj, vict, TO_NOTVICT);
...
ACMD(do_spoil) {
    ...
    if (type == 0) {
        act("@C$n@W reaches down and @rtears@W the head off of @R$p@W!@n", 1, ch, obj, 0, TO_ROOM);
        act("@WYou reach down and @rtear@W the head off of @R$p@W!@n", 1, ch, obj, 0, TO_CHAR);
    } else if (type == 1) {
        act("@C$n@W reaches down and @rcuts@W the head off of @R$p@W!@n", 1, ch, obj, 0, TO_ROOM);
        act("@WYou reach down and @rcut@W the head off of @R$p@W!@n", 1, ch, obj, 0, TO_CHAR);
    } else if (type == 2) {
        act("@C$n@W reaches down and @rcuts@W the head off of @R$p@W!@n", 1, ch, obj, 0, TO_ROOM);
        act("@WYou reach down and @rcut@W the head off of @R$p@W!@n", 1, ch, obj, 0, TO_CHAR);
    }
