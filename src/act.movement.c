...
static bool has_key(struct char_data *ch, obj_vnum key);

static void do_doorcmd(struct char_data *ch, struct obj_data *obj, int door, int scmd);

static bool ok_pick(struct char_data *ch, obj_vnum keynum, int pickproof, int dclock, int scmd, struct obj_data *obj);

static bool has_flight(struct char_data *ch);
...
void handle_teleport(struct char_data *ch, struct char_data *tar, int location) {
    ...
    if (success == 1) { /* We have made it. */
        act("@w$n@w appears in an instant out of nowhere!@n", 1, ch, 0, 0, TO_ROOM);
        if (DRAGGING(ch) && !IS_NPC(DRAGGING(ch))) {
            char_from_room(DRAGGING(ch));
            char_to_room(DRAGGING(ch), IN_ROOM(ch));
            act("@w$n@w appears in an instant out of nowhere being dragged by $N!@n", 1, DRAGGING(ch), 0, ch,
                TO_NOTVICT);
        }
        if (GRAPPLING(ch) && !IS_NPC(GRAPPLING(ch))) {
            char_from_room(GRAPPLING(ch));
            char_to_room(GRAPPLING(ch), IN_ROOM(ch));
            act("@w$n@w appears in an instant out of nowhere being grappled by $N!@n", 1, GRAPPLING(ch), 0, ch,
                TO_NOTVICT);
        }
        if (CARRYING(ch)) {
            char_from_room(CARRYING(ch));
            char_to_room(CARRYING(ch), IN_ROOM(ch));
            act("@w$n@w appears in an instant out of nowhere being carried by $N!@n", 1, CARRYING(ch), 0, ch,
                TO_NOTVICT);
        }
        if (GRAPPLED(ch) && !IS_NPC(GRAPPLED(ch))) {
            char_from_room(GRAPPLED(ch));
            char_to_room(GRAPPLED(ch), IN_ROOM(ch));
            act("@w$n@w appears in an instant out of nowhere being grappled by $N!@n", 1, GRAPPLED(ch), 0, ch,
                TO_NOTVICT);
        }
        if (DRAGGING(ch) && IS_NPC(DRAGGING(ch))) {
            act("@WYou stop dragging @C$N@W!@n", 1, ch, 0, DRAGGING(ch), TO_CHAR);
            act("@C$n@W stops dragging @c$N@W!@n", 1, ch, 0, DRAGGING(ch), TO_ROOM);
    ...
    if (CARRYING(ch)) { /* Already carrying someone. Put them down. Simple and clean. */
        ...
        if (GET_PC_WEIGHT(vict) + IS_CARRYING_W(vict) > CAN_CARRY_W(ch)) {
            act("@WYou try to pick up @C$N@W but have to put them down. They are too heavy for you at the moment.@n",
                1, ch, 0, vict, TO_CHAR);
            act("@C$n@W tries to pick up @c$N@W. After struggling for a moment $e has to put $M down.@n", 1, ch, 0,
                vict, TO_NOTVICT);
            WAIT_STATE(ch, PULSE_1SEC);
            return;
        } else { /* Let's carry that mofo! */
            act("@WYou pick up @C$N@W and put $M over your shoulder.@n", 1, ch, 0, vict, TO_CHAR);
            act("@C$n@W picks up $c$N@W and puts $M over $s shoulder.@n", 1, ch, 0, vict, TO_NOTVICT);
            if (SITS(vict)) {
...
void carry_drop(struct char_data *ch, int type) {
    ...
    switch (type) {
        case 0: /* Awww we were gentle >.> */
            act("@WYou gently set @C$N@W down on the ground.@n", 1, ch, 0, vict, TO_CHAR);
            act("@C$n @Wgently sets you down on the ground.@n", 1, ch, 0, vict, TO_VICT);
            act("@C$n @Wgently sets @c$N@W down on the ground.@n", 1, ch, 0, vict, TO_NOTVICT);
            break;
        case 1: /* We're not super nice. */
            act("@WYou set @C$N@W hastily onto the ground.@n", 1, ch, 0, vict, TO_CHAR);
            act("@C$n @Wsets you hastily onto the ground.@n", 1, ch, 0, vict, TO_VICT);
            act("@C$n @Wsets @c$N@W hastily onto the ground.@n", 1, ch, 0, vict, TO_NOTVICT);
            break;
        case 2: /* Uh oh we dropped them from being hit! */
            act("@WYou have @C$N@W knocked out of your arms and onto the ground!@n", 1, ch, 0, vict, TO_CHAR);
            act("@WYou are knocked out of @C$n's@W arms and onto the ground!@n", 1, ch, 0, vict, TO_VICT);
            act("@C$n @Whas @c$N@W knocked out of $s arms and onto the ground!@n", 1, ch, 0, vict, TO_NOTVICT);
            break;
        case 3: /* Uh oh they are being extracted! */
            act("@WYou stop carrying @C$N@W for some reason.@n", 1, ch, 0, vict, TO_CHAR);
            act("@C$n @Wstops carrying you for some reason.@n", 1, ch, 0, vict, TO_VICT);
            act("@C$n @Wstops carrying @c$N@W for some reason.@n", 1, ch, 0, vict, TO_NOTVICT);
            break;
    }
    CARRYING(ch) = NULL;
...
ACMD(do_land) {
    ...
    int above_planet = 1, inroom = GET_ROOM_VNUM(IN_ROOM(ch));
    ...
    if (landing != -1) {
        ...
        act("$n comes down from high above in the sky and quickly lands on the ground.", 1, ch, 0, 0, TO_ROOM);
        return;
    }
}
...
static bool has_flight(struct char_data *ch) {
    ...
    if (AFF_FLAGGED(ch, AFF_FLYING) && GET_MANA(ch) >= (GET_LEVEL(ch) + (GET_MAX_MANA(ch) / (GET_LEVEL(ch) * 30))) &&
        !IS_ANDROID(ch) && !IS_NPC(ch)) {
        return true;
    }
    if (AFF_FLAGGED(ch, AFF_FLYING) && GET_MANA(ch) < (GET_LEVEL(ch) + (GET_MAX_MANA(ch) / (GET_LEVEL(ch) * 30))) &&
        !IS_ANDROID(ch) && !IS_NPC(ch)) {
        act("@WYou crash to the ground, too tired to fly anymore!@n", 1, ch, 0, 0, TO_CHAR);
        act("@W$n@W crashes to the ground!@n", 1, ch, 0, 0, TO_ROOM);
...
bool has_o2(struct char_data *ch) {
    if (ADM_FLAGGED(ch, ADM_WALKANYWHERE))
        return true;
   ...
   */
    ...
    if (((SECT(IN_ROOM(ch)) == SECT_WATER_NOSWIM) || (SECT(EXIT(ch, dir)->to_room) == SECT_WATER_NOSWIM)) &&
        ...
        if (IS_KANASSAN(ch) && !has_flight(ch)) {
            act("@CYou swim swiftly.@n", 1, ch, 0, 0, TO_CHAR);
            act("@c$n@C swims swiftly.@n", 1, ch, 0, 0, TO_ROOM);
            ...
            if (!check_swim(ch)) {
                ...
                act("@CYou swim through the cold water.@n", 1, ch, 0, 0, TO_CHAR);
                act("@c$n@C swim through the cold water.@n", 1, ch, 0, 0, TO_ROOM);
                WAIT_STATE(ch, PULSE_1SEC);
            }
        }
    }
    ...
    if (SUNKEN(IN_ROOM(ch)) ||
        ...
        if (!has_o2(ch) && ((group_bonus(ch, 2) != 10 && GET_MANA(ch) < GET_MAX_MANA(ch) / 200) ||
                            (group_bonus(ch, 2) == 10 && GET_MANA(ch) < GET_MAX_MANA(ch) / 800))) {
    ...
    if (AFF_FLAGGED(ch, AFF_SNEAK) && !IS_NPC(ch)) {
        ...
        if (GET_SKILL(ch, SKILL_MOVE_SILENTLY)) {
            ...
            act(buf2, 1, ch, 0, 0, TO_ROOM | TO_SNEAKRESIST);
    ...
    was_in = IN_ROOM(ch);
    if (DRAGGING(ch)) {
        act("@C$n@w drags @c$N@w with $m.@n", 1, ch, 0, DRAGGING(ch), TO_ROOM);
    }
    if (CARRYING(ch)) {
        act("@C$n@w carries @c$N@w with $m.@n", 1, ch, 0, CARRYING(ch), TO_ROOM);
    }
    ...
    act("$n arrives from $T.", 1, ch, 0, buf2, TO_ROOM | TO_SNEAKRESIST);
    if (FIGHTING(ch)) {
    ...
    if (DRAGGING(ch)) {
        act("@wYou drag @C$N@w with you.@n", 1, ch, 0, DRAGGING(ch), TO_CHAR);
        act("@C$n@w drags @c$N@w with $m.@n", 1, ch, 0, DRAGGING(ch), TO_ROOM);
        char_from_room(DRAGGING(ch));
        char_to_room(DRAGGING(ch), IN_ROOM(ch));
        if (SITS(DRAGGING(ch))) {
    ...
    if (CARRYING(ch)) {
        act("@wYou carry @C$N@w with you.@n", 1, ch, 0, CARRYING(ch), TO_CHAR);
        act("@C$n@w carries @c$N@w with $m.@n", 1, ch, 0, CARRYING(ch), TO_ROOM);
        char_from_room(CARRYING(ch));
        char_to_room(CARRYING(ch), IN_ROOM(ch));
    ...
    if (ch->desc != NULL) {
        ...
        if (AFF_FLAGGED(ch, AFF_SNEAK) && !IS_NPC(ch) && GET_SKILL(ch, SKILL_MOVE_SILENTLY) &&
            ...
            act("@c$n@w makes a noise revealing $s sneaking!@n", 1, ch, 0, 0, TO_ROOM | TO_SNEAKRESIST);
            reveal_hiding(ch, 0);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_SNEAK);
        }
    }
    ...
    if (ROOM_EFFECT(IN_ROOM(ch)) == 6 || ROOM_EFFECT(was_in) == 6) {
        if (!IS_DEMON(ch) && !AFF_FLAGGED(ch, AFF_FLYING) && group_bonus(ch, 2) != 14) {
            act("@rYour legs are burned by the lava!@n", 1, ch, 0, 0, TO_CHAR);
            act("@R$n@r's legs are burned by the lava!@n", 1, ch, 0, 0, TO_ROOM);
            ...
            if (GET_SUPP(ch) >= GET_MAX_HIT(ch) / 20) {
                ...
                GET_SUPP(ch) = 0;
                if (GET_HIT(ch) < 0) {
                    act("@rYou have burned to death!@n", 1, ch, 0, 0, TO_CHAR);
                    act("@R$n@r has burned to death!@n", 1, ch, 0, 0, TO_ROOM);
                    die(ch, NULL);
                }
            }
        }
        if (DRAGGING(ch) && !IS_DEMON(DRAGGING(ch))) {
            act("@R$N@r gets burned!@n", 1, ch, 0, DRAGGING(ch), TO_CHAR);
            act("@R$N@r gets burned!@n", 1, ch, 0, DRAGGING(ch), TO_ROOM);
            GET_HIT(DRAGGING(ch)) -= GET_MAX_HIT(DRAGGING(ch)) / 20;
            if (GET_HIT(DRAGGING(ch)) < 0) {
                act("@rYou have burned to death!@n", 1, DRAGGING(ch), 0, 0, TO_CHAR);
                act("@R$n@r has burned to death!@n", 1, DRAGGING(ch), 0, 0, TO_ROOM);
...
bool perform_move(struct char_data *ch, int dir, int need_specials_check) {
    room_rnum was_in;
...
ACMD(do_move) {
    ...
    if (GET_COND(ch, DRUNK) > 4 && (rand_number(1, 9) + GET_COND(ch, DRUNK)) >= rand_number(14, 20)) {
        ...
        act("@C$n@W wobbles around before falling on $s ass@n.", 1, ch, 0, 0, TO_ROOM);
   ...
   */
    ...
    if (!IS_NPC(ch)) {
        ...
        if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_SPACE) && GET_ADMLEVEL(ch) < 1) {
            ...
            act("@wYou slowly pull yourself along with your arm...@n", 1, ch, 0, 0, TO_CHAR);
            act("@C$n@w slowly pulls $mself along with one arm...@n", 1, ch, 0, 0, TO_ROOM);
            if (GET_LIMBCOND(ch, 2) < 50) {
                ...
                if (GET_LIMBCOND(ch, 1) <= 0) {
                    act("@RYour left arm falls apart!@n", 1, ch, 0, 0, TO_CHAR);
                    act("@r$n's@R left arm falls apart!@n", 1, ch, 0, 0, TO_ROOM);
                }
            }
            WAIT_STATE(ch, PULSE_5SEC);
            ...
            if (GET_LIMBCOND(ch, 1) < 50) {
                ...
                if (GET_LIMBCOND(ch, 1) <= 0) {
                    act("@RYour right arm falls apart!@n", 1, ch, 0, 0, TO_CHAR);
                    act("@r$n's@R right arm falls apart!@n", 1, ch, 0, 0, TO_ROOM);
                }
            }
            WAIT_STATE(ch, PULSE_5SEC);
        } else if ((GET_LIMBCOND(ch, 3) <= 0 && GET_LIMBCOND(ch, 4) <= 0) && !AFF_FLAGGED(ch, AFF_FLYING)) {
            act("@wYou slowly pull yourself along with your arms...@n", 1, ch, 0, 0, TO_CHAR);
            act("@C$n@w slowly pulls $mself along with one arms...@n", 1, ch, 0, 0, TO_ROOM);
            if (GET_LIMBCOND(ch, 2) < 50) {
                send_to_char(ch, "@RYour left arm is damaged by the forced use!@n\r\n");
                GET_LIMBCOND(ch, 2) -= rand_number(1, 5);
            ...
            act("@wYou hop on one leg...@n", 1, ch, 0, 0, TO_CHAR);
            act("@C$n@w hops on one leg...@n", 1, ch, 0, 0, TO_ROOM);
            if (GET_LIMBCOND(ch, 4) < 50) {
                ...
                if (GET_LIMBCOND(ch, 4) <= 0) {
                    act("@RYour left leg falls apart!@n", 1, ch, 0, 0, TO_CHAR);
                    act("@r$n's@R left leg falls apart!@n", 1, ch, 0, 0, TO_ROOM);
                }
            }
            WAIT_STATE(ch, PULSE_2SEC);
            ...
            if (GET_LIMBCOND(ch, 3) < 50) {
                ...
                if (GET_LIMBCOND(ch, 3) <= 0) {
                    act("@RYour right leg falls apart!@n", 1, ch, 0, 0, TO_CHAR);
                    act("@r$n's@R right leg falls apart!@n", 1, ch, 0, 0, TO_ROOM);
                }
            }
            WAIT_STATE(ch, PULSE_2SEC);
        } else if (GET_POS(ch) == POS_RESTING) {
            act("@wYou crawl on your hands and knees.@n", 1, ch, 0, 0, TO_CHAR);
            act("@C$n@w crawls on $s hands and knees.@n", 1, ch, 0, 0, TO_ROOM);
            if (SITS(ch)) {
            ...
            act("@wYou shuffle on your hands and knees.@n", 1, ch, 0, 0, TO_CHAR);
            act("@C$n@w shuffles on $s hands and knees.@n", 1, ch, 0, 0, TO_ROOM);
            if (SITS(ch)) {
                struct obj_data *chair = SITS(ch);
                SITTING(chair) = NULL;
                SITS(ch) = NULL;
...
static int find_door(struct char_data *ch, const char *type, char *dir, const char *cmdname) {
    ...
    if (*dir) {            /* a direction was specified */
        if ((door = search_block(dir, dirs, 0)) < 0 &&
            (door = search_block(dir, abbr_dirs, 0)) < 0) {    /* Partial Match */
...
static void do_doorcmd(struct char_data *ch, struct obj_data *obj, int door, int scmd) {
            ...
            if (obj) {
                if ((obj) && GET_OBJ_TYPE(obj) == ITEM_HATCH && (vehicle)) {
                    ...
                    if (GET_OBJ_VNUM(obj) > 19199) {
                        ...
                        act("@wYou open @c$p@w.", 1, ch, obj, 0, TO_CHAR);
                        act("@C$n@w opens @c$p@w.", 1, ch, obj, 0, TO_ROOM);
            ...
            if (obj) {
                if ((obj) && GET_OBJ_TYPE(obj) == ITEM_HATCH && (vehicle)) {
                    ...
                    if (GET_OBJ_VNUM(obj) > 19199) {
                        ...
                        act("@wYou close @c$p@w.", 1, ch, obj, 0, TO_CHAR);
                        act("@C$n@w closes @c$p@w.", 1, ch, obj, 0, TO_ROOM);
    ...
    if (!obj || IN_ROOM(obj) != NOWHERE)
        act(buf, 0, ch, obj, obj ? 0 : EXIT(ch, door)->keyword, TO_ROOM);
...
static bool ok_pick(struct char_data *ch, obj_vnum keynum, int pickproof, int dclock, int scmd, struct obj_data *hatch) {
    int skill_lvl, found = 0;
    ...
    if (keynum == NOTHING) {
        ...
        act("@c$n@w puts a set of lockpick tools away.@n", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_gen_door) {
    ...
    if ((obj) || (door >= 0)) {
        ...
        if (!(DOOR_IS_OPENABLE(ch, obj, door)))
            act("You can't $F that!", 0, ch, 0, cmd_door[subcmd], TO_CHAR);
            ...
            send_to_char(ch, "You don't seem to have the proper key.\r\n");
                 ...
                 ok_pick(ch, keynum, DOOR_IS_PICKPROOF(ch, obj, door), DOOR_DCLOCK(ch, obj, door), subcmd, NULL))
...
static int do_simple_enter(struct char_data *ch, struct obj_data *obj, int need_specials_check) {
    ...
    act("$n enters $p.", 1, ch, obj, 0, TO_ROOM | TO_SNEAKRESIST);
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_PORTAL)
        act("$n arrives from $p.", 0, ch, obj, 0, TO_ROOM | TO_SNEAKRESIST);
    else
        act("$n arrives from outside.", 0, ch, 0, 0, TO_ROOM | TO_SNEAKRESIST);
    if (DRAGGING(ch)) {
        act("@wYou drag @C$N@w with you.@n", 1, ch, 0, DRAGGING(ch), TO_CHAR);
        act("@C$n@w drags @c$N@w with $m.@n", 1, ch, 0, DRAGGING(ch), TO_ROOM);
        if (!AFF_FLAGGED(DRAGGING(ch), AFF_KNOCKED) && !AFF_FLAGGED(DRAGGING(ch), AFF_SLEEP) && rand_number(1, 3)) {
...
static int do_simple_leave(struct char_data *ch, struct obj_data *obj, int need_specials_check) {
    ...
    act("$n leaves $p.", 1, ch, vehicle, 0, TO_ROOM | TO_SNEAKRESIST);
    ...
    if (vehicle) {
        act("$n arrives from inside $p.", 1, ch, vehicle, 0, TO_ROOM | TO_SNEAKRESIST);
    } else {
        act("$n arrives from inside", 1, ch, 0, 0, TO_ROOM | TO_SNEAKRESIST);
    }
    if (DRAGGING(ch)) {
        act("@wYou drag @C$N@w with you.@n", 1, ch, 0, DRAGGING(ch), TO_CHAR);
        act("@C$n@w drags @c$N@w with $m.@n", 1, ch, 0, DRAGGING(ch), TO_ROOM);
    ...
    if (ch->desc != NULL) {
        act(obj->action_description, 1, ch, obj, 0, TO_CHAR);
        look_at_room(IN_ROOM(ch), ch, 0);
    }
...
static void handle_fall(struct char_data *ch) {
    ...
    while (EXIT(ch, 5) && SECT(IN_ROOM(ch)) == SECT_FLYING) {
        ...
        if (!EXIT(ch, 5) || SECT(IN_ROOM(ch)) != SECT_FLYING) {
            act("@r$n slams into the ground!@n", 1, ch, 0, 0, TO_ROOM);
            if (GET_HIT(ch) - GET_MAX_HIT(ch) / 20 <= 0) {
                GET_HIT(ch) = 1;
            } else {
                GET_HIT(ch) -= GET_MAX_HIT(ch) / 20;
            }
            act("@rYou slam into the ground!@n", 1, ch, 0, 0, TO_CHAR);
            look_at_room(IN_ROOM(ch), ch, 0);
        } else {
            act("@r$n pummets down toward the ground below!@n", 1, ch, 0, 0, TO_ROOM);
        }
    }
    if (SECT(IN_ROOM(ch)) == SECT_WATER_NOSWIM && !CARRIED_BY(ch) && !IS_KANASSAN(ch)) {
        if (GET_MOVE(ch) >= gear_weight(ch)) {
            act("@bYou swim in place.@n", 1, ch, 0, 0, TO_CHAR);
            act("@C$n@b swims in place.@n", 1, ch, 0, 0, TO_ROOM);
            ...
            act("@RYou are drowning!@n", 1, ch, 0, 0, TO_CHAR);
            act("@C$n@b gulps water as $e struggles to stay above the water line.@n", 1, ch, 0, 0, TO_ROOM);
            if (GET_HIT(ch) - (gear_pl(ch) / 3) <= 0) {
                act("@rYou drown!@n", 1, ch, 0, 0, TO_CHAR);
                act("@R$n@r drowns!@n", 1, ch, 0, 0, TO_ROOM);
                die(ch, NULL);
                GET_HIT(ch) = 1;
            } else {
...
ACMD(do_fly) {
    ...
    if (!*arg) {
        if (AFF_FLAGGED(ch, AFF_FLYING) && SECT(IN_ROOM(ch)) != SECT_FLYING && SECT(IN_ROOM(ch)) != SECT_SPACE) {
            act("@WYou slowly settle down to the ground.@n", 1, ch, 0, 0, TO_CHAR);
            act("@W$n slowly settles down to the ground.@n", 1, ch, 0, 0, TO_ROOM);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_FLYING);
            GET_ALT(ch) = 0;
            return;
        }
        ...
        if (AFF_FLAGGED(ch, AFF_FLYING) && SECT(IN_ROOM(ch)) == SECT_FLYING) {
            act("@WYou begin to plummet to the ground!@n", 1, ch, 0, 0, TO_CHAR);
            act("@W$n starts to pummet to the ground below!@n", 1, ch, 0, 0, TO_ROOM);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_FLYING);
            GET_ALT(ch) = 0;
            handle_fall(ch);
            return;
        ...
        if (AFF_FLAGGED(ch, AFF_FLYING) && SECT(IN_ROOM(ch)) == SECT_SPACE) {
            act("@WYou let yourself drift aimlessly through space.@n", 1, ch, 0, 0, TO_CHAR);
            act("@W$n starts to drift slowly.!@n", 1, ch, 0, 0, TO_ROOM);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_FLYING);
            GET_ALT(ch) = 0;
            return;
        }
        if (GET_MANA(ch) < GET_MAX_MANA(ch) / 100 && !IS_ANDROID(ch)) {
            ...
            reveal_hiding(ch, 0);
            act("@WYou slowly take off into the sky.@n", 1, ch, 0, 0, TO_CHAR);
            act("@W$n slowly takes off into the sky.@n", 1, ch, 0, 0, TO_ROOM);
            if (SITS(ch)) {
    ...
    if (!strcasecmp("high", arg)) {
        if (GET_MANA(ch) < GET_MAX_MANA(ch) / 100 && !IS_ANDROID(ch)) {
            ...
            reveal_hiding(ch, 0);
            act("@WYou rocket high into the sky.@n", 1, ch, 0, 0, TO_CHAR);
            act("@W$n rockets high into the sky.@n", 1, ch, 0, 0, TO_ROOM);
            if (SITS(ch)) {
                SITTING(SITS(ch)) = NULL;
                SITS(ch) = NULL;
            }
    ...
    if (!strcasecmp("space", arg)) {
        ...
        if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_EARTH)) {
            ...
            act("@C$n blasts up from the atmosphere below and then comes to a stop.@n", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_stand) {
    ...
    switch (GET_POS(ch)) {
        ...
        case POS_SITTING:
            ...
            act("$n clambers to $s feet.", 1, ch, 0, 0, TO_ROOM);
            if (SITS(ch)) {
                if (CAN_WEAR(SITS(ch), ITEM_WEAR_TAKE) && GET_OBJ_TYPE(SITS(ch)) != ITEM_CHAIR &&
                    ...
                    act("You pick up $p.", 1, ch, SITS(ch), 0, TO_CHAR);
                    act("$n picks up $p.", 1, ch, SITS(ch), 0, TO_ROOM);
                }
                chair = SITS(ch);
                SITTING(chair) = NULL;
                SITS(ch) = NULL;
        ...
        case POS_RESTING:
            ...
            act("$n stops resting, and clambers to $s feet.", 1, ch, 0, 0, TO_ROOM);
            if (SITS(ch)) {
...
ACMD(do_sit) {
    ...
    if (!*arg) {
        ...
        switch (GET_POS(ch)) {
            case POS_STANDING:
                reveal_hiding(ch, 0);
                act("You sit down on $p.", 0, ch, chair, 0, TO_CHAR);
                act("$n sits down on $p.", 0, ch, chair, 0, TO_ROOM);
                GET_POS(ch) = POS_SITTING;
                SITS(ch) = chair;
                SITTING(chair) = ch;
                break;
    ...
    if (!*arg) {
        ...
        switch (GET_POS(ch)) {
            case POS_STANDING:
                ...
                act("$n lays down and rests.", 1, ch, 0, 0, TO_ROOM);
        ...
        switch (GET_POS(ch)) {
            case POS_STANDING:
                reveal_hiding(ch, 0);
                act("You lay down and rest on $p.", 1, ch, chair, 0, TO_CHAR);
                act("$n lays down and rests on $p.", 1, ch, chair, 0, TO_ROOM);
                SITS(ch) = chair;
                SITTING(chair) = ch;
                GET_POS(ch) = POS_RESTING;
                break;
...
ACMD(do_sleep) {
    ...
    if (!*arg) {
        ...
        switch (GET_POS(ch)) {
            ...
            case POS_STANDING:
                reveal_hiding(ch, 0);
                act("You lay down on $p and sleep.", 0, ch, chair, 0, TO_CHAR);
                act("$n lays down on $p and sleeps.", 0, ch, chair, 0, TO_ROOM);
                /* Fury Mode Loss for halfbreeds */
...
ACMD(do_wake) {
    ...
    if (*arg) {
        if (GET_POS(ch) == POS_SLEEPING)
            ...
            act("$E is already awake.", 0, ch, 0, vict, TO_CHAR);
        else if (AFF_FLAGGED(vict, AFF_SLEEP))
            act("You can't wake $M up!", 0, ch, 0, vict, TO_CHAR);
        else if (GET_POS(vict) < POS_SLEEPING)
            act("$E's in pretty bad shape!", 0, ch, 0, vict, TO_CHAR);
            ...
            act("You wake $M up.", 0, ch, 0, vict, TO_CHAR);
            act("You are awakened by $n.", 0, ch, 0, vict, TO_VICT | TO_SLEEP);
            GET_POS(vict) = POS_SITTING;
            if (DRAGGED(vict)) {
                act("@WYou stop dragging @C$N@W!@n", 1, DRAGGED(vict), 0, vict, TO_CHAR);
                act("@C$n@W stops dragging @c$N@W!@n", 1, DRAGGED(vict), 0, vict, TO_ROOM);
    ...
    if (AFF_FLAGGED(ch, AFF_SLEEP))
        ...
        if (DRAGGED(ch)) {
            act("@WYou stop dragging @C$N@W!@n", 1, DRAGGED(ch), 0, ch, TO_CHAR);
            act("@C$n@W stops dragging you!@n", 1, DRAGGED(ch), 0, ch, TO_VICT);
            act("@C$n@W stops dragging @c$N@W!@n", 1, DRAGGED(ch), 0, ch, TO_NOTVICT);
...
ACMD(do_follow) {
    ...
    if (ch->master == leader) {
        act("You are already following $M.", 0, ch, 0, leader, TO_CHAR);
        return;
    }
    if (AFF_FLAGGED(ch, AFF_CHARM) && (ch->master)) {
