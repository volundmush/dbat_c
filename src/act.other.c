...
static bool spell_in_book(struct obj_data *obj, int spellnum);

static bool spell_in_scroll(struct obj_data *obj, int spellnum);

static bool spell_in_domain(struct char_data *ch, int spellnum);
...
void bring_to_cap(struct char_data *ch) {
    int skippl = 0, skipki = 0, skipst = 0, mult = 1;
...
ACMD(do_rpp) {
    ...
    if (!*arg) { /* Display menu */
        ...
        send_to_char(ch,
                     "  @C3@D)@c Custom Equipment    @D[@G 30 RPP @D]  @C4@D)@c Alignment Change      @D[@G 20 RPP @D]\n");
        send_to_char(ch,
        ...
        send_to_char(ch,
                     ...
                     "%", tnlcost);
        send_to_char(ch,
                     " @C11@D)@c Reach Softcap       @D[@G  6 RPP @D] @C12@D)@c RPP Store             @D[@G ?? RPP @D]\n");
        send_to_char(ch,
                     " @C13@D)@c Extra Feature       @D[@G  1 RPP @D] @C14@D)@c Restring Equipment    @D[@G  1 RPP @D]\n");
        send_to_char(ch,
                     " @C15@D)@c Extra Skillslot     @D[@G  5 RPP @D] @C16@D)@R Disabled              @D[@G -- RPP @D]@n\n");
        send_to_char(ch, "@b  ------------------------------------------------------------------@n\n");
    ...
    if (selection > 2) {
        ...
        if (selection == 8) { /* Simple Revival Reward */
            ...
            if (GET_RP(ch) < pay) {
                ...
                act("$n's body forms in a pool of @Bblue light@n.", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_commune) {
    ...
    if (prob < perc) {
        ...
        act("@cYou close your eyes and try to commune with the Eldritch Star. You are unable to concentrate though.@n",
            1, ch, 0, 0, TO_CHAR);
        act("@W$n closes $s eyes for a moment. Then $e reopens them and frowns.@n", 1, ch, 0, 0, TO_ROOM);
        ...
        act("@cYou close your eyes and commune with the Eldritch Star spiritually. You feel your stamina replenish some.@n",
            1, ch, 0, 0, TO_CHAR);
        act("@W$n closes $s eyes for a moment. Then $e reopens them and smiles.@n", 1, ch, 0, 0, TO_ROOM);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    }
}
...
ACMD(do_grapple) {
  ...
  return;*/
    ...
    if (GRAPPLING(ch) != NULL) {
        act("@RYou stop grappling with @r$N@R!@n", 1, ch, 0, GRAPPLING(ch), TO_CHAR);
        act("@r$n@R stops grappling with @rYOU!!@n", 1, ch, 0, GRAPPLING(ch), TO_VICT);
        act("@r$n@R stops grappling with @r$N@R!@n", 1, ch, 0, GRAPPLING(ch), TO_NOTVICT);
    ...
    if (!strcasecmp("hold", arg2) || !strcasecmp("choke", arg2) || !strcasecmp("grab", arg2) ||
        ...
        if (((!IS_NPC(vict) && IS_ICER(vict) && rand_number(1, 30) >= 28) || AFF_FLAGGED(vict, AFF_ZANZOKEN)) &&
            ...
            if (!AFF_FLAGGED(ch, AFF_ZANZOKEN) || (AFF_FLAGGED(ch, AFF_ZANZOKEN) && GET_SPEEDI(ch) + rand_number(1, 5) <
                                                                                    ...
                                                                                    rand_number(1, 5))) {
                reveal_hiding(ch, 0);
                act("@C$N@c disappears, avoiding your grapple attempt before reappearing!@n", 0, ch, 0, vict,
                    TO_CHAR);
                act("@cYou disappear, avoiding @C$n's@c grapple attempt before reappearing!@n", 0, ch, 0, vict,
                    TO_VICT);
                act("@C$N@c disappears, avoiding @C$n's@c grapple attempt before reappearing!@n", 0, ch, 0, vict,
                    TO_NOTVICT);
                ...
                reveal_hiding(ch, 0);
                act("@C$N@c disappears, trying to avoid your grapple but your zanzoken is faster!@n", 0, ch, 0,
                    vict, TO_CHAR);
                act("@cYou zanzoken to avoid the grapple attempt but @C$n's@c zanzoken is faster!@n", 0, ch, 0,
                    vict, TO_VICT);
        ...
        if ((GET_HIT(ch) * 0.02) * GET_STR(ch) < (GET_HIT(vict) * 0.01) * GET_STR(vict)) {
            reveal_hiding(ch, 0);
            act("@RYou try to grapple with @r$N@R, but $E manages to overpower you!@n", 1, ch, 0, vict, TO_CHAR);
            act("@r$n@R tries to grapple with YOU, but you manage to overpower $m!@n", 1, ch, 0, vict, TO_VICT);
            act("@r$n@R tries to grapple with @r$N@R, but $E manages to overpower @r$n@R!@n", 1, ch, 0, vict,
                TO_NOTVICT);
            GET_MOVE(ch) -= cost;
            ...
            reveal_hiding(ch, 0);
            act("@RYou try to grapple with @r$N@R, but $E manages to avoid it!@n", 1, ch, 0, vict, TO_CHAR);
            act("@r$n@R tries to grapple with YOU, but you manage to avoid it!@n", 1, ch, 0, vict, TO_VICT);
            act("@r$n@R tries to grapple with @r$N@R, but $E manages to avoid it!@n", 1, ch, 0, vict, TO_NOTVICT);
            GET_MOVE(ch) -= cost;
            improve_skill(ch, SKILL_GRAPPLE, 1);
            WAIT_STATE(ch, PULSE_4SEC);
            return;
            ...
            reveal_hiding(ch, 0);
            act("@RYou rush at @r$N@R and manage to get $M in a hold from behind!@n", 1, ch, 0, vict, TO_CHAR);
            act("@r$n@R rushes at YOU and manages to get you in a hold from behind!@n", 1, ch, 0, vict, TO_VICT);
            act("@r$n@R rushes at @r$N@R and manages to get $M in a hold from behind!@n", 1, ch, 0, vict,
                TO_NOTVICT);
            ...
            reveal_hiding(ch, 0);
            act("@RYou rush at @r$N@R and manage to grab $S throat with both hands!@n", 1, ch, 0, vict, TO_CHAR);
            act("@r$n@R rushes at YOU and manages to grab your throat with both hands!@n", 1, ch, 0, vict, TO_VICT);
            act("@r$n@R rushes at @r$N@R and manages to grab $S throat with both hands!@n", 1, ch, 0, vict,
                TO_NOTVICT);
            ...
            reveal_hiding(ch, 0);
            act("@RYou rush at @r$N@R and manage to lock your arm onto $S!@n", 1, ch, 0, vict, TO_CHAR);
            act("@r$n@R rushes at YOU and manages to lock $s arm onto your's!@n", 1, ch, 0, vict, TO_VICT);
            act("@r$n@R rushes at @r$N@R and manages to lock $s arm onto @r$N's@R!@n", 1, ch, 0, vict, TO_NOTVICT);
    ...
    if (vict != NULL) {
        ...
        if (((!IS_NPC(vict) && IS_ICER(vict) && rand_number(1, 30) >= 28) || AFF_FLAGGED(vict, AFF_ZANZOKEN)) &&
            ...
            if (!AFF_FLAGGED(ch, AFF_ZANZOKEN) || (AFF_FLAGGED(ch, AFF_ZANZOKEN) && GET_SPEEDI(ch) + rand_number(1, 5) <
                                                                                    ...
                                                                                    rand_number(1, 5))) {
                reveal_hiding(ch, 0);
                act("@C$N@c disappears, avoiding your trip before reappearing!@n", 0, ch, 0, vict, TO_CHAR);
                act("@cYou disappear, avoiding @C$n's@c trip before reappearing!@n", 0, ch, 0, vict, TO_VICT);
                act("@C$N@c disappears, avoiding @C$n's@c trip before reappearing!@n", 0, ch, 0, vict, TO_NOTVICT);
                if (AFF_FLAGGED(ch, AFF_ZANZOKEN)) {
                    REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_ZANZOKEN);
                }
                REMOVE_BIT_AR(AFF_FLAGS(vict), AFF_ZANZOKEN);
                ...
                reveal_hiding(ch, 0);
                act("@C$N@c disappears, trying to avoid your trip but your zanzoken is faster!@n", 0, ch, 0, vict,
                    TO_CHAR);
                act("@cYou zanzoken to avoid the trip but @C$n's@c zanzoken is faster!@n", 0, ch, 0, vict, TO_VICT);
                act("@C$N@c disappears, trying to avoid @C$n's@c trip but @C$n's@c zanzoken is faster!@n", 0, ch, 0,
                    vict, TO_NOTVICT);
                REMOVE_BIT_AR(AFF_FLAGS(vict), AFF_ZANZOKEN);
                REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_ZANZOKEN);
            }
        ...
        if (perc < prob) { /* Fail! */
            reveal_hiding(ch, 0);
            act("@mYou move to trip $N@m, but you screw up and $E keeps $S footing!@n", 1, ch, 0, vict, TO_CHAR);
            act("@m$n@m moves to trip YOU, but $e screws up and you manage to keep your footing!@n", 1, ch, 0, vict,
                TO_VICT);
            act("@m$n@m moves to trip $N@m, but $e screws up and $N@m manages to keep $S footing!@n", 1, ch, 0, vict,
                TO_NOTVICT);
            ...
            reveal_hiding(ch, 0);
            act("@mYou move to trip $N@m, and manage to knock $M off $S feet!@n", 1, ch, 0, vict, TO_CHAR);
            act("@m$n@m moves to trip YOU, and manages to knock you off your feet!@n", 1, ch, 0, vict, TO_VICT);
            act("@m$n@m moves to trip $N@m, and manages to knock $N@m off $S feet!@n", 1, ch, 0, vict, TO_NOTVICT);
            improve_skill(ch, SKILL_TRIP, 0);
            GET_MOVE(ch) -= cost;
...
ACMD(do_train) {
    ...
    if (GET_ROOM_VNUM(IN_ROOM(ch)) == 1131 && GET_GOLD(ch) >= 8 && GET_PRACTICES(ch, GET_CLASS(ch)) >= 1) {
        ...
        send_to_char(ch,
                     "@YYou do not have enough zenni (5) in order to train with your sensei. Train on your own elsewhere.@n\r\n");
    } else if (GET_ROOM_VNUM(IN_ROOM(ch)) == 1131 && GET_PRACTICES(ch, GET_CLASS(ch)) < 1) {
        send_to_char(ch,
                     "@YYou do not have enough PS (1) in order to train with your sensei. Train on your own elsewhere.@n\r\n");
    ...
    if (!strcasecmp("str", arg)) {
        ...
        switch (rand_number(1, 3)) {
            case 1:
                act("@WYou throw a flurry of punches into the air at an invisible opponent.@n", 1, ch, 0, 0,
                    TO_CHAR);
                act("@W$n throws a flurry of punches into the air.@n", 1, ch, 0, 0, TO_ROOM);
                break;
            case 2:
                act("@WYou leap into the air and throw a wild kick at an invisible opponent@n", 1, ch, 0, 0,
                    TO_CHAR);
                act("@W$n leaps into the air and throws a wild kick at nothing.@n", 1, ch, 0, 0, TO_ROOM);
                break;
            case 3:
                act("@WYou leap high into the air and unleash a flurry of punches and kicks at an invisible opponent@n",
                    1, ch, 0, 0, TO_CHAR);
                act("@W$n leaps high into the air and unleashes a flurry of punches and kicks at nothing.@n", 1, ch,
                    0, 0, TO_ROOM);
                break;
        }
        ...
        switch (rand_number(1, 3)) {
            case 1:
                act("@WYou dash quickly around the surrounding area as fast as you can!@n", 1, ch, 0, 0, TO_CHAR);
                act("@W$n dashes quickly around the surrounding area as fast as $e can!@n", 1, ch, 0, 0, TO_ROOM);
                break;
            case 2:
                act("@WYou dodge to the side as fast as you can!@n", 1, ch, 0, 0, TO_CHAR);
                act("@W$n dodges to the side as fast as $e can!@n", 1, ch, 0, 0, TO_ROOM);
                break;
            case 3:
                act("@WYou dash backwards as fast as you can!@n", 1, ch, 0, 0, TO_CHAR);
                act("@W$n dashes backwards as fast as $e can!@n", 1, ch, 0, 0, TO_ROOM);
                break;
        }
        plus = (((total / 20) + (GET_MAX_MOVE(ch) / 50)) * 100) / GET_MAX_MOVE(ch);
        if (GET_LEVEL(ch) > 80) {
        ...
        switch (rand_number(1, 3)) {
            case 1:
                act("@WYou leap into the air and then slam into the ground with your feet outstretched!@n", 1, ch, 0,
                    0, TO_CHAR);
                act("@W$n leaps into the air and then slams into the ground with $s feet outstretched!?@n", 1, ch, 0,
                    0, TO_ROOM);
                break;
            case 2:
                act("@WYou leap into the air and then slam into the ground with your fists!@n", 1, ch, 0, 0,
                    TO_CHAR);
                act("@W$n leaps into the air and then slams into the ground with $s fists!?@n", 1, ch, 0, 0,
                    TO_ROOM);
                break;
            case 3:
                act("@WYou leap into the air and then slam into the ground with your body!@n", 1, ch, 0, 0, TO_CHAR);
                act("@W$n leaps into the air and then slams into the ground with $s body!?@n", 1, ch, 0, 0, TO_ROOM);
                break;
        }
        plus = (((total / 20) + (GET_MAX_MOVE(ch) / 50)) * 100) / GET_MAX_MOVE(ch);
        if (GET_LEVEL(ch) > 80) {
        ...
        switch (rand_number(1, 3)) {
            ...
            case 2:
                ...
                break;
            case 3:
                act("@WYou flip to the side off one hand and then land on your feet.@n", 1, ch, 0, 0, TO_CHAR);
                act("@W$n flips to the side off one hand and then lands on $s feet.@n", 1, ch, 0, 0, TO_ROOM);
                break;
        }
        plus = (((total / 20) + (GET_MAX_MOVE(ch) / 50)) * 100) / GET_MAX_MOVE(ch);
        if (GET_LEVEL(ch) > 80) {
        ...
        switch (rand_number(1, 3)) {
            case 1:
                act("@WConcentrating you fly high into the air as fast as you can before settling slowly back to the ground.@n",
                    1, ch, 0, 0, TO_CHAR);
                act("@W$n flies high into the air as fast as $e can before settling slowly back to the ground.@n", 1,
                    ch, 0, 0, TO_ROOM);
                break;
            case 2:
                act("@WYou focus your ki at your outstretched hand and send a mild shockwave in that direction!@n",
                    1, ch, 0, 0, TO_CHAR);
                act("@W$n focuses $s ki at $s outstretched hand and sends a mild shockwave in that direction!@n", 1,
                    ch, 0, 0, TO_ROOM);
                break;
            case 3:
                act("@WYou concentrate on your ki and force torrents of it to rush out from your body randomly!@n",
                    1, ch, 0, 0, TO_CHAR);
                act("@W$n seems to concentrate before torrents of ki randomly blasts out from $s body!@n", 1, ch, 0,
                    0, TO_ROOM);
                break;
        }
        ...
        switch (rand_number(1, 3)) {
            case 1:
                act("@WYou close your eyes and wage a mental battle against an imaginary opponent.@n", 1, ch, 0, 0,
                    TO_CHAR);
                act("@W$n closes $s eyes for a moment and an expression of intensity forms on it.@n", 1, ch, 0, 0,
                    TO_ROOM);
                break;
            case 2:
                act("@WYou look around and contemplate battle tactics for an imaginary scenario.@n", 1, ch, 0, 0,
                    TO_CHAR);
                act("@W$n looks around and appears to be imagining things that aren't there.@n", 1, ch, 0, 0,
                    TO_ROOM);
                break;
            case 3:
                act("@WYou invent a battle plan for a battle that doesn't exist!@n", 1, ch, 0, 0, TO_CHAR);
                act("@W$n seems to have thought of something.@n", 1, ch, 0, 0, TO_ROOM);
                break;
        }
        plus = (((total / 20) + (GET_MAX_MANA(ch) / 50)) * 100) / GET_MAX_MANA(ch);
        if (GET_LEVEL(ch) > 80) {
    ...
    if (ch != vict && GET_POS(ch) > POS_SLEEPING) {
        if (GET_MOVE(ch) < GET_MAX_MOVE(ch) / 20) {
            ...
            if (GET_HIT(ch) > GET_HIT(vict) * 2) {
                reveal_hiding(ch, 0);
                act("@rYou rush at @R$N@r and grab $S tail! With a powerful tug you pull it off!@n", 1, ch, 0, vict,
                    TO_CHAR);
                act("@R$n@r rushes at YOU and grabs your tail! With a powerful tug $e pulls it off!@n", 1, ch, 0,
                    vict, TO_VICT);
                act("@R$n@R rushes at @R$N@r and grab $S tail! With a powerful tug $e pulls it off!@n", 1, ch, 0,
                    vict, TO_NOTVICT);
                ...
                reveal_hiding(ch, 0);
                act("@rYou rush at @R$N@r and grab $S tail! You are too weak to pull it off though!@n", 1, ch, 0,
                    vict, TO_CHAR);
                act("@R$n@r rushes at YOU and grabs your tail! $e is too weak to pull it off though!@n", 1, ch, 0,
                    vict, TO_VICT);
                act("@R$n@R rushes at @R$N@r and grab $S tail! $e is too weak to pull it off though!@n", 1, ch, 0,
                    vict, TO_NOTVICT);
                return;
            }
        } else {
            GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 20;
            reveal_hiding(ch, 0);
            act("@rYou rush at @R$N@r and try to grab $S tail, but fail!@n", 1, ch, 0, vict, TO_CHAR);
            act("@R$n@r rushes at YOU and tries to grab your tail, but fails!@n", 1, ch, 0, vict, TO_VICT);
            act("@R$n@R rushes at @R$N@r and tries to grab $S tail, but fails!@n", 1, ch, 0, vict, TO_NOTVICT);
            return;
        }
    } else if (ch == vict) {
        reveal_hiding(ch, 0);
        act("@rYou grab your own tail and yank it off!@n", 1, ch, 0, 0, TO_CHAR);
        act("@R$n@r grabs $s own tail and yanks it off!@n", 1, ch, 0, 0, TO_ROOM);
        ...
        GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 20;
        reveal_hiding(ch, 0);
        act("@rYou reach and grab @R$N's@r tail! With a powerful tug you pull it off!@n", 1, ch, 0, vict, TO_CHAR);
        act("@RYou feel your tail pulled off!@n", 1, ch, 0, vict, TO_VICT);
        act("@R$n@R reaches and grabs @R$N's@r tail! With a powerful tug $e pulls it off!@n", 1, ch, 0, vict,
            TO_NOTVICT);
        REMOVE_BIT_AR(PLR_FLAGS(vict), PLR_TAIL);
        REMOVE_BIT_AR(PLR_FLAGS(vict), PLR_STAIL);
        oozaru_drop(vict);
...
ACMD(do_infuse) {
    ...
    if (AFF_FLAGGED(ch, AFF_INFUSE)) {
        act("You stop infusing ki into your attacks.", 1, ch, 0, 0, TO_CHAR);
        act("$n stops infusing ki into $s attacks.", 1, ch, 0, 0, TO_ROOM);
    ...
    reveal_hiding(ch, 0);
    act("You start infusing ki into your attacks.", 1, ch, 0, 0, TO_CHAR);
    act("$n starts infusing ki into $s attacks.", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_paralyze) {
    ...
    if (GET_BONUS(vict, BONUS_INSOMNIAC)) {
        ...
        act("@RYou focus ki and point both your arms at @r$N@R. However $E manages to avoid your attempt to paralyze $M!@n",
            1, ch, 0, vict, TO_CHAR);
        act("@r$n @Rfocuses ki and points both $s arms at YOU! You manage to avoid $s technique though...@n", 1, ch,
            0, vict, TO_VICT);
...
ACMD(do_taisha) {
    ...
    if (prob < perc) {
        reveal_hiding(ch, 0);
        act("@WYou hold up your hands while channeling ki. Your technique fails to produce an aura though....@n", 1,
            ch, 0, 0, TO_CHAR);
        act("@g$n@W holds up $s hands while channeling ki. $s technique fails to produce an aura though....", 1, ch,
            0, 0, TO_ROOM);
...
ACMD(do_candy) {
    ...
    if (rand_number(1, 6) == 6) {
        ...
        reveal_hiding(ch, 0);
        act("@cYou aim your forelock at @R$N@c and fire a beam of energy but it is dodged!@n", 1, ch, 0, vict,
            TO_CHAR);
        act("@C$n@c aims $s forelock at @R$N@c and fires a beam of energy but the beam is dodged!@n", 1, ch, 0, vict,
            TO_NOTVICT);
        if (!FIGHTING(ch)) {
            set_fighting(ch, vict);
        }
        ...
        if (GET_MAX_HIT(vict) >= GET_MAX_HIT(ch) * 1.5) {
            ...
            obj_to_char(obj, ch);
                   ...
                   (((GET_MAX_HIT(vict) >= GET_MAX_HIT(ch) * .5) && GET_LEVEL(ch) < 100) ||
                    ((GET_MAX_HIT(vict) >= GET_MAX_HIT(ch) * .1) && GET_LEVEL(ch) >= 100))) {
            send_to_char(ch, "You grab the candy as it falls.\r\n");
...
ACMD(do_future) {
    ...
    if (vict != ch) {
        ...
        GET_PRACTICES(ch, GET_CLASS(ch)) -= 100;
        reveal_hiding(ch, 0);
        act("@CYou focus your energy into your mind and awaken your latent Future Sight powers!@n", 1, ch, 0, vict,
            TO_CHAR);
        act("@C$n focuses $s energy while closing $s eyes for a moment.@n", 1, ch, 0, vict, TO_VICT);
        act("@C$n focuses $s energy while closing $s eyes for a moment.@n", 1, ch, 0, vict, TO_NOTVICT);
...
ACMD(do_drag) {
    ...
    if (DRAGGING(ch)) {
        ...
        DRAGGING(ch) = NULL;
        DRAGGED(vict) = NULL;
        act("@wYou stop dragging @C$N@W.@n", 1, ch, 0, vict, TO_CHAR);
        act("@C$n@W stops dragging @c$N@W.@n", 1, ch, 0, vict, TO_ROOM);
        return;
    }
    ...
    if (GET_POS(vict) != POS_SLEEPING) {
        reveal_hiding(ch, 0);
        act("@wYou try to grab and pull @C$N@W with you, but $E resists!@n", 1, ch, 0, vict, TO_CHAR);
        act("@C$n@W tries to grab and pull you! However you resist!@n", 1, ch, 0, vict, TO_VICT);
        act("@C$n@W tries to grab and pull @c$N@W but $E resists!@n", 1, ch, 0, vict, TO_NOTVICT);
        ...
        reveal_hiding(ch, 0);
        act("@wYou try to grab and pull @C$N@W with you, but $E is too heavy!@n", 1, ch, 0, vict, TO_CHAR);
        act("@C$n@W tries to grab and pull @c$N@W but $E is too heavy!@n", 1, ch, 0, vict, TO_ROOM);
        return;
    } else {
        reveal_hiding(ch, 0);
        act("@wYou grab and start dragging @C$N@W.@n", 1, ch, 0, vict, TO_CHAR);
        act("@C$n@W grabs and starts dragging @c$N@W.@n", 1, ch, 0, vict, TO_NOTVICT);
        DRAGGING(ch) = vict;
        DRAGGED(vict) = ch;
...
ACMD(do_stop) {
    ...
    if (!FIGHTING(ch)) {
        ...
        act("@CYou move out of your fighting posture.@n", 1, ch, 0, 0, TO_CHAR);
        act("@c$n@C moves out of $s fighting posture.@n", 1, ch, 0, 0, TO_ROOM);
        stop_fighting(ch);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    }
...
ACMD(do_suppress) {
    ...
    if (!strcasecmp(arg, "release")) {
        if (GET_SUPPRESS(ch)) {
            reveal_hiding(ch, 0);
            act("@GYou stop suppressing your current powerlevel!@n", 1, ch, 0, 0, TO_CHAR);
            act("@G$n smiles as a rush of power erupts around $s body briefly.@n", 1, ch, 0, 0, TO_ROOM);
    ...
    if (GET_HIT(ch) < amt && GET_SUPPRESS(ch) == 0) {
        ...
        if (GET_SUPP(ch) + GET_HIT(ch) < amt) {
            ...
            reveal_hiding(ch, 0);
            act("@GYou alter your suppression level!@n", 1, ch, 0, 0, TO_CHAR);
            act("@G$n seems to concentrate for a moment.@n", 1, ch, 0, 0, TO_ROOM);
    ...
    reveal_hiding(ch, 0);
    act("@GYou suppress your current powerlevel!@n", 1, ch, 0, 0, TO_CHAR);
    act("@G$n seems to concentrate for a moment.@n", 1, ch, 0, 0, TO_ROOM);
    int64_t diff = GET_HIT(ch) - amt;
    GET_SUPP(ch) = diff;
    GET_HIT(ch) = amt;
...
ACMD(do_hass) {
    ...
    if (perc < prob) {
        reveal_hiding(ch, 0);
        act("@WYou try to move your arms at incredible speeds but screw up and waste some of your stamina.@n", 1, ch,
            0, 0, TO_CHAR);
        act("@C$n@W tries to move $s arms at incredible speeds but screws up and wastes some of $s stamina.@n", 1,
            ch, 0, 0, TO_ROOM);
        ...
        reveal_hiding(ch, 0);
        act("@WYou concentrate and start to move your arms at incredible speeds.@n", 1, ch, 0, 0, TO_CHAR);
        act("@C$n@W concentrates and starts to move $s arms at incredible speeds.@n", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_implant) {
    ...
    for (obj = ch->carrying; obj; obj = next_obj) {
        ...
        if (found == 0 && GET_OBJ_VNUM(obj) == 66 &&
...
ACMD(do_pose) {
    ...
    if (prob < perc) {
        reveal_hiding(ch, 0);
        act("@WYou attempt to strike an awe inspiring pose, but end up falling on your face!@n", 1, ch, 0, 0,
            TO_CHAR);
        act("@C$n@W attempts to strike an awe inspiring pose, but ends up falling on $s face!@n", 1, ch, 0, 0,
            TO_ROOM);
        ...
        switch (rand_number(1, 4)) {
            ...
            case 2:
                ...
                break;
            case 3:
                act("@WYou extend one leg outward while you bend forward, balancing on a single leg!@n", 1, ch, 0, 0,
                    TO_CHAR);
                act("@C$n@W extends one leg outward while $e bends forward, balancing on a single leg!@n", 1, ch, 0,
                    0, TO_ROOM);
                break;
            case 4:
...
ACMD(do_fury) {
    ...
    reveal_hiding(ch, 0);
    act("You release your fury! Your very next attack is guaranteed to rip your foes a new one!", 1, ch, 0, 0,
        TO_CHAR);
    act("$n screams furiously as a look of anger appears on $s face!", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_telepathy) {
    ...
    if (!(strcmp(arg, "link"))) {
        ...
        if (MINDLINK(ch)) {
            act("@CYou remove the link your mind had with @w$N.@n", 1, ch, 0, MINDLINK(ch), TO_CHAR);
            act("@w$n@C removes the link $s mind had with yours.@n", 1, ch, 0, MINDLINK(ch), TO_VICT);
            ...
            act("@R$n@r tried to link $s mind with yours, but you manage to force a break in the link!@n", 0, ch, 0,
                vict, TO_VICT);
            act("@R$N@r manages to sense the intrusion and with $S intelligence push you out!@n", 0, ch, 0, vict,
                TO_CHAR);
            return;
        } else {
            act("@CYou link your mind with @w$N.@n", 1, ch, 0, vict, TO_CHAR);
            act("@w$n@C links $s mind with yours. You can speak your thoughts to $m with 'think'.@n", 1, ch, 0, vict,
                TO_VICT);
        ...
        if (!(vict = get_char_vis(ch, arg2, NULL, FIND_CHAR_ROOM))) {
            ...
            if (axion_dice(0) > GET_SKILL(ch, SKILL_TELEPATHY)) {
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 40;
                act("@wYou attempt to read $N's@w mind, but fail to see it clearly.@n", 1, ch, 0, vict, TO_CHAR);
                if (rand_number(1, 15) >= 14 && !AFF_FLAGGED(ch, AFF_SHOCKED)) {
                    act("@MYour mind has been shocked!@n", 1, ch, 0, 0, TO_CHAR);
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 40;
                act("@wYou fail to read @c$N's@w mind and they seemed to have noticed the attempt!@n", 1, ch, 0,
                    vict, TO_CHAR);
                act("@C$n@w attempts to read your mind, but you resist and force $m out!@n", 1, ch, 0, vict,
                    TO_VICT);
                improve_skill(ch, SKILL_TELEPATHY, 0);
                return;
            } else {
...
ACMD(do_potential) {
    ...
    if (IS_MAJIN(vict)) {
        ...
        act("$n places $s hand on top of your head. After a moment you feel a rush of power as your hidden potential is released!",
            1, ch, 0, vict, TO_VICT);
        act("$n places $s hand on $N's head. After a moment a rush of power explodes off of $N's body!", 1, ch, 0,
            vict, TO_NOTVICT);
...
ACMD(do_majinize) {
    ...
    if (MAJINIZED(vict) > 0 && MAJINIZED(vict) != GET_ID(ch)) {
        ...
        reveal_hiding(ch, 0);
        act("You remove $N's majinization, freeing them from your influence, but also weakening them.", 1, ch, 0,
            vict, TO_CHAR);
        act("$n removes your majinization, freeing you from their influence, and weakening you!", 1, ch, 0, vict,
            TO_VICT);
        act("$n waves a hand at $N, and instantly the glowing M on $S forehead disappears!", 1, ch, 0, vict,
            TO_NOTVICT);
        MAJINIZED(vict) = 0;
        GET_BOOSTS(ch) += 1;
        double mult = 1;
...
ACMD(do_spit) {
    ...
    if (AFF_FLAGGED(vict, AFF_PARALYZE)) {
        act("$N has already been turned to stone.", 1, ch, 0, vict, TO_CHAR);
        return;
    }
    if (FIGHTING(vict)) {
    ...
    if (GET_SKILL(ch, SKILL_SPIT) < axion_dice(0)) {
        ...
        act("@WGathering spit you concentrate ki into a wicked loogie and let it loose, but it falls short of hitting @c$N@W!@n",
            1, ch, 0, vict, TO_CHAR);
        act("@C$n@W seems to focus ki before hawking a loogie at you! Fortunatly the loogie falls short.@n", 1, ch,
            0, vict, TO_VICT);
        ...
        GET_MANA(ch) -= cost;
        reveal_hiding(ch, 0);
        act("@C$N@c disappears, avoiding your spit before reappearing!@n", 0, ch, 0, vict, TO_CHAR);
        act("@cYou disappear, avoiding @C$n's@c @rstone spit@c before reappearing!@n", 0, ch, 0, vict, TO_VICT);
        act("@C$N@c disappears, avoiding @C$n's@c @rstone spit@c before reappearing!@n", 0, ch, 0, vict,
            TO_NOTVICT);
        REMOVE_BIT_AR(AFF_FLAGS(vict), AFF_ZANZOKEN);
        WAIT_STATE(ch, PULSE_2SEC);
        improve_skill(ch, SKILL_SPIT, 1);
...
ACMD(do_form) {
    int skill = 0, senzu = 0, bag = 0, light = 0, sword = 0, mattress = 0, gi = 0, pants = 0, kachin = 0, boost = 0, shuriken = 0;
    int clothes = 0, wrist = 0, boots = 0, level = 0;
    double discount = 1.0;
    int64_t cost = 0;
    struct obj_data *obj;
    ...
    if (!(strcmp(arg, "food"))) {
        ...
        if (GET_MANA(ch) < cost) {
            ...
            reveal_hiding(ch, 0);
            GET_COOLDOWN(ch) = 10;
            act("You hold out your hand and create $p out of your ki!", 1, ch, obj, 0, TO_CHAR);
            act("$n holds out $s hand and creates $p out of thin air!", 1, ch, obj, 0, TO_ROOM);
            GET_MANA(ch) -= cost;
            return;
        }
...
ACMD(do_recharge) {
    ...
    if (!PLR_FLAGGED(ch, PLR_REPAIR)) {
        ...
        if (GET_MANA(ch) < cost) {
            ...
            reveal_hiding(ch, 0);
            GET_COOLDOWN(ch) = 10;
            act("You focus your ki into your energy reserves, recharging them some.", 1, ch, 0, 0, TO_CHAR);
            act("$n stops and glows green briefly.", 1, ch, 0, 0, TO_ROOM);
            GET_MANA(ch) -= cost;
    ...
    if (!IS_NPC(ch) && !PLR_FLAGGED(ch, PLR_REPAIR)) {
        ...
        if (GET_MOVE(ch) < cost) {
            ...
            act("You repair some of your outer casings and internal systems, with the small nano-robots contained in your body.",
                1, ch, 0, 0, TO_CHAR);
            act("$n stops a moment as small glowing particles move across $s body.", 1, ch, 0, 0, TO_ROOM);
    ...
    if (!strcasecmp("augment", arg)) {
        ...
        if (!(obj = get_obj_in_list_vis(ch, "Augmentation", NULL, ch->carrying))) {
            ...
            if (!strcasecmp("powerlevel", arg2)) {
                obj_from_char(obj);
                extract_obj(obj);
                act("@WYou install the circuits and upgrade your maximum powerlevel.@n", 1, ch, 0, 0, TO_CHAR);
                act("@C$n@W installs some circuits and upgrades $s systems.@n", 1, ch, 0, 0, TO_ROOM);
                GET_MAX_HIT(ch) += gain;
                GET_BASE_PL(ch) += gain;
                ...
                obj_from_char(obj);
                extract_obj(obj);
                act("@WYou install the circuits and upgrade your maximum ki.@n", 1, ch, 0, 0, TO_CHAR);
                act("@C$n@W installs some circuits and upgrades $s systems.@n", 1, ch, 0, 0, TO_ROOM);
                ...
                obj_from_char(obj);
                extract_obj(obj);
                act("@WYou install the circuits and upgrade your maximum stamina.@n", 1, ch, 0, 0, TO_CHAR);
                act("@C$n@W installs some circuits and upgrades $s systems.@n", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_ingest) {
    ...
    if (IS_MAJIN(ch)) {
        ...
        if (AFF_FLAGGED(vict, AFF_ZANZOKEN) && GET_MOVE(vict) >= 1 && GET_POS(vict) != POS_SLEEPING) {
            act("@C$N@c disappears, avoiding your attempted ingestion!@n", 0, ch, 0, vict, TO_CHAR);
            act("@cYou disappear, avoiding @C$n's@c attempted @ringestion@c before reappearing!@n", 0, ch, 0, vict,
                TO_VICT);
            act("@C$N@c disappears, avoiding @C$n's@c attempted @ringestion@c before reappearing!@n", 0, ch, 0,
                vict, TO_NOTVICT);
            REMOVE_BIT_AR(AFF_FLAGS(vict), AFF_ZANZOKEN);
            WAIT_STATE(ch, PULSE_3SEC);
            return;
        ...
        if (GET_SPEEDI(ch) + rand_number(1, 5) < GET_SPEEDI(ch) + rand_number(1, 5)) {
            act("@WYou fling a piece of goo at @c$N@W, and try to ingest $M! $E manages to avoid your blob of goo though!@n",
                1, ch, 0, vict, TO_CHAR);
            act("@C$n@W flings a piece of goo at you, you manage to avoid it though!@n", 1, ch, 0, vict, TO_VICT);
            act("@C$n@w flings a piece of goo at @c$N@W, but the goo misses $M@W!@n", 1, ch, 0, vict, TO_NOTVICT);
            WAIT_STATE(ch, PULSE_3SEC);
            return;
        } else {
            act("@WYou flings a piece of goo at @c$N@W! The goo engulfs $M and then returns to your body!@n", 1, ch,
                0, vict, TO_CHAR);
            act("@C$n@W flings a piece of goo at you! The goo engulfs your body and then returns to @C$n@W!@n", 1,
                ch, 0, vict, TO_VICT);
            act("@C$n@w flings a piece of goo at @c$N@W! The goo engulfs $M and then return to @C$n@W!@n", 1, ch, 0,
                vict, TO_NOTVICT);
            GET_ABSORBS(ch) += 1;
...
ACMD(do_absorb) {
    ...
    if (ABSORBING(ch)) {
        act("@WYou stop absorbing from @c$N@W!@n", 1, ch, 0, ABSORBING(ch), TO_CHAR);
        act("$n stops absorbing from you!", 1, ch, 0, ABSORBING(ch), TO_VICT);
        act("$n stops absorbing from $N!", 1, ch, 0, ABSORBING(ch), TO_NOTVICT);
    ...
    if (IS_ANDROID(ch)) {
        ...
        if (init_skill(ch, SKILL_ABSORB) < axion_dice(0)) {
            act("@WYou rush at @c$N@W and try to absorb from them, but $E manages to avoid you!@n", 1, ch, 0, vict,
                TO_CHAR);
            act("@C$n@W rushes at you and tries to grab you, but you manage to avoid $m!@n", 1, ch, 0, vict,
                TO_VICT);
            act("@C$n@w rushes at @c$N@W and tries to grab $M, but @c$N@W manages to avoid @c$n@W!@n", 1, ch, 0,
                vict, TO_NOTVICT);
            ...
            act("@WYou rush at @c$N@W and try to absorb from them, and manage to grab on!@n", 1, ch, 0, vict,
                TO_CHAR);
            act("@C$n@W rushes at you and tries to grab you, and manages to grab on!@n", 1, ch, 0, vict, TO_VICT);
            act("@C$n@w rushes at @c$N@W and tries to grab $M, and manages to grab on!@n", 1, ch, 0, vict,
                TO_NOTVICT);
            improve_skill(ch, SKILL_ABSORB, 1);
            ABSORBING(ch) = vict;
            ABSORBBY(vict) = ch;
...
ACMD(do_escape) {
    ...
    if (ABSORBBY(ch)) {
        ...
        if (num > skill) {
            ...
            act("@c$N@W struggles to break loose of @C$n's@W hold!@n", 1, ABSORBBY(ch), 0, ch, TO_NOTVICT);
            act("@WYou struggle to break loose of @C$n's@W hold!@n", 1, ABSORBBY(ch), 0, ch, TO_VICT);
            act("@c$N@W struggles to break loose of your hold!@n", 1, ABSORBBY(ch), 0, ch, TO_CHAR);
            if (rand_number(1, 3) == 3) {
        ...
        if (num > skill) {
            if (GRAPTYPE(GRAPPLED(ch)) == 4) {
                act("@c$N@M flexes with all $S might and causes your body to explode outward into gooey chunks!@n",
                    1, GRAPPLED(ch), 0, ch, TO_CHAR);
                act("@MYou flex with all your might and cause @C$n's@M body to explode outward into gooey chunks!@n",
                    1, GRAPPLED(ch), 0, ch, TO_VICT);
                act("@c$N@M flexes with all $S might and causes @C$n's@M body to explode outward into gooey chunks!@n",
                    1, GRAPPLED(ch), 0, ch, TO_NOTVICT);

                act("@MYou reform your body mere moments later.@n", 1, GRAPPLED(ch), 0, 0, TO_CHAR);
                act("@C$n@M reforms $s body mere moments later.", 1, GRAPPLED(ch), 0, 0, TO_ROOM);
            } else {
                act("@c$N@W manages to break loose of @C$n's@W hold!@n", 1, GRAPPLED(ch), 0, ch, TO_NOTVICT);
                act("@WYou manage to break loose of @C$n's@W hold!@n", 1, GRAPPLED(ch), 0, ch, TO_VICT);
                act("@c$N@W manages to break loose of your hold!@n", 1, GRAPPLED(ch), 0, ch, TO_CHAR);
            }
            if (FIGHTING(ch) == NULL) {
            ...
            act("@c$N@W struggles to break loose of @C$n's@W hold!@n", 1, GRAPPLED(ch), 0, ch, TO_NOTVICT);
            act("@WYou struggle to break loose of @C$n's@W hold!@n", 1, GRAPPLED(ch), 0, ch, TO_VICT);
            act("@c$N@W struggles to break loose of your hold!@n", 1, GRAPPLED(ch), 0, ch, TO_CHAR);
            if (rand_number(1, 3) == 3) {
                int64_t dmg = GET_MAX_HIT(ch) * 0.025;
...
ACMD(do_regenerate) {
    ...
    if (GET_HIT(ch) >= gear_pl(ch)) {
        act("You concentrate your ki and regenerate your body completely.", 1, ch, 0, 0, TO_CHAR);
        act("$n concentrates and regenerates $s body completely.", 1, ch, 0, 0, TO_ROOM);
    } else if (amt < GET_MAX_HIT(ch) / 10) {
        act("You concentrate your ki and regenerate your body a little.", 1, ch, 0, 0, TO_CHAR);
        act("$n concentrates and regenerates $s body a little.", 1, ch, 0, 0, TO_ROOM);
    } else if (amt < GET_MAX_HIT(ch) / 5) {
        act("You concentrate your ki and regenerate your body some.", 1, ch, 0, 0, TO_CHAR);
        act("$n concentrates and regenerates $s body some.", 1, ch, 0, 0, TO_ROOM);
    } else if (amt < GET_MAX_HIT(ch) / 2) {
        act("You concentrate your ki and regenerate your body a great deal.", 1, ch, 0, 0, TO_CHAR);
        act("$n concentrates and regenerates $s body a great deal.", 1, ch, 0, 0, TO_ROOM);
    } else if (GET_HIT(ch) < GET_MAX_HIT(ch)) {
        act("You concentrate your ki and regenerate you nearly completely.", 1, ch, 0, 0, TO_CHAR);
        act("$n concentrates and regenerates $s body nearly completely.", 1, ch, 0, 0, TO_ROOM);
    }
    improve_skill(ch, SKILL_REGENERATE, 0);
    if (AFF_FLAGGED(ch, AFF_BURNED)) {
        send_to_char(ch, "Your burns are healed now.\r\n");
        act("$n@w's burns are now healed.@n", 1, ch, 0, 0, TO_ROOM);
        null_affect(ch, AFF_BURNED);
    }

    if (!IS_NPC(ch)) {
        if (GET_LIMBCOND(ch, 1) <= 0) {
            act("You regrow your right arm!", 1, ch, 0, 0, TO_CHAR);
            act("$n regrows $s right arm!", 1, ch, 0, 0, TO_ROOM);
            GET_LIMBCOND(ch, 1) = 100;
        } else if (GET_LIMBCOND(ch, 1) >= 0 && GET_LIMBCOND(ch, 1) < 50) {
            act("Your broken right arm mends itself!", 1, ch, 0, 0, TO_CHAR);
            act("$n regenerates $s broken right arm!", 1, ch, 0, 0, TO_ROOM);
            GET_LIMBCOND(ch, 1) = 100;
        }
        if (GET_LIMBCOND(ch, 2) <= 0) {
            GET_LIMBCOND(ch, 2) = 100;
            act("You regrow your left arm!", 1, ch, 0, 0, TO_CHAR);
            act("$n regrows $s left arm!", 1, ch, 0, 0, TO_ROOM);
        } else if (GET_LIMBCOND(ch, 2) > 0 && GET_LIMBCOND(ch, 2) < 50) {
            act("Your broken left arm mends itself!", 1, ch, 0, 0, TO_CHAR);
            act("$n regenerates $s broken left arm!", 1, ch, 0, 0, TO_ROOM);
            GET_LIMBCOND(ch, 2) = 100;
        }
        if (GET_LIMBCOND(ch, 4) <= 0) {
            GET_LIMBCOND(ch, 4) = 100;
            act("You regrow your left leg!", 1, ch, 0, 0, TO_CHAR);
            act("$n regrows $s left leg!", 1, ch, 0, 0, TO_ROOM);
        } else if (GET_LIMBCOND(ch, 4) > 0 && GET_LIMBCOND(ch, 4) < 50) {
            act("Your broken left leg mends itself!", 1, ch, 0, 0, TO_CHAR);
            act("$n regenerates $s broken left leg!", 1, ch, 0, 0, TO_ROOM);
            GET_LIMBCOND(ch, 4) = 100;
        }
        if (GET_LIMBCOND(ch, 3) <= 0) {
            GET_LIMBCOND(ch, 3) = 100;
            act("You regrow your right leg!", 1, ch, 0, 0, TO_CHAR);
            act("$n regrows $s right leg!", 1, ch, 0, 0, TO_ROOM);
        } else if (GET_LIMBCOND(ch, 3) > 0 && GET_LIMBCOND(ch, 3) < 50) {
            GET_LIMBCOND(ch, 3) = 100;
            act("Your broken right leg mends itself!", 1, ch, 0, 0, TO_CHAR);
            act("$n regenerates $s broken right leg!", 1, ch, 0, 0, TO_ROOM);
        }
        if (!PLR_FLAGGED(ch, PLR_TAIL) && IS_BIO(ch)) {
            SET_BIT_AR(PLR_FLAGS(ch), PLR_TAIL);
            act("You regrow your tail!", 1, ch, 0, 0, TO_CHAR);
            act("$n regrows $s tail!", 1, ch, 0, 0, TO_ROOM);
        }
        improve_skill(ch, SKILL_REGENERATE, 0);
    }
}
    ...
    if (AFF_FLAGGED(ch, AFF_CURSE)) {
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_STONESKIN)) {
                ...
                reveal_hiding(ch, 0);
                act("You focus ki into your skin, but fail in making it tough!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki into $s skin, but fails in making it tough!", 1, ch, 0, 0, TO_ROOM);
                return;
            } else {
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki into your skin, making it tough!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki into $s skin, making it tough!", 1, ch, 0, 0, TO_ROOM);
                return;
            }
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_STONESKIN)) {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's skin, but fail in making it tough!", 1, ch, 0, vict, TO_CHAR);
                    act("$n focuses ki into your skin, but fails in making it tough!", 1, ch, 0, vict, TO_VICT);
                    act("$n focuses ki into $N's skin, but fails in making it tough!", 1, ch, 0, vict, TO_NOTVICT);
                    return;
                } else {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's skin, making it tough!", 1, ch, 0, vict, TO_CHAR);
                    act("$n focuses ki into your skin, making it tough!", 1, ch, 0, vict, TO_VICT);
                    act("$n focuses ki into $N's skin, making it tough!", 1, ch, 0, vict, TO_NOTVICT);
                    return;
                }
            }
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_MIGHT)) {
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki into your muscles, but fail in making them mighty!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki into $s muscles, but fails in making them mighty!", 1, ch, 0, 0, TO_ROOM);
                return;
            } else {
                ...
                reveal_hiding(ch, 0);
                act("You focus ki into your muscles, making them mighty!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki into $s muscles, making them mighty!", 1, ch, 0, 0, TO_ROOM);
                return;
            }
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_MIGHT)) {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's muscles, but fail in making them mighty!", 1, ch, 0, vict, TO_CHAR);
                    act("$n focuses ki into your muscles, but fails in making them mighty!", 1, ch, 0, vict,
                        TO_VICT);
                    act("$n focuses ki into $N's muscles, but fails in making them mighty!", 1, ch, 0, vict,
                        TO_NOTVICT);
                    return;
                } else {
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    ...
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's muscles, making them mighty!", 1, ch, 0, vict, TO_CHAR);
                    act("$n focuses ki into your muscles, making them mighty!", 1, ch, 0, vict, TO_VICT);
                    act("$n focuses ki into $N's muscles, making them mighty!", 1, ch, 0, vict, TO_NOTVICT);
                    return;
                }
            }
        ...
        if (AFF_FLAGGED(vict, AFF_WITHER)) {
            ...
            GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
            reveal_hiding(ch, 0);
            act("You focus ki into $N's body, but fail in withering it!", 1, ch, 0, vict, TO_CHAR);
            act("$n focuses ki into your body, but fails in withering it!", 1, ch, 0, vict, TO_VICT);
            act("$n focuses ki into $N's body, but fails in withering it!", 1, ch, 0, vict, TO_NOTVICT);
            return;
        } else {
            ...
            save_char(vict);
            reveal_hiding(ch, 0);
            act("You focus ki into $N's body, and succeed in withering it!", 1, ch, 0, vict, TO_CHAR);
            act("$n focuses ki into your body, and succeeds in withering it!", 1, ch, 0, vict, TO_VICT);
            act("$n focuses ki into $N's body, and succeeds in withering it!", 1, ch, 0, vict, TO_NOTVICT);
            return;
        }
    } // End of wither
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_ENLIGHTEN)) {
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki into your mind, but fail in awakening it to cosmic wisdom!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki into $s mind, but fails in awakening it to cosmic wisdom!", 1, ch, 0, 0, TO_ROOM);
                return;
            } else {
                int duration = roll_aff_duration(GET_INT(ch), 2);
                /* Str , Con, Int, Agl, Wis, Spd */
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki into your mind, awakening it to cosmic wisdom!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki into $s mind, awakening it to cosmic wisdom!", 1, ch, 0, 0, TO_ROOM);
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_ENLIGHTEN)) {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's mind, but fail in awakening it to cosmic wisdom!", 1, ch, 0, vict,
                        TO_CHAR);
                    act("$n focuses ki into your mind, but fails in awakening it to cosmic wisdom!", 1, ch, 0, vict,
                        TO_VICT);
                    act("$n focuses ki into $N's mind, but fails in awakening it to cosmic wisdom!", 1, ch, 0, vict,
                        TO_NOTVICT);
                    return;
                } else {
                    int duration = roll_aff_duration(GET_INT(ch), 2);
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's mind, awakening it to cosmic wisdom!", 1, ch, 0, vict, TO_CHAR);
                    act("$n focuses ki into your mind, awakening it to cosmic wisdom!", 1, ch, 0, vict, TO_VICT);
                    act("$n focuses ki into $N's mind, awakening it to cosmic wisdom!", 1, ch, 0, vict, TO_NOTVICT);
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_GENIUS)) {
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki into your mind, but fail in making it work faster!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki into $s muscles, but fails in making it work faster!", 1, ch, 0, 0, TO_ROOM);
                return;
            } else {
                int duration = roll_aff_duration(GET_INT(ch), 2);
                /* Str , Con, Int, Agl, Wis, Spd */
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki into your mind, making it work faster!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki into $s mind, making it work faster!", 1, ch, 0, 0, TO_ROOM);
                return;
            }
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_GENIUS)) {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's mind, but fail in making it work faster!", 1, ch, 0, vict, TO_CHAR);
                    act("$n focuses ki into your mind, but fails in making it work faster!", 1, ch, 0, vict,
                        TO_VICT);
                    act("$n focuses ki into $N's mind, but fails in making it work faster!", 1, ch, 0, vict,
                        TO_NOTVICT);
                    return;
                } else {
                    int duration = roll_aff_duration(GET_INT(ch), 2);;
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's mind, making it work faster!", 1, ch, 0, vict, TO_CHAR);
                    act("$n focuses ki into your mind, making it work faster!", 1, ch, 0, vict, TO_VICT);
                    act("$n focuses ki into $N's mind, making it work faster!", 1, ch, 0, vict, TO_NOTVICT);
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_FLEX)) {
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki into your limbs, but fail in making them more flexible!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki into $s muscles, but fails in making them more flexible!", 1, ch, 0, 0, TO_ROOM);
                return;
            } else {
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                int duration = roll_aff_duration(GET_INT(ch), 2);;
                ...
                reveal_hiding(ch, 0);
                act("You focus ki into your limbs, making them more flexible!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki into $s limbs, making them more flexible!", 1, ch, 0, 0, TO_ROOM);
                return;
            }
        } // End of no vict FLEX
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_FLEX)) {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's limbs, but fail in making them more flexible!", 1, ch, 0, vict,
                        TO_CHAR);
                    act("$n focuses ki into your limbs, but fails in making them more flexible!", 1, ch, 0, vict,
                        TO_VICT);
                    act("$n focuses ki into $N's limbs, but fails in making them more flexible!", 1, ch, 0, vict,
                        TO_NOTVICT);
                    return;
                } else {
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    ...
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's limbs, making them more flexible!", 1, ch, 0, vict, TO_CHAR);
                    act("$n focuses ki into your limbs, making them more flexible!", 1, ch, 0, vict, TO_VICT);
                    act("$n focuses ki into $N's limbs, making them more flexible!", 1, ch, 0, vict, TO_NOTVICT);
                    if ((vict->master == ch || ch->master == vict || ch->master == vict->master) &&
                        AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                        if (IS_KAI(ch) && level_exp(ch, GET_LEVEL(ch) + 1) - GET_EXP(ch) > 0 &&
                            rand_number(1, 3) == 3) {
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_BLESS)) {
                ...
                act("You focus ki while chanting spiritual words. Your blessing does nothing though, you must have messed up!",
                    1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki while chanting spiritual words. $n seems disappointed.", 1, ch, 0, 0, TO_ROOM);
                return;
            } else {
                ...
                act("You focus ki while chanting spiritual words. You feel your body recovering at above normal speed!",
                    1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki while chanting spiritual words. $n smiles after finishing $s chant.", 1, ch, 0, 0,
                    TO_ROOM);
                if (AFF_FLAGGED(ch, AFF_CURSE)) {
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_BLESS)) {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki while chanting spiritual words. Your blessing fails!", 1, ch, 0, 0, TO_CHAR);
                    ...
                    act("You focus ki while chanting spiritual words. Blessing $N with faster regeneration!", 1, ch,
                        0, vict, TO_CHAR);
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_CURSE)) {
                ...
                act("You focus ki while chanting demonic words. Your cursing does nothing though, you must have messed up!",
                    1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki while chanting demonic words. $n seems disappointed.", 1, ch, 0, 0, TO_ROOM);
                return;
            } else {
                int duration = roll_aff_duration(GET_INT(ch), 3);;
                /* Str , Con, Int, Agl, Wis, Spd */
                ...
                act("You focus ki while chanting demonic words. You feel your body recovering at below normal speed!",
                    1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki while chanting demonic words. $n grins after finishing $s chant.", 1, ch, 0, 0,
                    TO_ROOM);
                if (AFF_FLAGGED(ch, AFF_BLESS)) {
            ...
            if (!can_kill(ch, vict, NULL, 0)) {
                ...
                if (AFF_FLAGGED(vict, AFF_CURSE)) {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki while chanting demonic words. Your cursing fails!", 1, ch, 0, 0, TO_CHAR);
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki while chanting demonic words. cursing $N with slower regeneration!", 1, ch, 0,
                        vict, TO_CHAR);
        ...
        if (!can_kill(ch, vict, NULL, 0)) {
            ...
            if (AFF_FLAGGED(vict, AFF_SLEEP)) {
                ...
                act("You focus ki while moving your hands in lulling patterns, but $N doesn't look the least bit sleepy!",
                    1, ch, 0, vict, TO_CHAR);
                act("$n focuses ki while moving $s hands in a lulling pattern, but you just don't feel tired.", 1,
                    ch, 0, vict, TO_VICT);
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki while moving your hands in lulling patterns, but fail to put $N to sleep!", 1, ch,
                    0, vict, TO_CHAR);
                act("$n focuses ki while moving $s hands in a lulling pattern, but you resist the technique!", 1, ch,
                    0, vict, TO_VICT);
                act("$n focuses ki while moving $s hands in a lulling pattern, but $N resists the technique!", 1, ch,
                    0, vict, TO_NOTVICT);
                return;
            } else {
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki while moving your hands in lulling patterns, putting $N to sleep!", 1, ch, 0, vict,
                    TO_CHAR);
                act("$n focuses ki while moving $s hands in a lulling pattern, before you realise it you are asleep!",
                    1, ch, 0, vict, TO_VICT);
                act("$n focuses ki while moving $s hands in a lulling pattern, putting $N to sleep!", 1, ch, 0, vict,
                    TO_NOTVICT);
                GET_POS(vict) = POS_SLEEPING;
                if (AFF_FLAGGED(vict, AFF_FLYING)) {
        ...
        if (!*name) {
            if (GET_MANA(ch) < GET_MAX_MANA(ch) / 10) {
                ...
                reveal_hiding(ch, 0);
                act("You focus ki into your very cells, but fail at re-engerizing them!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki and glows green for a moment, $e then frowns.", 1, ch, 0, 0, TO_ROOM);
                WAIT_STATE(ch, PULSE_2SEC);
                return;
            } else if (GET_MOVE(ch) >= GET_MAX_MOVE(ch)) {
                ...
                reveal_hiding(ch, 0);
                act("You focus ki into your very cells, and manage to re-energize them!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki and glows green for a moment, $e then smiles.", 1, ch, 0, 0, TO_ROOM);
                WAIT_STATE(ch, PULSE_2SEC);
                return;
            }
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (IS_NPC(vict)) {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 10;
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's very cells, and fail at re-energizing them!", 1, ch, 0, vict,
                        TO_CHAR);
                    act("$n focuses ki into your very cells, but nothing happens!", 1, ch, 0, vict, TO_VICT);
                    act("$n focuses ki and $N glows green for a moment, $N frowns.", 1, ch, 0, vict, TO_NOTVICT);
                    WAIT_STATE(ch, PULSE_2SEC);
                    return;
                } else {
                    if (GET_BONUS(ch, BONUS_HEALER) > 0) {
                    ...
                    reveal_hiding(ch, 0);
                    act("You focus ki into $N's very cells, and manage to re-energize them!", 1, ch, 0, vict,
                        TO_CHAR);
                    act("$n focuses ki into your very cells, and manages to re-energize them!", 1, ch, 0, vict,
                        TO_VICT);
                    act("$n focuses ki and $N glows green for a moment, $N smiles.", 1, ch, 0, vict, TO_NOTVICT);
                    WAIT_STATE(ch, PULSE_2SEC);
                    return;
                }
            }
        ...
        if (!*name) {
            if (!AFF_FLAGGED(ch, AFF_POISON)) {
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki and aim a pulsing light at your body. Nothing happens!", 1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki and aims a pulsing light at $s body. Nothing seems to happen.", 1, ch, 0, 0,
                    TO_ROOM);
                return;
            } else {
                ...
                act("You focus ki and aim a pulsing light at your body. You feel the poison in your blood disappear!",
                    1, ch, 0, 0, TO_CHAR);
                act("$n focuses ki and aims a pulsing light at $s body. $n smiles.", 1, ch, 0, 0, TO_ROOM);
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (!AFF_FLAGGED(vict, AFF_POISON)) {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki and aim a pulsing light at $N's body. Nothing happens.", 1, ch, 0, vict,
                        TO_CHAR);
                    act("$n focuses ki and aims a pulsing light at your body. You are STILL poisoned!", 1, ch, 0,
                        vict, TO_VICT);
                    act("$n focuses ki and aims a pulsing light at $N's body. $N looks disappointed.", 1, ch, 0,
                        vict, TO_NOTVICT);
                    return;
                } else {
                    ...
                    GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                    reveal_hiding(ch, 0);
                    act("You focus ki and aim a pulsing light at $N's body. $e is cured.", 1, ch, 0, vict, TO_CHAR);
                    act("$n focuses ki and aims a pulsing light at your body. You have been cured of your poison!",
                        1, ch, 0, vict, TO_VICT);
                    act("$n focuses ki and aims a pulsing light at $N's body. $N smiles.", 1, ch, 0, vict,
                        TO_NOTVICT);
        ...
        if (!can_kill(ch, vict, NULL, 0)) {
            ...
            if (AFF_FLAGGED(vict, AFF_POISON)) {
                ...
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki and fling poison at $N. You missed!", 1, ch, 0, vict, TO_CHAR);
                act("$n focuses ki and flings poison at you, but misses!", 1, ch, 0, vict, TO_VICT);
                act("$n focuses ki and flings poison at $N, but misses!", 1, ch, 0, vict, TO_NOTVICT);
                return;
            } else {
                GET_MANA(ch) -= GET_MAX_MANA(ch) / 20;
                reveal_hiding(ch, 0);
                act("You focus ki and fling poison at $N! The poison burns into $s skin!", 1, ch, 0, vict, TO_CHAR);
                act("$n focuses ki and flings poison at you! The poison burns into your skin!", 1, ch, 0, vict,
                    TO_VICT);
                act("$n focuses ki and flings poison at $N! The poison burns into $s skin!", 1, ch, 0, vict,
                    TO_NOTVICT);
                if (IS_NPC(vict)) {
                    set_fighting(vict, ch);
                }
                if (IS_MUTANT(vict) && (GET_GENOME(vict, 0) == 7 || GET_GENOME(vict, 1) == 7)) {
                    act("However $N seems unaffected by the poison.", 1, ch, 0, vict, TO_CHAR);
                    act("Your natural immunity to poison prevents it from affecting you.", 1, ch, 0, vict, TO_VICT);
                    act("However $N seems unaffected by the poison.", 1, ch, 0, vict, TO_NOTVICT);
                } else {
                    vict->poisonby = ch;
                    if (GET_CHARGE(ch) > 0) {
...
ACMD(do_kaioken) {
    char arg[MAX_INPUT_LENGTH];
    int roll = axion_dice(0), x = 0, pass = 0;
    int64_t boost = 0;
    one_argument(argument, arg);
    ...
    if (GET_KAIOKEN(ch) > 0) {
        ...
        act("$n@w drops out of kaioken.@n", 1, ch, 0, 0, TO_ROOM);
    ...
    if (init_skill(ch, SKILL_KAIOKEN) < roll) {
        ...
        act("$n tries to use kaioken but messes up somehow.", 1, ch, 0, 0, TO_ROOM);
    ...
    send_to_char(ch, "@rA dark red aura bursts up around your body as you achieve Kaioken x %d!@n\r\n",
                 GET_KAIOKEN(ch));
    act("@rA dark red aura bursts up around @R$n@r as they achieve a level of Kaioken!@n", 1, ch, 0, 0, TO_ROOM);
    improve_skill(ch, SKILL_KAIOKEN, 1);
...
ACMD(do_plant) {
    ...
    if (roll <= detect && roll <= fail) {
        reveal_hiding(ch, 0);
        act("@C$n@w tries to plant $p@w on you!@n", 1, ch, obj, vict, TO_VICT);
        act("@C$n@w tries to plant $p@w on @c$N@w!@n", 1, ch, obj, vict, TO_NOTVICT);
        act("@wYou try and fail to plant $p@w on @c$N@w, and $E notices!@n", 1, ch, obj, vict, TO_CHAR);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    } else if (roll <= fail) {
        act("@wYou try and fail to plant $p@w on @c$N@w! However no one seemed to notice.@n", 1, ch, obj, vict,
            TO_CHAR);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
        ...
        act("@wYou try and fail to plant $p@w on @c$N@w because $E can't carry the weight. It seems $E noticed the attempt!@n",
            1, ch, obj, vict, TO_CHAR);
        WAIT_STATE(ch, PULSE_2SEC);
    } else if (roll <= detect) {
        act("@cYou feel like the weight of your inventory has changed.@n", 1, ch, obj, vict, TO_VICT);
        act("@c$N@w looks around after feeling $S pockets.@n", 1, ch, obj, vict, TO_NOTVICT);
        act("@wYou plant $p@w on @c$N@w! @c$N @wseems to notice the change in weight in their inventory.@n", 1, ch,
            obj, vict, TO_CHAR);
        obj_from_char(obj);
        obj_to_char(obj, vict);
        WAIT_STATE(ch, PULSE_2SEC);
        ...
        act("@wYou plant $p@w on @c$N@w! No one noticed, whew....@n", 1, ch, obj, vict, TO_CHAR);
        obj_from_char(obj);
        obj_to_char(obj, vict);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
...
ACMD(do_forgery) {
    ...
    for (obj = ch->carrying; obj; obj = next_obj) {
        next_obj = obj->next_content;
        if (found == 0 && GET_OBJ_VNUM(obj) == 19 &&
            (!OBJ_FLAGGED(obj, ITEM_BROKEN) && !OBJ_FLAGGED(obj, ITEM_FORGED))) {
            found = 1;
            obj4 = obj;
        }
    ...
    reveal_hiding(ch, 0);
    act("@c$n@w looks at $p, begins to work on forging a fake copy of it.@n", 1, ch, obj2, 0, TO_ROOM);
    ...
    if (GET_SKILL(ch, SKILL_FORGERY) < axion_dice(0)) {
        ...
        send_to_char(ch, "You start to make a forgery of %s but screw up and waste your forgery kit..\r\n",
                     obj2->short_description);
        act("@c$n@w tried to duplicate $p but screws up somehow.@n", 1, ch, obj2, 0, TO_ROOM);
        obj_from_char(obj4);
        extract_obj(obj4);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    ...
    act("@c$n@w makes a perfect forgery of $p.@n", 1, ch, obj2, 0, TO_ROOM);
    WAIT_STATE(ch, PULSE_2SEC);
}
...
ACMD(do_appraise) {
    ...
    reveal_hiding(ch, 0);
    act("@c$n@w looks at $p, turning it over in $s hands.@n", 1, ch, obj, 0, TO_ROOM);
    ...
    if (GET_SKILL(ch, SKILL_APPRAISE) < axion_dice(-10)) {
        ...
        act("@c$n@w looks stumped about $p.@n", 1, ch, obj, 0, TO_ROOM);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    }
    ...
    if (OBJ_FLAGGED(obj, ITEM_BROKEN)) {
        ...
        act("@c$n@w looks at $p and frowns.@n", 1, ch, obj, 0, TO_ROOM);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    }
    ...
    if (OBJ_FLAGGED(obj, ITEM_FORGED)) {
        ...
        act("@c$n@w looks at $p with an angry face.@n", 1, ch, obj, 0, TO_ROOM);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    }
    found = 0;
    ...
    act("@c$n@w looks at $p and nods, a satisfied look on $s face.@n", 1, ch, obj, 0, TO_ROOM);
    int percent = 0;
...
ACMD(do_disguise) {
    ...
    if (PLR_FLAGGED(ch, PLR_DISGUISED)) {
        ...
        act("@C$n @wpulls off $s disguise and reveals $mself!", 1, ch, 0, 0, TO_ROOM);
        return;
    }
    ...
    if (skill > roll) {
        ...
        act("@C$n @wmanages to disguise $mself with some skilled manipulation of $s headwear.", 1, ch, 0, 0,
            TO_ROOM);
        ...
        act("@C$n @wattempts and fails to disguise $mself properly and must try again.", 1, ch, 0, 0, TO_ROOM);
        if (GET_MOVE(ch) >= GET_MOVE(ch) / 50) {
...
ACMD(do_eavesdrop) {
    ...
    if ((dir = search_block(buf, dirs, 0)) < 0) {
...
ACMD(do_zanzoken) {
    ...
    act("@wYou focus your ki, preparing to move at super speeds if necessary.@n", 1, ch, 0, 0, TO_CHAR);
    GET_MANA(ch) -= cost;
...
ACMD(do_block) {
    ...
    if (!*arg) {
        ...
        if (BLOCKS(ch)) {
            act("@wYou stop blocking @c$N@w.@n", 1, ch, 0, BLOCKS(ch), TO_CHAR);
            act("@C$n@w stops blocking you.@n", 1, ch, 0, BLOCKS(ch), TO_VICT);
            act("@C$n@w stops blocking @c$N@w.@n", 1, ch, 0, BLOCKS(ch), TO_NOTVICT);
            vict = BLOCKS(ch);
            BLOCKED(vict) = NULL;
            BLOCKS(ch) = NULL;
            return;
    ...
    if (BLOCKS(ch)) {
        ...
        BLOCKED(vict) = ch;
        reveal_hiding(ch, 0);
        act("@wYou start blocking @c$N's@w escape.@n", 1, ch, 0, BLOCKS(ch), TO_CHAR);
        act("@C$n@w starts blocking your escape.@n", 1, ch, 0, BLOCKS(ch), TO_VICT);
        act("@C$n@w starts blocking @c$N's@w escape.@n", 1, ch, 0, BLOCKS(ch), TO_NOTVICT);
        return;
    } else {
        BLOCKS(ch) = vict;
        BLOCKED(vict) = ch;
...
ACMD(do_eyec) {
    ...
    if (PLR_FLAGGED(ch, PLR_EYEC)) {
        ...
        act("@wYou open your eyes.@n", 1, ch, 0, 0, TO_CHAR);
        act("@C$n@w opens $s eyes.@n", 1, ch, 0, 0, TO_ROOM);
    } else if (!PLR_FLAGGED(ch, PLR_EYEC)) {
        SET_BIT_AR(PLR_FLAGS(ch), PLR_EYEC);
        act("@wYou close your eyes.@n", 1, ch, 0, 0, TO_CHAR);
        act("@C$n@w closes $s eyes.@n", 1, ch, 0, 0, TO_ROOM);
    }
...
ACMD(do_solar) {
    ...
    for (vict = world[IN_ROOM(ch)].people; vict; vict = next_v) {
        ...
        if (vict == ch)
            ...
            act("@W$N@W is @YBLINDED@W!@n", 1, ch, 0, vict, TO_CHAR);
            act("@RYou are @YBLINDED@R!@n", 1, ch, 0, vict, TO_VICT);
            act("@W$N@W is @YBLINDED@W!@n", 1, ch, 0, vict, TO_NOTVICT);
        }
    }
    improve_skill(ch, SKILL_SOLARF, 0);
    GET_MANA(ch) -= cost;
...
ACMD(do_heal) {
    ...
    if (prob < perc) {
        if (vict != ch) {
            act("@WYou place your hands near @c$N@W, but fail to concentrate enough to heal them!@n", 1, ch, 0, vict,
                TO_CHAR);
            act("@C$n@W places $s hands near you, but nothing happens!@n", 1, ch, 0, vict, TO_VICT);
            act("@C$n@W places $s hands near @c$N@W, but nothing happens.", 1, ch, 0, vict, TO_NOTVICT);
            GET_MANA(ch) -= cost;
        ...
        if (vict == ch) {
            act("@WYou place your hands on your body, but fail to concentrate to heal yourself!@n", 1, ch, 0, vict,
                TO_CHAR);
            act("@C$n@W places $s hands on $s body, but nothing happens.", 1, ch, 0, vict, TO_NOTVICT);
            GET_MANA(ch) -= cost;
            improve_skill(ch, SKILL_HEAL, 0);
            WAIT_STATE(ch, PULSE_2SEC);
            return;
    ...
    if (vict != ch) {
        ...
        act("@WYou place your hands near @c$N@W and an orange glow surrounds $M!@n", 1, ch, 0, vict, TO_CHAR);
        act("@C$n@W places $s hands near you and an orange glow surrounds you!@n", 1, ch, 0, vict, TO_VICT);
        act("@C$n@W places $s hands near @c$N@W and an orange glow surrounds $M.", 1, ch, 0, vict, TO_NOTVICT);
        GET_MANA(ch) -= cost;
        ...
        if (AFF_FLAGGED(vict, AFF_BURNED)) {
            ...
            act("$n@w's burns are now healed.@n", 1, vict, 0, 0, TO_ROOM);
        ...
        if (AFF_FLAGGED(vict, AFF_HYDROZAP)) {
            ...
            act("$n@w no longer looks as if they could drink an ocean.@n", 1, vict, 0, 0, TO_ROOM);
        ...
        improve_skill(ch, SKILL_HEAL, 0);
        if (vict->master == ch || ch->master == vict || ch->master == vict->master) {
            if (IS_NAIL(ch) && IS_NAMEK(ch) && level_exp(ch, GET_LEVEL(ch) + 1) - GET_EXP(ch) > 0 &&
                GET_HIT(vict) <= gear_pl(vict) * 0.85 && rand_number(1, 3) == 3) {
    ...
    if (vict == ch) {
        ...
        act("@WYou place your hands on your body and an orange glow surrounds you!@n", 1, ch, 0, vict, TO_CHAR);
        act("@C$n@W places $s hands on $s body and an orange glow surrounds $m.", 1, ch, 0, vict, TO_NOTVICT);
        GET_MANA(ch) -= cost;
        if (GET_HIT(vict) + heal <= gear_pl(vict)) {
            GET_HIT(vict) += heal;
        } else {
...
ACMD(do_barrier) {
    ...
    if (AFF_FLAGGED(ch, AFF_SANCTUARY) && !strcasecmp("release", arg)) {
        act("@BYou dispel your barrier, releasing its energy.@n", 1, ch, 0, 0, TO_CHAR);
        act("@B$n@B dispels $s barrier, releasing its energy.@n", 1, ch, 0, 0, TO_ROOM);
        GET_BARRIER(ch) = 0;
    ...
    if (size > prob) {
        ...
        if (GET_SKILL(ch, SKILL_BARRIER)) {
            act("@BYou shout as you form a barrier of ki around your body!@n", 1, ch, 0, 0, TO_CHAR);
            act("@B$n@B shouts as $e forms a barrier of ki around $s body!@n", 1, ch, 0, 0, TO_ROOM);
        } else {
            act("@BYou shout as you form a barrier of ki and raging waters around your body!@n", 1, ch, 0, 0,
                TO_CHAR);
            act("@B$n@B shouts as $e forms a barrier of ki and raging waters around $s body!@n", 1, ch, 0, 0,
                TO_ROOM);
        }
...
ACMD(do_instant) {
    ...
    if (tar != NULL) {
        ...
        act("@wPlacing two fingers on your forehead you close your eyes and concentrate. Accelerating to such a speed that you move through the molecules of the universe faster than the speed of light. You stop as you arrive at $N@w!@n",
            1, ch, 0, tar, TO_CHAR);
        act("@w$n@w appears in an instant out of nowhere right next to you!@n", 1, ch, 0, tar, TO_VICT);
        act("@w$n@w places two fingers on $s forehead and disappears in an instant!@n", 1, ch, 0, tar, TO_NOTVICT);
        ...
        act("@wPlacing two fingers on your forehead you close your eyes and concentrate. Accelerating to such a speed that you move faster than light and arrive almost instantly at your destination. Having located the planet by its collective population's ki.@n",
            1, ch, 0, 0, TO_CHAR);
        act("@w$n@w places two fingers on $s forehead and disappears in an instant!@n", 1, ch, 0, 0, TO_NOTVICT);
...
void wishSYS(void) {
    if (SHENRON == 1) {
        if (SELFISHMETER < 10) {
            ...
            int num = rand_number(200, 20000), done = 0, place = 1;
            DRAGONC = 0;
            WISH[0] = 0;
            WISH[1] = 0;
            while (done == 0) {
    ...
    if (!*arg) {
        ...
        if (IS_HALFBREED(ch)) {
            ...
            if (GET_BASE_PL(ch) < trans_req(ch, 1)) {
                ...
                if (!PLR_FLAGGED(ch, PLR_LSSJ) && !PLR_FLAGGED(ch, PLR_FPSSJ) && rand_number(1, 500) >= 491 &&
                    GET_MAX_HIT(ch) > 19200000) {
                    send_to_char(ch,
                                 "You have mastered the super saiyan first transformation and have achieved Full Power Super Saiyan! You will now no longer use stamina while in this form.\r\n");
        ...
        if (IS_HUMAN(ch)) {
            ...
            if (GET_BASE_PL(ch) < trans_req(ch, 2)) {
                ...
                if (GET_TRANSCOST(ch, 2) == 0) {
                    if (GET_RP(ch) < 1) {
                        send_to_char(ch, "You need 1 RPP in order to obtain a transformation for the first time.\r\n");
                        return;
                    } else {
                        GET_RP(ch) -= 1;
                        GET_TRANSCOST(ch, 2) = 1;
                    }
                }
                int zone = 0;
        ...
        if (IS_HUMAN(ch)) {
            ...
            if (GET_BASE_PL(ch) < trans_req(ch, 3)) {
                ...
                if (GET_TRANSCOST(ch, 3) == 0) {
                    if (GET_RP(ch) < 1) {
                        send_to_char(ch, "You need 1 RPP in order to obtain a transformation for the first time.\r\n");
                        return;
                    } else {
                        GET_RP(ch) -= 1;
                        GET_TRANSCOST(ch, 3) = 1;
                    }
                }
                int zone = 0;
        ...
        if (IS_HUMAN(ch)) {
            ...
            if (GET_BASE_PL(ch) < trans_req(ch, 4)) {
                ...
                if (GET_TRANSCOST(ch, 4) == 0) {
                    if (GET_RP(ch) < 2) {
                        send_to_char(ch, "You need 2 RPP in order to obtain a transformation for the first time.\r\n");
                        return;
                    } else {
                        GET_RP(ch) -= 2;
                        GET_TRANSCOST(ch, 4) = 1;
                    }
                }
                int zone = 0;
            ...
            if (GET_BASE_PL(ch) < trans_req(ch, 1)) {
                ...
                if (GET_TRANSCOST(ch, 1) == 0) {
                    if (GET_PRACTICES(ch, GET_CLASS(ch)) < 50) {
                        send_to_char(ch,
                                     "You need 50 practice points in order to obtain a transformation for the first time.\r\n");
                        return;
                    } else {
                        GET_PRACTICES(ch, GET_CLASS(ch)) -= 50;
                        GET_TRANSCOST(ch, 1) = 1;
        ...
        if (IS_ANDROID(ch)) {
            ...
            if (GET_BASE_PL(ch) < trans_req(ch, 5)) {
                ...
                if (GET_TRANSCOST(ch, 5) == 0) {
                    if (GET_RP(ch) < 1) {
                        send_to_char(ch, "You need 1 RPP in order to obtain a transformation for the first time.\r\n");
                        return;
                    } else {
                        GET_RP(ch) -= 1;
                        GET_TRANSCOST(ch, 5) = 1;
                    }
                }
                act("@WYou stop for a moment as the nano-machines within your body reprogram and restructure you. You are now more powerful and efficient!@n",
                    1, ch, 0, 0, TO_CHAR);
        ...
        if (IS_ANDROID(ch)) {
            ...
            if (GET_BASE_PL(ch) < trans_req(ch, 6)) {
                ...
                if (GET_TRANSCOST(ch, 6) == 0) {
                    if (GET_RP(ch) < 2) {
                        send_to_char(ch, "You need 2 RPP in order to obtain a transformation for the first time.\r\n");
                        return;
                    } else {
                        GET_RP(ch) -= 2;
                        GET_TRANSCOST(ch, 6) = 1;
                    }
                }
                act("@WYou stop for a moment as the nano-machines within your body reprogram and restructure you. You are now more powerful and efficient!@n",
                    1, ch, 0, 0, TO_CHAR);
        ...
        if (IS_HUMAN(ch)) {
            if (PLR_FLAGGED(ch, PLR_TRANS4)) {
                ...
                act("@wYou revert from @CSuper @cHuman @GFourth@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cHuman @GFourth@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS4);
                ...
                act("@wYou revert from @CSuper @cHuman @GThird@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cHuman @GThird@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS3);
                ...
                act("@wYou revert from @CSuper @cHuman @GSecond@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cHuman @GSecond@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS2);
                ...
                act("@wYou revert from @CSuper @cHuman @GFirst@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cHuman @GFirst.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS1);
            ...
            if (PLR_FLAGGED(ch, PLR_TRANS4)) {
                ...
                act("@wYou revert from @CSuper @cSaiyan @GFourth@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cSaiyan @GFourth@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS4);
                ...
                act("@wYou revert from @CSuper @cSaiyan @GThird@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cSaiyan @GThird@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS3);
                ...
                if (!PLR_FLAGGED(ch, PLR_LSSJ)) {
                    act("@wYou revert from @CSuper @cSaiyan @GSecond@w.@n", 1, ch, 0, 0, TO_CHAR);
                    act("@w$n@w reverts from @CSuper @cSaiyan @GSecond@w.@n", 1, ch, 0, 0, TO_ROOM);
                    if ((GET_CHARGE(ch) > 0)) {
                        do_charge(ch, "release", 0, 0);
                    }
                    REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS2);
                    ...
                    act("@wYou revert from your @YLegendary @CSuper Saiyan@w form.@n", 1, ch, 0, 0, TO_CHAR);
                    act("@w$n@w reverts from $s @YLegendary @CSuper Saiyan@w form@w.@n", 1, ch, 0, 0, TO_ROOM);
                    if ((GET_CHARGE(ch) > 0)) {
                        do_charge(ch, "release", 0, 0);
                    }
                    REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS2);
                ...
                act("@wYou revert from @CSuper @cSaiyan @GFirst@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cSaiyan @GFirst.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS1);
            ...
            if (PLR_FLAGGED(ch, PLR_TRANS4)) {
                ...
                act("@wYou revert from @CSuper @cNamek @GFourth@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cNamek @GFourth@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS4);
                ...
                act("@wYou revert from @CSuper @cNamek @GThird@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cNamek @GThird@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS3);
                ...
                act("@wYou revert from @CSuper @cNamek @GSecond@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cNamek @GSecond@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS2);
                ...
                act("@wYou revert from @CSuper @cNamek @GFirst@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CSuper @cNamek @GFirst.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS1);
            ...
            if (PLR_FLAGGED(ch, PLR_TRANS4)) {
                ...
                act("@wYou revert from @CTransform @GFourth@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CTransform @GFourth@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS4);
                ...
                act("@wYou revert from @CTransform @GThird@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CTransform @GThird@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS3);
                ...
                act("@wYou revert from @CTransform @GSecond@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CTransform @GSecond@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS2);
                ...
                act("@wYou revert from @CTransform @GFirst@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CTransform @GFirst.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS1);
            ...
            if (PLR_FLAGGED(ch, PLR_TRANS3)) {
                ...
                act("@wYou revert from @CMutate @GThird@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CMutate @GThird@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS3);
                ...
                act("@wYou revert from @CMutate @GSecond@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CMutate @GSecond@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS2);
                ...
                act("@wYou revert from @CMutate @GFirst@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CMutate @GFirst.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS1);
            ...
            if (PLR_FLAGGED(ch, PLR_TRANS3)) {
                ...
                act("@wYou revert from @CShadow @GThird@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CShadow @GThird@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS3);
            ...
            if (PLR_FLAGGED(ch, PLR_TRANS2)) {
                ...
                act("@wYou revert from @CShadow @GSecond@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CShadow @GSecond@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS2);
                ...
                act("@wYou revert from @CShadow @GFirst@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CShadow @GFirst.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS1);
            ...
            if (PLR_FLAGGED(ch, PLR_TRANS3)) {
                ...
                act("@wYou revert from @CMystic @GThird@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CMystic @GThird@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS3);
            ...
            if (PLR_FLAGGED(ch, PLR_TRANS2)) {
                ...
                act("@wYou revert from @CMystic @GSecond@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CMystic @GSecond@w.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS2);
                ...
                act("@wYou revert from @CMystic @GFirst@w.@n", 1, ch, 0, 0, TO_CHAR);
                act("@w$n@w reverts from @CMystic @GFirst.@n", 1, ch, 0, 0, TO_ROOM);
                if ((GET_CHARGE(ch) > 0)) {
                    do_charge(ch, "release", 0, 0);
                }
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_TRANS1);
...
ACMD(do_situp) {
    ...
    if (AFF_FLAGGED(ch, AFF_FLYING)) {
        ...
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 0) {
            bonus = 1;
            act("@gYou do a situp.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a situp.@n", 1, ch, 0, 0, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 10) {
            bonus = rand_number(3, 7);
            act("@gYou do a situp, feeling the strain of gravity.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a situp, while sweating.@n", 1, ch, 0, 0, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 20) {
            bonus = rand_number(8, 14);
            act("@gYou do a situp, and feel gravity's pull.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a situp, while sweating.@n", 1, ch, 0, 0, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 30) {
            bonus = rand_number(14, 20);
            act("@gYou do a situp, and feel the burn.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a situp, while sweating.@n", 1, ch, 0, 0, TO_ROOM);
            ...
            act("@gYou do a situp, and really strain against the gravity.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a situp, while sweating.@n", 1, ch, 0, 0, TO_ROOM);
            ...
            act("@gYou do a situp, and it was a really hard one to finish.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a situp, while sweating profusely.@n", 1, ch, 0, 0, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 5000) {
...
ACMD(do_meditate) {
    ...
    if (!strcasecmp(arg, "expand")) {
        ...
        if (MINDLINK(ch) == NULL) {
            ...
            act("@rYou manage to break the mind link between you and @R$N@r!@n", 0, ch, 0, MINDLINK(ch), TO_CHAR);
            act("$n closes their eyes for a few seconds.", 0, ch, 0, MINDLINK(ch), TO_ROOM);
            ...
            act("@rYou struggle to free your mind of @R$N's@r link, but fail!@n", 0, ch, 0, MINDLINK(ch), TO_CHAR);
            act("$n closes their eyes for a few seconds, and appears to struggle quite a bit.", 0, ch, 0,
                MINDLINK(ch), TO_ROOM);
    ...
    if (GET_MANA(ch) < cost) {
        ...
        /* Meditate Message */
        act("@cYou close your eyes and concentrate, lifting $p@c with your ki.@n", 1, ch, obj, 0, TO_CHAR);
        act("@c$n closes $s eyes and lifts $p@c with $s ki.@n", 1, ch, obj, 0, TO_ROOM);
...
ACMD(do_pushup) {
    ...
    if (AFF_FLAGGED(ch, AFF_FLYING)) {
        ...
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 0) {
            bonus = 1;
            act("@gYou do a pushup.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a pushup.@n", 1, ch, 0, 0, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 10) {
            bonus = rand_number(3, 7);
            act("@gYou do a pushup, feeling the strain of gravity.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a pushup, while sweating.@n", 1, ch, 0, 0, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 20) {
            bonus = rand_number(8, 14);
            act("@gYou do a pushup, and feel gravity's pull.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a pushup, while sweating.@n", 1, ch, 0, 0, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 30) {
            bonus = rand_number(14, 20);
            act("@gYou do a pushup, and feel the burn.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a pushup, while sweating.@n", 1, ch, 0, 0, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 40) {
            bonus = rand_number(20, 35);
            act("@gYou do a pushup, and feel the burn.@n", 1, ch, 0, 0, TO_CHAR);
            ...
            bonus = rand_number(180, 250);
            act("@gYou do a pushup, and really strain against the gravity.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a pushup, while sweating.@n", 1, ch, 0, 0, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 200) {
            bonus = rand_number(400, 600);
            act("@gYou do a pushup, and really strain against the gravity.@n", 1, ch, 0, 0, TO_CHAR);
            ...
            bonus = rand_number(9000, 10000);
            act("@gYou do a pushup, and it was a really hard one to finish.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a pushup, while sweating profusely.@n", 1, ch, 0, 0, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 5000) {
            bonus = rand_number(15000, 20000);
            act("@gYou do a pushup, and it was a really hard one to finish.@n", 1, ch, 0, 0, TO_CHAR);
            act("@g$n does a pushup, while sweating profusely.@n", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_spar) {
    if (IS_NPC(ch)) {
        ...
        if (PLR_FLAGGED(ch, PLR_SPAR)) {
            act("@wYou cease your sparring stance.@n", 0, ch, 0, 0, TO_CHAR);
            act("@C$n@w ceases $s sparring stance.@n", 0, ch, 0, 0, TO_ROOM);
        }
        if (!PLR_FLAGGED(ch, PLR_SPAR)) {
            act("@wYou move into your sparring stance.@n", 0, ch, 0, 0, TO_CHAR);
            act("@C$n@w moves into $s sparring stance.@n", 0, ch, 0, 0, TO_ROOM);
        }
...
static void check_eq(struct char_data *ch) {
    ...
    for (i = 0; i < NUM_WEARS; i++) {
        if (GET_EQ(ch, i)) {
            obj = GET_EQ(ch, i);
            if (OBJ_FLAGGED(obj, ITEM_BROKEN)) {
                act("@W$p@W falls apart and you remove it.@n", 0, ch, obj, 0, TO_CHAR);
                act("@W$p@W falls apart and @C$n@W remove it.@n", 0, ch, obj, 0, TO_ROOM);
                perform_remove(ch, i);
                return;
            }
            if (obj == GET_EQ(ch, WEAR_WIELD1) && GET_LIMBCOND(ch, 1) <= 0) {
                act("@WWithout your right arm you let go of @c$p@W!@n", 0, ch, obj, 0, TO_CHAR);
                act("@C$n@W lets go of @c$p@W!@n", 0, ch, obj, 0, TO_ROOM);
                perform_remove(ch, i);
                return;
            }
            if (obj == GET_EQ(ch, WEAR_WIELD2) && GET_LIMBCOND(ch, 2) <= 0) {
                act("@WWithout your left arm you let go of @c$p@W!@n", 0, ch, obj, 0, TO_CHAR);
                act("@C$n@W lets go of @c$p@W!@n", 0, ch, obj, 0, TO_ROOM);
                perform_remove(ch, i);
                return;
            }
...
void base_update(void) {
    ...
    for (d = descriptor_list; d; d = d->next) {
        ...
        if (PLR_FLAGGED(d->character, PLR_GOOP) && d->character->gooptime == 60) {
            if (IS_BIO(d->character)) {
                act("@GConciousness slowly returns to you. You realize quickly that some of your cells have survived. You take control of your regenerative processes and focus on growing a new body!@n",
                    1, d->character, 0, 0, TO_CHAR);
            } else {
                act("@MSlowly you regain conciousness. The various split off chunks of your body begin to likewise stir.@n",
                    1, d->character, 0, 0, TO_CHAR);
                act("@MYou think you notice the chunks of @m$n@M's moving slightly.@n", 1, d->character, 0, 0,
                    TO_ROOM);
            }
            d->character->gooptime -= 1;
        } else if (PLR_FLAGGED(d->character, PLR_GOOP) && d->character->gooptime == 30) {
            if (IS_BIO(d->character)) {
                act("@GFrom the collection of cells growing a crude form of your body starts to take shape!@n", 1,
                    d->character, 0, 0, TO_CHAR);
                act("@GYou start to notice a large mass of pulsing flesh growing before you!@n", 1, d->character, 0,
                    0, TO_ROOM);
            } else {
        ...
        if (MOON_UP && (IS_SAIYAN(d->character) || IS_HALFBREED(d->character)) &&
            !PLR_FLAGGED(d->character, PLR_OOZARU)) {
            oozaru_add(d->character);
        }
        if (cash == 1 && GET_BANK_GOLD(d->character) > 0) {
        ...
        if (!IS_NPC(d->character) && ROOM_EFFECT(IN_ROOM(d->character)) >= 1 && rand_number(1, 100) >= 96) {
            if (ROOM_EFFECT(IN_ROOM(d->character)) <= 4) {
                switch (rand_number(1, 4)) {
                    case 1:
                        act("@RLava spews up violently from the cracks in the ground!@n", 0, d->character, 0, 0,
                            TO_ROOM);
                        act("@RLava spews up violently from the cracks in the ground!@n", 0, d->character, 0, 0,
                            TO_CHAR);
                        break;
                    case 2:
                        act("@RThe lava bubbles and gives off tremendous heat!@n", 0, d->character, 0, 0, TO_ROOM);
                        act("@RThe lava bubbles and gives off tremendous heat!@n", 0, d->character, 0, 0, TO_CHAR);
                        break;
                    case 3:
                        act("@RNoxious fumes rise from the bubbling lava!@n", 0, d->character, 0, 0, TO_ROOM);
                        act("@RNoxious fumes rise from the bubbling lava!@n", 0, d->character, 0, 0, TO_CHAR);
                        break;
                    case 4:
                        act("@RSome of the lava cools as it spreads further from the source!@n", 0, d->character, 0,
                            0, TO_ROOM);
                        act("@RSome of the lava cools as it spreads further from the source!@n", 0, d->character, 0,
                            0, TO_CHAR);
                        break;
                ...
                act("@RLava covers the entire area now!@n", 0, d->character, 0, 0, TO_ROOM);
                act("@RLava covers the entire area now!@n", 0, d->character, 0, 0, TO_CHAR);
                ROOM_EFFECT(IN_ROOM(d->character)) += 1;
            }
        }
        if (ABSORBING(d->character) && IN_ROOM(d->character) != IN_ROOM(ABSORBING(d->character))) {
        ...
        if (IS_ANDROID(d->character) && ABSORBING(d->character)) {
            if (GET_MOVE(ABSORBING(d->character)) < (GET_MAX_MOVE(d->character) / 15) &&
                GET_MANA(ABSORBING(d->character)) < (GET_MAX_MANA(d->character) / 15)) {
                act("@WYou stop absorbing stamina and ki from @c$N as they don't have enough for you to take@W!@n",
                    1, d->character, 0, ABSORBING(d->character), TO_CHAR);
                act("@C$n@W stops absorbing stamina and ki from you!@n", 1, d->character, 0, ABSORBING(d->character),
                    TO_VICT);
                act("@C$n@W stops absorbing stamina and ki from @c$N@w!@n", 1, d->character, 0,
        ...
        if (IS_ANDROID(d->character) && ABSORBING(d->character) && rand_number(1, 9) >= 6) {
            if (GET_MOVE(ABSORBING(d->character)) > (GET_MAX_MOVE(d->character) / 15) ||
                ...
                act("@WYou absorb stamina and ki from @c$N@W!@n", 1, d->character, 0, ABSORBING(d->character),
                    TO_CHAR);
                act("@C$n@W absorbs stamina and ki from you!@n", 1, d->character, 0, ABSORBING(d->character),
                    TO_VICT);
                send_to_char(ABSORBING(d->character), "@wTry 'escape'!@n\r\n");
                act("@C$n@W absorbs stamina and ki from @c$N@w!@n", 1, d->character, 0, ABSORBING(d->character),
                    TO_NOTVICT);
                ...
                if (GET_MOVE(d->character) > GET_MAX_MOVE(d->character) &&
                    ...
                    GET_MANA(d->character) = GET_MAX_MANA(d->character);
                    GET_MOVE(d->character) = GET_MAX_MOVE(d->character);
                    act("@WYou stop absorbing stamina and ki from @c$N as you are full@W!@n", 1, d->character, 0,
                        ABSORBING(d->character), TO_CHAR);
                    act("@C$n@W stops absorbing stamina and ki from you!@n", 1, d->character, 0,
        ...
        if (GET_OVERFLOW(d->character) == 1) {
            mudlog(NRM, ADMLVL_GOD, 1, "OVERFLOW: %s has caused an overflow, check for illegal activity.",
                   GET_NAME(d->character));
            GET_OVERFLOW(d->character) = 0;
        }
...
ACMD(do_snet) {
    int channel = 0, global = 0, call = -1, reached = 0;
    ...
    if (channel > 0) {
        ...
        act("@wYou push some buttons on $p@w and change its channel.", 1, ch, obj, 0, TO_CHAR);
        act("@C$n@w pushes some buttons on $p@w and changes its channel.", 1, ch, obj, 0, TO_ROOM);
        return;
    } else {
        ...
        for (i = descriptor_list; i; i = i->next) {
            ...
            if (GET_EQ(i->character, WEAR_EYE)) {
                ...
                if (global == 0 && call <= -1 && SFREQ(obj2) == SFREQ(obj) && GET_ADMLEVEL(i->character) < 1) {
                ...
                else if (global == 1 && call <= -1 && GET_ADMLEVEL(i->character) < 1) {
...
ACMD(do_scouter) {
    ...
    if (!obj) {
                ...
                if (STATE(i) != CON_PLAYING) {
                    ...
                    if (OBJ_FLAGGED(obj, ITEM_BSCOUTER) && GET_HIT(i->character) >= 150000) {
                        ...
                        send_to_char(ch, "@D<@GPowerlevel Detected@D: [@Y%s@D]@w ---> @C%s@n\r\n",
                                     add_commas(GET_HIT(i->character)), same == 1 ? pathway : blah);
                    }
                    ++count;
                    free(blah);
                }
        ...
        if (IS_ANDROID(vict)) {
            act("$n points $s scouter at you.", 0, ch, 0, vict, TO_VICT);
            act("$n points $s scouter at $N.", 0, ch, 0, vict, TO_NOTVICT);
            send_to_char(ch, "@D,==================================|@n\r\n");
            ...
            if (OBJ_FLAGGED(obj, ITEM_BSCOUTER) && GET_HIT(vict) >= 150000) {
                ...
                act("$n's scouter explodes!", 0, ch, 0, 0, TO_ROOM);
                extract_obj(obj);
                save_char(ch);
                return;
...
ACMD(do_quit) {
    ...
    if (subcmd != SCMD_QUIT)
        ...
        mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1, "%s has quit the game.", GET_NAME(ch));
...
ACMD(do_steal) {
    ...
    if (axion_dice(0) > 100 && GET_POS(vict) != POS_SLEEPING) { /* Unlucky! */
        reveal_hiding(ch, 0);
        act("@r$N@R just happens to glance in your direction! What terrible luck!@n", 1, ch, 0, vict, TO_CHAR);
        act("@RYou just happen to glance behind you and spot @r$n@R trying to STEAL from you!@n", 1, ch, 0, vict,
            TO_VICT);
        act("@r$N@R just happens to glance in @r$n's@R direction and catches $m trying to STEAL!@n", 1, ch, 0, vict,
            TO_NOTVICT);
        prob = -1000;
    }
    ...
    if (prob + 20 < perc && GET_POS(vict) != POS_SLEEPING) { /* Critical failure! */
        reveal_hiding(ch, 0);
        act("@rYou are caught trying to stick your hand in @R$N's@r possessions!@n", 1, ch, 0, vict, TO_CHAR);
        act("@rYou catch @R$n@r trying to rummage through your possessions!@n", 1, ch, 0, vict, TO_VICT);
        act("@R$n@R is caught by @R$N@r as $e sticks $s hand in @R$N's@r possessions!@n", 1, ch, 0, vict,
            TO_NOTVICT);
        WAIT_STATE(ch, PULSE_3SEC);
        if (IS_NPC(vict)) {
            set_fighting(vict, ch);
        ...
        if (!strcasecmp(arg, "zenni")) { /* MOOLA! */
            if (prob > perc) {
                if (GET_GOLD(vict) > 0) {
                    ...
                    if (!IS_NPC(vict)) {
                        ...
                        mudlog(NRM, MAX(ADMLVL_GRGOD, GET_INVIS_LEV(ch)), 1,
                    ...
                    if (axion_dice(0) > prob) {
                        ...
                        reveal_hiding(ch, 0);
                        act("@R$n@r just stole zenni from @R$N@r!@n", 1, ch, 0, vict, TO_ROOM);
                ...
                reveal_hiding(ch, 0);
                act("@rYou are caught trying to steal zenni from @R$N@r!@n", 1, ch, 0, vict, TO_CHAR);
                act("@rYou catch @R$n's@r hand trying to snatch your zenni!@n", 1, ch, 0, vict, TO_VICT);
                act("@R$N@r catches @R$n's@r hand trying to snatch $S zenni!@n", 1, ch, 0, vict, TO_NOTVICT);
                WAIT_STATE(ch, PULSE_3SEC);
                if (IS_NPC(vict)) {
                    set_fighting(vict, ch);
                }
            ...
            if (!(obj = get_obj_in_list_vis(ch, arg, NULL, vict->carrying))) {
                ...
                if (!obj) {
                    act("$E isn't wearing that item.", 0, ch, 0, vict, TO_CHAR);
                    return;
                } else if (GET_POS(vict) > POS_SLEEPING) {
                    send_to_char(ch,
                    ...
                    act("You unequip $p and steal it.", 0, ch, obj, vict, TO_CHAR);
                    if (axion_dice(0) > prob) {
                        send_to_char(ch, "You think that your movements might have been a bit obvious.\r\n");
                        reveal_hiding(ch, 0);
                        act("@R$n@r just stole $p@r from @R$N@r!@n", 1, ch, obj, vict, TO_ROOM);
                    ...
                    reveal_hiding(ch, 0);
                    GET_POS(vict) = POS_SITTING;
                    act("@rYou are caught trying to steal $p@r from @R$N@r!@n", 1, ch, obj, vict, TO_CHAR);
                    act("@rYou feel your body being shifted while you sleep and wake up to find @R$n@r trying to steal $p@r from you!@n",
                        1, ch, obj, vict, TO_VICT);
                    act("@R$N@r catches @R$n's@r trying to $p@r from $M during $S sleep!@n", 1, ch, obj, vict,
                        TO_NOTVICT);
                    WAIT_STATE(ch, PULSE_3SEC);
                    if (IS_NPC(vict)) {
                ...
                if (!give_otrigger(obj, vict, ch) || !receive_mtrigger(ch, vict, obj)) {
                    ...
                    act("You steal $p from $N.", 0, ch, obj, vict, TO_CHAR);
                    obj_from_char(obj);
                    obj_to_char(obj, ch);
                    if (!IS_NPC(vict)) {
                        SET_BIT_AR(PLR_FLAGS(vict), PLR_STOLEN);
                        mudlog(NRM, MAX(ADMLVL_GRGOD, GET_INVIS_LEV(ch)), 1, "THEFT: %s has stolen %s@n from %s",
                    ...
                    reveal_hiding(ch, 0);
                    act("@rYou are caught trying to steal $p@r from @R$N@r!@n", 1, ch, obj, vict, TO_CHAR);
                    act("@rYou catch @R$n@r trying to steal $p@r from you!@n", 1, ch, obj, vict, TO_VICT);
                    act("@R$N@r catches @R$n's@r trying to $p@r!@n", 1, ch, obj, vict, TO_NOTVICT);
                    WAIT_STATE(ch, PULSE_3SEC);
                    if (IS_NPC(vict)) {
                        GET_POS(vict) = POS_STANDING;
                        set_fighting(vict, ch);
...
ACMD(do_visible) {
    ...
    if (AFF_FLAGGED(ch, AFF_ETHEREAL) && affectedv_by_spell(ch, ART_EMPTY_BODY)) {
        ...
        if (AFF_FLAGGED(ch, AFF_ETHEREAL)) {
            ...
            if (!appeared)
                act("$n flashes into existence.", 0, ch, 0, 0, TO_ROOM);
        }
        appeared = 1;
    }
...
perform_group(struct char_data *ch, struct char_data *vict, int highlvl, int lowlvl, int64_t highpl, int64_t lowpl) {
    ...
    if (GET_BONUS(vict, BONUS_LONER) > 0) {
        act("$n is the loner type and refuses to be in your group.", 1, vict, 0, ch, TO_VICT);
        return false;
    }

    if (GET_LEVEL(vict) + 12 < highlvl) {
        act("$n isn't experienced enough to be in your group with its current members.", 1, vict, 0, ch, TO_VICT);
        return false;
    }

    if (GET_LEVEL(vict) > lowlvl + 12) {
        act("$n is too experienced to be in your group with its current members.", 1, vict, 0, ch, TO_VICT);
        return false;
    }

    if (highlvl >= 100) {
        if (gear_pl(vict) > highpl * 1.5) {
            act("$n is too powerful right now to be in a level 100 group with you.", 1, vict, 0, ch, TO_VICT);
            return false;
        }

        if (gear_pl(vict) < lowpl * 0.5) {
            act("$n is too weak right now to be in a level 100 group with you.", 1, vict, 0, ch, TO_VICT);
            return false;
        }
    }
    ...
    if (ch != vict)
        act("$N is now a member of your group.", 0, ch, 0, vict, TO_CHAR);
    act("You are now a member of $n's group.", 0, ch, 0, vict, TO_VICT);
    act("$N is now a member of $n's group.", 0, ch, 0, vict, TO_NOTVICT);
    return true;
}
        ...
        for (f = k->followers; f; f = f->next) {
            ...
            if (GET_HIT(f->follower) > (GET_MAX_HIT(f->follower) - gear_weight(f->follower)) / 10) {
                snprintf(buf, sizeof(buf),
                         ...
                         add_commas(GET_HIT(f->follower)), add_commas(GET_MANA(f->follower)),
                         add_commas(GET_MOVE(f->follower)),
...
ACMD(do_group) {
    ...
    if (!(vict = get_char_vis(ch, buf, NULL, FIND_CHAR_ROOM)))
        ...
        act("$N must follow you to enter your group.", 0, ch, 0, vict, TO_CHAR);
    else {
        if (!AFF_FLAGGED(vict, AFF_GROUP)) {
            ...
            if (ch != vict)
                act("$N is no longer a member of your group.", 0, ch, 0, vict, TO_CHAR);
            act("You have been kicked out of $n's group!", 0, ch, 0, vict, TO_VICT);
            act("$N has been kicked out of $n's group!", 0, ch, 0, vict, TO_NOTVICT);
...
ACMD(do_ungroup) {
    ...
    if (!*buf) {
        ...
        for (f = ch->followers; f; f = next_fol) {
            ...
            if (AFF_FLAGGED(f->follower, AFF_GROUP)) {
                ...
                act("$N has disbanded the group.", 1, f->follower, NULL, ch, TO_CHAR);
    ...
    act("$N is no longer a member of your group.", 0, ch, 0, tch, TO_CHAR);
    act("You have been kicked out of $n's group!", 0, ch, 0, tch, TO_VICT);
    act("$N has been kicked out of $n's group!", 0, ch, 0, tch, TO_NOTVICT);
...
ACMD(do_report) {
    ...
    for (f = k->followers; f; f = f->next)
        if (AFF_FLAGGED(f->follower, AFF_GROUP) && f->follower != ch)
            act(buf, 1, ch, NULL, f->follower, TO_VICT);

    if (k != ch)
        act(buf, 1, ch, NULL, k, TO_VICT);
                ...
                switch (GET_OBJ_VNUM(mag_item)) {
                    case 381:
                        ...
                        act("@WYou place the $p@W against your chest and feel a rush of stamina as it automatically administers the dose.@n",
                            1, ch, mag_item, 0, TO_CHAR);
                        act("@C$n@W places an $p@W against $s chest and a loud click is heard.@n", 1, ch, mag_item,
                            0, TO_ROOM);
                    ...
                    case 382:
                        if (AFF_FLAGGED(ch, AFF_BURNED)) {
                            act("@WYou gently apply the salve to your burns.@n", 1, ch, mag_item, 0, TO_CHAR);
                            act("@C$n@W gently applies a burn salve to $s burns.@n", 1, ch, mag_item, 0, TO_ROOM);
                    ...
                    case 383:
                        if (AFF_FLAGGED(ch, AFF_POISON)) {
                            act("@WYou place the $p@W against your neck and feel a rush of relief as the antitoxiin enters your bloodstream.@n",
                                1, ch, mag_item, 0, TO_CHAR);
                            act("@C$n@W places an $p@W against $s neck and a loud click is heard.@n", 1, ch,
                                mag_item, 0, TO_ROOM);
                            null_affect(ch, AFF_POISON);
                            extract_obj(mag_item);
                        } else {
                        ...
                        return;
                    case 385:
                        act("@WYou drink the contents of the vial before disposing of it.@n", 1, ch, mag_item, 0,
                            TO_CHAR);
                        act("@C$n@W dinks a $p and then disposes of it.@n", 1, ch, mag_item, 0, TO_ROOM);
                        if (AFF_FLAGGED(ch, AFF_BLIND)) {
                            act("@WYour eyesight has returned!@n", 1, ch, mag_item, 0, TO_CHAR);
                            act("@C$n@W eyesight seems to have returned.@n", 1, ch, mag_item, 0, TO_ROOM);
                            null_affect(ch, AFF_BLIND);
                        }
                        int refreshed = 0;
    ...
    switch (subcmd) {
        ...
        case SCMD_AFK:
            ...
            if (PRF_FLAGGED(ch, PRF_AFK))
                act("$n has gone AFK.", 1, ch, 0, 0, TO_ROOM);
            else {
                act("$n has come back from AFK.", 1, ch, 0, 0, TO_ROOM);
...
ACMD(do_compare) {
    ...
    act(msg, 0, ch, obj1, obj2, TO_CHAR);
}
...
ACMD(do_break) {
    ...
    act("$n ruins $p.", 0, ch, obj, 0, TO_ROOM);
...
ACMD(do_fix) {
    ...
    struct char_data *dummy = NULL;
    int cmbrk, found = 0, self = 0, custom = 0;
    ...
    for (rep = ch->carrying; rep; rep = next_obj) {
        ...
        if (custom == 0) {
            if (found == 0 && GET_OBJ_VNUM(rep) == 48 &&
                (!OBJ_FLAGGED(rep, ITEM_BROKEN) && !OBJ_FLAGGED(rep, ITEM_FORGED))) {
                found = 1;
                obj4 = rep;
            }
        } else {
            if (found == 0 && GET_OBJ_VNUM(rep) == 13593 &&
                (!OBJ_FLAGGED(rep, ITEM_BROKEN) && !OBJ_FLAGGED(rep, ITEM_FORGED))) {
                found = 1;
                obj4 = rep;
            }
    ...
    if (self == 0) {
        if (GET_SKILL(ch, SKILL_REPAIR) < axion_dice(0)) {
            act("You try to repair $p but screw up..", 1, ch, obj, 0, TO_CHAR);
            act("$n tries to repair $p but screws up..", 1, ch, obj, 0, TO_ROOM);
            extract_obj(obj4);
        ...
        if (GET_OBJ_VAL(obj, VAL_ALL_HEALTH) + GET_SKILL(ch, SKILL_REPAIR) < 100) {
            ...
            act("$n repairs $p a bit.", 0, ch, obj, 0, TO_ROOM);
            ...
            act("$n repairs $p completely.", 0, ch, obj, 0, TO_ROOM);
        ...
        if (GET_HIT(ch) >= gear_pl(ch)) {
            ...
            act("You try to repair your body but screw up..", 1, ch, 0, 0, TO_CHAR);
            act("$n tries to repair $s body but screws up..", 1, ch, 0, 0, TO_ROOM);
            extract_obj(obj4);
            improve_skill(ch, SKILL_REPAIR, 1);
            WAIT_STATE(ch, PULSE_5SEC);
            return;
        } else {
            act("You use the repair kit to fix part of your body...", 1, ch, 0, 0, TO_CHAR);
            act("$n works on their body with a repair kit.", 1, ch, 0, 0, TO_ROOM);
 ...
 */
bool is_innate(struct char_data *ch, int spellnum) {
    return (spellnum == SPELL_FAERIE_FIRE && IS_ANIMAL(ch));
}
...
*/
bool is_innate_ready(struct char_data *ch, int spellnum) {
    struct innate_node *inn;

    for (inn = ch->innate; inn; inn = inn->next) {
...
void add_innate_affects(struct char_data *ch) {
    switch (GET_RACE(ch)) {
        ...
        case RACE_ANDROID:
        case RACE_BIO:
            affect_modify(ch, APPLY_NONE, 0, 0, AFF_INFRAVISION, 1);
            break;
    }
    affect_total(ch);
}
...
ACMD(do_clan) {
    ...
    if (GET_ADMLEVEL(ch) >= ADMLVL_IMPL) {
        ...
        if (!(strcmp(arg1, "create"))) {
            if (!*arg2)
                ...
                clanCreate(arg2);
                mudlog(BRF, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) %s has created a clan named %s.",
                       GET_NAME(ch), arg2);
            }
            return;
    ...
    if (!(strcmp(arg1, "join"))) {
        if (!*arg2)
            ...
            send_to_char(ch, "You are already a member of %s.\r\n", arg2);
        else if (GET_CLAN(ch) != NULL && checkCLAN(ch) == 1 && strstr(GET_CLAN(ch), "Applying") == 0)
        ...
        if (GET_CLAN(ch) == NULL || !(strcmp(GET_CLAN(ch), "None."))) {
            ...
            if (clanIsModerator(GET_CLAN(ch), ch) == 0)
        ...
        if (GET_CLAN(ch) == NULL || !(strcmp(GET_CLAN(ch), "None."))) {
            ...
            if (!clanIsMember(GET_CLAN(ch), ch) && !clanIsModerator(GET_CLAN(ch), ch)) {
                send_to_char(ch, "You are not in a clan.\r\n");
                return;
            } else if (!ROOM_FLAGGED(IN_ROOM(ch), ROOM_CBANK) && clanBANY(GET_CLAN(ch), ch) == 0) {
        ...
        if (!*arg3 || !*name)
            ...
            send_to_char(ch, "%s is not a valid clan.\r\n", arg3);
        else if ((clanIsModerator(arg3, ch) == 0 && clanIsMember(arg3, ch) == 0 &&
...
ACMD(do_aid) {
    ...
    char arg[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
    int dc = 0, found = 0, num = 47, num2 = 0, survival = 0;
    ...
    for (obj = ch->carrying; obj; obj = next_obj) {
        next_obj = obj->next_content;
        if (found == 0 && GET_OBJ_VNUM(obj) == num &&
            (!OBJ_FLAGGED(obj, ITEM_BROKEN) && !OBJ_FLAGGED(obj, ITEM_FORGED))) {
            found = 1;
            aid_obj = obj;
        }
    ...
    if (num == 47) {
        ...
        if (!AFF_FLAGGED(vict, AFF_SPIRIT) && !PLR_FLAGGED(vict, PLR_BANDAGED)) {
            ...
            act("$n attempts to bandage $N's wounds.", 1, ch, 0, vict, TO_ROOM);
            dc = axion_dice(0);
            if ((GET_SKILL(ch, SKILL_FIRST_AID) + 1) > dc) {
                ...
                act("$n's wounds are stablized by $N!", 1, vict, 0, ch, TO_NOTVICT);
                ...
                if (vict != ch) {
                    ...
                    act("$N fails to bandage $n's wounds properly, wasting an entire set of bandages...", 1, vict, 0,
                        ch, TO_NOTVICT);
                    act("$N fails to bandage your wounds properly, wasting an entire set of bandages...", 1, vict, 0,
                        ch, TO_CHAR);
                } else {
                    act("$N fails to bandage $s wounds properly, wasting an entire set of bandages...", 1, vict, 0,
                        ch, TO_NOTVICT);
                    act("You fail to bandage your wounds properly, wasting an entire set of bandages...", 1, vict, 0,
                        ch, TO_VICT);
                }
                extract_obj(aid_obj);
            }
    ...
    if (!strcasecmp(arg, "light")) {
        if (GET_SKILL(ch, SKILL_FOCUS) && GET_SKILL(ch, SKILL_CONCENTRATION) < 75) {
            ...
            if (PLR_FLAGGED(ch, PLR_AURALIGHT)) {
                ...
                act("$n's aura fades as they stop shining light on the area.", 1, ch, 0, 0, TO_ROOM);
