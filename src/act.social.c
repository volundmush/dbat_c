...
ACMD(do_action) {
    ...
    if (GET_POS(vict) < action->min_victim_position)
        act("$N is not in a proper position for that.", 0, ch, 0, vict, TO_CHAR | TO_SLEEP);
    else {
        if (*part) {
                    ...
                    case 0:
                        if (GET_SEX(ch) == SEX_MALE) {
                            if (GET_SEX(victim) == SEX_MALE)
                                act("$n accuses you of fighting like a woman!", 0, ch, 0, victim, TO_VICT);
                            else
                                act("$n says that women can't fight.", 0, ch, 0, victim, TO_VICT);
                        } else {        /* Ch == Woman */
                            if (GET_SEX(victim) == SEX_MALE)
...
void boot_social_messages(void) {
    ...
    for (;;) {
        ...
        if (CONFIG_NEW_SOCIALS == 0 && !soc_mess_list[curr_soc].char_found)
            continue;
