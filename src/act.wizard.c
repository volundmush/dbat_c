...
static bool perform_set(struct char_data *ch, struct char_data *vict, int mode, char *val_arg);
...
ACMD(do_news) {
    ...
    int entries = 0, lookup = 0, nr, found = 0, first = 1, exit = 0;
...
ACMD(do_newsedit) {
    ...
    *(tmstr + strlen(tmstr) - 1) = '\0';
    ...
    char *backstr = NULL;
    act("$n begins to edit the news.", 1, ch, 0, 0, TO_ROOM);
...
static void print_lockout(struct char_data *ch) {
    ...
    while (!feof(file)) {
        get_line(file, line);
        sscanf(line, "%s\n", filler);
        if (first != 1 && strstr(buf, filler) == NULL) {
            if (count == 0) {
...
ACMD(do_transobj) {
    ...
    if (!(obj = get_obj_in_list_vis(ch, arg, NULL, ch->carrying))) {
        ...
        act("You send $p to everyone in the game.", 1, ch, obj, 0, TO_CHAR);
        ...
        for (d = descriptor_list; d; d = d->next) {
            if (IS_NPC(d->character))
                ...
                act("$N sends $p across the universe to you.", 1, d->character, obj, ch, TO_CHAR);
        ...
        act("You send $p to $N.", 1, ch, obj, vict, TO_CHAR);
        act("$n sends $p across the universe to you.", 1, ch, obj, vict, TO_VICT);
        obj_from_char(obj);
...
ACMD(do_recall) {
    if (GET_ADMLEVEL(ch) < 1) {
        ...
        act("$n disappears in a burst of light!", 0, ch, 0, 0, TO_ROOM);
    ...
    if (!*argument)
        ...
        if (NoName == 1) {
            ...
            sprintf(blom, "@D(@GOOC@W: @gSmote by user %s@D)@n",
                    IS_NPC(ch) ? GET_NAME(ch) : (ch->desc->user == NULL ? "ERROR REPORT" : ch->desc->user));
            act(blom, 0, ch, 0, 0, TO_ROOM);
        }
        if (found == 0) {
            ...
            act(buf, 0, ch, 0, tch, TO_CHAR);
            act(buf, 0, ch, 0, tch, TO_NOTVICT);
            search_replace(buf, "$N", "you");
            act(buf, 0, ch, 0, tch, TO_VICT);
        }
    }
}
...
ACMD(do_trans) {
    ...
    if (!*buf)
        ...
        if (!(victim = get_char_vis(ch, buf, NULL, FIND_CHAR_WORLD)))
            ...
            act("$n disappears in a mushroom cloud.", 0, victim, 0, 0, TO_ROOM);
...
ACMD(do_teleport) {
    ...
    if (!*buf)
        ...
        send_to_char(ch, "%s", CONFIG_OK);
        act("$n disappears in a puff of smoke.", 0, victim, 0, 0, TO_ROOM);
        char_from_room(victim);
        char_to_room(victim, target);
        act("$n arrives from a puff of smoke.", 0, victim, 0, 0, TO_ROOM);
        act("$n has teleported you!", 0, ch, 0, (char *) victim, TO_VICT);
        look_at_room(IN_ROOM(victim), victim, 0);
        enter_wtrigger(&world[IN_ROOM(victim)], victim, -1);
    }
}
...
ACMD(do_load) {
    ...
    if (is_abbrev(buf, "mob")) {
        ...
        for (i = 0; i < n; i++) {
            ...
            act("$n makes a quaint, magical gesture with one hand.", 1, ch, 0, 0, TO_ROOM);
            act("$n has created $N!", 0, ch, 0, mob, TO_ROOM);
            act("You create $N.", 0, ch, 0, mob, TO_CHAR);
            load_mtrigger(mob);
        }
        ...
        for (i = 0; i < n; i++) {
            ...
            act("You create $p.", 0, ch, obj, 0, TO_CHAR);
            load_otrigger(obj);
        }
    } else
...
ACMD(do_purge) {
    ...
    if (*buf) {
        if ((vict = get_char_vis(ch, buf, NULL, FIND_CHAR_WORLD)) != NULL) {
            ...
            if (!IS_NPC(vict)) {
                mudlog(BRF, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) %s has purged %s.", GET_NAME(ch),
                       GET_NAME(vict));
...
ACMD(do_syslog) {
    ...
    if (((tp = search_block(arg, logtypes, 0)) == -1)) {
...
ACMD(do_advance) {
    ...
    if (newlevel < GET_LEVEL(victim)) {
        ...
        act("$n makes some strange gestures.\r\n"
            ...
            "You feel slightly different.", 0, ch, 0, victim, TO_VICT);
    }
...
ACMD(do_restore) {
    ...
    if (!*buf)
        ...
        for (j = descriptor_list; j; j = j->next) {
            ...
            update_pos(vict);
            act("You have been fully healed by $N!", 0, vict, 0, ch, TO_CHAR | TO_SLEEP);
        ...
        act("You have been fully healed by $N!", 0, vict, 0, ch, TO_CHAR);
        if (GET_SUPPRESS(vict) > 0 && GET_HIT(vict) > (((gear_pl(vict)) / 100) * GET_SUPPRESS(vict))) {
            GET_HIT(vict) = (((gear_pl(vict)) / 100) * GET_SUPPRESS(vict));
            send_to_char(vict, "@mYou are healed to your suppression limit.@n\r\n");
        }
...
static void perform_immort_invis(struct char_data *ch, int level) {
    ...
    for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room) {
        ...
        if (GET_ADMLEVEL(tch) >= GET_INVIS_LEV(ch) && GET_ADMLEVEL(tch) < level)
            act("You blink and suddenly realize that $n is gone.", 0, ch, 0,
                tch, TO_VICT);
        if (GET_ADMLEVEL(tch) < GET_INVIS_LEV(ch) && GET_ADMLEVEL(tch) >= level)
            act("You suddenly realize that $n is standing beside you.", 0, ch, 0,
                tch, TO_VICT);
    }
...
ACMD(do_force) {
    ...
    if (!*arg || !*to_force)
        ...
        if (!(vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD)))
            ...
            send_to_char(ch, "%s", CONFIG_OK);
            act(buf1, 1, ch, NULL, vict, TO_VICT);
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) %s forced %s to %s", GET_NAME(ch),
                   GET_NAME(vict), to_force);
            command_interpreter(vict, to_force);
        }
    } else if (!strcasecmp("room", arg)) {
        send_to_char(ch, "%s", CONFIG_OK);
        mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) %s forced room %d to %s",
        ...
        send_to_char(ch, "%s", CONFIG_OK);
        mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) %s forced all to %s", GET_NAME(ch), to_force);
...
ACMD(do_zreset) {
    ...
    if (*arg == '*') {
        if (GET_ADMLEVEL(ch) < ADMLVL_VICE) {
            ...
            mudlog(NRM, MAX(ADMLVL_GRGOD, GET_INVIS_LEV(ch)), 1, "(GC) %s reset all MUD zones.", GET_NAME(ch));
    ...
    if (i <= top_of_zone_table && (can_edit_zone(ch, i) || GET_ADMLEVEL(ch) > ADMLVL_IMMORT)) {
        ...
        mudlog(NRM, MAX(ADMLVL_GRGOD, GET_INVIS_LEV(ch)), 1, "(GC) %s reset zone %d (%s)", GET_NAME(ch),
               zone_table[i].number, zone_table[i].name);
    ...
    if (!*arg)
        ...
        switch (subcmd) {
            ...
            case SCMD_PARDON:
                ...
                mudlog(BRF, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) %s pardoned by %s", GET_NAME(vict),
                       GET_NAME(ch));
                break;
            case SCMD_NOTITLE:
                result = PLR_TOG_CHK(vict, PLR_NOTITLE);
                mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) Notitle %s for %s by %s.",
                       ONOFF(result), GET_NAME(vict), GET_NAME(ch));
                send_to_char(ch, "(GC) Notitle %s for %s by %s.\r\n", ONOFF(result), GET_NAME(vict), GET_NAME(ch));
                break;
            case SCMD_SQUELCH:
                result = PLR_TOG_CHK(vict, PLR_NOSHOUT);
                mudlog(BRF, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) Squelch %s for %s by %s.",
                       ONOFF(result), GET_NAME(vict), GET_NAME(ch));
            ...
            case SCMD_FREEZE:
                ...
                act("A sudden cold wind conjured from nowhere freezes $n!", 0, vict, 0, 0, TO_ROOM);
                mudlog(BRF, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) %s frozen by %s.", GET_NAME(vict),
                       GET_NAME(ch));
                break;
            case SCMD_THAW:
                if (!PLR_FLAGGED(vict, PLR_FROZEN)) {
                ...
                mudlog(BRF, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) %s un-frozen by %s.", GET_NAME(vict),
                       GET_NAME(ch));
                ...
                act("A sudden fireball conjured from nowhere thaws $n!", 0, vict, 0, 0, TO_ROOM);
                break;
            case SCMD_UNAFFECT:
...
static bool perform_set(struct char_data *ch, struct char_data *vict, int mode,
                       char *val_arg) {
    int i, on = 0, off = 0;
    int64_t value = 0;
    room_rnum rnum;
    ...
    switch (mode) {
        ...
        case 4:
            vict->max_hit = value;
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set maxpl for %s.", GET_NAME(ch),
                   GET_NAME(vict));
            log_imm_action("SET: %s has set maxpl for %s.", GET_NAME(ch), GET_NAME(vict));
            break;
        case 5:
            vict->max_mana = value;
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set maxki for %s.", GET_NAME(ch),
                   GET_NAME(vict));
            log_imm_action("SET: %s has set maxki for %s.", GET_NAME(ch), GET_NAME(vict));
            break;
        case 6:
            vict->max_move = value;
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set maxsta for %s.", GET_NAME(ch),
                   GET_NAME(vict));
            log_imm_action("SET: %s has set maxsta for %s.", GET_NAME(ch), GET_NAME(vict));
            break;
        case 7:
            vict->hit = value;
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set pl for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 8:
            vict->mana = value;
            affect_total(vict);
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set ki for %s.", GET_NAME(ch),
                   GET_NAME(vict));
            log_imm_action("SET: %s has set ki for %s.", GET_NAME(ch), GET_NAME(vict));
            break;
        case 9:
            vict->move = value;
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set st for %s.", GET_NAME(ch),
                   GET_NAME(vict));
            log_imm_action("SET: %s has set st for %s.", GET_NAME(ch), GET_NAME(vict));
            break;
        case 10:
            GET_ALIGNMENT(vict) = RANGE(-1000, 1000);
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set align for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 11:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set str for %s.", GET_NAME(ch),
                   GET_NAME(vict));
    ...
    affect_total(vict);
       break; */
        case 13:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set intel for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 14:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set wis for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 15:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set dex for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 16:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set con for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 17:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set speed for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 18:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set armor index for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 19:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set zenni for %s.", GET_NAME(ch),
                   GET_NAME(vict));
            log_imm_action("SET: %s has set zenni for %s.", GET_NAME(ch), GET_NAME(vict));
            break;
        case 20:
            GET_BANK_GOLD(vict) = RANGE(0, 100000000);
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set bank for %s.", GET_NAME(ch),
                   GET_NAME(vict));
            log_imm_action("SET: %s has set bank for %s.", GET_NAME(ch), GET_NAME(vict));
            break;
        case 21:
            vict->exp = RANGE(0, 50000000);
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set exp for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 28:
            if (GET_CLASS_LEVEL(vict)) {
                ...
                mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set PS for %s.", GET_NAME(ch),
                       GET_NAME(vict));
        ...
        case 38: SET_OR_REMOVE(PLR_FLAGS(vict), PLR_DELETED);
            break;
        case 39:
            if ((i = search_block(val_arg, class_names, 0)) < 0) {
        ...
        case 46: SET_OR_REMOVE(PLR_FLAGS(vict), PLR_NODELETE);
            break;
        case 47:
            if ((i = search_block(val_arg, genders, 0)) < 0) {
     ...
     */
        ...
        case 52:
            if ((i = search_block(val_arg, race_names, 0)) < 0) {
        ...
        case 64:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set basepl for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 65:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set baseki for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 66:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set basest for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 68:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set absorbs for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 69:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set upgrade points for %s.",
                   GET_NAME(ch), GET_NAME(vict));
        ...
        case 70:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set aura for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 74:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set death count for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 78:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set skill slots for %s.", GET_NAME(ch),
                   GET_NAME(vict));
        ...
        case 80:
            ...
            mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "SET: %s has set transformation class for %s.",
                   GET_NAME(ch), GET_NAME(vict));
...
ACMD(do_raise) {
    ...
    if (GET_LEVEL(vict) > 9 && statpunish == 1) {
        send_to_char(vict,
    ...
    act("$n's body forms in a pool of @Bblue light@n.", 1, vict, 0, 0, TO_ROOM);
}
...
ACMD(do_chown) {
    ...
    if (!*buf2)
        ...
        act("@n$n makes a magical gesture and $p@n flies from $N to $m.", 0, ch, obj, victim, TO_NOTVICT);
        act("@n$n makes a magical gesture and $p@n flies away from you to $m.", 0, ch, obj, victim, TO_VICT);
        act("@nYou make a magical gesture and $p@n flies away from $N to you.", 0, ch, obj, victim, TO_CHAR);
...
ACMD(do_zpurge) {
    ...
    mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "(GC) %s has purged zone %d.", GET_NAME(ch), zone);
}
...
ACMD(do_findkey) {
    ...
    } else if ((dir = search_block(arg, dirs, 0)) >= 0 ||
               (dir = search_block(arg, abbr_dirs, 0)) >= 0) {
        if (!EXIT(ch, dir)) {
