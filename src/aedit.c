...
ACMD(do_oasis_aedit) {
    ...
    if (!strcasecmp("save", arg)) {
        mudlog(CMP, MAX(ADMLVL_BUILDER, GET_INVIS_LEV(ch)), 1, "OLC: %s saves socials.", GET_NAME(ch));
    ...
    mudlog(CMP, ADMLVL_IMMORT, 1, "OLC: %s starts editing actions.", GET_NAME(ch));
}
        ...
        case AEDIT_CONFIRM_SAVESTRING:
            switch (*arg) {
                ...
                case 'Y':
                    ...
                    mudlog(CMP, ADMLVL_IMPL, 1, "OLC: %s edits action %s",
