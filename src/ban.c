...
ACMD(do_ban) {
    ...
    mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "%s has banned %s for %s players.",
...
ACMD(do_unban) {
    ...
    mudlog(NRM, MAX(ADMLVL_GOD, GET_INVIS_LEV(ch)), 1, "%s removed the %s-player ban on %s.",
