...
bool save_board(struct board_info *ts) {
...
bool mesglookup(struct board_msg *message, struct char_data *ch, struct board_info *board) {
    int mem = 0;
...
void write_board_message(obj_vnum board_vnum, struct char_data *ch, char *arg) {
    ...
    act("@C$n@w starts writing on the board.@n", 1, ch, 0, 0, TO_ROOM);
    skip_spaces(&arg);
