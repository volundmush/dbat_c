...
bool passcomm(struct char_data *ch, char *comm);
    ...
    if (d->showstr_count) {
        ...
        *prompt = '\0';
        ...
        if (PRF_FLAGGED(d->character, PRF_DISPAUTO) && GET_LEVEL(d->character) >= 500 && len < sizeof(prompt)) {
            ...
            if (flagged == 1 && len < sizeof(prompt)) {
            ...
            if (!PRF_FLAGGED(d->character, PRF_DISPERC)) {
                if (PRF_FLAGGED(d->character, PRF_DISPHP) && len < sizeof(prompt) &&
                    ...
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@RPL@n@Y: @m%s@D]@n",
                                     add_commas(GET_HIT(d->character)));
                    if (count >= 0)
                        len += count;
                } else if (PRF_FLAGGED(d->character, PRF_DISPHP) && len < sizeof(prompt) &&
                           GET_HIT(d->character) > GET_MAX_HIT(d->character)) {
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@RPL@n@Y: @G%s@D]@n",
                                     add_commas(GET_HIT(d->character)));
                    if (count >= 0)
                        len += count;
                } else if (PRF_FLAGGED(d->character, PRF_DISPHP) && len < sizeof(prompt) &&
                           GET_HIT(d->character) > (gear_pl(d->character)) / 2) {
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@RPL@n@Y: @c%s@D]@n",
                                     add_commas(GET_HIT(d->character)));
                    if (count >= 0)
                        len += count;
                } else if (PRF_FLAGGED(d->character, PRF_DISPHP) && len < sizeof(prompt) &&
                           GET_HIT(d->character) > (gear_pl(d->character)) / 10) {
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@RPL@n@Y: @y%s@D]@n",
                                     add_commas(GET_HIT(d->character)));
                    if (count >= 0)
                        len += count;
                } else if (PRF_FLAGGED(d->character, PRF_DISPHP) && len < sizeof(prompt) &&
                           GET_HIT(d->character) <= (gear_pl(d->character)) / 10) {
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@RPL@n@Y: @r%s@D]@n",
                                     add_commas(GET_HIT(d->character)));
                    if (count >= 0)
                        len += count;
                }
            ...
            if (!PRF_FLAGGED(d->character, PRF_DISPERC)) {
                if (PRF_FLAGGED(d->character, PRF_DISPKI) && len < sizeof(prompt) &&
                    ...
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@CKI@Y: @c%s@D]@n",
                                     add_commas(GET_MANA(d->character)));
                    if (count >= 0)
                        len += count;
                } else if (PRF_FLAGGED(d->character, PRF_DISPKI) && len < sizeof(prompt) &&
                           GET_MANA(d->character) > GET_MAX_MANA(d->character) / 10) {
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@CKI@Y: @y%s@D]@n",
                                     add_commas(GET_MANA(d->character)));
                    if (count >= 0)
                        len += count;
                } else if (PRF_FLAGGED(d->character, PRF_DISPKI) && len < sizeof(prompt) &&
                           GET_MANA(d->character) <= GET_MAX_MANA(d->character) / 10) {
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@CKI@Y: @r%s@D]@n",
                                     add_commas(GET_MANA(d->character)));
                    if (count >= 0)
                        len += count;
                }
            ...
            if (!PRF_FLAGGED(d->character, PRF_DISPERC)) {
                if (PRF_FLAGGED(d->character, PRF_DISPMOVE) && len < sizeof(prompt) &&
                    ...
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@GSTA@Y: @c%s@D]@n",
                                     add_commas(GET_MOVE(d->character)));
                    if (count >= 0)
                        len += count;
                } else if (PRF_FLAGGED(d->character, PRF_DISPMOVE) && len < sizeof(prompt) &&
                           GET_MOVE(d->character) > GET_MAX_MOVE(d->character) / 10) {
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@GSTA@Y: @y%s@D]@n",
                                     add_commas(GET_MOVE(d->character)));
                    if (count >= 0)
                        len += count;
                } else if (PRF_FLAGGED(d->character, PRF_DISPMOVE) && len < sizeof(prompt) &&
                           GET_MOVE(d->character) <= GET_MAX_MOVE(d->character) / 10) {
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@GSTA@Y: @r%s@D]@n",
                                     add_commas(GET_MOVE(d->character)));
                    if (count >= 0)
                        len += count;
                }
 ...
 */
bool perform_subst(struct descriptor_data *t, char *orig, char *subst) {
...
void close_socket(struct descriptor_data *d) {
    ...
    if (d->character) {
        ...
        if (IS_PLAYING(d) || STATE(d) == CON_DISCONNECT) {
            ...
            act("$n has lost $s link.", 1, link_challenged, 0, 0, TO_ROOM);
            save_char(link_challenged);
            mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(link_challenged)), 1, "Closing link to: %s.",
                   GET_NAME(link_challenged));
        } else {
            free_char(d->character);
        }
    } else
        mudlog(CMP, ADMLVL_IMMORT, 1, "Losing descriptor without char.");
...
void send_to_eaves(const char *messg, struct char_data *tch, ...) {
    ...
    for (d = descriptor_list; d; d = d->next) {
        ...
        if (GET_EAVESDROP(d->character) == GET_ROOM_VNUM(IN_ROOM(tch)) &&
        ...
        for (d = descriptor_list; d; d = d->next) {
            ...
            if (GET_EAVESDROP(d->character) > 0) {
                ...
                if (!resskill || (roll_skill(d->character, resskill) >= dcval)) {
                    if (ch != NULL && GET_EAVESDROP(d->character) == GET_ROOM_VNUM(IN_ROOM(ch)) &&
                        GET_SKILL(d->character, SKILL_EAVESDROP) > roll) {
                        char buf3[1000];
                        *buf3 = '\0';
                        sprintf(buf3, "-----Eavesdrop-----\r\n%s\r\n-----Eavesdrop-----\r\n", str);
                        perform_act(buf3, ch, obj, vict_obj, d->character);
                    } else if (obj != NULL && GET_EAVESDROP(d->character) == GET_ROOM_VNUM(IN_ROOM(obj)) &&
                               GET_SKILL(d->character, SKILL_EAVESDROP) > roll) {
                        char buf3[1000];
                        *buf3 = '\0';
                        sprintf(buf3, "-----Eavesdrop-----\r\n%s\r\n-----Eavesdrop-----\r\n", str);
...
bool open_logfile(const char *filename, FILE *stderr_fp) {
