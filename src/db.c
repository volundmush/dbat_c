...
int SHENRON = 0;            /* Shenron has been summoned     */
...
static bool parse_simple_mob(FILE *mob_f, struct char_data *ch, int nr);
...
static bool parse_enhanced_mob(FILE *mob_f, struct char_data *ch, int nr);
...
void auc_save() {
    ...
    if ((fl = fopen(AUCTION_FILE, "w")) == NULL)
        ...
        for (obj = world[real_room(80)].contents; obj; obj = obj->next_content) {
            if (obj) {
...
static void setup_dir(FILE *fl, int room, int dir) {
    ...
    if (((retval = sscanf(line, " %d %d %d %d %d %d %d %d %d %d %d", t, t + 1, t + 2, t + 3, t + 4, t + 5, t + 6, t + 7,
                          t + 8, t + 9, t + 10)) == 3) && (bitwarning == 1)) {
        log("SYSERR: Format error, %s", buf2);
        exit(1);
    } else if (bitwarning == 0) {
        ...
        world[room].dir_option[dir]->key = ((t[1] == NOTHING) ? NOTHING : t[1]);
        world[room].dir_option[dir]->to_room = ((t[2] == NOTHING) ? NOWHERE : t[2]);
...
bool parse_mobile_from_file(FILE *mob_f, struct char_data *ch) {
    int j, t[10], retval;
...
void log_dupe_objects(struct obj_data *obj1, struct obj_data *obj2) {
    mudlog(BRF, ADMLVL_GOD, 1, "DUPE: Dupe object found: %s [%d] [%" TMT ":%" I64T "]",
           obj1->short_description ? obj1->short_description : "<No name>",
           GET_OBJ_VNUM(obj1), obj1->generation, obj1->unique_id);
    mudlog(BRF, ADMLVL_GOD, 1, "DUPE: First: In room: %d (%s), "
    ...
    mudlog(BRF, ADMLVL_GOD, 1, "DUPE: Newer: In room: %d (%s), "
                                  "In object: %s, Carried by: %s, Worn by: %s",
...
void zone_update(void) {
    ...
    for (update_u = reset_q.head; update_u; update_u = update_u->next)
        if (zone_table[update_u->zone_to_reset].reset_mode == 2 ||
            ...
            mudlog(CMP, ADMLVL_GOD, 0, "Auto zone reset: %s (Zone %d)",
...
static void log_zone_error(zone_rnum zone, int cmd_no, const char *message) {
    mudlog(NRM, ADMLVL_GOD, 1, "SYSERR: zone file: %s", message);
    mudlog(NRM, ADMLVL_GOD, 1, "SYSERR: ...offending cmd: '%c' cmd in zone #%d, line %d",
...
void reset_zone(zone_rnum zone) {
    ...
    if (pre_reset(zone_table[zone].number) == 0) {
        for (cmd_no = 0; ZCMD2.command != 'S'; cmd_no++) {
...
static int check_object(struct obj_data *obj) {
    ...
    switch (GET_OBJ_TYPE(obj)) {
        case ITEM_DRINKCON: {
            ...
            if (search_block(onealias, drinknames, 1) < 0 && (error = 1)) {
                //log("SYSERR: Object #%d (%s) doesn't have drink type as last alias. (%s)", GET_OBJ_VNUM(obj), obj->short_description, obj->name);
            }
        }
            /* Fall through. */
        case ITEM_FOUNTAIN:
            if ((GET_OBJ_VAL(obj, 0) > 0) && (GET_OBJ_VAL(obj, 1) > GET_OBJ_VAL(obj, 0) && (error = 1)))
        ...
        case ITEM_STAFF:
            error |= check_object_level(obj, 0);
            error |= check_object_spell_number(obj, 3);
            if (GET_OBJ_VAL(obj, 2) > GET_OBJ_VAL(obj, 1) && (error = 1))
...
static int check_object_spell_number(struct obj_data *obj, int val) {
   ...
   */
    ...
    if ((spellname == unused_spellname || !strcasecmp("UNDEFINED", spellname)) && (error = 1))
...
static int check_object_level(struct obj_data *obj, int val) {
    ...
    if ((GET_OBJ_VAL(obj, val) < 0) && (error = 1))
...
bool my_obj_save_to_disk(FILE *fp, struct obj_data *obj, int locate) {
    int counter2, i;
