...
void send_char_pos(struct char_data *ch, int dam) {
    switch (GET_POS(ch)) {
        case POS_MORTALLYW:
            act("$n is mortally wounded, and will die soon, if not aided.", 1, ch, 0, 0, TO_ROOM);
            send_to_char(ch, "You are mortally wounded, and will die soon, if not aided.\r\n");
            break;
        case POS_INCAP:
            act("$n is incapacitated and will slowly die, if not aided.", 1, ch, 0, 0, TO_ROOM);
            send_to_char(ch, "You are incapacitated and will slowly die, if not aided.\r\n");
            break;
        case POS_STUNNED:
            act("$n is stunned, but will probably regain consciousness again.", 1, ch, 0, 0, TO_ROOM);
        ...
        default:                        /* >= POSITION SLEEPING */
            if (dam > (GET_MAX_HIT(ch) >> 2))
                act("That really did HURT!", 0, ch, 0, 0, TO_CHAR);
 ...
 */
bool valid_dg_target(struct char_data *ch, int bitvector) {
    if (IS_NPC(ch))
...
void script_damage(struct char_data *vict, int dam) {
    ...
    if (GET_POS(vict) == POS_DEAD) {
        if (!IS_NPC(vict))
            mudlog(BRF, 0, 1, "%s killed by script at %s",
