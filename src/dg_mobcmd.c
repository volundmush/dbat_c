...
ACMD(do_masound) {
    ...
    for (door = 0; door < NUM_OF_DIRS; door++) {
        ...
        if (((newexit = world[was_in_room].dir_option[door]) != NULL) &&
            ...
            sub_write(argument, ch, 1, TO_ROOM);
        }
    }
...
ACMD(do_mtransform) {
    char arg[MAX_INPUT_LENGTH];
    struct char_data *m, tmpmob;
    struct obj_data *obj[NUM_WEARS];
