...
ACMD(do_oasis_trigedit) {
    ...
    mudlog(CMP, ADMLVL_IMMORT, 1, "OLC: %s starts editing zone %d [trigger](allowed zone %d)",
...
void trigedit_save(struct descriptor_data *d) {
    ...
    if (!(trig_file = fopen(fname, "w"))) {
        mudlog(BRF, MAX(ADMLVL_GOD, GET_INVIS_LEV(d->character)), 1,
...
bool dg_script_edit_parse(struct descriptor_data *d, char *arg) {
...
int format_script(struct descriptor_data *d) {
    ...
    int indent = 0, indent_next = 0, found_case = 0, i, line_num = 0;
    ...
    *nsc = '\0';
    ...
    while (t) {
        ...
        if (!strncasecmp(t, "if ", 3) ||
            ...
            found_case = 1;  /* so you can 'break' a loop without complains */
            indent_next = 1;
