...
bool eval_lhs_op_rhs(char *expr, char *result, void *gohere, struct script_data *sc,
                    struct trig_data *trig, int type);

bool process_if(char *cond, void *gohere, struct script_data *sc,
               struct trig_data *trig, int type);
...
bool can_wear_on_pos(struct obj_data *obj, int pos) {
    switch (pos) {
        case WEAR_WIELD1:
 ...
 */
bool perform_set_dg_var(struct char_data *ch, struct char_data *vict, char *val_arg) {
    ...
    for (cl = cl; cl && GET_TRIG_DEPTH(trig); cl = cl->next) {
        ...
        if (*p == '*') /* comment */
            ...
            if (!strncasecmp(cmd, "eval ", 5))
                ...
                mudlog(NRM, ADMLVL_GOD, 1, "%s", DG_SCRIPT_VERSION);
