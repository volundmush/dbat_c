 ...
 */
bool word_check(char *str, char *wordlist) {
...
bool greet_mtrigger(struct char_data *actor, int dir) {
    struct trig_data *t;
    struct char_data *ch;
    char buf[MAX_INPUT_LENGTH];
    int intermediate, final = 1;
...
bool entry_mtrigger(struct char_data *ch) {
    struct trig_data *t;
...
bool command_mtrigger(struct char_data *actor, char *cmd, char *argument) {
        ...
        if (SCRIPT_CHECK(ch, MTRIG_COMMAND) && !AFF_FLAGGED(ch, AFF_CHARM) &&
            ...
            for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
                ...
                if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
                    mudlog(NRM, ADMLVL_BUILDER, 1, "SYSERR: Command Trigger #%d has no text argument!",
                           GET_TRIG_VNUM(t));
                    continue;
                }
    ...
    for (ch = world[IN_ROOM(actor)].people; ch; ch = ch_next) {
        ...
        if (SCRIPT_CHECK(ch, MTRIG_SPEECH) && AWAKE(ch) &&
            ...
            for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
                ...
                if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
                    mudlog(NRM, ADMLVL_BUILDER, 1, "SYSERR: Speech Trigger #%d has no text argument!",
                           GET_TRIG_VNUM(t));
                    continue;
                }
...
void act_mtrigger(const struct char_data *ch, char *str, struct char_data *actor, const struct char_data *victim,
                  struct obj_data *object, const struct obj_data *target, const char *arg) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
    ...
    if (SCRIPT_CHECK(ch, MTRIG_ACT) && !AFF_FLAGGED(ch, AFF_CHARM) &&
        ...
        for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
            ...
            if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
                mudlog(NRM, ADMLVL_BUILDER, 1, "SYSERR: Act Trigger #%d has no text argument!",
                       GET_TRIG_VNUM(t));
                continue;
            }
...
bool receive_mtrigger(struct char_data *ch, struct char_data *actor, struct obj_data *obj) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
    int ret_val;
...
bool death_mtrigger(struct char_data *ch, struct char_data *actor) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
...
bool cast_mtrigger(struct char_data *actor, struct char_data *ch, int spellnum) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
...
bool leave_mtrigger(struct char_data *actor, int dir) {
    struct trig_data *t;
    struct char_data *ch;
    char buf[MAX_INPUT_LENGTH];
...
bool door_mtrigger(struct char_data *actor, int subcmd, int dir) {
    struct trig_data *t;
    struct char_data *ch;
    char buf[MAX_INPUT_LENGTH];
...
bool get_otrigger(struct obj_data *obj, struct char_data *actor) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
    int ret_val;
...
bool cmd_otrig(struct obj_data *obj, struct char_data *actor, char *cmd,
    ...
    if (obj && SCRIPT_CHECK(obj, OTRIG_COMMAND))
        for (t = TRIGGERS(SCRIPT(obj)); t; t = t->next) {
            ...
            if (IS_SET(GET_TRIG_NARG(t), type) &&
                ...
                mudlog(NRM, ADMLVL_BUILDER, 1, "SYSERR: O-Command Trigger #%d has no text argument!",
                       GET_TRIG_VNUM(t));
                continue;
            }
...
bool command_otrigger(struct char_data *actor, char *cmd, char *argument) {
    struct obj_data *obj;
    int i;
...
bool wear_otrigger(struct obj_data *obj, struct char_data *actor, int where) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
    int ret_val;
...
bool remove_otrigger(struct obj_data *obj, struct char_data *actor) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
    int ret_val;
...
bool drop_otrigger(struct obj_data *obj, struct char_data *actor) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
    int ret_val;
...
bool give_otrigger(struct obj_data *obj, struct char_data *actor, struct char_data *victim) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
    int ret_val;
...
bool cast_otrigger(struct char_data *actor, struct obj_data *obj, int spellnum) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
...
bool leave_otrigger(struct room_data *room, struct char_data *actor, int dir) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
    int temp, final = 1;
...
bool consume_otrigger(struct obj_data *obj, struct char_data *actor, int cmd) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
    int ret_val;
...
bool enter_wtrigger(struct room_data *room, struct char_data *actor, int dir) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
...
bool command_wtrigger(struct char_data *actor, char *cmd, char *argument) {
    ...
    for (t = TRIGGERS(SCRIPT(room)); t; t = t->next) {
        ...
        if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
            mudlog(NRM, ADMLVL_BUILDER, 1, "SYSERR: W-Command Trigger #%d has no text argument!",
                   GET_TRIG_VNUM(t));
            continue;
        }
...
void speech_wtrigger(struct char_data *actor, char *str) {
    ...
    for (t = TRIGGERS(SCRIPT(room)); t; t = t->next) {
        ...
        if (!GET_TRIG_ARG(t) || !*GET_TRIG_ARG(t)) {
            mudlog(NRM, ADMLVL_BUILDER, 1, "SYSERR: W-Speech Trigger #%d has no text argument!",
                   GET_TRIG_VNUM(t));
            continue;
        }
...
bool drop_wtrigger(struct obj_data *obj, struct char_data *actor) {
    struct room_data *room;
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
    int ret_val;
...
bool cast_wtrigger(struct char_data *actor, struct char_data *vict, struct obj_data *obj, int spellnum) {
    struct room_data *room;
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
...
bool leave_wtrigger(struct room_data *room, struct char_data *actor, int dir) {
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
...
bool door_wtrigger(struct char_data *actor, int subcmd, int dir) {
    struct room_data *room;
    struct trig_data *t;
    char buf[MAX_INPUT_LENGTH];
