...
bool char_has_item(char *item, struct char_data *ch) {

    /* If this works, no more searching needed */
    if (get_object_in_equip(ch, item) != NULL)
        return true;

    return !(item_in_list(item, ch->carrying) == 0);
}

bool text_processed(char *field, char *subfield, struct trig_var_data *vd,
...
void find_replacement(void *gohere, struct script_data *sc, struct trig_data *trig, int type, char *var, char *field,
...
/* see note in dg_scripts.h */
                ...
                case 's':
                    if (!strcasecmp(field, "sex"))
                        ...
                        if (subfield && *subfield) {
                            int ns;
                            if ((ns = search_block(subfield, size_names, 0)) > -1) {
                                (c)->size = ns;
                            }
                        }
