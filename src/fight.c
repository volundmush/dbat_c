...
static bool pick_n_throw(struct char_data *ch, char *buf);


bool group_bonus(struct char_data *ch, int type) {
...
void mutant_limb_regen(struct char_data *ch) {
    if (GET_LIMBCOND(ch, 1) > 0 && GET_LIMBCOND(ch, 1) < 50) {
        act("The bones in your right arm have mended them selves.", 1, ch, 0, 0, TO_CHAR);
        act("$n starts moving $s right arm gingerly for a moment.", 1, ch, 0, 0, TO_ROOM);
        GET_LIMBCOND(ch, 1) = 100;
    } else if (GET_LIMBCOND(ch, 1) <= 0) {
        act("Your right arm begins to grow back very quickly. Within moments it is whole again!", 1, ch, 0, 0,
            TO_CHAR);
        act("$n's right arm starts to regrow! Within moments the arm is whole again!.", 1, ch, 0, 0, TO_ROOM);
        GET_LIMBCOND(ch, 1) = 100;
    }
    if (GET_LIMBCOND(ch, 2) > 0 && GET_LIMBCOND(ch, 2) < 50) {
        act("The bones in your left arm have mended them selves.", 1, ch, 0, 0, TO_CHAR);
        act("$n starts moving $s left arm gingerly for a moment.", 1, ch, 0, 0, TO_ROOM);
    ...
    if (GET_LIMBCOND(ch, 3) > 0 && GET_LIMBCOND(ch, 3) < 50) {
        act("The bones in your right leg have mended them selves.", 1, ch, 0, 0, TO_CHAR);
        act("$n starts moving $s right leg gingerly for a moment.", 1, ch, 0, 0, TO_ROOM);
    ...
    if (GET_LIMBCOND(ch, 4) > 0 && GET_LIMBCOND(ch, 4) < 50) {
        act("The bones in your left leg have mended them selves.", 1, ch, 0, 0, TO_CHAR);
        act("$n starts moving $s left leg gingerly for a moment.", 1, ch, 0, 0, TO_ROOM);
...
static void mob_attack(struct char_data *ch, char *buf) {
    ...
    if (axion_dice(-10) > 90 && GET_HIT(ch) <= GET_MAX_HIT(ch) / 2 && !MOB_FLAGGED(ch, MOB_POWERUP) &&
        GET_MOB_VNUM(ch) != 25) {
        do_powerup(ch, NULL, 0, 0);
        return;
    }

    if (GET_MANA(ch) >= GET_MAX_MANA(ch) * 0.05 && IS_HUMANOID(ch) && (!IS_DRAGON(ch) || dragonpass == 1)) {
        if (ch->mobcharge <= 0 && rand_number(1, 10) >= 8) {
            act("@wAn aura flares up around @R$n@w!@n", 1, ch, 0, 0, TO_ROOM);
            ch->mobcharge += 1;
            if (GET_LEVEL(ch) > 80) {
                ch->mobcharge += 1;
            }
        } else if (ch->mobcharge <= 5) {
            act("@wThe aura burns brighter around @R$n@w!@n", 1, ch, 0, 0, TO_ROOM);
            ch->mobcharge += 1;
            if (GET_LEVEL(ch) > 80) {
                ch->mobcharge += 1;
            }
        } else if (ch->mobcharge == 6) {
            act("@wThe aura around @R$n@w flashes!@n", 1, ch, 0, 0, TO_ROOM);
            ch->mobcharge += 1;
            special = 100;
        }
    }

    if (IS_HUMANOID(ch) && dragonpass == 1) { /* Is a humanoid mob */
        if (AFF_FLAGGED(ch, AFF_PARALYZE)) {
            ...
            } else if (IS_ANDROID(ch) && MOB_FLAGGED(ch, MOB_REPAIR) && GET_HIT(ch) <= gear_pl(ch) * 0.5 &&
                       rand_number(1, 20) >= 16) {
    ...
    } else if (!IS_HUMANOID(ch) || dragonpass == 0) { /* Is not a humanoid mob */
...
void fight_stack() {
    ...
    for (tch = character_list; tch; tch = tch->next) {
        ...
        if (PLR_FLAGGED(ch, PLR_SPIRAL)) {
            handle_spiral(ch, NULL, GET_SKILL(ch, SKILL_SPIRAL), 0);
        }
        ...
        if (IS_NPC(ch) && MOB_FLAGGED(ch, MOB_POWERUP) && axion_dice(0) >= 90) {
            if (GET_HIT(ch) >= GET_MAX_HIT(ch)) {
                ...
                act("@g$n@G continues powering up as torrents of energy crackle within $s aura.@n", 1, ch, 0, 0,
                    TO_ROOM);
                GET_HIT(ch) += GET_MAX_HIT(ch) / 10;
            } else if (GET_HIT(ch) > GET_MAX_HIT(ch) / 4) {
                act("@g$n@G powers up as a steady aura around $s body grow brighter.@n", 1, ch, 0, 0, TO_ROOM);
                GET_HIT(ch) += GET_MAX_HIT(ch) / 8;
            } else if (GET_HIT(ch) > 0) {
                act("@g$n@G powers up, as a weak aura flickers around $s body.@n", 1, ch, 0, 0, TO_ROOM);
        ...
        if (FIGHTING(ch) && DRAGGING(ch)) {
            act("@WYou are forced to stop dragging @C$N@W!@n", 1, ch, 0, DRAGGING(ch), TO_CHAR);
            act("@C$n@W is forced to stop dragging @c$N@W!@n", 1, ch, 0, DRAGGING(ch), TO_ROOM);
        ...
        if (GET_HIT(ch) <= (gear_pl(ch) * 0.01) * GET_LIFEPERC(ch) && GET_LIFEFORCE(ch) > 0 && !IS_ANDROID(ch)) {
            if (rand_number(1, 15) >= 14) {
                if (GET_LIFEFORCE(ch) >= GET_LIFEMAX(ch) * 0.05 || AFF_FLAGGED(ch, AFF_HEALGLOW) ||
                    (IS_KANASSAN(ch) && GET_LIFEFORCE(ch) >= GET_LIFEMAX(ch) * 0.03)) {
        ...
        if (!AFF_FLAGGED(ch, AFF_POSITION)) {
            if (roll_balance(ch) > axion_dice(0) && rand_number(1, 10) >= 7) {
                if (FIGHTING(ch)) {
                    if (!AFF_FLAGGED(FIGHTING(ch), AFF_POSITION)) {
                        act("@YYou manage to move into an advantageous position!@n", 1, ch, 0, 0, TO_CHAR);
                        act("@y$n@Y manages to move into an advantageous position!@n", 1, ch, 0, 0, TO_ROOM);
                        SET_BIT_AR(AFF_FLAGS(ch), AFF_POSITION);
                    } else {
                        struct char_data *vict = FIGHTING(ch);
                        if (roll_balance(ch) > roll_balance(vict)) {
                            act("@YYou struggle to gain a better position than @y$N@Y and succeed!@n", 1, ch, 0,
                                vict, TO_CHAR);
                            act("@y$n@Y struggles to gain a better position than you and succeeds!@n", 1, ch, 0,
                                vict, TO_VICT);
                            act("@y$n@Y struggles to gain a better position than @y$N@Y and succeeds!@n", 1, ch, 0,
                                vict, TO_NOTVICT);
            ...
            if (roll_balance(ch) < axion_dice(-30) || GET_POS(ch) < POS_STANDING) {
                act("@YYou are moved out of your position!@n", 1, ch, 0, 0, TO_CHAR);
                act("@y$n@Y is moved out of $s position!@n", 1, ch, 0, 0, TO_ROOM);
        ...
        if (GRAPPLING(ch) && GRAPTYPE(ch) == 2 && rand_number(1, 11) >= 8) {
            if (GET_MOVE(GRAPPLING(ch)) >= GET_MAX_MOVE(GRAPPLING(ch)) / 8) {
                act("@WYou choke @C$N@W!@n", 1, ch, 0, GRAPPLING(ch), TO_CHAR);
                act("@C$n@W chokes YOU@W!@n", 1, ch, 0, GRAPPLING(ch), TO_VICT);
                act("@C$n@W chokes @c$N@W!@n", 1, ch, 0, GRAPPLING(ch), TO_NOTVICT);
                GET_MOVE(GRAPPLING(ch)) -= GET_MAX_MOVE(GRAPPLING(ch)) / 8;
            } else {
                act("@WYou choke @C$N@W, and $E passes out!@n", 1, ch, 0, GRAPPLING(ch), TO_CHAR);
                act("@C$n@W chokes YOU@W, and you pass out!@n", 1, ch, 0, GRAPPLING(ch), TO_VICT);
                act("@C$n@W chokes @c$N@W, and $E passes out!@n", 1, ch, 0, GRAPPLING(ch), TO_NOTVICT);
            ...
            act("@WYou crush @C$N@W some more!@n", 1, ch, 0, GRAPPLING(ch), TO_CHAR);
            act("@C$n@W crushes YOU@W some more!@n", 1, ch, 0, GRAPPLING(ch), TO_VICT);
            act("@C$n@W crushes @c$N@W some more!@n", 1, ch, 0, GRAPPLING(ch), TO_NOTVICT);
        ...
        if (!IS_NPC(ch) && IS_TRANSFORMED(ch)) {
            if (IS_NONPTRANS(ch) && !IS_ICER(ch) && GET_MOVE(ch) < GET_MAX_MOVE(ch) / 60) {
                act("@mExhausted of stamina, your body forcibly reverts from its form.@n", 1, ch, 0, 0, TO_CHAR);
                act("@C$n @wbreathing heavily, reverts from $s form, returning to normal.@n", 1, ch, 0, 0, TO_ROOM);
                if (GET_KAIOKEN(ch) < 1) {
            ...
            } else if (IS_NONPTRANS(ch) && !IS_ICER(ch) && GET_MOVE(ch) >= GET_MAX_MOVE(ch) / 900 &&
                       PLR_FLAGGED(ch, PLR_TRANS1) && !IS_KONATSU(ch) && !IS_KAI(ch) && !IS_NAMEK(ch)) {
            ...
            } else if (IS_NONPTRANS(ch) && !IS_ICER(ch) && GET_MOVE(ch) >= GET_MAX_MOVE(ch) / 800 &&
                       PLR_FLAGGED(ch, PLR_TRANS1)) {
                if (IS_SAIYAN(ch) && GET_LIFEFORCE(ch) >= GET_LIFEMAX(ch) * 0.7) {
                    GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 900;
                } else
                    GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 800;
            } else if (IS_NONPTRANS(ch) && !IS_ICER(ch) && GET_MOVE(ch) >= GET_MAX_MOVE(ch) / 600 &&
                       PLR_FLAGGED(ch, PLR_TRANS2) && !IS_KONATSU(ch) && !IS_KAI(ch) && !IS_NAMEK(ch)) {
                if (IS_SAIYAN(ch) && GET_LIFEFORCE(ch) >= GET_LIFEMAX(ch) * 0.7) {
                    GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 700;
                } else
                    GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 600;
            } else if (IS_NONPTRANS(ch) && !IS_ICER(ch) && GET_MOVE(ch) >= GET_MAX_MOVE(ch) / 500 &&
                       PLR_FLAGGED(ch, PLR_TRANS2)) {
                GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 500;
            } else if (IS_NONPTRANS(ch) && !IS_ICER(ch) && GET_MOVE(ch) >= GET_MAX_MOVE(ch) / 400 &&
                       PLR_FLAGGED(ch, PLR_TRANS3) && !IS_SAIYAN(ch)) {
                GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 400;
            } else if (IS_NONPTRANS(ch) && !IS_ICER(ch) && GET_MOVE(ch) >= GET_MAX_MOVE(ch) / 250 &&
                       PLR_FLAGGED(ch, PLR_TRANS3)) {
                if (IS_SAIYAN(ch) && GET_LIFEFORCE(ch) >= GET_LIFEMAX(ch) * 0.7) {
                    GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 300;
                } else
                    GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 250;
            } else if (IS_NONPTRANS(ch) && !IS_ICER(ch) && GET_MOVE(ch) >= GET_MAX_MOVE(ch) / 200 &&
                       PLR_FLAGGED(ch, PLR_TRANS4) && !IS_SAIYAN(ch)) {
                GET_MOVE(ch) -= GET_MAX_MOVE(ch) / 200;
            } else if (IS_NONPTRANS(ch) && !IS_ICER(ch) && GET_MOVE(ch) >= GET_MAX_MOVE(ch) / 170 &&
        ...
        if (!IS_NPC(ch) && PLR_FLAGGED(ch, PLR_DISGUISED) && GET_SKILL(ch, SKILL_DISGUISE) < rand_number(1, 125)) {
            ...
            act("@W$n's@W disguise comes off because of $s swift movements!@n", 0, ch, 0, 0, TO_ROOM);
        }
        if (IS_NPC(ch) && AFF_FLAGGED(ch, AFF_BLIND) && rand_number(1, 200) >= 190) {
            act("@W$n@W is no longer blind.@n", 0, ch, 0, 0, TO_ROOM);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_BLIND);
        }
        if (AFF_FLAGGED(ch, AFF_KNOCKED) && rand_number(1, 200) >= 195) {
            act("@W$n@W is no longer senseless, and wakes up.@n", 0, ch, 0, 0, TO_ROOM);
            ...
            if (IS_NPC(ch) && rand_number(1, 20) >= 12) {
                act("@W$n@W stands up.@n", 0, ch, 0, 0, TO_ROOM);
                GET_POS(ch) = POS_STANDING;
            }
        }
        ...
        if (FIGHTING(ch) && IS_NPC(ch) && !MOB_FLAGGED(ch, MOB_DUMMY)) {
            ...
            if (AFF_FLAGGED(FIGHTING(ch), AFF_FLYING) && !IS_HUMANOID(ch) && !AFF_FLAGGED(ch, AFF_FLYING) &&
                ...
                if (rand_number(1, 30) >= 22 && !block_calc(ch)) {
                    act("$n@G flees in terror and you lose sight of $m!", 1, ch, 0, 0, TO_ROOM);
                    while (ch->carrying)
            ...
            if (AFF_FLAGGED(FIGHTING(ch), AFF_FLYING) && IS_HUMANOID(ch) && GET_LEVEL(ch) <= 10) {
                if (rand_number(1, 30) >= 22 && !block_calc(ch)) {
                    act("$n@G turns and runs away. You lose sight of $m!", 1, ch, 0, 0, TO_ROOM);
                    while (ch->carrying)
                        extract_obj(ch->carrying);
                    extract_char(ch);
                    continue;
            ...
            if (IS_AFFECTED(ch, AFF_PARA) && IS_NPC(ch) && GET_INT(ch) + 10 < rand_number(1, 60)) {
                act("@yYou fail to overcome your paralysis!@n", 1, ch, 0, 0, TO_CHAR);
                act("@Y$n @ystruggles with $s paralysis!@n", 1, ch, 0, 0, TO_ROOM);
                continue;
            }
        ...
        if (PLR_FLAGGED(ch, PLR_POWERUP) && rand_number(1, 3) == 3) {
            ...
            if (GET_HIT(ch) >= gear_pl(ch) && GET_MANA(ch) >= GET_MAX_MANA(ch) / 20 &&
                ...
                dispel_ash(ch);
                act("@RYou have reached your maximum!@n", 1, ch, 0, 0, TO_CHAR);
                act("@R$n stops powering up in a flash of light!@n", 1, ch, 0, 0, TO_ROOM);
                ...
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_POWERUP);
            } else if (GET_HIT(ch) >= gear_pl(ch) && GET_MANA(ch) >= (GET_MAX_MANA(ch) * 0.0375) + 1 &&
                       GET_PREFERENCE(ch) == PREFERENCE_KI) {
            ...
            if (GET_MANA(ch) < GET_MAX_MANA(ch) / 20 && GET_PREFERENCE(ch) != PREFERENCE_KI) {
                GET_MANA(ch) = 0;
                act("@RYou have run out of ki.@n", 1, ch, 0, 0, TO_CHAR);
                act("@R$n stops powering up in a flash of light!@n", 1, ch, 0, 0, TO_ROOM);
                send_to_sense(0, "You sense someone stop powering up", ch);
                sprintf(buf3, "@D[@GBlip@D]@r Rising Powerlevel Final@D: [@Y%s@D]", add_commas(GET_HIT(ch)));
                send_to_scouter(buf3, ch, 1, 0);
            ...
            if (GET_HIT(ch) < gear_pl(ch) &&
                ((GET_PREFERENCE(ch) != PREFERENCE_KI && GET_MANA(ch) >= GET_MAX_MANA(ch) / 20) ||
                 (GET_PREFERENCE(ch) == PREFERENCE_KI && GET_MANA(ch) >= (GET_MAX_MANA(ch) * 0.0375) + 1))) {
                ...
                if (GET_MAX_HIT(ch) < 50000) {
                    act("@RYou continue to powerup, as wind billows out from around you!@n", 1, ch, 0, 0, TO_CHAR);
                    act("@R$n continues to powerup, as wind billows out from around $m!@n", 1, ch, 0, 0, TO_ROOM);
                } else if (GET_MAX_HIT(ch) < 500000) {
                    act("@RYou continue to powerup, as the ground splits beneath you!@n", 1, ch, 0, 0, TO_CHAR);
                    act("@R$n continues to powerup, as the ground splits beneath $m!@n", 1, ch, 0, 0, TO_ROOM);
                } else if (GET_MAX_HIT(ch) < 5000000) {
                    act("@RYou continue to powerup, as the ground shudders and splits beneath you!@n", 1, ch, 0, 0,
                        TO_CHAR);
                    act("@R$n continues to powerup, as the ground shudders and splits beneath $m!@n", 1, ch, 0, 0,
                        TO_ROOM);
                } else if (GET_MAX_HIT(ch) < 50000000) {
                    act("@RYou continue to powerup, as a huge depression forms beneath you!@n", 1, ch, 0, 0,
                        TO_CHAR);
                    act("@R$n continues to powerup, as a huge depression forms beneath $m!@n", 1, ch, 0, 0, TO_ROOM);
                } else if (GET_MAX_HIT(ch) < 100000000) {
                    act("@RYou continue to powerup, as the entire area quakes around you!@n", 1, ch, 0, 0, TO_CHAR);
                    act("@R$n continues to powerup, as the entire area quakes around $m!@n", 1, ch, 0, 0, TO_ROOM);
                } else if (GET_MAX_HIT(ch) < 300000000) {
                    act("@RYou continue to powerup, as huge chunks of ground are ripped apart beneath you!@n", 1, ch,
                        0, 0, TO_CHAR);
                    act("@R$n continues to powerup, as huge chunks of ground are ripped apart beanth $m!@n", 1, ch,
                        0, 0, TO_ROOM);
                } else {
                    act("@RYou continue to powerup, as the very air around you crackles and burns!@n", 1, ch, 0, 0,
                        TO_CHAR);
                    act("@R$n continues to powerup, as the very air around $m crackles and burns!@n", 1, ch, 0, 0,
                        TO_ROOM);
                }
        ...
        if (!PLR_FLAGGED(ch, PLR_CHARGE) && rand_number(1, 40) >= 38 && !FIGHTING(ch) &&
            ...
            if (GET_CHARGE(ch) >= GET_MAX_MANA(ch) / 100) {
                ...
                switch (rand_number(1, 3)) {
                    case 1:
                        act("$n@w's aura flickers weakly.@n", 1, ch, 0, 0, TO_ROOM);
                        break;
                    case 2:
                        act("$n@w's aura sheds energy.@n", 1, ch, 0, 0, TO_ROOM);
                        break;
                    case 3:
                        act("$n@w's aura flickers brightly before growing dimmer.@n", 1, ch, 0, 0, TO_ROOM);
                        break;
                    default:
                        act("$n@w's aura shrinks some.@n", 1, ch, 0, 0, TO_ROOM);
                        break;
                }
                ...
                act("$n@w's aura fades away dimmly.@n", 1, ch, 0, 0, TO_ROOM);
                GET_CHARGE(ch) = 0;
            }
        }
        ...
        if (PLR_FLAGGED(ch, PLR_CHARGE)) {
            ...
            if (GET_MANA(ch) <= 0) {
                ...
                act("$n@w's aura grows calm.@n", 1, ch, 0, 0, TO_ROOM);
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_CHARGE);
            } else if (((GET_MAX_MANA(ch) * 0.01) * perc) >= GET_MANA(ch)) {
                send_to_char(ch, "You have charged the last that you can.\r\n");
                act("$n@w's aura @Yflashes@w spectacularly, rushing upwards in torrents!@n", 1, ch, 0, 0, TO_ROOM);
                ...
                if (GET_CHARGE(ch) >= GET_CHARGETO(ch)) {
                    ...
                    act("$n@w's aura burns steadily.@n", 1, ch, 0, 0, TO_ROOM);
                    GET_CHARGETO(ch) = 0;
                    REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_CHARGE);
                    ...
                    act("$n@w's aura flares up brightly and then burns steadily.@n", 1, ch, 0, 0, TO_ROOM);
                    GET_CHARGETO(ch) = 0;
                    REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_CHARGE);
                } else {
                    ...
                    switch (rand_number(1, 3)) {
                        case 1:
                            act("$n@w's aura ripples magnificantly while growing brighter!@n", 1, ch, 0, 0, TO_ROOM);
                            send_to_char(ch, "Your aura grows bright as you charge more ki.\r\n");
                            break;
                        case 2:
                            act("$n@w's aura ripples with power as it grows larger!@n", 1, ch, 0, 0, TO_ROOM);
                            send_to_char(ch, "Your aura ripples with power as you charge more ki.\r\n");
                            break;
                        case 3:
                            act("$n@w's aura throws sparks off violently!.@n", 1, ch, 0, 0, TO_ROOM);
                    ...
                    if (GET_CHARGE(ch) >= GET_CHARGETO(ch)) {
                        ...
                        act("$n@w's aura burns brightly and then evens out.@n", 1, ch, 0, 0, TO_ROOM);
                        REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_CHARGE);
                        GET_CHARGETO(ch) = 0;
                    }
                }
...
void appear(struct char_data *ch) {
    ...
    act("$n slowly fades into existence.", 0, ch, 0, 0, TO_ROOM);
}
...
static void make_pcorpse(struct char_data *ch) {
    ...
    if (AFF_FLAGGED(ch, AFF_ASHED)) {
        act("@WSome ashes fall off the corpse.@n", 1, ch, 0, 0, TO_ROOM);
...
void raw_kill(struct char_data *ch, struct char_data *killer) {
    ...
    if (IS_NPC(ch) && !MOB_FLAGGED(ch, MOB_DUMMY)) {
        ...
        if (GET_LEVEL(ch) >= 9 && !IS_NPC(ch) && !ROOM_FLAGGED(IN_ROOM(ch), ROOM_PAST) &&
            (GET_ROOM_VNUM(IN_ROOM(ch)) < 17900 || GET_ROOM_VNUM(IN_ROOM(ch)) > 17999)) {
            /*af.type = -1;
            af.duration = -1;
            af.modifier = 0;
        ...
        if (IS_ANDROID(ch) && !PLR_FLAGGED(ch, PLR_ABSORB) && !AFF_FLAGGED(ch, AFF_SPIRIT) &&
            !ROOM_FLAGGED(IN_ROOM(ch), ROOM_PAST) &&
            (GET_ROOM_VNUM(IN_ROOM(ch)) < 17900 || GET_ROOM_VNUM(IN_ROOM(ch)) > 17999) && GET_UP(ch) > 5) {
...
void die(struct char_data *ch, struct char_data *killer) {
    if (!IS_NPC(ch)) {
        ...
        if ((IS_MAJIN(ch) || IS_BIO(ch)) && (GET_LIFEFORCE(ch) >= GET_LIFEMAX(ch) * 0.75 ||
                                             (PLR_FLAGGED(ch, PLR_SELFD2) &&
                                              GET_LIFEFORCE(ch) >= GET_LIFEMAX(ch) * 0.5))) {
            GET_LIFEFORCE(ch) = -1;
            GET_HIT(ch) = 1;
        ...
        if (PLR_FLAGGED(ch, PLR_IMMORTAL)) {
            act("@c$n@w disappears right before dying. $n appears to be immortal.@n", 1, ch, 0, 0, TO_CHAR);
            act("@c$n@w disappears right before dying. $n appears to be immortal.@n.", 1, ch, 0, 0, TO_ROOM);
            GET_HIT(ch) = 1;
            GET_MANA(ch) = 1;
            GET_MOVE(ch) = 1;
...
static void perform_group_gain(struct char_data *ch, int base, struct char_data *victim) {
    ...
    if (LASTHIT(victim) != 0 && LASTHIT(victim) != GET_IDNUM(ch)) {
        ...
        if (checkit == 0 && ch->master != NULL && GET_IDNUM(ch->master) == LASTHIT(victim)) {
            checkit = 1;
        }
        if (checkit == 0 && ch->master != NULL) {
...
void group_gain(struct char_data *ch, struct char_data *victim) {
    ...
    if (AFF_FLAGGED(k, AFF_GROUP) && IN_ROOM(k) == IN_ROOM(ch)) {
        if (!IS_WEIGHTED(k)) {
            ...
            if (k == ch) {
                send_to_char(ch,
                             "You can not group gain while your powerlevel is weighted down more than half of your max.\r\n");
            } else {
                send_to_char(ch,
                             "You can not group gain while your powerlevel is weighted down more than half of the leader's adjusted powerlevel.\r\n");
            }
        }
    }
