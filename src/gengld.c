...
int add_guild(struct guild_data *ngld) {
    ...
    if ((rguild = real_guild(G_NUM(ngld))) != NOWHERE) {
        ...
        if (rznum != NOWHERE) {
            ...
            mudlog(BRF, ADMLVL_BUILDER, 1, "SYSERR: GenOLC: Cannot determine guild zone.");
        return rguild;
    }

    mudlog(BRF, ADMLVL_BUILDER, 1, "SYSERR: GenOLC: Creating new guild.");
...
bool save_guilds(zone_rnum zone_num) {
    int i, j, rguild;
    FILE *guild_file;
    char fname[64];
    ...
    if (!(guild_file = fopen(fname, "w"))) {
        mudlog(BRF, ADMLVL_GOD, 1, "SYSERR: OLC: Cannot open Guild file!");
        return false;
    }
