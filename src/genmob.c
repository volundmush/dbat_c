...
void update_mobile_strings(struct char_data *t, struct char_data *f);

void check_mobile_strings(struct char_data *mob);

void check_mobile_string(mob_vnum i, char **string, const char *dscr);

bool write_mobile_espec(mob_vnum mvnum, struct char_data *mob, FILE *fd);

void copy_mobile_strings(struct char_data *t, struct char_data *f);
...
void copy_mobile(struct char_data *to, struct char_data *from) {
...
void free_mobile_strings(struct char_data *mob) {
    if (mob->name)
        free(mob->name);
    if (mob->title)
        free(mob->title);
 ...
 * and their mob_proto!  */
bool free_mobile(struct char_data *mob) {
    mob_rnum i;
...
int save_mobiles(zone_rnum zone_num) {
    ...
    if ((mobfd = fopen(mobfname, "w")) == NULL) {
        mudlog(BRF, ADMLVL_GOD, 1, "SYSERR: GenOLC: Cannot open mob file for writing.");
        return false;
    }
...
bool write_mobile_record(mob_vnum mvnum, struct char_data *mob, FILE *fd) {
