...
static bool copy_object_main(struct obj_data *to, struct obj_data *from, int free_object);
...
bool save_objects(zone_rnum zone_num) {
    ...
    if (!(fp = fopen(cmfname, "w+"))) {
        mudlog(BRF, ADMLVL_IMMORT, 1, "SYSERR: OLC: Cannot open objects file %s!", cmfname);
        return false;
    }
    /*
            ...
            if (obj->ex_description) {    /* Yes, save them too. */
                for (ex_desc = obj->ex_description; ex_desc; ex_desc = ex_desc->next) {
                    ...
                    if (!ex_desc->keyword || !ex_desc->description || !*ex_desc->keyword || !*ex_desc->description) {
                        mudlog(BRF, ADMLVL_IMMORT, 1, "SYSERR: OLC: oedit_save_to_disk: Corrupt ex_desc!");
                        continue;
                    }
...
int copy_object(struct obj_data *to, struct obj_data *from) {
    ...
    return copy_object_main(to, from, 1);
}

int copy_object_preserve(struct obj_data *to, struct obj_data *from) {
    return copy_object_main(to, from, 0);
}
