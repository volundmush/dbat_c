...
int add_shop(struct shop_data *nshp) {
    ...
    if ((rshop = real_shop(S_NUM(nshp))) != NOWHERE) {
        ...
        if (rznum != NOWHERE)
            ...
            mudlog(BRF, ADMLVL_BUILDER, 1, "SYSERR: GenOLC: Cannot determine shop zone.");
        return rshop;
    }
...
bool save_shops(zone_rnum zone_num) {
    int i, j, rshop;
    FILE *shop_file;
    ...
    if (!(shop_file = fopen(fname, "w"))) {
        mudlog(BRF, ADMLVL_GOD, 1, "SYSERR: OLC: Cannot open shop file!");
        return false;
    } else if (fprintf(shop_file, "CircleMUD v3.0 Shop File~\n") < 0) {
        mudlog(BRF, ADMLVL_GOD, 1, "SYSERR: OLC: Cannot write to shop file!");
        fclose(shop_file);
        return false;
    }
    /*
