...
bool delete_room(room_rnum rnum) {
    room_rnum i;
    int j;
        ...
        for (j = 0; ZCMD(i, j).command != 'S'; j++)
            switch (ZCMD(i, j).command) {
                ...
                case '*':
                    ...
                    break;
                default:
                    mudlog(BRF, ADMLVL_GOD, 1, "SYSERR: GenOLC: delete_room: Unknown zone entry found!");
            }
...
bool save_rooms(zone_rnum zone_num) {
    int i;
    struct room_data *room;
    FILE *sf;
    char filename[128];
...
bool copy_room(struct room_data *to, struct room_data *from) {
 ...
 */
bool copy_room_strings(struct room_data *dest, struct room_data *source) {
    int i;
...
bool free_room_strings(struct room_data *room) {
    int i;
