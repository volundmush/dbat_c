...
zone_rnum create_new_zone(zone_vnum vzone_num, room_vnum bottom, room_vnum top, const char **error) {
    ...
    if (!(fp = fopen(buf, "w"))) {
        mudlog(BRF, ADMLVL_IMPL, 1, "SYSERR: OLC: Can't write new zone file.");
    ...
    if (!(fp = fopen(buf, "w"))) {
        mudlog(BRF, ADMLVL_IMPL, 1, "SYSERR: OLC: Can't write new world file.");
    ...
    if (!(fp = fopen(buf, "w"))) {
        mudlog(BRF, ADMLVL_IMPL, 1, "SYSERR: OLC: Can't write new mob file.");
    ...
    if (!(fp = fopen(buf, "w"))) {
        mudlog(BRF, ADMLVL_IMPL, 1, "SYSERR: OLC: Can't write new obj file.");
    ...
    if (!(fp = fopen(buf, "w"))) {
        mudlog(BRF, ADMLVL_IMPL, 1, "SYSERR: OLC: Can't write new shop file.");
    ...
    if (!(fp = fopen(buf, "w"))) {
        mudlog(BRF, ADMLVL_IMPL, 1, "SYSERR: OLC: Can't write new trigger file");
    ...
    if (!(fp = fopen(buf, "w"))) {
        mudlog(BRF, ADMLVL_IMPL, 1, "SYSERR: OLC: Can't write new guild file");
...
void create_world_index(int znum, const char *type) {
    ...
    if (!(oldfile = fopen(old_name, "r"))) {
        mudlog(BRF, ADMLVL_IMPL, 1, "SYSERR: OLC: Failed to open %s.", old_name);
        return;
    } else if (!(newfile = fopen(new_name, "w"))) {
        mudlog(BRF, ADMLVL_IMPL, 1, "SYSERR: OLC: Failed to open %s.", new_name);
        fclose(oldfile);
        return;
    }
 ...
 */
bool save_zone(zone_rnum zone_num) {
    ...
    if (!(zfile = fopen(fname, "w"))) {
        mudlog(BRF, ADMLVL_BUILDER, 1, "SYSERR: OLC: save_zones:  Can't write zone %d.",
    ...
    for (subcmd = 0; ZCMD(zone_num, subcmd).command != 'S'; subcmd++) {
        switch (ZCMD(zone_num, subcmd).command) {
            ...
            case '*':
                ...
                continue;
            default:
                mudlog(BRF, ADMLVL_BUILDER, 1, "SYSERR: OLC: z_save_to_disk(): Unknown cmd '%c' - NOT saving",
 ...
 */
bool new_command(struct zone_data *zone, int pos) {
    int subcmd = 0;
