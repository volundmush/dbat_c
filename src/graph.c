...
/* local functions */
bool VALID_EDGE(room_rnum x, int y);
...
bool VALID_EDGE(room_rnum x, int y) {
    if (world[x].dir_option[y] == NULL || TOROOM(x, y) == NOWHERE)
        return false;
    if (CONFIG_TRACK_T_DOORS == 0 && IS_CLOSED(x, y))
        return false;
...
ACMD(do_sradar) {
    ...
    if (noship == 0 && SECT(IN_ROOM(vehicle)) != SECT_SPACE) {
        send_to_char(ch, "@wYour ship is not in space!\r\n");
        return;
    }
    if (noship == 1 && SECT(IN_ROOM(ch)) != SECT_SPACE) {
        send_to_char(ch, "@wYou are not even in space!\r\n");
        return;
    }

    if (!*arg) {
        if (GET_ADMLEVEL(ch) >= 1 && noship == 1) {
...
ACMD(do_radar) {
    int room = 0, dir, num = 0, found = 0, found2 = 0, fcount = 0;
    ...
    if (IS_NPC(ch)) {
        ...
        act("$n holds up a dragon radar and pushes its button.", 0, ch, 0, 0, TO_ROOM);
        while (num < 20000) {
...
ACMD(do_track) {
    ...
    if (!*arg && !FIGHTING(ch)) {
        ...
        act("You look at $N@n intently for a moment.", 1, ch, 0, vict, TO_CHAR);
        act("$n looks at you intently for a moment.", 1, ch, 0, vict, TO_VICT);
        act("$n looks at $N@n intently for a moment.", 1, ch, 0, vict, TO_NOTVICT);
        if (!IS_ANDROID(vict)) {
    ...
    if ((GET_SKILL_BASE(ch, SKILL_SENSE) == 100) &&
        ...
        switch (dir) {
            ...
            case BFS_ALREADY_THERE:
                act("You look at $N@n intently for a moment.", 1, ch, 0, vict, TO_CHAR);
                act("$n looks at you intently for a moment.", 1, ch, 0, vict, TO_VICT);
                act("$n looks at $N intently for a moment.", 1, ch, 0, vict, TO_NOTVICT);
                if (!IS_ANDROID(vict)) {
                    if (GET_ALIGNMENT(vict) > 50 && GET_ALIGNMENT(vict) < 200) {
                        send_to_char(ch, "You sense slightly pure and good ki from them.\r\n");
                    } else if (GET_ALIGNMENT(vict) > 200 && GET_ALIGNMENT(vict) < 500) {
