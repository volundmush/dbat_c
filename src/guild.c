...
ACMD(do_teach) {
    ...
    int cost = calculate_skill_cost(vict, skill), free = 0;
    ...
    if (!vict->master) {
        ...
        act(tochar, 1, ch, 0, vict, TO_CHAR);
        act(tovict, 1, ch, 0, vict, TO_VICT);
        act(toother, 1, ch, 0, vict, TO_NOTVICT);
...
int print_skills_by_type(struct char_data *ch, char *buf, int maxsz, int sktype, char *argument) {
    ...
    for (i = 1; i <= SKILL_TABLE_SIZE; i++) {
        ...
        if (*arg) {
            if (atoi(arg) <= 0 && strstr(spell_info[i].name, arg) == 0) {
                known = 0;
...
int slot_count(struct char_data *ch) {
    int i, skills = -1, fail = 0;
    int punch = 0, kick = 0, knee = 0, elbow = 0, kiball = 0, kiblast = 0, beam = 0, renzo = 0, shogekiha = 0;
    ...
    if (punch == 1 && kick == 1 && elbow == 1 && knee == 1) {
        skills += 1;
    }
    if (kiball == 1 && kiblast == 1 && beam == 1 && shogekiha == 1 && renzo == 1) {
        skills += 1;
    }
...
bool is_guild_open(struct char_data *keeper, int guild_nr, int msg) {
    char buf[200];
    *buf = 0;
...
bool is_guild_ok_char(struct char_data *keeper, struct char_data *ch, int guild_nr) {
    char buf[200];
...
bool is_guild_ok(struct char_data *keeper, struct char_data *ch, int guild_nr) {
    if (is_guild_open(keeper, guild_nr, 1))
...
bool does_guild_know(int guild_nr, int i) {
    return ((int) (guild_index[guild_nr].skills[i]));
}

bool does_guild_know_feat(int guild_nr, int i) {
...
void what_does_guild_know(int guild_nr, struct char_data *ch) {
    ...
    for (sortpos = 1; sortpos <= NUM_FEATS_DEFINED; sortpos++) {
        ...
        if (does_guild_know_feat(guild_nr, i) && feat_is_available(ch, i, 0, NULL) && feat_list[i].in_game &&
...
bool prereq_pass(struct char_data *ch, int snum) {
...
void handle_exp(struct char_data *keeper, int guild_nr, struct char_data *ch, char *argument, int unused) {
    ...
    if (GET_EXP(ch) > level_exp(ch, GET_LEVEL(ch) + 1) && GET_LEVEL(ch) != 100) {
        ...
        act("@c$n@W spends time training you in $s fighting style.@n", 1, keeper, 0, ch, TO_VICT);
        act("@c$n@W spends time training @C$N@W in $s fighting style.@n", 1, keeper, 0, ch, TO_NOTVICT);
...
void handle_study(struct char_data *keeper, int guild_nr, struct char_data *ch, char *argument, int unused) {
    ...
    act("@c$N@W spends time lecturing you on various subjects.@n", 1, ch, 0, keeper, TO_CHAR);
    act("@c$N@W spends time lecturing @C$n@W on various subjects.@n", 1, ch, 0, keeper, TO_ROOM);
...
void handle_learn(struct char_data *keeper, int guild_nr, struct char_data *ch, char *argument, int unused) {
    ...
    switch (feat_num) {
        ...
        case FEAT_GREATER_SPELL_FOCUS:
            ...
            subval = search_block(ptr, (sftype == 2) ? spell_schools : weapon_type, 0);
            if (subval == -1) {
    ...
    *buf = '\0';
    for (gm_nr = 0; gm_nr <= top_guild && len < sizeof(buf); gm_nr++) {
        ...
        len += snprintf(buf + len, sizeof(buf) - len, "%6d	%s		%5.2f	%s\r\n",
                        GM_NUM(gm_nr), buf1, GM_CHARGE(gm_nr), guild_customer_string(gm_nr, 0));
    }
...
void list_detailed_guild(struct char_data *ch, int gm_nr) {
    ...
    sprintf(buf, "%s Whom will train: %s\r\n", buf, guild_customer_string(gm_nr, 1));
