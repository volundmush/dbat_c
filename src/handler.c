        ...
        if (GET_EQ(ch, i))
            for (j = 0; j < MAX_OBJ_AFFECT; j++)
                affect_modify_ar(ch, GET_EQ(ch, i)->affected[j].location,
                                 ...
                                 GET_OBJ_PERM(GET_EQ(ch, i)), 0);
    }


    for (af = ch->affected; af; af = af->next)
        affect_modify(ch, af->location, af->modifier, af->specific, af->bitvector, 0);
        ...
        if (GET_EQ(ch, i)) {
            ...
            for (j = 0; j < MAX_OBJ_AFFECT; j++)
                affect_modify_ar(ch, GET_EQ(ch, i)->affected[j].location,
                                 GET_EQ(ch, i)->affected[j].modifier,
                                 GET_EQ(ch, i)->affected[j].specific,
                                 GET_OBJ_PERM(GET_EQ(ch, i)), 1);
        }
    }


    for (af = ch->affected; af; af = af->next)
        affect_modify(ch, af->location, af->modifier, af->specific, af->bitvector, 1);
...
bool invalid_align(struct char_data *ch, struct obj_data *obj) {
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos) {
    ...
    if (invalid_align(ch, obj) || invalid_class(ch, obj) || invalid_race(ch, obj)) {
        act("You stop wearing $p as something prevents you.", 0, ch, obj, 0, TO_CHAR);
        act("$n stops wearing $p as something prevents $m.", 0, ch, obj, 0, TO_ROOM);
...
void obj_to_room(struct obj_data *object, room_rnum room) {
    ...
    if (!object || room == NOWHERE || room > top_of_world)
        ...
        if (EXIT(object, 5) &&
            ...
            act("$p @Bsinks to deeper waters.@n", 1, 0, object, 0, TO_ROOM);
        ...
        if (EXIT(object, 5) && SECT(IN_ROOM(object)) == SECT_FLYING &&
            ...
            act("$p @Cfalls down.@n", 1, 0, object, 0, TO_ROOM);
            int numb = GET_ROOM_VNUM(EXIT(object, 5)->to_room);
            obj_from_room(object);
            obj_to_room(object, real_room(numb));
            if (SECT(IN_ROOM(object)) != SECT_FLYING) {
                act("$p @Cfalls down and smacks the ground.@n", 1, 0, object, 0, TO_ROOM);
            }
        }
        if (GET_OBJ_VAL(object, 0) != 0) {
            if (GET_OBJ_VNUM(object) == 16705 || GET_OBJ_VNUM(object) == 16706 || GET_OBJ_VNUM(object) == 16707) {
        ...
        if (GET_EQ(ch, i)) {
            if (GET_OBJ_TYPE(GET_EQ(ch, i)) == ITEM_LIGHT && GET_OBJ_VAL(GET_EQ(ch, i), VAL_LIGHT_HOURS) > 0 &&
                ...
                if (j == 1) {
                    ...
                    act("$n's light begins to flicker and fade.", 0, ch, 0, 0, TO_ROOM);
                } else if (j == 0) {
                    send_to_char(ch, "Your light sputters out and dies.\r\n");
                    act("$n's light sputters out and dies.", 0, ch, 0, 0, TO_ROOM);
                    world[IN_ROOM(ch)].light--;
                }
...
void extract_char_final(struct char_data *ch) {
    ...
    if (GRAPPLING(ch)) {
        act("@WYou stop grappling with @C$N@W!@n", 1, ch, 0, GRAPPLING(ch), TO_CHAR);
        act("@C$n@W stops grappling with @c$N@W!@n", 1, ch, 0, GRAPPLING(ch), TO_ROOM);
    ...
    if (GRAPPLED(ch)) {
        act("@WYou stop being grappled with by @C$N@W!@n", 1, ch, 0, GRAPPLED(ch), TO_CHAR);
        act("@C$n@W stops being grappled with by @c$N@W!@n", 1, ch, 0, GRAPPLED(ch), TO_ROOM);
    ...
    if (DRAGGED(ch)) {
        act("@WYou stop being dragged by @C$N@W!@n", 1, ch, 0, DRAGGED(ch), TO_CHAR);
        act("@C$n@W stops being dragged by @c$N@W!@n", 1, ch, 0, DRAGGED(ch), TO_ROOM);
...
int generic_find(char *arg, bitvector_t bitvector, struct char_data *ch,
    ...
    *tar_obj = NULL;
    ...
    if (IS_SET(bitvector, FIND_OBJ_EQUIP)) {
        for (found = 0, i = 0; i < NUM_WEARS && !found; i++)
...
bool is_better(struct obj_data *object, struct obj_data *object2) {
