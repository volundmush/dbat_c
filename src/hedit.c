...
ACMD(do_oasis_hedit) {
    ...
    if (!strcasecmp("save", arg)) {
        mudlog(CMP, MAX(ADMLVL_BUILDER, GET_INVIS_LEV(ch)), 1, "OLC: %s saves help files.",
               GET_NAME(ch));
    ...
    if (d->olc) {
        mudlog(BRF, ADMLVL_IMMORT, 1, "SYSERR: do_oasis: Player already had olc structure.");
        free(d->olc);
    }
    ...
    mudlog(CMP, ADMLVL_IMMORT, 1, "OLC: %s starts editing help files.", GET_NAME(ch));
}
...
void hedit_parse(struct descriptor_data *d, char *arg) {
    ...
    switch (OLC_MODE(d)) {
        ...
        case HEDIT_ENTRY:
            ...
            mudlog(1, ADMLVL_BUILDER, BRF, "SYSERR: Reached HEDIT_ENTRY case in parse_hedit");
            break;
        ...
        default:
            ...
            mudlog(1, ADMLVL_BUILDER, BRF, "SYSERR: Reached default case in parse_hedit");
            break;
    }
