...
/* local functions */
bool House_get_filename(room_vnum vnum, char *filename, size_t maxlen);

bool House_load(room_vnum rvnum);

bool House_save(struct obj_data *obj, FILE *fp, int location);
...
void hcontrol_build_house(struct char_data *ch, char *arg) {
    ...
    if ((exit_num = search_block(arg1, dirs, 0)) < 0 &&
        (exit_num = search_block(arg1, abbr_dirs, 0)) < 0) {
...
void hcontrol_pay_house(struct char_data *ch, char *arg) {
    ...
    if (!*arg)
        ...
        mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1, "Payment for house %s collected by %s.", arg,
               GET_NAME(ch));
...
bool House_can_enter(struct char_data *ch, room_vnum house) {
    int i, j;
