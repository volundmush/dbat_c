...
void hsedit_save_internally(struct descriptor_data *d) {
    ...
    if (house_rnum != NOWHERE) {
        ...
        if (house_rnum < MAX_HOUSES) {
            ...
            mudlog(NRM, ADMLVL_BUILDER, 1, "HSEDIT: Max houses limit reached - Unable to save OLC data");
        }
    }
...
void hedit_delete_house(struct descriptor_data *d, int house_vnum) {
    ...
    if ((i = find_house(house_vnum)) == NOWHERE) {
        mudlog(BRF, ADMLVL_BUILDER, 1, "SYSERR: hsedit: Invalid house vnum in hedit_delete_house\r\n");
...
void hsedit_dir_menu(struct descriptor_data *d) {
    ...
    mudlog(CMP, ADMLVL_BUILDER, 1, "(LOG) hsedit_dir_menu: house vnum = %d", house->vnum);
...
void hsedit_parse(struct descriptor_data *d, char *arg) {
    ...
    mudlog(CMP, ADMLVL_BUILDER, 0, "(LOG) hsedit_parse: OLC mode %d", OLC_MODE(d));
        ...
        case HSEDIT_CONFIRM_SAVESTRING:
            switch (*arg) {
                ...
                case 'Y':
                    ...
                    mudlog(CMP, ADMLVL_BUILDER, 1, "OLC: %s edits house %d", GET_NAME(d->character), OLC_NUM(d));
                    if (CONFIG_OLC_SAVE) {
                        hsedit_save_to_disk();
            ...
            switch (*arg) {
                ...
                case '2':
                    if ((OLC_HOUSE(d)->vnum == NOWHERE) || (real_room(OLC_HOUSE(d)->vnum) == NOWHERE)) {
                        ...
                        mudlog(NRM, ADMLVL_GRGOD, 1, "SYSERR: Invalid house VNUM in hsedit");
                    } else {
                ...
                case '3':
                    if ((OLC_HOUSE(d)->vnum == NOWHERE) || (real_room(OLC_HOUSE(d)->vnum) == NOWHERE)) {
                        send_to_char(d->character, "ERROR: Invalid house VNUM\r\n(Press Enter)\r\n");
                        mudlog(NRM, ADMLVL_BUILDER, 1, "SYSERR: Invalid house VNUM in hsedit");
                    } else {
                        hsedit_dir_menu(d);
                    }
                    break;
...
*/
        ...
        default:
            ...
            mudlog(BRF, ADMLVL_BUILDER, 1, "SYSERR: Reached default case in parse_hsedit");
            break;
    }
...
/****************************************************************************/
    if (d->olc) {
        mudlog(BRF, ADMLVL_BUILDER, 1, "SYSERR: do_oasis_hsedit: Player already had olc structure.");
        free(d->olc);
    }
...
/****************************************************************************/
    if (!can_edit_zone(ch, OLC_ZNUM(d))) {
        ...
        mudlog(BRF, ADMLVL_BUILDER, 1, "OLC: %s tried to edit zone %d allowed zone %d",
...
/****************************************************************************/
    ...
    if (real_num == NOWHERE) {
        ...
        if (num_of_houses >= MAX_HOUSES) {
            ...
            mudlog(NRM, ADMLVL_BUILDER, 1, "HSEDIT: MAX houses limit reached (%d)\r\n", MAX_HOUSES);
            return;
        } else {
            hsedit_setup_new(d);
        }
...
/****************************************************************************/
    mudlog(CMP, ADMLVL_BUILDER, 1, "OLC: (hsedit) %s starts editing zone %d allowed zone %d",
           GET_NAME(ch), zone_table[OLC_ZNUM(d)].number, GET_OLC_ZONE(ch));
}
