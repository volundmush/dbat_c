...
void parse_edit_action(int command, char *string, struct descriptor_data *d) {
    ...
    switch (command) {
        ...
        default:
            write_to_output(d, "Invalid option.\r\n");
            mudlog(BRF, ADMLVL_IMPL, 1, "SYSERR: invalid command passed to parse_edit_action");
            return;
    }
}
 ...
 */
bool format_text(char **ptr_string, int mode, struct descriptor_data *d, unsigned int maxlen, int low, int high) {
    int line_chars, cap_next = 1, cap_next_next = 0, color_chars = 0, i, pass_line = 0;
