...
bool command_pass(char *cmd, struct char_data *ch);
 ...
 */
bool perform_alias(struct descriptor_data *d, char *orig, size_t maxlen) {
...
void topWrite(struct char_data *ch) {
    ...
    char *positions[25];
    int64_t points[25] = {0};
    int x = 0, writeEm = 0, placed = 0, start = 0, finish = 25, location = -1;
    int progress = 0;
    ...
    for (x = start; x < finish; x++) { /* Save the new spots */
        if (placed == 0) { /* They Haven't Placed */
    ...
    } else if (placed == 1 && location != finish) {
...
int reserved_word(char *argument) {
    return (search_block(argument, reserved, 1) >= 0);
}
...
void display_races(struct descriptor_data *d) {
    ...
    for (x = 0; x < NUM_RACES; x++)
        if (race_ok_gender[(int) GET_SEX(d->character)][x])
            send_to_char(d->character, "@B%2d@W) @C%-15s@D[@R%-6s@D]@n%s", (x + 1) != 21 ? x + 1 : 16, pc_race_types[x],
                         cost[x], !(++i % 2) ? "\r\n" : "   ");
...
bool special(struct char_data *ch, int cmd, char *arg) {
    struct obj_data *i;
    struct char_data *k;
    int j;
  ...
  */
    ...
    switch (mode) {
        case RECON:
            ...
            act("$n has reconnected.", 1, d->character, 0, 0, TO_ROOM);
            mudlog(NRM, MAX(ADMLVL_NONE, GET_INVIS_LEV(d->character)), 1, "%s [%s] has reconnected.",
        ...
        case USURP:
            ...
            d->character->rp = d->rpp;
            mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(d->character)), 1,
                   "%s has re-logged in ... disconnecting old socket.", GET_NAME(d->character));
            break;
        case UNSWITCH:
            write_to_output(d, "Reconnecting to unswitched char.");
            mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(d->character)), 1, "%s [%s] has reconnected.",
                   GET_NAME(d->character), d->host);
            break;
    }
...
int enter_player_game(struct descriptor_data *d) {
    ...
    if (MOON_UP == 0 && (IS_SAIYAN(d->character) || IS_HALFBREED(d->character))) {
        oozaru_drop(d->character);
    }
    if (MOON_UP == 1 && (IS_SAIYAN(d->character) || IS_HALFBREED(d->character))) {
...
void nanny(struct descriptor_data *d, char *arg) {
    ...
    int total, rr, moveon = 0, penalty = 0;
    int player_i;
   ...
   */
    ...
    switch (STATE(d)) {
	  ...
	  */
                        ...
                        if (isbanned(d->host) == BAN_SELECT &&
                            ...
                            mudlog(NRM, ADMLVL_GOD, 1, "Connection attempt for %s denied from %s",
                                   GET_NAME(d->character), d->host);
                            STATE(d) = CON_CLOSE;
                            return;
                            ...
                            write_to_output(d, "The game is temporarily restricted to at least %d level.\r\n",
                                            circle_restrict);
                            STATE(d) = CON_UMENU;
                            mudlog(NRM, ADMLVL_GOD, 1, "Request for character load denied for %s [%s] (wizlock)",
                                   GET_NAME(d->character), d->host);
                            return;
                        }
        ...
        case CON_NAME_CNFRM:        /* wait for conf. of new name    */
            if (toupper(*arg) == 'Y') {
                if (isbanned(d->host) >= BAN_NEW) {
                    mudlog(NRM, ADMLVL_GOD, 1, "Request for new char %s denied from [%s] (siteban)",
                ...
                if (circle_restrict) {
                    ...
                    mudlog(NRM, ADMLVL_GOD, 1, "Request for new char %s denied from [%s] (wizlock)",
                           GET_PC_NAME(d->character), d->host);
                    STATE(d) = CON_CLOSE;
                    return;
                }
        ...
        case CON_QCLASS:
            ...
            if (load_result == CLASS_UNDEFINED) {
                write_to_output(d, "\r\nThat's not a sensei.\r\nSensei: ");
                return;
            } else if (load_result == CLASS_KABITO && !IS_KAI(d->character) && d->character->desc->rbank < 10 &&
                       d->character->rbank < 10) {
        ...
        case CON_QROLLSTATS:
            ...
            total -= 30;
            mudlog(CMP, ADMLVL_GOD, 1, "New player: %s [%s %s]",
        ...
        case CON_QSTATS:
            if (parse_stats(d, arg)) {
                ...
                mudlog(CMP, ADMLVL_GOD, 1, "New player: %s [%s %s]",
                       GET_NAME(d->character), pc_race_types[GET_RACE(d->character)],
                       pc_class_types[GET_CLASS(d->character)]);
                mudlog(CMP, ADMLVL_GOD, 1, "Str: %2d Dex: %2d Con: %2d Int: %2d "
	 ...
	 */
      ...
      */
                ...
                mudlog(NRM, ADMLVL_GOD, 1, "User %s has deleted character %s (lev %d).", d->user,
...
ACMD(do_disable) {
    ...
    if (p) { /* this command is disabled */
        ...
        mudlog(BRF, ADMLVL_IMMORT, 1, "(GC) %s has enabled the command '%s'.",
        ...
        mudlog(BRF, ADMLVL_IMMORT, 1, "(GC) %s has disabled the command '%s'.",
               GET_NAME(ch), p->command->command);
        save_disabled(); /* save to disk */
    }
}
...
bool check_disabled(const struct command_info *command) {
    DISABLED_DATA *p;
