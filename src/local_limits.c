...
static bool wearing_stardust(struct char_data *ch);
...
static void barrier_shed(struct char_data *ch) {
    ...
    if (GET_BARRIER(ch) <= 0) {
        GET_BARRIER(ch) = 0;
        act("@cYour barrier disappears.@n", 1, ch, 0, 0, TO_CHAR);
        act("@c$n@c's barrier disappears.@n", 1, ch, 0, 0, TO_ROOM);
    } else {
        act("@cYour barrier loses some energy.@n", 1, ch, 0, 0, TO_CHAR);
        send_to_char(ch, "@D[@C%s@D]@n\r\n", add_commas(loss));
        act("@c$n@c's barrier sends some sparks into the air as it seems to get a bit weaker.@n", 1, ch, 0, 0,
            TO_ROOM);
    }
...
static void healthy_check(struct char_data *ch) {
    ...
    int chance = 70, roll = rand_number(1, 100), change = 0;
...
static void update_flags(struct char_data *ch) {
    ...
    if (GET_BONUS(ch, BONUS_LATE) && GET_POS(ch) == POS_SLEEPING && rand_number(1, 3) == 3) {
        if (GET_HIT(ch) >= gear_pl(ch) && GET_MOVE(ch) >= GET_MAX_MOVE(ch) && GET_MANA(ch) >= GET_MAX_MANA(ch)) {
            ...
            act("$n wakes up.", 1, ch, 0, 0, TO_ROOM);
            GET_POS(ch) = POS_SITTING;
        }
    }

    if (AFF_FLAGGED(ch, AFF_KNOCKED) && !FIGHTING(ch)) {
        act("@W$n is no longer senseless, and wakes up.@n", 0, ch, 0, 0, TO_ROOM);
    ...
    if (!IS_NPC(ch) && !PLR_FLAGGED(ch, PLR_STAIL) && !PLR_FLAGGED(ch, PLR_NOGROW) &&
        ...
        if (GET_TGROWTH(ch) == 10) {
            ...
            act("$n@w's tail grows back.@n", 1, ch, 0, 0, TO_ROOM);
    ...
    if (AFF_FLAGGED(ch, AFF_SHOCKED) && rand_number(1, 4) == 4) {
        ...
        if (GET_SKILL(ch, SKILL_TELEPATHY) > 0) {
            int skill = GET_SKILL(ch, SKILL_TELEPATHY), stop = 0;
    ...
    if (AFF_FLAGGED(ch, AFF_FROZEN) && rand_number(1, 2) == 2) {
        ...
        act("$n@W breaks out of the ice holding $m prisoner!", 1, ch, 0, 0, TO_ROOM);
    ...
    if (AFF_FLAGGED(ch, AFF_WITHER) && rand_number(1, 6 + sick_fail) == 2) {
        ...
        act("$n@W's looks more fit now.", 1, ch, 0, 0, TO_ROOM);
        ch->real_abils.str += 3;
        ch->real_abils.cha += 3;
        save_char(ch);
        REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_WITHER);
...
void gain_level(struct char_data *ch, int whichclass) {
    ...
    if (GET_LEVEL(ch) < 100 && GET_EXP(ch) >= level_exp(ch, GET_LEVEL(ch) + 1)) {
        ...
        mudlog(BRF, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1, "%s advanced level to level %d.",
        ...
        if (GET_LEVEL(ch) < 100) {
            if (MINDLINK(ch) && gain > 0 && LINKER(ch) == 0) {
                if (GET_LEVEL(ch) + 20 < GET_LEVEL(MINDLINK(ch)) || GET_LEVEL(ch) - 20 > GET_LEVEL(MINDLINK(ch))) {
                    ...
                    act("@GYou've absorbed some new experiences from @W$n@G!@n", 0, ch, 0, MINDLINK(ch), TO_VICT);
                    int read = gain * 0.12;
                    gain -= read;
                    if (read == 0)
                        read = 1;
                    gain_exp(MINDLINK(ch), read);
                    act("@RYou sense that @W$N@R has stolen some of your experiences with $S mind!@n", 0, ch, 0,
                        MINDLINK(ch), TO_CHAR);
                }
            }
    ...
    if (PLR_FLAGGED(ch, PLR_WRITING))
                    ...
                    if (condition != DRUNK && prior >= 48 && !IS_MAJIN(ch)) {
                        ...
                        if (pukeroll > GET_CON(ch) + 19) {
                            act("@r@6You retch violently until your stomach is empty! Your constitution couldn't handle being that stuffed!@n",
                                1, ch, 0, 0, TO_CHAR);
                            act("@m@6$n@r@6 retches violently! It seems $e stuffed $mself too much!@n", 1, ch, 0, 0,
                                TO_ROOM);
                            ...
                            act("@r@6You puke violently! Your constitution couldn't handle being that stuffed!@n", 1,
                                ch, 0, 0, TO_CHAR);
                            act("@m@6$n@r@6 pukes violently! It seems $e stuffed $mself too much!@n", 1, ch, 0, 0,
                                TO_ROOM);
                            SET_BIT_AR(AFF_FLAGS(ch), AFF_PUKED);
                            if (!IS_NAMEK(ch)) {
                            ...
                            act("@r@6You puke a little! Your constitution couldn't handle being that stuffed!@n", 1,
                                ch, 0, 0, TO_CHAR);
                            act("@m@6$n@r@6 pukes a little! It seems $e stuffed $mself too much!@n", 1, ch, 0, 0,
                                TO_ROOM);
                            SET_BIT_AR(AFF_FLAGS(ch), AFF_PUKED);
                            if (!IS_NAMEK(ch)) {
        ...
        if (!AFF_FLAGGED(ch, AFF_SPIRIT) &&
            ...
            if (GET_HIT(ch) <= 0 && GET_COND(ch, HUNGER) == 0) {
                ...
                GET_MOVE(ch) = 0;
                act("@W$n@W falls down dead before you...@n", 0, ch, 0, 0, TO_ROOM);
                die(ch, NULL);
...
static void check_idling(struct char_data *ch) {
    ...
    if (++(ch->timer) > CONFIG_IDLE_VOID) {
        if (GET_WAS_IN(ch) == NOWHERE && IN_ROOM(ch) != NOWHERE) {
            ...
            Crash_rentsave(ch, 0);
            cp(ch);
            mudlog(CMP, ADMLVL_GOD, 1, "%s force-rented and extracted (idle).", GET_NAME(ch));
            extract_char(ch);
        }
    }
}
    ...
    if (healrate > 0) {
        if (GET_LIMBCOND(ch, 1) > 0 && GET_LIMBCOND(ch, 1) < 50) {
            if (GET_LIMBCOND(ch, 1) + healrate >= 50) {
                act("You realize your right arm is no longer broken.", 1, ch, 0, 0, TO_CHAR);
        ...
        if (GET_LIMBCOND(ch, 2) > 0 && GET_LIMBCOND(ch, 2) < 50) {
            if (GET_LIMBCOND(ch, 2) + healrate >= 50) {
                act("You realize your left arm is no longer broken.", 1, ch, 0, 0, TO_CHAR);
        ...
        if (GET_LIMBCOND(ch, 3) > 0 && GET_LIMBCOND(ch, 3) < 50) {
            if (GET_LIMBCOND(ch, 3) + healrate >= 50) {
                act("You realize your right leg is no longer broken.", 1, ch, 0, 0, TO_CHAR);
        ...
        if (GET_LIMBCOND(ch, 4) > 0 && GET_LIMBCOND(ch, 4) < 50) {
            if (GET_LIMBCOND(ch, 4) + healrate >= 50) {
                act("You realize your left leg is no longer broken.", 1, ch, 0, 0, TO_CHAR);
        ...
        if (!PLR_FLAGGED(ch, PLR_BANDAGED) && recovered == 1) {
...
void point_update(void) {
    ...
    for (i = character_list; i; i = next_char) {
        ...
        if (GET_POS(i) >= POS_STUNNED) {
            ...
            if (GET_KAIOKEN(i) > 0 &&
                (GET_SKILL(i, SKILL_KAIOKEN) < rand_number(1, x) || GET_MOVE(i) <= GET_MAX_MOVE(i) / 10)) {
                send_to_char(i, "You lose focus and your kaioken disappears.\r\n");
                act("$n loses focus and $s kaioken aura disappears.", 1, i, 0, 0, TO_ROOM);
            ...
            if (AFF_FLAGGED(i, AFF_BURNED)) {
                if (rand_number(1, 5) >= 4) {
                    ...
                    act("$n@w's burns are now healed.@n", 1, i, 0, 0, TO_ROOM);
            ...
            if (SECT(IN_ROOM(i)) == SECT_WATER_NOSWIM && !CARRIED_BY(i) && !IS_KANASSAN(i)) {
                if (GET_MOVE(i) >= gear_weight(i)) {
                    act("@bYou swim in place.@n", 1, i, 0, 0, TO_CHAR);
                    act("@C$n@b swims in place.@n", 1, i, 0, 0, TO_ROOM);
                    ...
                    act("@RYou are drowning!@n", 1, i, 0, 0, TO_CHAR);
                    act("@C$n@b gulps water as $e struggles to stay above the water line.@n", 1, i, 0, 0, TO_ROOM);
                    if (GET_HIT(i) - (gear_pl(i) / 3) <= 0) {
                        act("@rYou drown!@n", 1, i, 0, 0, TO_CHAR);
                        act("@R$n@r drowns!@n", 1, i, 0, 0, TO_ROOM);
                        die(i, NULL);
                        GET_HIT(i) = 1;
                    } else {
            ...
            if (!has_o2(i) && SUNKEN(IN_ROOM(i)) && !ROOM_FLAGGED(IN_ROOM(i), ROOM_SPACE)) {
                if ((GET_MANA(i) - mana_gain(i)) > GET_MAX_MANA(i) / 200) {
                    ...
                    if (GET_SUPP(i) > 0 && GET_SUPP(i) > gear_pl(i) * 0.05) {
                        ...
                        GET_HIT(i) = 1;
                        act("@W$n@W drowns right in front of you.@n", 0, i, 0, 0, TO_ROOM);
                        die(i, NULL);
                    }
                }
            }
            ...
            if (!AFF_FLAGGED(i, AFF_FLYING) && ROOM_EFFECT(IN_ROOM(i)) == 6 && !MOB_FLAGGED(i, MOB_NOKILL) &&
                !IS_DEMON(i)) {
                act("@rYour legs are burned by the lava!@n", 1, i, 0, 0, TO_CHAR);
                act("@R$n@r's legs are burned by the lava!@n", 1, i, 0, 0, TO_ROOM);
                ...
                if (GET_SUPP(i) > gear_pl(i) * 0.05) {
                    ...
                    if (GET_HIT(i) < 0) {
                        act("@rYou have burned to death!@n", 1, i, 0, 0, TO_CHAR);
                        act("@R$n@r has burned to death!@n", 1, i, 0, 0, TO_ROOM);
                        die(i, NULL);
                    }
                }
            }
            if (change == 1 && !AFF_FLAGGED(i, AFF_POISON)) {
                if (PLR_FLAGGED(i, PLR_HEALT) && SITS(i) != NULL) {
                    ...
                    if (HCHARGE(SITS(i)) == 0) {
                        ...
                        act("You step out of the now empty healing tank.", 1, i, 0, 0, TO_CHAR);
                        act("@C$n@w steps out of the now empty healing tank.@n", 1, i, 0, 0, TO_ROOM);
                        REMOVE_BIT_AR(PLR_FLAGS(i), PLR_HEALT);
                        SITTING(SITS(i)) = NULL;
                        SITS(i) = NULL;
                    } else if (GET_HIT(i) == (gear_pl(i)) && GET_MANA(i) == GET_MAX_MANA(i) &&
                               GET_MOVE(i) == GET_MAX_MOVE(i)) {
            ...
            if (AFF_FLAGGED(i, AFF_POISON)) {
                ...
                if (GET_HIT(i) - GET_MAX_HIT(i) * cost > 0) {
                    ...
                    act("$n shivers and then pukes.", 1, i, 0, 0, TO_ROOM);
                    GET_HIT(i) -= GET_MAX_HIT(i) * cost;
                } else {
                    send_to_char(i, "The poison claims your life!\r\n");
                    act("$n pukes up blood and falls down dead!", 1, i, 0, 0, TO_ROOM);
                    if (i->poisonby) {
        ...
        if (GET_MANA(i) >= GET_MAX_MANA(i) * 0.5 && GET_CHARGE(i) < GET_MAX_MANA(i) * 0.1 &&
    ...
    for (j = object_list; j; j = next_thing) {
        ...
        if (OBJ_FLAGGED(j, ITEM_NORENT) && j->worn_by == NULL && j->carried_by == NULL && obj_selling != j &&
            GET_OBJ_VNUM(j) != 7200) {
            time_t diff = 0;
            ...
            if (!strstr(j->name, "android") && !strstr(j->name, "Android") && !OBJ_FLAGGED(j, ITEM_BURIED)) {
                if (GET_OBJ_TIMER(j) == 5) {
                    if ((IN_ROOM(j) != NOWHERE) && (world[IN_ROOM(j)].people)) {
                        act("@DFlies start to gather around $p@D.@n", 1, world[IN_ROOM(j)].people, j, 0, TO_CHAR);
                        act("@DFlies start to gather around $p@D.@n", 1, world[IN_ROOM(j)].people, j, 0, TO_ROOM);
                    }
                }
                if (GET_OBJ_TIMER(j) == 3) {
                    if ((IN_ROOM(j) != NOWHERE) && (world[IN_ROOM(j)].people)) {
                        act("@DA cloud of flies has formed over $p@D.@n", 1, world[IN_ROOM(j)].people, j, 0,
                            TO_CHAR);
                        act("@DA cloud of flies has formed over $p@D.@n", 1, world[IN_ROOM(j)].people, j, 0,
                            TO_ROOM);
                    }
                ...
                if (GET_OBJ_TIMER(j) == 2) {
                    if ((IN_ROOM(j) != NOWHERE) && (world[IN_ROOM(j)].people)) {
                        act("@DMaggots can be seen crawling all over $p@D.@n", 1, world[IN_ROOM(j)].people, j, 0,
                            TO_CHAR);
                        act("@DMaggots can be seen crawling all over $p@D.@n", 1, world[IN_ROOM(j)].people, j, 0,
                            TO_ROOM);
                    }
                ...
                if (GET_OBJ_TIMER(j) == 1) {
                    if ((IN_ROOM(j) != NOWHERE) && (world[IN_ROOM(j)].people)) {
                        act("@DMaggots have nearly stripped $p of all its flesh@D.@n", 1, world[IN_ROOM(j)].people,
                            j, 0, TO_CHAR);
                        act("@DMaggots have nearly stripped $p of all its flesh@D.@n", 1, world[IN_ROOM(j)].people,
                            j, 0, TO_ROOM);
                    }
            ...
            if (!GET_OBJ_TIMER(j)) {
                ...
                if (j->carried_by) {
                    if (!strstr(j->name, "android")) {
                        act("$p decays in your hands.", 0, j->carried_by, j, 0, TO_CHAR);
                        if ((IN_ROOM(j) != NOWHERE) && (world[IN_ROOM(j)].people)) {
                            act("A quivering horde of maggots consumes $p.",
                                1, world[IN_ROOM(j)].people, j, 0, TO_ROOM);
                            act("A quivering horde of maggots consumes $p.",
                                1, world[IN_ROOM(j)].people, j, 0, TO_CHAR);
                        }
                    } else {
                        act("$p decays in your hands.", 0, j->carried_by, j, 0, TO_CHAR);
                        if ((IN_ROOM(j) != NOWHERE) && (world[IN_ROOM(j)].people)) {
