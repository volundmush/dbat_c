...
/* local functions */
bool mag_materials(struct char_data *ch, int item0, int item1, int item2, int extract, int verbose);
...
bool mag_materials(struct char_data *ch, int item0, int item1, int item2,
    ...
    if (verbose) {
        ...
        act("A puff of smoke rises from $n's pack.", 1, ch, NULL, NULL, TO_ROOM);
    }
    return true;
}


bool mag_newsaves(struct char_data *ch, struct char_data *victim, int spellnum, int level, int cast_stat) {
    int stype;
    int dc;
    int total;
    ...
    switch (spellnum) {
        ...
        case SPELL_DISPEL_EVIL:
            ...
            if (IS_EVIL(ch)) {
                ...
                act("The gods protect $N.", 0, ch, 0, victim, TO_CHAR);
                return false;
            }
            break;
...
void mag_affects(int level, struct char_data *ch, struct char_data *victim,
    ...
    bool accum_affect = 0, accum_duration = 0;
    ...
    switch (spellnum) {
        ...
        case SPELL_SLEEP:
            ...
            if (GET_POS(victim) > POS_SLEEPING) {
                ...
                act("$n goes to sleep.", 1, victim, 0, 0, TO_ROOM);
    ...
    for (i = 0; i < MAX_SPELL_AFFECTS; i++)
        if (af[i].bitvector || (af[i].location != APPLY_NONE))
            affect_join(victim, af + i, accum_duration, 0, accum_affect, 0);

    if (to_vict != NULL)
        act(to_vict, 0, victim, 0, ch, TO_CHAR);
    if (to_room != NULL)
        act(to_room, 1, victim, 0, ch, TO_ROOM);
}
...
void mag_areas(int level, struct char_data *ch, int spellnum) {
    ...
    if (to_char != NULL)
        act(to_char, 0, ch, 0, 0, TO_CHAR);
    if (to_room != NULL)
        act(to_room, 0, ch, 0, 0, TO_ROOM);
...
void mag_summons(int level, struct char_data *ch, struct obj_data *obj, int spellnum, char *arg) {
    ...
    int msg = 0, num = 1, handle_corpse = 0, affs = 0, affvs = 0, assist = 0, i, j, count;
    char *buf = NULL;
    ...
    for (i = 0; i < num; i++) {
        ...
        act(mag_summon_msgs[msg], 0, ch, 0, mob, TO_ROOM);
        load_mtrigger(mob);
        add_follower(mob, ch);
...
void mag_points(int level, struct char_data *ch, struct char_data *victim,
    ...
    switch (spellnum) {
        ...
        case SPELL_SENSU:
            ...
            act("@C$n@W suddenly looks a lot better!@b", 0, victim, 0, 0, TO_NOTVICT);
            affect_from_char(victim, SPELL_POISON);
            if (AFF_FLAGGED(victim, AFF_BURNED)) {
                send_to_char(victim, "Your burns are healed now.\r\n");
                act("$n@w's burns are now healed.@n", 1, victim, 0, 0, TO_ROOM);
...
void mag_unaffects(int level, struct char_data *ch, struct char_data *victim,
                   int spellnum) {
    int spell = 0, msg_not_affected = 1;
    const char *to_vict = NULL, *to_room = NULL;
...
void mag_alter_objs(int level, struct char_data *ch, struct obj_data *obj,
    ...
    if (to_char == NULL)
        ...
        act(to_char, 1, ch, obj, 0, TO_CHAR);

    if (to_room != NULL)
        act(to_room, 1, ch, obj, 0, TO_ROOM);
    else if (to_char != NULL)
        act(to_char, 1, ch, obj, 0, TO_ROOM);
...
void mag_creations(int level, struct char_data *ch, int spellnum) {
    ...
    add_unique_id(tobj);
    obj_to_char(tobj, ch);
    act("$n creates $p.", 0, ch, tobj, 0, TO_ROOM);
    act("You create $p.", 0, ch, tobj, 0, TO_CHAR);
    load_otrigger(tobj);
}
...
void mag_affectsv(int level, struct char_data *ch, struct char_data *victim,
    ...
    for (i = 0; i < MAX_SPELL_AFFECTS; i++)
        if (af[i].bitvector || (af[i].location != APPLY_NONE))
            affectv_join(victim, af + i, accum_duration, 0, accum_affect, 0);
