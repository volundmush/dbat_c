...
void postmaster_send_mail(struct char_data *ch, struct char_data *mailman,
    ...
    act(buf, 0, mailman, 0, ch, TO_VICT);
    act("@C$n@w starts writing a letter.@n", 1, ch, 0, 0, TO_ROOM);
...
void postmaster_check_mail(struct char_data *ch, struct char_data *mailman,
    ...
    if (has_mail(GET_IDNUM(ch)))
        act("$n tells you, 'You have mail waiting.'", 0, mailman, 0, ch, TO_VICT);
    else
        act("$n tells you, 'Sorry, you don't have any mail waiting.'", 0, mailman, 0, ch, TO_VICT);
}
