...
//    }
    ...
    if (d->character) {
        ...
        act("$n stops using OLC.", 1, d->character, NULL, NULL, TO_ROOM);

        if (cleanup_type == CLEANUP_CONFIG)
            mudlog(BRF, ADMLVL_IMMORT, 1, "OLC: %s stops editing the game configuration", GET_NAME(d->character));
        else if (STATE(d) == CON_TEDIT)
            mudlog(BRF, ADMLVL_IMMORT, 1, "OLC: %s stops editing text files.", GET_NAME(d->character));
        else if (STATE(d) == CON_HEDIT)
            mudlog(CMP, ADMLVL_IMMORT, 1, "OLC: %s stops editing help files.", GET_NAME(d->character));
        else
            mudlog(BRF, ADMLVL_IMMORT, 1, "OLC: %s stops editing zone %d allowed zone %d", GET_NAME(d->character),
...
/**                                                                          **/
/******************************************************************************/
bool can_edit_zone(struct char_data *ch, zone_rnum rnum) {
...
void send_cannot_edit(struct char_data *ch, zone_vnum zone) {
    ...
    send_to_char(ch, "\r\n");
    mudlog(BRF, ADMLVL_IMPL, 1, "OLC: %s tried to edit zone %d allowed zone %d",
