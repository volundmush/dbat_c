...
ACMD(do_oasis_copy) {
    ...
    if (d->olc) {
        mudlog(BRF, ADMLVL_IMMORT, 1, "SYSERR: do_oasis_copy: Player already had olc structure.");
        free(d->olc);
    }
...
ACMD(do_dig) {
    ...
    if ((dir = search_block(sdir, abbr_dirs, 0)) < 0)
        dir = search_block(sdir, dirs, 0);
    ...
    if (rrnum == NOWHERE) {
         ...
         */
        if (d->olc) {
            mudlog(BRF, ADMLVL_IMMORT, 1, "SYSERR: do_dig: Player already had olc structure.");
            free(d->olc);
        }
        CREATE(d->olc, struct oasis_olc_data, 1);
        OLC_ZNUM(d) = zone;
...
bool buildwalk(struct char_data *ch, int dir) {
    ...
    if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_BUILDWALK) &&
        ...
        if (!can_edit_zone(ch, world[IN_ROOM(ch)].zone)) {
             ...
             */
            if (d->olc) {
                mudlog(BRF, ADMLVL_IMMORT, 1, "SYSERR: buildwalk(): Player already had olc structure.");
                free(d->olc);
            }
            CREATE(d->olc, struct oasis_olc_data, 1);
