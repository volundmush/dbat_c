...
static bool gen_receptionist(struct char_data *ch, struct char_data *recep, int cmd, char *arg, int mode);

static bool Crash_save(struct obj_data *obj, FILE *fp, int location);
...
static bool Crash_is_unrentable(struct obj_data *obj);
...
void delete_inv_backup(struct char_data *ch) {
    ...
    sprintf(source_file, ""SLASH"home"SLASH"m053car2"SLASH"dbat"SLASH"lib"SLASH"plrobjs"SLASH"%s"SLASH"%s.copy", alpha,
            ch->name);
...
bool Obj_to_store(struct obj_data *obj, FILE *fl, int location) {
    ...
    if (location > 0) {    /* Was wearing it. */
        ...
        if (location > 0) {        /* Wearable. */
            if (!GET_EQ(ch, j)) {
                ...
                mudlog(BRF, ADMLVL_IMMORT, 1, "SYSERR: autoeq: '%s' already equipped in position %d.", GET_NAME(ch),
                       location);
                location = LOC_INVENTORY;
            }
        }
...
bool Crash_delete_file(char *name) {
    char filename[50];
    FILE *fl;
...
bool Crash_delete_crashfile(struct char_data *ch) {
...
bool Crash_clean_file(char *name) {
...
static int Crash_offer_rent(struct char_data *ch, struct char_data *recep,
    ...
    if (display) {
        ...
        if (totalcost > GET_GOLD(ch) + GET_BANK_GOLD(ch)) {
            act("$n tells you, '...which I see you can't afford.'", 0, recep, 0, ch, TO_VICT);
            return false;
...
static bool gen_receptionist(struct char_data *ch, struct char_data *recep,
    ...
    if (!CAN_SEE(recep, ch)) {
        act("$n says, 'I don't deal with people I can't see!'", 0, recep, 0, 0, TO_ROOM);
        return true;
    }
    ...
    if (CMD_IS("rent")) {
        ...
        if (!(cost = Crash_offer_rent(ch, recep, 0, mode)))
            return true;
        if (mode == RENT_FACTOR)
        ...
        if (mode == RENT_FACTOR) {
            act("$n stores your belongings and helps you into your private chamber.", 0, recep, 0, ch, TO_VICT);
            Crash_rentsave(ch, cost);
            mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1, "%s has rented (%d/day, %d tot.)",
            ...
            mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1, "%s has cryo-rented.", GET_NAME(ch));
            SET_BIT_AR(PLR_FLAGS(ch), PLR_CRYO);
        }
        ...
        Crash_offer_rent(ch, recep, 1, mode);
...
bool Crash_load(struct char_data *ch) {
    FILE *fl;
    ...
    if (!(fl = fopen(cmfname, "r+b"))) {
        ...
        if (GET_LEVEL(ch) > 1) {
            mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1,
    ...
    switch (orig_rent_code = rentcode) {
        case RENT_RENTED:
            mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1, "%s un-renting and entering game.", GET_NAME(ch));
            break;
        case RENT_CRASH:
            mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1,
            ...
            break;
        case RENT_CRYO:
            mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1, "%s un-cryo'ing and entering game.", GET_NAME(ch));
            break;
        case RENT_FORCED:
        case RENT_TIMEDOUT:
            mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1,
    ...
    mudlog(NRM, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1, "%s (level %d) has %d objects (max %d).",
