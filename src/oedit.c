...
void oedit_parse(struct descriptor_data *d, char *arg) {
    ...
    switch (OLC_MODE(d)) {
                    ...
                    if (STATE(d) != CON_IEDIT) {
                        ...
                        *obj = *(OLC_OBJ(d));
                        ...
                        if (d->character) {
                            ...
                            STATE(d) = CON_PLAYING;
                            act("$n stops using OLC.", 1, d->character, 0, 0, TO_ROOM);
                        }
                        free(d->olc);
                        d->olc = NULL;
                    }
        ...
        default:
            mudlog(BRF, ADMLVL_BUILDER, 1, "SYSERR: OLC: Reached default case in oedit_parse()!");
...
ACMD(do_iedit) {
    ...
    act("$n starts using OLC.", 1, ch, 0, 0, TO_ROOM);
