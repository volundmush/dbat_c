...
bool can_modify(struct char_data *ch, int vnum);
...
ACMD(do_olc) {
    ...
    if ((olc_mode = search_block(mode_arg, olc_modes, 0)) < 0) {
...
void olc_interpreter(void *targ, int mode, char *arg) {
    ...
    if ((command = search_block(command_string, olc_commands, 0)) < 0) {
...
void olc_bitvector(int *bv, const char **names, char *arg) {
    ...
    while (*arg) {
        ...
        if ((flagnum = search_block(this_name, names, 1)) < 0)
