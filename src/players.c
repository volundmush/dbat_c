...
int load_char(const char *name, struct char_data *ch) {
    ...
    if ((id = get_ptable_by_name(name)) < 0)
        ...
        if (!get_filename(fname, sizeof(fname), PLR_FILE, player_table[id].name))
            return (-1);
        if (!(fl = fopen(fname, "r"))) {
            mudlog(NRM, ADMLVL_GOD, 1, "SYSERR: Couldn't open player file %s", fname);
            return (-1);
        }
        ...
        while (get_line(fl, line)) {
            ...
            switch (*tag) {
                ...
                case 'B':
                    ...
                    else if (!strcmp(tag, "Bonu")) load_bonuses(fl, ch, 0);
                ...
                case 'S':
                    ...
                    else if (!strcmp(tag, "Skil")) load_skills(fl, ch, 0);
                    else if (!strcmp(tag, "Skn ")) GET_SKIN(ch) = atoi(line);
                    else if (!strcmp(tag, "Size")) ch->size = atoi(line);
                    else if (!strcmp(tag, "SklB")) load_skills(fl, ch, 1);
...
void save_char_pets(struct char_data *ch) {
    ...
    if (!(fl = fopen(fname, "w"))) {
        mudlog(NRM, ADMLVL_GOD, 1, "SYSERR: Couldn't open pet file %s for write", fname);
        return;
    }
...
void load_char_pets(struct char_data *ch) {
    ...
    for (foll = ch->followers; foll; foll = foll->next) {
        ...
        act("You are joined by $N.", 0, ch, 0, foll->follower, TO_CHAR);
    }
}
...
void save_char(struct char_data *ch) {
    ...
    if (!get_filename(fname, sizeof(fname), PLR_FILE, GET_NAME(ch)))
        return;
    if (!(fl = fopen(fname, "w"))) {
        mudlog(NRM, ADMLVL_GOD, 1, "SYSERR: Couldn't open player file %s for write", fname);
        return;
    }
