...
get_selling_obj(struct char_data *ch, char *name, struct char_data *keeper, int shop_nr, int msg);
...
static struct obj_data *
get_purchase_obj(struct char_data *ch, char *arg, struct char_data *keeper, int shop_nr, int msg);
...
static bool is_ok_char(struct char_data *keeper, struct char_data *ch, int shop_nr);

static bool is_ok_obj(struct char_data *keeper, struct char_data *ch, struct obj_data *obj, int shop_nr);

static bool is_open(struct char_data *keeper, int shop_nr, int msg);

static bool is_ok(struct char_data *keeper, struct char_data *ch, int shop_nr);
...
static bool same_obj(struct obj_data *obj1, struct obj_data *obj2);
...
static bool add_to_list(struct shop_buy_data *list, int type, int *len, int *val);
...
static void shopping_app(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr) {
    ...
    if (!(obj = get_purchase_obj(ch, arg, keeper, shop_nr, 1))) {
        do_appraise(ch, arg, 0, 0);
        return;
    }

    act("@C$N@W hands you @G$p@W for a moment and let's you examine it before taking it back.@n", 1, ch, obj, keeper,
        TO_CHAR);
    act("@c$N@W hands @C$n@W @G$p@W for a moment and let's $m examine it before taking it back.@n", 1, ch, obj,
        keeper, TO_ROOM);
...
static void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr) {
    ...
    act(tempbuf, 0, ch, obj, 0, TO_ROOM);
...
static void shopping_sell(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr) {
    ...
    one_argument(arg, name);
    if (!(obj = get_selling_obj(ch, name, keeper, shop_nr, 1)))
        return;
    ...
    while (obj && GET_GOLD(keeper) + SHOP_BANK(shop_nr) >= sell_price(obj, shop_nr, keeper, ch) && sold < sellnum) {
        ...
        obj = get_selling_obj(ch, name, keeper, shop_nr, 0);
    }
...
static char *
list_object(struct obj_data *obj, int cnt, int aindex, int shop_nr, struct char_data *keeper, struct char_data *ch) {
...
SPECIAL(shop_keeper) {
    ...
    if (CMD_IS("steal")) {
        ...
        if (!SHOP_ALLOW_STEAL(shop_nr)) {
            ...
            act(argm, 0, ch, 0, keeper, TO_CHAR);
            act(argm, 0, ch, 0, keeper, TO_ROOM);
...
bool ok_damage_shopkeeper(struct char_data *ch, struct char_data *victim) {
    int sindex;
...
static void list_detailed_shop(struct char_data *ch, int shop_nr) {
    ...
    send_to_char(ch, "Customers:  %s\r\n", (ptrsave = customer_string(shop_nr, 1)) ? ptrsave : "None");
