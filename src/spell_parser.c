...
void say_spell(struct char_data *ch, int spellnum, struct char_data *tch,
    ...
    *buf = '\0';
    ...
    if (tch != NULL && tch != ch && IN_ROOM(tch) == IN_ROOM(ch)) {
        ...
        act(buf1, 0, ch, NULL, tch, TO_VICT);
    }
}
 ...
 */
bool call_magic(struct char_data *caster, struct char_data *cvict,
    ...
    if (ROOM_FLAGGED(IN_ROOM(caster), ROOM_PEACEFUL) && GET_ADMLEVEL(caster) < ADMLVL_IMPL &&
        ...
        act("White light from no particular source suddenly fills the room, then vanishes.", 0, caster, 0, 0,
            TO_ROOM);
        return false;
    }
...
void mag_objectmagic(struct char_data *ch, struct obj_data *obj,
    ...
    switch (GET_OBJ_TYPE(obj)) {
        case ITEM_STAFF:
            act("You tap $p three times on the ground.", 0, ch, obj, 0, TO_CHAR);
            if (obj->action_description)
                act(obj->action_description, 0, ch, obj, 0, TO_ROOM);
            else
                act("$n taps $p three times on the ground.", 0, ch, obj, 0, TO_ROOM);

            if (GET_OBJ_VAL(obj, VAL_STAFF_CHARGES) <= 0) {
                send_to_char(ch, "It seems powerless.\r\n");
                act("Nothing seems to happen.", 0, ch, obj, 0, TO_ROOM);
            } else {
        ...
        case ITEM_WAND:
            if (k == FIND_CHAR_ROOM) {
                if (tch == ch) {
                    act("You point $p at yourself.", 0, ch, obj, 0, TO_CHAR);
                    act("$n points $p at $mself.", 0, ch, obj, 0, TO_ROOM);
                } else {
                    act("You point $p at $N.", 0, ch, obj, tch, TO_CHAR);
                    if (obj->action_description)
                        act(obj->action_description, 0, ch, obj, tch, TO_ROOM);
                    else
                        act("$n points $p at $N.", 1, ch, obj, tch, TO_ROOM);
                }
            } else if (tobj != NULL) {
                act("You point $p at $P.", 0, ch, obj, tobj, TO_CHAR);
                if (obj->action_description)
                    act(obj->action_description, 0, ch, obj, tobj, TO_ROOM);
                else
                    act("$n points $p at $P.", 1, ch, obj, tobj, TO_ROOM);
                ...
                act("You point $p outward.", 0, ch, obj, NULL, TO_CHAR);
                act("$n points $p outward.", 1, ch, obj, NULL, TO_ROOM);
            } else {
                act("At what should $p be pointed?", 0, ch, obj, NULL, TO_CHAR);
                return;
            }
        ...
        case ITEM_SCROLL:
            if (*arg) {
                if (!k) {
                    act("There is nothing to here to affect with $p.", 0,
                        ch, obj, NULL, TO_CHAR);
                    return;
                }
            } else
            ...
            act("You recite $p which dissolves.", 1, ch, obj, 0, TO_CHAR);
            if (obj->action_description)
                act(obj->action_description, 0, ch, obj, tch, TO_ROOM);
            else
                act("$n recites $p.", 0, ch, obj, NULL, TO_ROOM);
        ...
        case ITEM_POTION:
            ...
            act("You swallow $p.", 0, ch, obj, NULL, TO_CHAR);
            if (obj->action_description)
                act(obj->action_description, 0, ch, obj, NULL, TO_ROOM);
            else
                act("$n swallows $p.", 1, ch, obj, NULL, TO_ROOM);
...
ACMD(do_cast) {
    ...
    int mana, percent; */
    int ki = 0;
    int spellnum, i, target = 0, innate = 0;
...
void mag_assign_spells(void) {
    ...
    spello(SKILL_SOLARF, "blind", 25, 10, 1, POS_STANDING,
           TAR_CHAR_ROOM | TAR_NOT_SELF, 1, MAG_ACTION_FULL | MAG_AFFECTSV, MAGSAVE_FORT | MAGSAVE_NONE, 0,
    ...
    spello(SPELL_DETECT_ALIGN, "detect alignment", 20, 10, 2, POS_STANDING,
           TAR_CHAR_ROOM | TAR_SELF_ONLY, 0, MAG_ACTION_FULL | MAG_AFFECTS, 0, 0,
           "You feel less aware.",
    ...
    spello(SPELL_DETECT_POISON, "detect poison", 15, 5, 1, POS_STANDING,
           TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_ROOM, 0, MAG_ACTION_FULL | MAG_MANUAL, 0, 0,
           "The detect poison wears off.",
           0, SCHOOL_DIVINATION, DOMAIN_UNIVERSAL);

    spello(SPELL_DISPEL_EVIL, "dispel evil", 40, 25, 3, POS_FIGHTING,
           TAR_CHAR_ROOM | TAR_FIGHT_VICT, 1, MAG_ACTION_FULL | MAG_DAMAGE, MAGSAVE_WILL | MAGSAVE_NONE, 0,
           NULL,
    ...
    spello(SPELL_GROUP_ARMOR, "group armor", 50, 30, 2, POS_STANDING,
           TAR_IGNORE, 0, MAG_ACTION_FULL | MAG_GROUPS, 0, 0,
           NULL,
           5, SCHOOL_CONJURATION, DOMAIN_UNDEFINED);

    spello(SPELL_FAERIE_FIRE, "faerie fire", 20, 10, 2, POS_STANDING,
           TAR_CHAR_WORLD | TAR_NOT_SELF, 0, MAG_ACTION_FULL | MAG_MANUAL, 0, 0,
           NULL,
           1, SCHOOL_EVOCATION, DOMAIN_UNIVERSAL);

    spello(SPELL_FIREBALL, "fireball", 40, 30, 2, POS_FIGHTING,
           TAR_CHAR_ROOM | TAR_FIGHT_VICT, 1, MAG_ACTION_FULL | MAG_DAMAGE, MAGSAVE_REFLEX | MAGSAVE_HALF, 0,
           NULL,
    ...
    spello(SPELL_HARM, "harm", 75, 45, 3, POS_FIGHTING,
           TAR_CHAR_ROOM | TAR_FIGHT_VICT, 1, MAG_ACTION_FULL | MAG_DAMAGE, MAGSAVE_FORT | MAGSAVE_NONE, 0,
           NULL,
           6, SCHOOL_UNDEFINED, DOMAIN_DESTRUCTION);

    spello(SPELL_HEAL, "heal", 60, 40, 3, POS_FIGHTING,
           TAR_CHAR_ROOM, 0, MAG_ACTION_FULL | MAG_POINTS | MAG_UNAFFECTS, 0, 0,
           NULL,
    ...
    spello(SPELL_IDENTIFY, "identify", 50, 25, 5, POS_STANDING,
           TAR_OBJ_INV | TAR_OBJ_ROOM, 0, MAG_ACTION_FULL | MAG_MANUAL, 0, 0,
           NULL,
    ...
    spello(SPELL_INVISIBLE, "invisibility", 35, 25, 1, POS_STANDING,
           TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_ROOM, 0, MAG_ACTION_FULL | MAG_AFFECTS | MAG_ALTER_OBJS, 0, 0,
    ...
    spello(SPELL_LOCATE_OBJECT, "locate object", 25, 20, 1, POS_STANDING,
           TAR_OBJ_WORLD, 0, MAG_ACTION_FULL | MAG_MANUAL, 0, 0,
           NULL,
           3, SCHOOL_DIVINATION, DOMAIN_TRAVEL);

    spello(SPELL_MAGIC_MISSILE, "magic missile", 25, 10, 3, POS_FIGHTING,
           TAR_CHAR_ROOM | TAR_FIGHT_VICT, 1, MAG_ACTION_FULL | MAG_DAMAGE, 0, 0,
           NULL,
    ...
    spello(SKILL_POISON, "poison", 50, 20, 3, POS_STANDING,
           TAR_CHAR_ROOM | TAR_NOT_SELF | TAR_OBJ_INV, 1,
    ...
    spello(SPELL_REMOVE_CURSE, "remove curse", 45, 25, 5, POS_STANDING,
           TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_EQUIP, 0,
    ...
    spello(SPELL_NEUTRALIZE_POISON, "neutralize poison", 40, 8, 4, POS_STANDING,
           TAR_CHAR_ROOM | TAR_OBJ_INV | TAR_OBJ_ROOM, 0, MAG_ACTION_FULL | MAG_UNAFFECTS | MAG_ALTER_OBJS, 0, 0,
           NULL,
           4, SCHOOL_CONJURATION, DOMAIN_UNIVERSAL);

    spello(SPELL_SANCTUARY, "sanctuary", 110, 85, 5, POS_STANDING,
           TAR_CHAR_ROOM, 0, MAG_ACTION_FULL | MAG_AFFECTS, 0, 0,
    ...
    spello(SPELL_SLEEP, "sleep", 40, 25, 5, POS_FIGHTING,
           TAR_CHAR_ROOM, 1, MAG_ACTION_FULL | MAG_AFFECTS, MAGSAVE_WILL | MAGSAVE_NONE, 0,
    ...
    spello(SPELL_TELEPORT, "teleport", 75, 50, 3, POS_STANDING,
           TAR_CHAR_ROOM, 0, MAG_ACTION_FULL | MAG_MANUAL, 0, 0,
           NULL,
    ...
    spello(SPELL_RESISTANCE, "resistance", 40, 20, 0, POS_STANDING,
           TAR_CHAR_ROOM, 0, MAG_ACTION_FULL | MAG_AFFECTSV,
    ...
    spello(SPELL_ACID_SPLASH, "acid splash", 40, 20, 0, POS_STANDING,
           TAR_CHAR_ROOM, 0, MAG_ACTION_FULL | MAG_AFFECTS,
    ...
    spello(SPELL_SUMMON_MONSTER_I, "summon monster i", 40, 20, 2, POS_FIGHTING,
           TAR_IGNORE, 0, MAG_ACTION_FULL | MAG_SUMMONS, 0, 0,
           NULL,
           1, SCHOOL_CONJURATION, DOMAIN_UNDEFINED);
    ...
    spello(SPELL_FIRE_SHIELD, "fire shield", 40, 20, 2, POS_FIGHTING,
           TAR_CHAR_ROOM, 0, MAG_ACTION_FULL | MAG_AFFECTSV, 0, 0,
           NULL,
