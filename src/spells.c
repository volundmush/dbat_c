...
ASPELL(spell_teleport) {
    ...
    act("$n slowly fades out of existence and is gone.",
        0, victim, 0, 0, TO_ROOM);
    char_from_room(victim);
    char_to_room(victim, to_room);
    act("$n slowly fades into existence.", 0, victim, 0, 0, TO_ROOM);
    look_at_room(IN_ROOM(victim), victim, 0);
    entry_memory_mtrigger(victim);
    greet_mtrigger(victim, -1);
    greet_memory_mtrigger(victim);
...
ASPELL(spell_summon) {
    ...
    if (!CONFIG_PK_ALLOWED) {
        ...
        if (!IS_NPC(victim) && !PRF_FLAGGED(victim, PRF_SUMMONABLE) &&
            ...
            mudlog(BRF, ADMLVL_IMMORT, 1, "%s failed summoning %s to %s.", GET_NAME(ch), GET_NAME(victim),
    ...
    act("$n disappears suddenly.", 1, victim, 0, 0, TO_ROOM);

    char_from_room(victim);
    char_to_room(victim, IN_ROOM(ch));

    act("$n arrives suddenly.", 1, victim, 0, 0, TO_ROOM);
    act("$n has summoned you!", 0, ch, 0, victim, TO_VICT);
    look_at_room(IN_ROOM(victim), victim, 0);
    entry_memory_mtrigger(victim);
    greet_mtrigger(victim, -1);
    greet_memory_mtrigger(victim);
    ...
    if (victim) {
        if (victim == ch) {
            ...
            if (AFF_FLAGGED(victim, AFF_POISON))
                ...
                act("You sense that $E is healthy.", 0, ch, 0, victim, TO_CHAR);
        }
    }
        ...
        switch (GET_OBJ_TYPE(obj)) {
            ...
            case ITEM_FOOD:
                if (GET_OBJ_VAL(obj, VAL_FOOD_POISON))
                    act("You sense that $p has been contaminated.", 0, ch, obj, 0, TO_CHAR);
                else
                    act("You sense that $p is safe for consumption.", 0, ch, obj, 0,
                        TO_CHAR);
                break;
            default:
...
ASPELL(art_abundant_step) {
    ...
    act("$n is suddenly absent.", 1, ch, 0, 0, TO_ROOM);

    char_from_room(ch);
    char_to_room(ch, r);

    act("$n is suddenly present.", 1, ch, 0, 0, TO_ROOM);
