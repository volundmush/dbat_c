    ...
    switch (terminator) {
        case STRINGADD_SAVE:
            if (!(fl = fopen(storage, "a")))
                mudlog(CMP, ADMLVL_IMPL, 1, "SYSERR: Can't write file '%s'.", storage);
            if (!*d->str)
                mudlog(CMP, ADMLVL_IMPL, 1, "SYSERR: Can't write file '%s'.", storage);
            else {
                char *tmstr;
        ...
        case STRINGADD_ABORT:
            ...
            act("$n stops editing the news.", 1, d->character, 0, 0, TO_ROOM);
            break;
        default:
    ...
    switch (terminator) {
        case STRINGADD_SAVE:
            if (!(fl = fopen(storage, "w")))
                ...
                fclose(fl);
                mudlog(CMP, ADMLVL_GOD, 1, "OLC: %s saves '%s'.", GET_NAME(d->character), storage);
        ...
        case STRINGADD_ABORT:
            write_to_output(d, "Edit aborted.\r\n");
            act("$n stops editing some scrolls.", 1, d->character, 0, 0, TO_ROOM);
            break;
        default:
...
ACMD(do_tedit) {
    ...
    if (ch->desc->olc) {
        mudlog(BRF, ADMLVL_IMMORT, 1, "SYSERR: do_tedit: Player already had olc structure.");
        free(ch->desc->olc);
    }
    ...
    act("$n begins editing a text file.", 1, ch, 0, 0, TO_ROOM);
