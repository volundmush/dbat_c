...
void dispel_ash(struct char_data *ch) {
    ...
    if (ash) {
        ...
        if (GET_OBJ_COST(ash) == 3) {
            if (GET_INT(ch) > roll) {
                act("@GYou clear the air with the shockwaves of your power!@n", 1, ch, ash, 0, TO_CHAR);
                act("@C$n@G clears the air with the shockwaves of $s power!@n", 1, ch, ash, 0, TO_ROOM);
                extract_obj(ash);
            }
...
bool has_group(struct char_data *ch) {
...
bool roll_pursue(struct char_data *ch, struct char_data *vict) {
    ...
    if (skill > perc) {
        ...
        act("@C$n@R pursues after the fleeing @c$N@R!@n", 1, ch, 0, vict, TO_NOTVICT);
        char_from_room(ch);
        char_to_room(ch, IN_ROOM(vict));
        act("@GYou pursue right after @c$N@G!@n", 1, ch, 0, vict, TO_CHAR);
        act("@C$n@R pursues after you!@n", 1, ch, 0, vict, TO_VICT);
        act("@C$n@R pursues after the fleeing @c$N@R!@n", 1, ch, 0, vict, TO_NOTVICT);
        ...
        if (ch->followers) {
            for (k = ch->followers; k; k = next) {
                ...
                if ((GET_ROOM_VNUM(IN_ROOM(k->follower)) == inroom) && (GET_POS(k->follower) >= POS_STANDING) &&
                    ...
                    act("You follow $N.", 1, k->follower, 0, ch, TO_CHAR);
                    act("$n follows after $N.", 1, k->follower, 0, ch, TO_NOTVICT);
                    act("$n follows after you.", 1, k->follower, 0, ch, TO_VICT);
...
void broken_update() {
    ...
    int dice = rand_number(2, 12), grav_roll = 0, grav_change = 0, health = 0;
...
void randomize_eq(struct obj_data *obj) {
    if (wearable_obj(obj) && !OBJ_FLAGGED(obj, ITEM_NORANDOM)) {
        ...
        int stat = 0;
        int strength = 0, wisdom = 0, intelligence = 0, dexterity = 0, speed = 0, constitution = 0;
...
void reveal_hiding(struct char_data *ch, int type) {
    ...
    if (type == 0) { /* Automatic reveal. */
        act("@MYou feel as though what you just did may have revealed your hiding spot...@n", 1, ch, 0, 0, TO_CHAR);
        act("@M$n moves a little and you notice them!@n", 1, ch, 0, 0, TO_ROOM);
        REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_HIDE);
    } else if (type == 1) { /* Their skill at hiding failed, reveal */
        if (GET_SKILL(ch, SKILL_HIDE) + bonus < axion_dice(0)) {
            act("@MYou feel as though what you just did may have revealed your hiding spot...@n", 1, ch, 0, 0,
                TO_CHAR);
            act("@M$n moves a little and you notice them!@n", 1, ch, 0, 0, TO_ROOM);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_HIDE);
        }
        ...
        for (d = descriptor_list; d; d = d->next) {
            ...
            if (GET_SKILL(tch, SKILL_SPOT) + rand1 >= GET_SKILL(ch, SKILL_HIDE) + rand2) {
                REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_HIDE);
                act("@M$N seems to notice you!@n", 1, ch, 0, tch, TO_CHAR);
                act("@MYou notice $n's movements reveal $s hiding spot!@n", 1, ch, 0, tch, TO_VICT);
                act("@MYou notice $N look keenly somewhere nearby. At that spot you see $n hiding!@n", 1, ch, 0, tch,
                    TO_NOTVICT);
                return;
            }
        }
        ...
        act("@MThe scouter makes some beeping sounds as you tinker with its buttons.@n", 1, ch, 0, 0, TO_CHAR);
        for (d = descriptor_list; d; d = d->next) {
            if (STATE(d) != CON_PLAYING)
                continue;
            tch = d->character;
            ...
            if (GET_SKILL(tch, SKILL_LISTEN) > axion_dice(0)) {
                switch (type) {
                    case 3:
                        act("@MYou notice some beeping sounds that sound really close by.@n", 1, ch, 0, tch,
                            TO_VICT);
                        break;
                    default:
...
bool block_calc(struct char_data *ch) {
    ...
    if (GET_SPEEDI(ch) < GET_SPEEDI(blocker) && GET_POS(blocker) > POS_SITTING) {
        if (!AFF_FLAGGED(blocker, AFF_BLIND) && !PLR_FLAGGED(blocker, PLR_EYEC)) {
            ...
            if (!GET_SKILL(ch, SKILL_ESCAPE_ARTIST) || (GET_SKILL(ch, SKILL_ESCAPE_ARTIST) &&
                ...
                act("$n tries to leave, but can't outrun $N!", 1, ch, 0, blocker, TO_NOTVICT);
                act("$n tries to leave, but can't outrun you!", 1, ch, 0, blocker, TO_VICT);
                act("You try to leave, but can't outrun $N!", 1, ch, 0, blocker, TO_CHAR);
                ...
                act("$n proves $s great skill and escapes from $N's attempted block!", 1, ch, 0, blocker,
                    TO_NOTVICT);
                act("$n proves $s great skill and escapes from your attempted block!", 1, ch, 0, blocker, TO_VICT);
                act("Using your great skill you manage to escape from $N's attempted block!", 1, ch, 0, blocker,
                    TO_CHAR);
                BLOCKED(ch) = NULL;
                BLOCKS(blocker) = NULL;
            }
        } else {
            act("$n proves $s great skill and escapes from $N's attempted block!", 1, ch, 0, blocker, TO_NOTVICT);
            act("$n proves $s great skill and escapes from your attempted block!", 1, ch, 0, blocker, TO_VICT);
            act("Using your great skill you manage to escape from $N's attempted block!", 1, ch, 0, blocker,
                TO_CHAR);
            BLOCKED(ch) = NULL;
        ...
        act("$n proves $s great skill and escapes from $N!", 1, ch, 0, blocker, TO_NOTVICT);
        act("$n proves $s great skill and escapes from you!", 1, ch, 0, blocker, TO_VICT);
        act("Using your great skill you manage to escape from $N!", 1, ch, 0, blocker, TO_CHAR);
        BLOCKED(ch) = NULL;
        BLOCKS(blocker) = NULL;
    } else if (GET_POS(blocker) > POS_SITTING) {
        act("$n proves $s great skill and escapes from $N's attempted block!", 1, ch, 0, blocker, TO_NOTVICT);
        act("$n proves $s great skill and escapes from your attempted block!", 1, ch, 0, blocker, TO_VICT);
        act("Using your great skill you manage to escape from $N's attempted block!", 1, ch, 0, blocker, TO_CHAR);
        BLOCKED(ch) = NULL;
        BLOCKS(blocker) = NULL;
    }
...
void demon_refill_lf(struct char_data *ch, int64_t num) {
    ...
    for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room) {
        ...
        if (GET_LIFEFORCE(tch) >= GET_LIFEMAX(tch))
            ...
            act("@CYou feel the life energy from @c$N@C's cursed body flow out and you draw it into yourself!@n", 1,
                tch, 0, ch, TO_CHAR);
        }
    }
}
...
bool mob_respond(struct char_data *ch, struct char_data *vict, const char *speech) {
    if (ch != NULL && vict != NULL) {
        if (!IS_NPC(ch) && IS_NPC(vict)) {
            if ((strstr(speech, "hello") || strstr(speech, "greet") || strstr(speech, "Hello") ||
                ...
                if (IS_HUMAN(vict) || IS_HALFBREED(vict)) {
                    switch (rand_number(1, 4)) {
                        case 1:
                            act("@w$n@W says, '@CYes, hello to you as well $N.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 2:
                            act("@w$n@W says, '@CHello!@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 3:
                            act("@w$n@W says, '@CHi, $N, how are you doing?@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 4:
                            act("@w$n@W says, '@CGreetings, $N. What are you up to?@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                    }
                } /* End Human Section */
                    ...
                    switch (rand_number(1, 4)) {
                        case 1:
                            act("@w$n@W says, '@CHmph, hi.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 2:
                            act("@w$n@W says, '@CHello weakling.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 3:
                            act("@w$n@W says, '@C$N do all weaklings like you waste time in idle talk?@W'@n", 1,
                                vict, 0, ch, TO_ROOM);
                            break;
                        case 4:
                            act("@w$n@W says, '@C$N, you are not welcome around me.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                    }
                } /* End Saiyan Section */
                    ...
                    switch (rand_number(1, 4)) {
                        case 1:
                            act("@w$n@W says, '@CHa ha... Yes, hello.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 2:
                            ...
                            break;
                        case 3:
                            act("@w$n@W says, '@C$N, hello. Now leave me be.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 4:
                            act("@w$n@W says, '@C$N, you are below me. Now begone.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                    }
                } /* End Icer Section */
                    ...
                    switch (rand_number(1, 4)) {
                        case 1:
                            act("@w$n@W says, '@CGreetings, $N, may your travels be well.@W'@n", 1, vict, 0, ch,
                                TO_ROOM);
                            break;
                        case 2:
                            act("@w$n@W says, '@CHello.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 3:
                            act("@w$n@W says, '@C$N, hello.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 4:
                            act("@w$n@W says, '@C$N, it is nice to meet you.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                    }
                } /* End Konatsu Section */
                    ...
                    switch (rand_number(1, 4)) {
                        case 1:
                            ...
                            break;
                        case 2:
                            act("@w$n@W says, '@CA peaceful greeting to you, $N.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 3:
                            act("@w$n@W says, '@C$N, hello. What is your business here?@W'@n", 1, vict, 0, ch,
                                TO_ROOM);
                            break;
                        case 4:
                            act("@w$n@W says, '@C$N, greetings.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                    }
                } /* End Namek Section */
                    ...
                    switch (rand_number(1, 4)) {
                        case 1:
                            act("@w$n@W says, '@CPeace, stranger.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 2:
                            act("@w$n@W says, '@CStay out of my way.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 3:
                            act("@w$n@W says, '@C$N, what is your business here?@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 4:
                            act("@w$n@W says, '@C...Hello.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                    }
                } /* End Arlian Section */
                else if (IS_ANDROID(vict)) {
                    act("@w$n@W says, '@C...@W'@n", 1, vict, 0, ch, TO_ROOM);
                } /* End Android Section */
                else if (IS_MAJIN(vict)) {
                    switch (rand_number(1, 2)) {
                        case 1:
                            act("@w$n@W says, '@CHa ha...@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                        case 2:
                            act("@w$n@W says, '@CHello. What candy you want to be?@W'@n", 1, vict, 0, ch, TO_ROOM);
                            break;
                    }
                } /* End MAJIN Section */
                    ...
                    switch (rand_number(1, 3)) {
                        case 1:
                            if (IS_SAIYAN(ch)) {
                                act("@w$n@W says, '@CEwww, dirty monkey...@W'@n", 1, vict, 0, ch, TO_ROOM);
                            } else {
                                act("@w$n@W says, '@CHello.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            }
                            break;
                        case 2:
                            if (IS_SAIYAN(ch)) {
                                ...
                                act("@w$n@W says, '@C$N, hello. You are a curious individual.@W'@n", 1, vict, 0, ch,
                                    TO_ROOM);
                            }
                            break;
                        case 3:
                            if (IS_SAIYAN(ch)) {
                                ...
                                act("@w$n@W says, '@C$N, hello. What's your IQ?@W'@n", 1, vict, 0, ch, TO_ROOM);
                            }
                            break;
                    }
                } /* End Truffle Section */
                else {
                    act("Hmph, yeah hi.", 1, vict, 0, ch, TO_ROOM);
                }
            } /* End Hello Section */
            ...
            if ((strstr(speech, "spar") || strstr(speech, "Spar")) && !FIGHTING(vict)) {
                ...
                if (GET_LEVEL(vict) > 4 && GET_ALIGNMENT(vict) >= 0) {
                    ...
                    if (remember == 1) {
                        act("@w$n@W says, '@C$N you will die by my hand!@W'@n", 1, vict, 0, ch, TO_ROOM);
                        return true;
                    } else if (MOB_FLAGGED(vict, MOB_NOKILL)) {
                        act("@w$n@W says, '@C$N, I have no need to spar with you.@W'@n", 1, vict, 0, ch, TO_ROOM);
                        return true;
                    } else if (MOB_FLAGGED(vict, MOB_AGGRESSIVE)) {
                        act("@w$n@W says, '@C$N, I will kill you instead.@W'@n", 1, vict, 0, ch, TO_ROOM);
                        return true;
                    } else if (MOB_FLAGGED(vict, MOB_DUMMY)) {
                        act("@w$n@W says, '@C...@W'@n", 1, vict, 0, ch, TO_ROOM);
                        return true;
                    } else if (GET_MAX_HIT(ch) > GET_MAX_HIT(vict) * 2) {
                        act("@w$n@W says, '@C$N, no way will I spar. I already know I would lose badly.@W'@n", 1,
                            vict, 0, ch, TO_ROOM);
                        return true;
                    } else if (GET_MAX_HIT(vict) > GET_MAX_HIT(ch) * 2) {
                        act("@w$n@W says, '@C$N, you wouldn't last very long.@W'@n", 1, vict, 0, ch, TO_ROOM);
                        return true;
                    } else if (GET_HIT(vict) < GET_MAX_HIT(vict) * .8) {
                        act("@w$n@W says, '@C$N, I need to recover first.@W'@n", 1, vict, 0, ch, TO_ROOM);
                        return true;
                    } else if (rand_number(1, 50) >= 40 && !MOB_FLAGGED(vict, MOB_SPAR)) {
                        act("@w$n@W says, '@C$N, maybe in a bit.@W'@n", 1, vict, 0, ch, TO_ROOM);
                        return true;
                    } else {
                        if (MOB_FLAGGED(vict, MOB_SPAR)) {
                            act("@w$n@W says, '@C$N, fine our match will wait till later then.@W'@n", 1, vict, 0, ch,
                                TO_ROOM);
                            REMOVE_BIT_AR(MOB_FLAGS(vict), MOB_SPAR);
                        } else {
                            act("@w$n@W says, '@C$N, sure. I'll spar with you.@W'@n", 1, vict, 0, ch, TO_ROOM);
                            SET_BIT_AR(MOB_FLAGS(vict), MOB_SPAR);
                        }
                        return false;
                    }
                } else if (GET_LEVEL(vict) > 4 && GET_ALIGNMENT(vict) < 0) {
                    act("@w$n@W says, '@CSpar? I don't play games, I play for blood...@W'@n", 1, vict, 0, ch,
                        TO_ROOM);
                    return true;
                } else {
                    act("@w$n@W says, '@CSpar? I prefer not to thank you...@W'@n", 1, vict, 0, ch, TO_ROOM);
                    return true;
                }
            ...
            if (strstr(speech, "goodbye") || strstr(speech, "Goodbye") || strstr(speech, "bye") ||
                ...
                if (GET_ALIGNMENT(vict) >= 0) {
                    if (GET_SEX(vict) == SEX_MALE) {
                        if (GET_SEX(ch) == SEX_FEMALE) {
                            act("@w$n@W says, '@C$N, goodbye babe.@W'@n", 1, vict, 0, ch, TO_ROOM);
                        } else {
                            act("@w$n@W says, '@C$N, goodbye.@W'@n", 1, vict, 0, ch, TO_ROOM);
                        }
                    } else if (GET_SEX(vict) == SEX_FEMALE) {
                        if (GET_SEX(ch) == SEX_MALE) {
                            act("@w$n@W says, '@C$N, goodbye...@W'@n", 1, vict, 0, ch, TO_ROOM);
                        } else {
                            act("@w$n@W says, '@C$N, bye.@W'@n", 1, vict, 0, ch, TO_ROOM);
                        }
                    } else {
                        act("@w$n@W says, '@C$N, goodbye.@W'@n", 1, vict, 0, ch, TO_ROOM);
                    }
                ...
                if (GET_ALIGNMENT(vict) < 0) {
                    if (GET_SEX(vict) == SEX_MALE) {
                        if (GET_SEX(ch) == SEX_FEMALE) {
                            act("@w$n@W says, '@CGoodbye. Eh heh heh.@W'@n", 1, vict, 0, ch, TO_ROOM);
                        } else {
                            act("@w$n@W says, '@CSo long and good ridance.@W'@n", 1, vict, 0, ch, TO_ROOM);
                        }
                    } else if (GET_SEX(vict) == SEX_FEMALE) {
                        if (GET_SEX(ch) == SEX_MALE) {
                            act("@w$n@W says, '@CGoodbye then...@W'@n", 1, vict, 0, ch, TO_ROOM);
                        } else {
                            act("@w$n@W says, '@C$N, no one wanted you around anyway.@W'@n", 1, vict, 0, ch,
                                TO_ROOM);
                        }
                    } else {
                        act("@w$n@W says, '@CFine get lost.@W'@n", 1, vict, 0, ch, TO_ROOM);
                    }
                }
            } /* End goodbye If */
            if (strstr(speech, "train") || strstr(speech, "Train") || strstr(speech, "exercise") ||
                ...
                if (GET_ALIGNMENT(vict) >= 0 && !MOB_FLAGGED(vict, MOB_NOKILL)) {
                    ...
                    if (GET_LEVEL(vict) >= 10 && GET_LEVEL(vict) < 30) {
                        act("@w$n@W says, '@CI think I might need a little more training...@W'@n", 1, vict, 0, ch,
                            TO_ROOM);
                    }
                    ...
                    if (GET_LEVEL(vict) >= 60) {
                        act("@w$n@W says, '@CI'm on top of my game.@W'@n", 1, vict, 0, ch, TO_ROOM);
                    }
                    if (GET_LEVEL(vict) < 5) {
                        act("@w$n@W says, '@CI really need to bust ass and train.@W'@n", 1, vict, 0, ch, TO_ROOM);
                    }
                }
                if (GET_ALIGNMENT(vict) < 0 && !MOB_FLAGGED(vict, MOB_NOKILL)) {
                    if (GET_LEVEL(vict) > 4 && GET_LEVEL(vict) < 10) {
                        act("@w$n@W says, '@CWell maybe I could use some more training.@W'@n", 1, vict, 0, ch,
                            TO_ROOM);
                    }
                    if (GET_LEVEL(vict) >= 10 && GET_LEVEL(vict) < 30) {
                        act("@w$n@W says, '@CTrain? Yeah it has become harder to take what I want....@W'@n", 1, vict,
                            0, ch, TO_ROOM);
                    }
                    if (GET_LEVEL(vict) >= 30 && GET_LEVEL(vict) < 60) {
                        act("@w$n@W says, '@CTrain? I don't need to train to take you!@W'@n", 1, vict, 0, ch,
                            TO_ROOM);
                    }
                    if (GET_LEVEL(vict) >= 60) {
                        act("@w$n@W says, '@CTraining won't save you when I tire of your continued life.@W'@n", 1,
                            vict, 0, ch, TO_ROOM);
                    }
                    if (GET_LEVEL(vict) < 5) {
    ...
    for (obj = world[IN_ROOM(ch)].contents; obj; obj = next_obj) {
        ...
        if (GET_OBJ_VNUM(obj) == 80 || GET_OBJ_VNUM(obj) == 81) {
            if (TARGET(obj) == ch || USER(obj) == ch) {
                act("$p @wloses its target and flies off into the distance.@n", 1, 0, obj, 0, TO_ROOM);
                extract_obj(obj);
                continue;
            }
        }
...
void improve_skill(struct char_data *ch, int skill, int num) {
    ...
    if ((IS_JINTO(ch) || IS_TSUNA(ch) || IS_DABURA(ch) || IS_TAPION(ch) && GET_SKILL(ch, SKILL_SENSE) >= 75) &&
        skill == SKILL_SENSE) {
        return;
    }
...
void log_death_trap(struct char_data *ch) {
    mudlog(BRF, ADMLVL_IMMORT, 1, "%s hit death trap #%d (%s)", GET_NAME(ch), GET_ROOM_VNUM(IN_ROOM(ch)),
...
void stop_follower(struct char_data *ch) {
    ...
    act("$n stops following $N.", 1, ch, 0, ch->master, TO_NOTVICT);
    if (!(DEAD(ch->master) || (ch->master->desc && STATE(ch->master->desc) == CON_MENU)))
        act("$n stops following you.", 1, ch, 0, ch->master, TO_VICT);
...
void add_follower(struct char_data *ch, struct char_data *leader) {
    ...
    if (IN_ROOM(ch) != NOWHERE && IN_ROOM(leader) != NOWHERE && CAN_SEE(leader, ch)) {
        ...
        act("\r\n$n starts to follow $N.", 1, ch, 0, leader, TO_NOTVICT);
    }
}
...
bool get_filename(char *filename, size_t fbufsize, int mode, const char *orig_name) {
...
void admin_set(struct char_data *ch, int value) {
    ...
    if (GET_ADMLEVEL(ch) < value) { /* Promotion */
        mudlog(BRF, MAX(ADMLVL_IMMORT, GET_INVIS_LEV(ch)), 1,
...
bool masadv(char *tmp, struct char_data *ch) {
...
int GET_PC_HEIGHT(struct char_data *ch) {
    return !IS_NPC(ch) ? age(ch)->year <= 10 ? (int) (ch->height * 0.68) : age(ch)->year <= 12 ? (int) (ch->height *
                                                                                                        0.72) :
                                                                           age(ch)->year <= 14 ? (int) (ch->height *
                                                                                                        0.85) :
                                                                           age(ch)->year <= 16 ? (int) (ch->height *
                                                                                                        0.92)
                                                                                               : ch->height
                       : ch->height;
}
...
int GET_PC_WEIGHT(struct char_data *ch) {
    return !IS_NPC(ch) ? age(ch)->year <= 10 ? (int) (ch->weight * 0.48) : age(ch)->year <= 12 ? (int) (ch->weight *
                                                                                                        0.55) :
                                                                           age(ch)->year <= 14 ? (int) (ch->weight *
                                                                                                        0.7) :
                                                                           age(ch)->year <= 16 ? (int) (ch->weight *
                                                                                                        0.85)
                                                                                               : ch->weight
                       : ch->weight;
}
...
int GET_MUTBOOST(struct char_data *ch) {
    return IS_MUTANT(ch) ? ((GET_GENOME(ch, 0) == 1 || GET_GENOME(ch, 1) == 1) ?
                            (GET_SPEEDCALC(ch) + GET_SPEEDBONUS(ch) + GET_SPEEDBOOST(ch)) * 0.3 : 0) : 0;
}
...
int GET_SPEEDCALC(struct char_data *ch) {
    return IS_GRAP(ch) ? GET_CHA(ch) : (IS_INFERIOR(ch) ? (AFF_FLAGGED(ch, AFF_FLYING) ? (GET_SPEEDVAR(ch) * 1.25)
                                                                                       : GET_SPEEDVAR(ch))
                                                        : GET_SPEEDVAR(ch));
}

int GET_SPEEDBONUS(struct char_data *ch) {
    return IS_ARLIAN(ch) ? AFF_FLAGGED(ch, AFF_SHELL) ? GET_SPEEDVAR(ch) * -0.5 : (IS_MALE(ch) ? (AFF_FLAGGED(ch,
                                                                                                              AFF_FLYING)
                                                                                                  ? (GET_SPEEDVAR(ch) *
                                                                                                     0.5) : 0) : 0) : 0;
}
...
int GET_SPEEDINT(struct char_data *ch) {
    return IS_BIO(ch) ? ((GET_CHA(ch) * GET_DEX(ch)) * (GET_MAX_HIT(ch) / 1200) / 1200) +
                        (GET_CHA(ch) * (GET_KAIOKEN(ch) * 100)) :
           ((GET_CHA(ch) * GET_DEX(ch)) * (GET_MAX_HIT(ch) / 1000) / 1000) + (GET_CHA(ch) * (GET_KAIOKEN(ch) * 100));
}
...
int GET_BLESSBONUS(struct char_data *ch) {
    return AFF_FLAGGED(ch, AFF_BLESS) ? (GET_BLESSLVL(ch) >= 100 ?
                                         ((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * 0.1 :
                                         GET_BLESSLVL(ch) >= 60 ?
                                         ((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * 0.05 :
                                         GET_BLESSLVL(ch) >= 40 ?
                                         ((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * 0.02 : 0) : 0;
}

double GET_POSELF(struct char_data *ch) {
    return !IS_NPC(ch) ? PLR_FLAGGED(ch, PLR_POSE) ? GET_SKILL(ch, SKILL_POSE) >= 100 ? 0.15 :
                                                     GET_SKILL(ch, SKILL_POSE) >= 60 ? 0.1 :
                                                     GET_SKILL(ch, SKILL_POSE) >= 40 ? 0.05 : 0 : 0 : 0;
}
...
int GET_LIFEBONUS(struct char_data *ch) {
    return IS_ARLIAN(ch) ? ((GET_MAX_MANA(ch) * 0.01) * (GET_MOLT_LEVEL(ch) / 100)) +
                           ((GET_MAX_MOVE(ch) * 0.01) * (GET_MOLT_LEVEL(ch) / 100)) : 0;
}

double GET_LIFEBONUSES(struct char_data *ch) {
    return ch->lifebonus > 0 ? (GET_LIFEBONUS(ch) + GET_BLESSBONUS(ch) + GET_POSEBONUS(ch)) *
                               (((ch)->lifebonus + 100) * 0.01) : (GET_LIFEBONUS(ch) + GET_BLESSBONUS(ch) +
                                                                   GET_POSEBONUS(ch));
}
...
int GET_LIFEMAX(struct char_data *ch) {
    return IS_DEMON(ch) ? (((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * 0.75) + GET_LIFEBONUSES(ch)
                        : (IS_KONATSU(ch) ? (((GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5)) * 0.85) +
                                            GET_LIFEBONUSES(ch) : (GET_MAX_MANA(ch) * 0.5) + (GET_MAX_MOVE(ch) * 0.5) +
                                                                  GET_LIFEBONUSES(ch));
}
...
bool CAN_SEE_IN_DARK(struct char_data *ch) {
    return AFF_FLAGGED(ch, AFF_INFRAVISION) || (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_HOLYLIGHT)) ||
           (IS_MUTANT(ch) && (GET_GENOME(ch, 0) == 4 || GET_GENOME(ch, 1) == 4)) || PLR_FLAGGED(ch, PLR_AURALIGHT);
}
...
bool LIGHT_OK(struct char_data *ch) {
   ...
   (IS_LIGHT(IN_ROOM(ch)) || AFF_FLAGGED((ch), AFF_INFRAVISION) ||
    (IS_MUTANT(ch) && (GET_GENOME(ch, 0) == 4 || GET_GENOME(ch, 1) == 4)) || PLR_FLAGGED(ch, PLR_AURALIGHT));
}
...
bool MORT_CAN_SEE_OBJ(struct char_data *sub, struct obj_data *obj) {
    return (LIGHT_OK(sub) || obj->carried_by == sub || obj->worn_by) && INVIS_OK_OBJ(sub, obj) &&
...
bool INTROD(struct char_data *ch, struct char_data *vict) {
    return ch == vict || readIntro(ch, vict) == 1 ||
           (IS_NPC(vict) || IS_NPC(ch) || (GET_ADMLEVEL(ch) > 0 || GET_ADMLEVEL(vict) > 0));
}
...
const char *JUGGLERACE(struct char_data *ch) {
    return IS_HOSHIJIN(ch) ? (ch->mimic > 0 ? pc_race_types[ch->mimic - 1] : pc_race_types[ch->race]) : handle_racial(
            ch, 1);
}
...
bool ETHER_STREAM(struct char_data *ch) {
    return ROOM_FLAGGED(IN_ROOM(ch), ROOM_EARTH) || ROOM_FLAGGED(IN_ROOM(ch), ROOM_AETHER) ||
           ROOM_FLAGGED(IN_ROOM(ch), ROOM_NAMEK) || PLANET_ZENITH(IN_ROOM(ch));
}
