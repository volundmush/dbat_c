...
static void drive_outof_vehicle(struct char_data *ch, struct obj_data *vehicle) {
    ...
    if (!(hatch = get_obj_in_list_type(ITEM_HATCH, world[IN_ROOM(vehicle)].contents))) {
        ...
        if (ch->desc != NULL)
            act("@wThe @De@Wn@wg@Di@wn@We@Ds@w of the ship @rr@Ro@ra@Rr@w as it moves.", 1, ch, 0, 0, TO_ROOM);
        send_to_char(ch, "@wThe ship flies onward:\r\n");
        look_at_room(IN_ROOM(vehicle), ch, 0);
        int door;
...
ACMD(do_drive) {
    ...
    if (!strcasecmp(arg3, "unready") && !IS_NPC(ch)) {
        if (!PLR_FLAGGED(ch, PLR_PILOTING)) {
            ...
            act("@w$n stands up and stops piloting the ship.", 1, ch, 0, 0, TO_ROOM);
    ...
    if (confirmed == 1) {
        ...
        act("@w$n sits down and begins piloting the ship.", 1, ch, 0, 0, TO_ROOM);
        ...
        act("@wYou are zapped by $p@w and instantly step away from it.", 0, ch, controls, 0, TO_CHAR);
        act("@w$n@w is zapped by $p@w and instantly steps away from it.", 0, ch, controls, 0, TO_ROOM);
        ...
        if (!*arg) {
            ...
            if (!strcasecmp(arg, "north") || !strcasecmp(arg, "n")) {
                ...
                *buf3 = '\0';
                ...
                /* For Earth */
                act("@wYou set the controls to descend.@n", 0, ch, 0, 0, TO_CHAR);
                act("@C$n @wmanipulates the ship controls.@n", 0, ch, 0, 0, TO_ROOM);
                act("@RThe ship rocks and shakes as it descends through the atmosphere!@n", 0, ch, 0, 0, TO_CHAR);
                act("@RThe ship rocks and shakes as it descends through the atmosphere!@n", 0, ch, 0, 0, TO_ROOM);
                if (land_location <= 50) {
                    act("@wThe ship has landed.@n", 0, ch, 0, 0, TO_CHAR);
                    act("@wThe ship has landed.@n", 0, ch, 0, 0, TO_ROOM);
                }
                if (land_location > 50) {
                    act("@wThe ship slams into the ground and forms a small crater!@n", 0, ch, 0, 0, TO_CHAR);
                    act("@wThe ship slams into the ground and forms a small crater!@n", 0, ch, 0, 0, TO_ROOM);
                    obj_from_room(vehicle);
                    ...
                    if (!strcasecmp(arg2, "1")) {
                        ...
                        act("@wLanding sequence aborted, improper coordinates.@n", 0, ch, 0, 0, TO_CHAR);
                        act("@wLanding sequence aborted, improper coordinates.@n", 0, ch, 0, 0, TO_ROOM);
                    }
                }
                    /* For Zenith */
                ...
                act("@wYou set the controls to launch.@n", 0, ch, 0, 0, TO_CHAR);
                act("@C$n @wmanipulates the ship controls.@n", 0, ch, 0, 0, TO_ROOM);
                act("@RThe ship shudders as it launches up into the sky!@n", 0, ch, 0, 0, TO_CHAR);
                act("@RThe ship shudders as it launches up into the sky!@n", 0, ch, 0, 0, TO_ROOM);
                act("@wThe ship has reached low orbit.@n", 0, ch, 0, 0, TO_CHAR);
                act("@wThe ship has reached low orbit.@n", 0, ch, 0, 0, TO_ROOM);
